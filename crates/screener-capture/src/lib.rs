...
//! Paired screenshot + accessibility-tree capture, persisted to the screener DB.
//!
//! Extracted from `screener-engine` so that both the engine binary and the
//! `screener-sdk` napi addon can run the same capture pipeline without the
