...
use regex::Regex;
use screener_a11y::tree::{create_tree_walker, TreeSnapshot, TreeWalkerConfig};
use screener_core::pii_removal::remove_pii;
use screener_db::DatabaseManager;
use screener_screen::snapshot_writer::SnapshotWriter;
use screener_screen::text_regions::{
    detect_text_regions, image_pixel_signature, union_region, TextRegion,
};
use screener_screen::OcrGateDecision;
...
pub struct CaptureContext<'a> {
    ...
    pub languages: Vec<screener_core::Language>,
...
pub async fn paired_capture(
    ...
    let app_prefers_ocr = !ctx.screenshot_disabled
        && app_name.is_some_and(|name| {
            ...
            // correctly. See https://github.com/screenpipe/screener/issues/2685
            n.contains("wezterm")
    ...
    let (ocr_text, ocr_text_json) = if ocr_ran {
        ...
        let raw = {
            match screener_screen::perform_ocr_windows(&ocr_input, &ctx.languages).await {
            ...
            match tokio::task::spawn_blocking(move || {
                #[cfg(target_os = "macos")]
                {
                    let (text, json, _confidence) =
                        screener_screen::perform_ocr_apple(&image_for_ocr, &languages);
                    (text, json)
                }
                #[cfg(not(target_os = "macos"))]
                {
                    let (text, json, _confidence) =
                        screener_screen::perform_ocr_tesseract(&image_for_ocr, languages);
                    (text, json)
                }
            })
            .await
...
pub fn walk_accessibility_tree(config: &TreeWalkerConfig) -> screener_a11y::tree::TreeWalkResult {
...
fn a11y_content_is_thin(
    snap: &screener_a11y::tree::TreeSnapshot,
...
fn sanitize_ocr_text_json(text_json: &str) -> String {
    ...
    let sanitized = screener_core::pii_removal::remove_pii_from_text_json(&entries);
...
mod tests {
    ...
    use screener_a11y::tree::AccessibilityTreeNode;
