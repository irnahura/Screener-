...
// `screener-audio` configures `ort` with `load-dynamic` on Windows (both x86_64 and
...
// binary by the workflow (and by `install_onnxruntime()` in screener-audio's
...
fn main() {
    ...
    // future for the ~2300-line async main in src/bin/screener-engine.rs sits
