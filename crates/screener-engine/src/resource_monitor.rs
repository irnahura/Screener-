...
use chrono::Local;
use reqwest::Client;
use screener_resource::{ProcessBreakdown, ResourceSampler, ResourceSnapshot};
...
// A leak that users actually feel ("screener ate 10GB overnight") is slow
...
/// governance live in `screener-resource`; this type owns only reporting,
...
/// governance in `screener-resource`.
...
impl ResourceTelemetryReporter {
    ...
    async fn send_to_posthog(
        ...
        // `screener-resource` obtains this from `proc_pid_rusage` on macOS.
        properties.insert(
    ...
    fn log_process_breakdown(breakdown: &ProcessBreakdown) {
        ...
        } else if env::var("SCREENER_RESOURCE_BREAKDOWN").is_ok()
