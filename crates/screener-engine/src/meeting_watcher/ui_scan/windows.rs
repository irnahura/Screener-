...
pub fn find_running_meeting_apps(
    ...
    for (idx, profile) in profiles.iter().enumerate() {
        ...
        for (pid, title) in &window_titles {
            ...
            if matched.is_some() {
                // Confirms screener saw the meeting window; pairs with the
...
mod windows_live_tests {
    ...
    /// Run with: cargo test -p screener-engine --no-default-features --lib -- windows_live_tests::test_live_zoom_detection --nocapture --ignored
    #[test]
    #[ignore]
