...
// Public surface consumed by other crates via `screener_engine::meeting_watcher::…`.
...
fn selected_detector_mode() -> MeetingDetectorMode {
    ...
    // back to UI scanning. Override on any platform with SCREENER_MEETING_DETECTOR.
    let audio_process_is_default = cfg!(target_os = "macos") || cfg!(target_os = "windows");
    selected_detector_mode_from(
        std::env::var("SCREENER_MEETING_DETECTOR").ok().as_deref(),
...
mod tests {
    ...
    fn detector_override_accepts_hyphenated_and_mixed_case_spellings() {
        ...
        // `SCREENER_MEETING_DETECTOR=ui-scan`. Both hyphen and underscore
