...
use screener_audio::meeting_processes::{self, AudioInputProcess};
use screener_db::{DatabaseManager, MEETING_END_REASON_AUTO_END, MEETING_END_REASON_SHUTDOWN};
use screener_events::subscribe_to_event;
...
pub async fn run_audio_process_meeting_detection_loop(
    ...
    if let Ok(Some(meeting)) = db.get_most_recent_active_meeting().await {
        ...
        sync_meeting_flag(
            true,
            Some(screener_audio::meeting_detector::ActiveMeeting {
                pid: None,
                bundle_id: None,
