...
use axum::Json;
use screener_connect::{connections, mcp_servers};
use screener_core::pipes::{
...
pub async fn run_pipe_now(
    ...
    if !required_connections.is_empty() {
        ...
        let missing =
            screener_connect::missing_pipe_connections(ss, &screener_dir, &required_connections)
                .await;
        if !missing.is_empty() {
            return Json(json!({
                "error": format!(
    ...
    let conn_ctx = join_context_blocks([
        connections::render_context(&screener_dir, api_port, ss).await,
        mcp_servers::render_context(&screener_dir, api_port).await,
    ]);
...
pub async fn stop_pipe(State(pm): State<SharedPipeManager>, Path(id): Path<String>) -> Json<Value> {
    ...
    match mgr.stop_pipe(&id).await {
        Ok(status) => Json(json!({
            "success": matches!(
                status,
                screener_core::pipes::PipeStopStatus::Stopping
                    | screener_core::pipes::PipeStopStatus::StopPending
            ),
            "status": status,
        })),
        Err(e) => Json(json!({ "error": e.to_string() })),
...
pub async fn list_favorites(State(pm): State<SharedPipeManager>) -> Json<Value> {
    ...
    let favorites = screener_core::pipes::favorites::load(&dir);
...
pub async fn set_pipe_favorite(
    ...
    let dir = mgr.pipes_dir().to_path_buf();
    drop(mgr);
    match screener_core::pipes::favorites::set(&dir, &id, body.favorite) {
...
mod tests {
    ...
    use screener_core::pipes::PipeStore;
    use std::collections::HashMap;
    use std::path::Path;
    ...
    async fn run_api_returns_and_reads_the_exact_new_execution() {
        ...
        let db = Arc::new(
            screener_db::DatabaseManager::new(db_path.to_str().unwrap(), Default::default())
                .await
                .unwrap(),
        );
