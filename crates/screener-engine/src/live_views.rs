...
fn store_path(screener_dir: &Path) -> PathBuf {
    screener_dir.join("live-views").join("templates.json")
}

fn legacy_store_path(screener_dir: &Path) -> PathBuf {
    screener_dir
        .join("desktop")
...
fn load_store_unlocked(screener_dir: &Path) -> Result<(LiveViewStore, bool), LiveViewError> {
    let path = store_path(screener_dir);
    if path.exists() {
        return read_store(&path);
    }
    let legacy_path = legacy_store_path(screener_dir);
...
fn write_store_unlocked(screener_dir: &Path, store: &LiveViewStore) -> Result<(), LiveViewError> {
    let path = store_path(screener_dir);
...
fn with_store<T>(
    ...
    let (mut store, migrated) = load_store_unlocked(screener_dir)?;
    let (result, changed) = operation(&mut store)?;
    if changed || migrated {
        write_store_unlocked(screener_dir, &store)?;
    }
    ...
    sync_targets(screener_dir, &store.templates)?;
    Ok(result)
}
...
pub fn scheduled_live_view_run_context(
    ...
    const SCHEDULED_REFRESH_RULES: &str = concat!(
        ...
        "source-backed Screener APIs inside that dashboard's exact time_range and submit an ",
...
pub fn scheduled_live_view_run_context_for_dir(
    ...
    let templates = list_live_view_templates(screener_dir).ok()?;
        ...
        .flat_map(|template| {
            template.blocks.iter().filter_map(move |block| {
                ...
                if let Some(object) = schema.as_object_mut() {
                    object.insert(
                        "x-screener-time-range".to_string(),
                        json!({
...
fn sync_targets(screener_dir: &Path, templates: &[LiveViewTemplate]) -> Result<(), LiveViewError> {
    ...
    replace_consumer_targets(screener_dir, LEGACY_CONSUMER_ID, Vec::new())
...
fn hydrate(
    ...
    let targets: HashMap<String, OutputTarget> = list_output_targets(screener_dir)
...
pub fn list_live_view_templates(
    ...
    with_store(screener_dir, |store| Ok((store.templates.clone(), false)))
}
...
pub fn get_live_view_template(
    ...
    list_live_view_templates(screener_dir)?
        .into_iter()
        .find(|template| template.id == id)
        .ok_or_else(|| LiveViewError::not_found(format!("Live View '{id}' not found")))
}

pub fn list_live_views(screener_dir: &Path) -> Result<Vec<LiveView>, LiveViewError> {
    let templates = with_store(screener_dir, |store| Ok((store.templates.clone(), false)))?;
    hydrate(screener_dir, templates)
}

pub fn get_live_view(screener_dir: &Path, id: &str) -> Result<LiveView, LiveViewError> {
    validate_slug(id, "Live View id")?;
    list_live_views(screener_dir)?
        .into_iter()
...
pub fn save_live_view(
    ...
    let templates = with_store(screener_dir, |store| {
...
pub fn apply_live_view_template(
    ...
    validate_live_view_template(&template)?;
    let expected_revision = list_live_view_templates(screener_dir)?
        .into_iter()
...
pub fn delete_live_view(screener_dir: &Path, id: &str) -> Result<(), LiveViewError> {
    validate_slug(id, "Live View id")?;
    with_store(screener_dir, |store| {
...
mod tests {
    ...
    fn template_compiles_only_bound_blocks_into_stable_targets() {
        ...
        assert_eq!(targets[0].schema["x-screener-time-range"]["preset"], "7d");
        assert_eq!(
            targets[0].schema["x-screener-time-range"]["authority"],
            "target"
        );
