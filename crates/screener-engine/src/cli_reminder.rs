...
//! Periodic CLI nudges shown to users running the standalone `screener`
//! binary from a terminal. Fires only from the CLI entrypoint — never from
//! the desktop app, which embeds the engine as a library.
//!
//! Opt out with `SCREENER_NO_REMINDERS=1` (silences all tips).
//! Opt out of the version-check nudge only with `SCREENER_NO_UPDATE_CHECK=1`.
...
const DESKTOP_APP_URL: &str = "https://screener.com";
const SURVEY_URL: &str = "https://screener.com/survey/cli";
const NPM_LATEST_URL: &str = "https://registry.npmjs.org/screener/latest";

/// Spawn the background reminder loop. Safe to call once at CLI startup.
pub fn spawn() {
    if env::var("SCREENER_NO_REMINDERS").is_ok() {
        return;
    }
...
pub async fn check_for_updates() {
    if env::var("SCREENER_NO_UPDATE_CHECK").is_ok() {
        return;
    }
    ...
    if latest != current {
        eprintln!(
            "\n  {} screener {} available (you have {})",
            "update:".yellow().bold(),
            latest.green(),
            current,
        );
        eprintln!("  run: {}", "npx screener@latest record".cyan());
        eprintln!();
    }
}
...
fn print_desktop_app_tip() {
    eprintln!();
    eprintln!(
        "  {} get the screener desktop app for the full experience",
        "tip:".cyan().bold(),
    );
...
fn print_mcp_tip() {
    eprintln!();
    eprintln!(
        "  {} connect screener to every AI tool detected on this computer:",
        "tip:".cyan().bold(),
    );
    eprintln!("       {}", "npx screener@latest setup".green().bold());
    eprintln!(
...
fn print_install_bundle_tip() {
    ...
    eprintln!(
        "       {}",
        "npx screener install https://screener.com/start.json"
            .green()
            .bold(),
    );
    eprintln!();
...
fn print_login_tip() {
    ...
    eprintln!("       {}", "npx screener login".green().bold());
    eprintln!();
}
...
fn print_survey_tip() {
    eprintln!();
    eprintln!(
        "  {} help shape the screener CLI. what do you use it for? (60s)",
        "survey:".cyan().bold(),
    );
    eprintln!("       {}", "npx screener survey".green().bold());
