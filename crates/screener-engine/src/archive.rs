...
use screener_core::sync::{BlobType, DownloadedBlob, SyncClientConfig, SyncManager};
...
pub struct ArchiveDownloadRequest {
    ...
    pub token: String,
    /// Destination folder. Defaults to `<screener_dir>/archive-export`.
...
fn archive_password(token: &str) -> String {
    format!("screener-archive-{:x}", md5::compute(token.as_bytes()))
}
...
pub async fn archive_download(
        ...
        .filter(|s| !s.is_empty())
    {
        ...
        None => state.screener_dir.join("archive-export"),
    };
...
mod tests {
    ...
    fn archive_password_is_stable_and_token_specific() {
        ...
        assert!(archive_password("tok-123").starts_with("screener-archive-"));
