...
//! by the standalone `screener-engine` binary and the Tauri desktop app.
//!
...
//!
//! Override the target with the `SCREENER_FD_LIMIT` env var.

/// Raise the soft `RLIMIT_NOFILE` toward `SCREENER_FD_LIMIT` (default 8192),
...
pub fn set_fd_limit() {
    ...
    let desired_limit: u64 = env::var("SCREENER_FD_LIMIT")
        .ok()
