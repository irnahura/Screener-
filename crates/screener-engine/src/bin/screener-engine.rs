...
#[cfg(target_os = "macos")]
use screener_audio::core::device::{
...
use screener_core::agents::AgentExecutor;
use screener_core::find_ffmpeg_path;
use screener_core::paths;
use screener_db::DatabaseManager;
use screener_engine::{
    analytics,
    cli::{
...
use screener_screen::monitor::list_monitors;
use serde_json::json;
use std::{
    env, fs,
...
async fn doctor_check_system_audio_capture() -> bool {
    ...
    match get_cpal_device_and_config(&device).await {
        ...
        Err(e) => {
            let terminal = screener_core::permissions::detect_terminal();
            eprintln!("  system audio capture: failed");
            eprintln!("    ScreenCaptureKit could not open System Audio for this CLI session.");
            eprintln!(
                "    macOS grants CLI capture through the terminal app, not the screener app."
            );
            eprintln!(
            ...
            eprintln!(
                "    if you only need a workaround: screener record --experimental-coreaudio-system-audio"
            );
...
fn get_base_dir(custom_path: &Option<String>) -> anyhow::Result<PathBuf> {
    let default_path = paths::default_screener_data_dir();
...
fn setup_logging(
    ...
    let file_appender = screener_engine::logging::SizedRollingWriter::builder()
    ...
    let make_env_filter = || {
        ...
        let filter = env::var("SCREENER_LOG")
            .unwrap_or_default()
            .split(',')
            .filter(|s| !s.is_empty())
        ...
        if debug {
            filter.add_directive("screener=debug".parse().unwrap())
        } else {
            filter
        }
    };
...
async fn main() -> anyhow::Result<()> {
    ...
    screener_engine::fd_limit::set_fd_limit();

    debug!("starting screener server");
    ...
    let record_args = match cli.command {
        ...
        Command::Agent { ref subcommand } => {
            screener_engine::cli::agent::handle_agent_command(subcommand).await?;
            return Ok(());
        }
        Command::Setup { ref api_url } => {
            screener_engine::cli::agent::setup_all_detected(api_url)?;
            return Ok(());
        }
        Command::Service { ref subcommand } => {
            screener_engine::cli::service::handle_service_command(subcommand).await?;
            return Ok(());
        }
        ...
        Command::Connection { ref subcommand } => {
            screener_engine::cli::connection::handle_connection_command(subcommand).await?;
            return Ok(());
        }
        Command::Vault { ref subcommand } => {
            screener_engine::cli::vault::handle_vault_command(subcommand).await?;
            return Ok(());
        }
        Command::Install {
            ref url,
            ...
            screener_engine::cli::install::handle_install(url, allow_untrusted).await?;
            return Ok(());
        }
        Command::Login => {
            screener_engine::cli::login::handle_login_command().await?;
            return Ok(());
        }
        Command::Logout => {
            screener_engine::cli::login::handle_logout_command().await?;
            return Ok(());
        }
        Command::Whoami => {
            screener_engine::cli::login::handle_whoami_command().await?;
            return Ok(());
        }
        Command::Survey => {
            screener_engine::cli::survey::handle_survey_command().await?;
            return Ok(());
        }
        Command::Auth { ref subcommand } => {
            screener_engine::cli::auth::handle_auth_command(subcommand).await?;
            return Ok(());
        }
        Command::Db { ref subcommand } => {
            screener_engine::cli::db::handle_db_command(subcommand).await?;
            return Ok(());
        }
        Command::Backup {
            ref subcommand,
            ...
            screener_engine::cli::backup::handle_backup_command(subcommand, data_dir).await?;
            return Ok(());
        }
        Command::Export(ref args) => {
            screener_engine::cli::export::handle_export_command(args).await?;
            return Ok(());
        }
        Command::Doctor => {
            eprintln!("screener doctor");
            eprintln!("=================");
            eprintln!();

            // permissions
            eprintln!("permissions:");
            let perms_ok = screener_core::permissions::preflight_check(true, true);
            ...
            if port_ok {
                ...
                eprintln!("  port 3030: in use (screener may already be running)");
            ...
            if perms_ok && system_audio_ok && ffmpeg_ok && port_ok {
                ...
                eprintln!("some checks failed — fix the issues above before running screener");
                std::process::exit(1);
            }
            return Ok(());
        }
        Command::Diagnose {
            ...
            let local_data_dir = get_base_dir(data_dir)?;
            screener_engine::cli::diagnose::handle_diagnose_command(
                &local_data_dir,
                message.as_deref(),
                port,
                dry_run,
    ...
    screener_engine::cli::agent::maybe_prompt_connect_detected(&local_data_dir);
    ...
    if config.analytics_enabled && screener_engine::analytics::telemetry_disabled_by_env() {
    ...
    screener_connect::mdns::set_enabled(record_args.enable_mdns);
    ...
    if let Err(e) = screener_engine::power::set_keep_awake(config.keep_computer_awake) {
    ...
    screener_engine::process_priority::set_background_priority();

    // Non-blocking update check — runs in background, prints banner if outdated
    tokio::spawn(async {
        screener_engine::cli_reminder::check_for_updates().await;
    });

    // Periodic terminal nudge to install the desktop app (CLI-only).
    screener_engine::cli_reminder::spawn();
    ...
    let _sentry_guard = if config.analytics_enabled {
        ...
        let guard = sentry::init((
            ...
            sentry::ClientOptions {
                ...
                server_name: Some("screener-cli".into()),
                before_send: Some(std::sync::Arc::new(|mut event| {
                    ...
                    // Tauri-app filter in apps/screener-app-tauri/src-tauri/
                    ...
                    let env_patterns = USER_ENV_PATTERNS.get_or_init(|| {
                        [
                            ...
                            // Port conflict — another screener instance is already bound
                            // (CLI-2J: 659 events / 649 users — user environment, not a bug)
                            r"you're likely already running screener instance",
        ...
        sentry::configure_scope(|scope| {
            ...
            // customers can set SCREENER_SUPPORT_ID to make standalone CLI
            ...
            let telemetry_context =
                screener_engine::telemetry_context::TelemetryContext::from_env();
    ...
    // apps/screener-app-tauri/src-tauri/src/main.rs.
    {
        // Write to the resolved data dir (honors --data-dir) so the crash log
        // sits next to screener.log, and an embedder running with its own
        // --data-dir doesn't collide with the desktop app's
        // ~/.screener/last-panic.log (the app runs its engine in-process and
        // owns that file).
        ...
        // Reuse the existing embedder attribution (SCREENER_EMBEDDER /
        // SCREENER_CUSTOMER_ID / ...) so the local crash record is identifiable
        ...
        let attribution = {
            use screener_engine::telemetry_context::TelemetryContext;
    ...
    if !is_local_ipv4_port_free(config.port) {
        error!(
            "you're likely already running screener instance in a different environment, e.g. terminal/ide, close it and restart or use different port"
        );
    ...
    let _permission_monitor_handle = screener_engine::permission_monitor::start();
    ...
    screener_engine::schedule_monitor::reset_schedule_paused();

    // Start work-hours schedule monitor if enabled
    if config.schedule_enabled {
        screener_engine::schedule_monitor::start_schedule_monitor(
        ...
        Ok(mut manager) => {
            ...
            if !config.disable_timeline {
                ...
                manager.set_on_transcription_insert(std::sync::Arc::new(move |info| {
                    ...
                    rt.spawn(async move {
                        use screener_engine::hot_frame_cache::HotAudio;
                        cache
                            .push_audio(HotAudio {
    ...
    let vision_metrics = Arc::new(screener_screen::PipelineMetrics::new());
    ...
    if !config.disable_snapshot_compaction {
        screener_engine::start_snapshot_compaction(
            db.clone(),
    ...
    // the only safety net is the 4-hour hard cap.
    {
        ...
        tokio::spawn(async move {
            ...
            let mut sub = screener_events::subscribe_to_event::<serde_json::Value>("meeting_ended");
    ...
    //   to the existing notification).
    {
        ...
        tokio::spawn(async move {
            use futures::StreamExt;
            let mut sub =
                screener_events::subscribe_to_event::<serde_json::Value>("meeting_started");
            while let Some(event) = sub.next().await {
                let meeting_id = event
                    .data
                    .get("meeting_id")
                ...
                if !matches!(
                    snap.default_mode,
                    screener_engine::high_fps_controller::DefaultMode::Always
                ) {
                    continue;
                }
                let already_bound = matches!(
                    snap.kind,
                    Some(screener_engine::high_fps_controller::SessionKind::Meeting { .. })
                );
                if !already_bound {
    ...
    if config.api_auth {
        info!("API auth enabled — run `screener auth token` to view your key");
    }

    // Standalone CLI users miss out on the chat/timeline UI — point them at the
    // desktop app. SCREENER_ANALYTICS_ID is only set when the Tauri app spawns
    // the engine, so its absence is a reliable "this is a bare CLI run" signal.
    if std::env::var("SCREENER_ANALYTICS_ID").is_err() {
        eprintln!();
    ...
    // SCREENER_API_KEY directly; desktop path overrides via
    // SCServer::cloud_token_handle after spawn.
    if let Ok(t) = std::env::var("SCREENER_API_KEY") {
        if !t.is_empty() {
    ...
    let encryption_requested =
        config.encrypt_secrets || screener_secrets::is_encryption_requested(&local_data_dir);
    {
        ...
        let secret_store_result = if config.encrypt_secrets {
            let secret_key = match screener_secrets::keychain::get_or_create_key() {
                Some(k) => {
                    info!("keychain: encryption key ready (--encrypt-secrets)");
                    if let Err(e) = screener_secrets::mark_encryption_enabled(&local_data_dir) {
            ...
            screener_secrets::SecretStore::open_for_data_dir(&local_data_dir, secret_key).await
        } else {
            screener_secrets::SecretStore::open_for_data_dir_with_vault_key(&local_data_dir).await
        };
        match secret_store_result {
            Ok(store) => {
                // Run startup permission sweep
                let fixed = screener_secrets::fix_secret_file_permissions(&local_data_dir);
                if fixed > 0 {
                    info!("fixed permissions on {} credential files", fixed);
                }

                // Run legacy migration
                match screener_secrets::migrate_legacy_secrets(&store, &local_data_dir).await {
                    Ok(report) => {
    ...
    let user_token = std::env::var("SCREENER_API_KEY").ok();
    let pi_executor = std::sync::Arc::new(
        screener_core::agents::pi::PiExecutor::new(user_token.clone())
    ...
    // can override with SCREENER_EVENT_CLASSIFIER_URL.
    if config.enable_workflow_events {
        let classifier_url = std::env::var("SCREENER_EVENT_CLASSIFIER_URL").unwrap_or_else(|_| {
            screener_engine::workflow_classifier::DEFAULT_CLASSIFIER_URL.to_string()
        });
        let token = user_token.clone().unwrap_or_default();
        let port = config.port;
        tokio::spawn(async move {
            screener_engine::workflow_classifier::start_workflow_classifier(
                classifier_url,
                token,
                port,
    ...
    let mut agent_executors: std::collections::HashMap<
        String,
        std::sync::Arc<dyn screener_core::agents::AgentExecutor>,
    > = std::collections::HashMap::new();
    agent_executors.insert("pi".to_string(), pi_executor.clone());

    // Create pipe store backed by the main SQLite DB
    let pipe_store: Option<std::sync::Arc<dyn screener_core::pipes::PipeStore>> =
        Some(std::sync::Arc::new(
            screener_engine::pipe_store::SqlitePipeStore::new(db.clone()),
        ));

    let mut pipe_manager =
        screener_core::pipes::PipeManager::new(pipes_dir, agent_executors, pipe_store, config.port);
    let mcp_session_access = screener_core::pipes::mcp_access::McpSessionAccessRegistry::new();
    ...
    pipe_manager.set_token_registry(std::sync::Arc::new(
        screener_engine::pipe_permissions_middleware::DashMapTokenRegistry::new(
    ...
    // pipes that are still in "setup mode" (declared connections not paired).
    {
        ...
        let screener_dir_for_check = local_data_dir.clone();
        pipe_manager.set_connection_check(std::sync::Arc::new(move |required| {
            let ss = secret_store_for_check.clone();
            let dir = screener_dir_for_check.clone();
            Box::pin(async move {
                screener_connect::missing_pipe_connections(ss.as_deref(), &dir, &required).await
            })
        }));
    }
    ...
    {
        let screener_dir_for_live_views = local_data_dir.clone();
        pipe_manager.set_scheduled_run_context(std::sync::Arc::new(move |pipe_name: &str| {
            screener_engine::live_views::scheduled_live_view_run_context_for_dir(
                &screener_dir_for_live_views,
                pipe_name,
                chrono::Local::now(),
            )
            .map(|context| screener_engine::pipes_api::format_run_context(&context))
        }));
    }
    ...
    println!(
        ...
        if record_args.retention_days == 0 {
            ...
            match record_args.retention_mode {
                screener_engine::retention::RetentionMode::Media => {
                    "media-only (keep transcripts)".to_string()
                }
                screener_engine::retention::RetentionMode::Lean => {
                    "lean (keep text+memories)".to_string()
                }
                screener_engine::retention::RetentionMode::All => "all (full delete)".to_string(),
            }
        }
    );
    ...
    println!(
        "\n{}",
        "check latest changes here: https://github.com/screenpipe/screener/releases"
            .bright_blue()
            .italic()
    );
    ...
    if !config.disable_audio {
        ...
        tokio::spawn(async move {
            ...
            if drm_pause && screener_engine::drm_detector::drm_content_paused() {
    ...
    let redact_database_error_hook: screener_redact::DatabaseErrorHook = {
        let db = Arc::clone(&db);
        Arc::new(move |error| {
    ...
    if config.redact_agent_session_secrets {
        if let Ok(pi_dir) = screener_core::agents::pi::pi_config_dir() {
            use screener_redact::worker::{Worker, WorkerConfig};
            use std::sync::Arc;
            // A sessions-only worker (empty tables) never uses this redactor; the
            // session scrub runs its own secrets-only regex pipeline.
            let placeholder = Arc::new(screener_redact::Pipeline::regex_only())
                as Arc<dyn screener_redact::Redactor>;
    ...
    if config.async_pii_redaction {
        ...
        //      from huggingface.co/screener/pii-redactor under
        ...
        tokio::spawn(async move {
            ...
            info!(
                "fetching v45 phase 3 ONNX text redactor (~278 MB INT8 on first run, \
                 cached at ~/.screener/models/v45_phase3_onnx/)"
            );
    ...
    if config.async_image_pii_redaction {
        use screener_redact::adapters::rfdetr::{RfdetrConfig, RfdetrRedactor};
        use screener_redact::image::worker::{ImageWorker, ImageWorkerConfig};
        use screener_redact::{ImageRedactionPolicy, ImageRedactor};
        use std::sync::Arc;
        ...
        {
            if std::env::var_os("SCREENER_ENABLE_EXPERIMENTAL_RFDETR_MLX").is_some() {
                use screener_redact::adapters::rfdetr_mlx::{RfdetrMlxConfig, RfdetrMlxRedactor};
                let mlx_cfg = RfdetrMlxConfig::default();
                // Mirrors the ONNX adapter: download once, verify SHA-256,
                // cache at ~/.screener/models/rfdetr_v9.safetensors.
                ...
                tracing::info!(
                    "rfdetr-mlx disabled by default for CLI stability; \
                     set SCREENER_ENABLE_EXPERIMENTAL_RFDETR_MLX=1 to opt in"
                );
            }
        }
                ...
                Err(e) => {
                    ...
                    tracing::warn!(
                        "image-PII redaction enabled but couldn't load model; skipping: {e}. \
                         ...
                         rfdetr_v9.onnx at ~/.screener/models/."
                    );
                }
            }
        }
