...
use screener_audio::audio_manager::AudioManager;
use screener_core::sync::SyncServiceHandle;
...
pub struct AppState {
    ...
    pub audio_metrics: Arc<screener_audio::metrics::AudioPipelineMetrics>,
    ...
    pub pipe_permissions:
        Arc<DashMap<String, Arc<screener_core::pipes::permissions::PipePermissions>>>,
    ...
    pub vault: screener_vault::VaultManager,
    ...
    pub browser_registry: Arc<screener_connect::connections::browser::BrowserRegistry>,
    ...
    pub owned_browser: Arc<screener_connect::connections::browser::OwnedBrowser>,
    /// When true, non-localhost requests require Authorization: Bearer <api_key>
    pub api_auth: bool,
    /// The API key to validate against (from SCREENER_API_KEY or auth.json)
    pub api_auth_key: Option<String>,
    /// Cloud JWT (Clerk) used to authenticate proxied requests to api.screener.com.
    ...
    pub secret_store: Option<Arc<screener_secrets::SecretStore>>,
...
pub struct SCServer {
    ...
    pub mcp_session_access: Option<screener_core::pipes::mcp_access::McpSessionAccessRegistry>,
    ...
    pub owned_browser: Option<Arc<screener_connect::connections::browser::OwnedBrowser>>,
...
fn is_api_auth_token_authorized(
    ...
    pipe_permissions: &DashMap<String, Arc<screener_core::pipes::permissions::PipePermissions>>,
    token: Option<&str>,
) -> bool {
...
impl SCServer {
    ...
    pub fn new(
        ...
        SCServer {
            ...
            video_quality,
            pipe_manager: None,
            vision_metrics: Arc::new(screener_screen::PipelineMetrics::new()),
            audio_metrics,
    ...
    /// to api.screener.com. Safe to call before or after `start()` — the route
    ...
    pub fn with_mcp_session_access(
        mut self,
        registry: screener_core::pipes::mcp_access::McpSessionAccessRegistry,
    ) -> Self {
    ...
    pub async fn start(self) -> Result<(), std::io::Error> {
        ...
        if self.advertise_mdns {
            if let Err(e) = screener_connect::mdns::advertise(self.addr.port()) {
    ...
    async fn create_router_inner(&self) -> Router {
        ...
        let app_state = Arc::new(AppState {
            ...
            vault: screener_vault::VaultManager::new(self.screener_dir.clone()),
            manual_meeting: self
                .manual_meeting
                .clone()
                .unwrap_or_else(|| Arc::new(tokio::sync::RwLock::new(None))),
            browser_bridge: crate::routes::browser::BrowserBridge::new(),
            browser_registry: screener_connect::connections::browser::BrowserRegistry::new(),
        ...
        // OwnedWebviewHandle.
        {
            use screener_connect::connections::browser::UserBrowser;
        ...
        let server = Server::axum()
            ...
            // HTTP twin of the `screener export` CLI; used by the MCP export-video tool.
        ...
        let router = Router::new()
            ...
            // Local proxy → api.screener.com/v1/chat/completions. Lets the
            ...
            // Local proxy → api.screener.com/v1/web-search. Lets coding-agent
            ...
            // Blocking connect broker for the harness MCP `screener_connect_app`
        ...
        let cm: crate::connections_api::SharedConnectionManager = Arc::new(Mutex::new(
            screener_connect::connections::ConnectionManager::new(
                self.screener_dir.clone(),
                self.secret_store.clone(),
            ),
        ));
        let wa: crate::connections_api::SharedWhatsAppGateway = Arc::new(Mutex::new(
            screener_connect::whatsapp::WhatsAppGateway::new(self.screener_dir.clone()),
        ));
        ...
        let mcp_store: crate::mcp_servers_api::SharedMcpServerStore =
            Arc::new(screener_connect::mcp_servers::McpServerStore::new(
                self.screener_dir.clone(),
                self.secret_store.clone(),
            ));
        let router = router.nest(
                ...
                axum::middleware::from_fn(
                    move |req: axum::extract::Request, next: axum::middleware::Next| {
                        ...
                        async move {
                            ...
                            // 2. screener_auth=<token> cookie (img src, WebSocket)
                            ...
                            let cookie_token = req
                                ...
                                .and_then(|cookies| {
                                    cookies.split(';')
                                        .map(|c| c.trim())
                                        .find_map(|c| c.strip_prefix("screener_auth="))
                                        .map(|s| s.to_string())
                                });
                            ...
                            if authorized {
                                ...
                                if upgrade {
                                    ...
                                    debug!(
                                        path = %path,
                                        "api auth: rejected WebSocket upgrade (missing/invalid token; use Cookie screener_auth, Authorization Bearer, or ?token=)"
                                    );
                                }
                                axum::response::Response::builder()
                                    ...
                                    .body(axum::body::Body::from(
                                        ...
                                        r#"{"error":"unauthorized: API access requires authentication. Pass `Authorization: Bearer <your-api-key>`. Get the key with `screener auth token`, or set the `SCREENER_API_KEY` env var before starting screener. (Desktop app users: Settings > Privacy.)"}"#,
                                    ))
                                    .unwrap()
                            }
                        }
...
mod tests {
    ...
    use dashmap::DashMap;
    use screener_core::pipes::permissions::PipePermissions;
