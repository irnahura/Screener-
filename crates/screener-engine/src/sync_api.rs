...
use crate::sync_provider::ScreenerSyncProvider;
...
async fn sync_init_inner(
    ...
    let machine_id = request
        .machine_id
        .unwrap_or_else(screener_core::sync::get_or_create_machine_id);
    ...
    let provider = Arc::new(
        ScreenerSyncProvider::new(state.db.clone(), machine_id.clone())
            .with_sync_gating(state.screener_dir.clone()),
    );
    ...
    tokio::spawn(async move {
        use screener_core::sync::SyncEvent;
    ...
    let download_provider = Arc::new(ScreenerSyncProvider::new(
        state.db.clone(),
        machine_id.clone(),
    ));
    let download_cursor = runtime_state.last_download_at.clone();
    let download_machine_id = machine_id.clone();
    let pipe_sync_dir = state.screener_dir.clone();
    ...
    // and the same screener_dir for the toggle file, but it talks to the
...
const SYNC_ACCOUNT_KEY_MISMATCH_MESSAGE: &str = concat!(
    ...
    "with an older device-local key; open screener on a device where sync still ",
...
async fn resolve_cloud_token(state: &Arc<AppState>) -> String {
    ...
    match in_mem.filter(|token| !token.is_empty()) {
        Some(token) => token,
        None => crate::auth_key::find_cloud_token(&state.screener_dir)
            .await
            .unwrap_or_default(),
    }
}
...
pub async fn ensure_sync_runtime(
    ...
    let machine_id = screener_core::sync::get_or_create_machine_id();
...
pub async fn sync_connections_push(
    ...
    use screener_core::connections::sync::*;
...
pub async fn sync_connections_pull(
    ...
    if has_deleted_connection_action(&actions) {
        clear_connection_tombstones(&state.screener_dir, &actions);
    }
...
async fn download_connection_manifest(
    manager: &Arc<screener_core::sync::SyncManager>,
) -> screener_core::connections::sync::ConnectionSyncManifest {
    use screener_core::connections::sync::ConnectionSyncManifest;
    use screener_core::sync::BlobType;
...
async fn upload_connection_manifest(
    manager: &Arc<screener_core::sync::SyncManager>,
    manifest: &screener_core::connections::sync::ConnectionSyncManifest,
) -> Vec<String> {
    use screener_core::sync::BlobType;
...
async fn build_connections_local_manifest(
    ...
    machine_id: &str,
    secret_store: Option<&screener_secrets::SecretStore>,
) -> screener_core::connections::sync::ConnectionSyncManifest {
    screener_core::connections::sync::build_local_manifest(screener_dir, machine_id, secret_store)
        .await
}

async fn apply_connections_manifest_to_disk(
    manifest: &screener_core::connections::sync::ConnectionSyncManifest,
    actions: &[screener_core::connections::sync::ConnectionSyncAction],
    screener_dir: &std::path::Path,
    secret_store: Option<&screener_secrets::SecretStore>,
) -> Vec<String> {
    screener_core::connections::sync::apply_manifest_to_disk(
        manifest,
        actions,
        screener_dir,
        secret_store,
...
fn has_deleted_connection_action(
    ...
    actions.iter().any(|action| {
        matches!(
            action,
            screener_core::connections::sync::ConnectionSyncAction::Deleted(_)
        )
    })
}

fn connection_action_str(
    action: &screener_core::connections::sync::ConnectionSyncAction,
) -> String {
    use screener_core::connections::sync::ConnectionSyncAction;
    match action {
...
async fn run_background_connections_sync(
    ...
    let local = build_connections_local_manifest(screener_dir, machine_id, secret_store).await;
    let (merged, actions) = merge_manifests(&local, &remote, machine_id);

    let mut all_errors =
        apply_connections_manifest_to_disk(&merged, &actions, screener_dir, secret_store).await;
    if has_deleted_connection_action(&actions) {
        clear_connection_tombstones(screener_dir, &actions);
    }
...
pub async fn sync_pipes_push(
    ...
    let pipes_dir = state.screener_dir.join("pipes");
    let machine_id = &runtime.machine_id;
...
async fn download_pipe_manifest(
    manager: &Arc<screener_core::sync::SyncManager>,
) -> screener_core::pipes::sync::PipeSyncManifest {
    use screener_core::pipes::sync::PipeSyncManifest;
    use screener_core::sync::BlobType;
...
async fn upload_pipe_manifest(
    manager: &Arc<screener_core::sync::SyncManager>,
    manifest: &screener_core::pipes::sync::PipeSyncManifest,
) -> Vec<String> {
    use screener_core::sync::BlobType;
...
fn is_pipe_sync_enabled(screener_dir: &std::path::Path) -> bool {
    is_settings_bool_enabled(screener_dir, "pipeSyncEnabled")
}

/// Check if memories sync is enabled by reading store.bin → settings.memoriesSyncEnabled.
fn is_memories_sync_enabled(screener_dir: &std::path::Path) -> bool {
    is_settings_bool_enabled(screener_dir, "memoriesSyncEnabled")
}

/// Check if connections sync is enabled by reading store.bin → settings.connectionsSyncEnabled.
fn is_connections_sync_enabled(screener_dir: &std::path::Path) -> bool {
    is_settings_bool_enabled(screener_dir, "connectionsSyncEnabled")
}

/// Read `settings.<key>` as a bool from `<screener_dir>/store.bin`.
...
fn is_settings_bool_enabled(screener_dir: &std::path::Path, key: &str) -> bool {
    ...
    let store = match crate::cli::store_file::read_store_for(screener_dir) {
        Ok(v) => v,
        Err(_) => return false,
    };
...
async fn run_background_pipe_sync(
    ...
    let pipes_dir = screener_dir.join("pipes");
...
fn read_memory_tombstones(
    ...
    let path = screener_dir.join(MEMORY_TOMBSTONES_FILE);
...
fn write_memory_tombstones(
    ...
    let path = screener_dir.join(MEMORY_TOMBSTONES_FILE);
    let tmp_path = screener_dir.join(format!("{}.tmp", MEMORY_TOMBSTONES_FILE));
...
pub fn record_memory_tombstone(screener_dir: &std::path::Path, sync_uuid: Option<&str>) {
    let Some(uuid) = sync_uuid else { return };
    let mut t = read_memory_tombstones(screener_dir);
    t.insert(uuid.to_string(), chrono::Utc::now().to_rfc3339());
    if let Err(e) = write_memory_tombstones(screener_dir, &t) {
        warn!(
...
async fn build_memories_local_manifest(
    ...
    screener_dir: &std::path::Path,
    machine_id: &str,
) -> screener_core::memories::sync::MemoriesSyncManifest {
    use screener_core::memories::sync::{MemoriesSyncManifest, SyncedMemory};
    ...
    // older than 30 days.
    for (uuid, ts) in read_memory_tombstones(screener_dir) {
...
async fn apply_memories_manifest_to_db(
    db: &Arc<DatabaseManager>,
    manifest: &screener_core::memories::sync::MemoriesSyncManifest,
    actions: &[screener_core::memories::sync::MemorySyncAction],
) -> Vec<String> {
    use screener_core::memories::sync::MemorySyncAction;
...
async fn download_memories_manifest(
    ...
    use screener_core::memories::sync::MemoriesSyncManifest;
    use screener_core::sync::BlobType;
...
pub async fn sync_memories_push(
    ...
    use screener_core::memories::sync::*;
    ...
    let machine_id = &runtime.machine_id;
    let local = build_memories_local_manifest(&state.db, &state.screener_dir, machine_id).await;
...
pub async fn sync_memories_pull(
    ...
    {
        let mut t = read_memory_tombstones(&state.screener_dir);
        for a in &actions {
            if let MemorySyncAction::Deleted(uuid) = a {
                t.remove(uuid);
            }
        }
        if let Err(e) = write_memory_tombstones(&state.screener_dir, &t) {
...
async fn run_background_memories_sync(
    ...
    let local = build_memories_local_manifest(db, screener_dir, machine_id).await;
    let (merged, actions) = merge_manifests(&local, &remote, machine_id);
...
fn action_str(a: &screener_core::memories::sync::MemorySyncAction) -> String {
    use screener_core::memories::sync::MemorySyncAction;
    match a {
