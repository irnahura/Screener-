...
const DISTINCT_ID_ENV_VARS: &[&str] = &[
    "SCREENER_ANALYTICS_ID",
    "SCREENER_SUPPORT_ID",
    "SCREENER_TELEMETRY_ID",
];

const SUPPORT_ID_ENV_VARS: &[&str] = &["SCREENER_SUPPORT_ID", "SCREENER_TELEMETRY_ID"];
const CUSTOMER_ID_ENV_VARS: &[&str] = &[
    "SCREENER_CUSTOMER_ID",
    "SCREENER_ORG_ID",
    "SCREENER_TELEMETRY_CUSTOMER_ID",
];
const DEPLOYMENT_ID_ENV_VARS: &[&str] =
    &["SCREENER_DEPLOYMENT_ID", "SCREENER_TELEMETRY_DEPLOYMENT_ID"];
const EMBEDDER_ENV_VARS: &[&str] = &[
    "SCREENER_EMBEDDER",
    "SCREENER_HOST_APP",
    "SCREENER_TELEMETRY_HOST_APP",
];
const EMBEDDER_VERSION_ENV_VARS: &[&str] = &[
    "SCREENER_EMBEDDER_VERSION",
    "SCREENER_HOST_VERSION",
    "SCREENER_TELEMETRY_HOST_VERSION",
];
const DISTRIBUTION_ENV_VARS: &[&str] = &["SCREENER_DISTRIBUTION", "SCREENER_DIST"];
const ENTERPRISE_LICENSE_HASH_ENV_VARS: &[&str] = &["SCREENER_ENTERPRISE_LICENSE_HASH"];
const ENTERPRISE_DEVICE_ID_ENV_VARS: &[&str] = &["SCREENER_ENTERPRISE_DEVICE_ID"];

/// How this engine was launched: "desktop-app" (Tauri app), "cli" (npm/bunx),
/// or "source"/"source-dev" (built locally). The app and CLI set
/// `SCREENER_DISTRIBUTION` explicitly; without it we infer a source build
...
impl TelemetryContext {
    ...
    /// enterprise support/telemetry id). For a bare `screener` CLI run — none
    ...
    pub fn distinct_id() -> String {
        Self::distinct_id_from_env().unwrap_or_else(screener_core::sync::get_or_create_machine_id)
    }
    ...
    pub fn pairs(&self) -> Vec<(&'static str, &str)> {
        ...
        push_if_some(&mut pairs, "screener_support_id", &self.support_id);
        push_if_some(&mut pairs, "screener_customer_id", &self.customer_id);
        push_if_some(&mut pairs, "screener_deployment_id", &self.deployment_id);
        push_if_some(&mut pairs, "screener_embedder", &self.embedder);
        push_if_some(
            &mut pairs,
            "screener_embedder_version",
        ...
        push_if_some(
            &mut pairs,
            "screener_enterprise_license_hash",
            &self.enterprise_license_hash,
        );
        push_if_some(
            &mut pairs,
            "screener_enterprise_device_id",
    ...
    pub fn insert_posthog_properties(&self, properties: &mut Map<String, Value>) {
        ...
        // be split by how screener was installed (separates the free OSS engine
        // WAU from the signed app + paid users).
        let distribution = resolve_distribution();
        properties.insert(
            "screener_distribution".to_string(),
        ...
        if let Some(set_obj) = set.as_object_mut() {
            set_obj.insert("screener_distribution".to_string(), json!(distribution));
...
mod tests {
    ...
    const ALL_ENV_VARS: &[&str] = &[
        ...
        "SCREENER_ORG_ID",
        "SCREENER_TELEMETRY_CUSTOMER_ID",
        "SCREENER_DEPLOYMENT_ID",
        "SCREENER_TELEMETRY_DEPLOYMENT_ID",
        "SCREENER_EMBEDDER",
        "SCREENER_HOST_APP",
        "SCREENER_TELEMETRY_HOST_APP",
        "SCREENER_EMBEDDER_VERSION",
    ...
    fn distinct_id_prefers_existing_analytics_id() {
        with_env(
            &[
                ("SCREENER_ANALYTICS_ID", "analytics-user"),
                ("SCREENER_SUPPORT_ID", "support-user"),
            ],
            || {
                assert_eq!(
    ...
    fn support_id_can_supply_standalone_distinct_id() {
        with_env(&[("SCREENER_SUPPORT_ID", "spcust_acme_123")], || {
            assert_eq!(
                TelemetryContext::distinct_id_from_env(),
    ...
    fn distinct_id_prefers_launcher_id_over_machine_id() {
        with_env(&[("SCREENER_ANALYTICS_ID", "analytics-user")], || {
    ...
    fn posthog_properties_include_person_set_values() {
        with_env(
            &[
                ("SCREENER_SUPPORT_ID", "spcust_acme_123"),
                ("SCREENER_ORG_ID", "acme"),
                ("SCREENER_DEPLOYMENT_ID", "prod-fleet"),
                ("SCREENER_EMBEDDER", "acme-agent"),
            ],
            || {
                ...
                assert_eq!(
                    properties.get("screener_support_id"),
                    Some(&json!("spcust_acme_123"))
                );
                assert_eq!(properties.get("screener_customer_id"), Some(&json!("acme")));
                assert_eq!(
                    properties.get("screener_deployment_id"),
                    Some(&json!("prod-fleet"))
                );

                let set = properties.get("$set").and_then(|value| value.as_object());
                assert_eq!(
                    set.and_then(|value| value.get("screener_embedder")),
    ...
    fn distribution_prefers_env_then_falls_back_to_source() {
        with_env(&[("SCREENER_DISTRIBUTION", "desktop-app")], || {
    ...
    fn distribution_always_tagged_even_without_context() {
        with_env(&[("SCREENER_DISTRIBUTION", "cli")], || {
            let context = TelemetryContext::from_env();
            ...
            assert_eq!(properties.get("screener_distribution"), Some(&json!("cli")));
            let set = properties.get("$set").and_then(|value| value.as_object());
            assert_eq!(
                set.and_then(|value| value.get("screener_distribution")),
                Some(&json!("cli"))
            );
        });
    }
    ...
    fn posthog_properties_include_enterprise_tags_and_group() {
        with_env(
            &[
                ("SCREENER_ENTERPRISE_LICENSE_HASH", "ent_abc123"),
                ("SCREENER_ENTERPRISE_DEVICE_ID", "device-1"),
            ],
            || {
                let context = TelemetryContext::from_env();
                let mut properties = Map::new();
                ...
                assert_eq!(
                    properties.get("screener_enterprise_license_hash"),
                    Some(&json!("ent_abc123"))
                );
                assert_eq!(
                    properties.get("screener_enterprise_device_id"),
                    Some(&json!("device-1"))
                );
                ...
                let set = properties.get("$set").and_then(|value| value.as_object());
                assert_eq!(
                    set.and_then(|value| value.get("screener_enterprise_device_id")),
                    Some(&json!("device-1"))
                );
            },
        );
