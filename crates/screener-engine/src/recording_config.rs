...
use screener_audio::audio_manager::builder::{AudioCaptureMode, TranscriptionMode};
use screener_audio::audio_manager::AudioManagerBuilder;
use screener_audio::core::engine::AudioTranscriptionEngine;
use screener_audio::meeting_streaming::MeetingStreamingConfig;
use screener_audio::transcription::deepgram::DeepgramTranscriptionConfig;
use screener_audio::transcription::VocabularyEntry;
use screener_audio::vad::VadEngineEnum;
use screener_config::{ChannelConfig, DbConfig, SemanticContextMode};
use screener_core::Language;
use screener_screen::PipelineMetrics;
...
pub struct RecordingConfig {
    ...
    /// downloads ~108 MB from huggingface.co/screener/pii-image-redactor.
    pub async_image_pii_redaction: bool,
    /// "local" or "tinfoil" — flips both async-PII workers between
    /// on-device ONNX and the screener-hosted Tinfoil enclave.
    ...
    /// `Authorization: Bearer <SCREENER_API_KEY>`.
    pub api_auth: bool,

    /// The API key for this instance (from SCREENER_API_KEY env or auth.json).
...
impl RecordingConfig {
    ...
    pub fn from_settings(
        ...
        screener_config::set_record_while_locked(settings.record_while_locked);
        let (screener_aec_enabled, windows_input_aec_enabled, macos_input_vpio_enabled) =
        ...
        Self {
            ...
            deepgram_config: match engine_str {
                "screener-cloud" => settings
                    .effective_user_id()
                    .map(|s| DeepgramTranscriptionConfig::screener_cloud(s.to_string())),
                "deepgram" => settings
    ...
    pub fn to_audio_manager_builder(
        ...
        AudioManagerBuilder::new()
            ...
            .screener_aec_enabled(self.screener_aec_enabled)
...
mod tests {
    ...
    fn settings_with(lan: bool, api_auth: bool) -> screener_config::RecordingSettings {
    ...
    fn build(s: &screener_config::RecordingSettings) -> RecordingConfig {
        RecordingConfig::from_settings(s, std::path::PathBuf::from("/tmp/sp_test"), None)
    }

    #[test]
    fn defaults_to_loopback() {
        let c = build(&screener_config::RecordingSettings::default());
    ...
    fn semantic_context_setting_is_preserved_when_enabled() {
        let settings = screener_config::RecordingSettings {
    ...
    fn aec_mode_produces_one_effective_backend() {
        let legacy_conflict_without_mode = screener_config::RecordingSettings {
            screener_aec_enabled: false,
            windows_input_aec_enabled: true,
            macos_input_vpio_enabled: true,
            ..Default::default()
        };
        let c = build(&legacy_conflict_without_mode);
        assert!(!c.screener_aec_enabled);
        assert!(!c.windows_input_aec_enabled);
        assert!(!c.macos_input_vpio_enabled);

        let explicit_screener = screener_config::RecordingSettings {
            aec_mode: screener_config::AecMode::Screener,
            screener_aec_enabled: false,
            windows_input_aec_enabled: true,
            macos_input_vpio_enabled: true,
            ..Default::default()
        };
        let c = build(&explicit_screener);
        assert!(c.screener_aec_enabled);
        assert!(!c.windows_input_aec_enabled);
        assert!(!c.macos_input_vpio_enabled);

        let explicit_macos = screener_config::RecordingSettings {
            aec_mode: screener_config::AecMode::Macos,
            screener_aec_enabled: true,
            windows_input_aec_enabled: true,
    ...
    fn privacy_capture_toggles_flow_to_ui_recorder_config() {
        ...
        assert_eq!(
            ...
            screener_a11y::ExtractionThreadPriority::Lowest
        );
    ...
    fn click_capture_defaults_on_and_rows_follow_setting() {
        ...
        let ui = build(&screener_config::RecordingSettings::default()).to_ui_recorder_config();
        assert!(ui.capture_clicks);
        assert!(ui.record_click_events);
