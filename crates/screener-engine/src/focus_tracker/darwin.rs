...
fn monitor_for_point(
    monitors: &[screener_screen::monitor::SafeMonitor],
    x: f64,
    y: f64,
    current: Option<&MonitorIdentity>,
) -> Option<MonitorIdentity> {
...
impl Inner {
    ...
    fn resolve_and_emit(&self, monitors: &[screener_screen::monitor::SafeMonitor]) {
...
impl DarwinFocusTracker {
    pub fn start() -> Result<Self> {
        ...
        std::thread::Builder::new()
            .name("screener-focus-tracker".into())
            .spawn(move || {
        ...
        let _poll_handle = handle.spawn(async move {
            ...
            loop {
                ...
                let monitors = screener_screen::monitor::list_monitors_cached().await;
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        let activate_cb = move |_note: &ns::Notification| {
            ...
            cidre::objc::ar_pool(|| {
                ...
                let monitors =
                    futures::executor::block_on(screener_screen::monitor::list_monitors_cached());
