...
use screener_connect::mcp_servers::{
    McpAuthMode, McpHeader, McpServerConfig, McpServerStore, McpTransport,
};
use screener_core::pipes::mcp_access::McpSessionAccessRegistry;
...
async fn oauth_callback(
    ...
    if let Some(error) = query.get("error") {
        return html_response(
            ...
            &format!("screener MCP OAuth failed: {}", error),
        );
    }
    let Some(state_value) = query.get("state") else {
        return html_response(
            StatusCode::BAD_REQUEST,
            "screener MCP OAuth failed: missing state",
        );
    };
    let Some(code) = query.get("code") else {
        return html_response(
            StatusCode::BAD_REQUEST,
            "screener MCP OAuth failed: missing code",
        );
    };
    match state.store.complete_oauth(state_value, code).await {
        Ok(server_id) if server_id == id => html_response(
            StatusCode::OK,
            "screener MCP OAuth connected. You can close this tab.",
        ),
        Ok(_) => html_response(
            StatusCode::BAD_REQUEST,
            "screener MCP OAuth failed: callback server mismatch",
        ),
        Err(e) => html_response(
            StatusCode::BAD_REQUEST,
            &format!("screener MCP OAuth failed: {}", e),
        ),
    }
}
...
fn html_response(status: StatusCode, message: &str) -> Response {
    ...
    let detail = if ok {
        "screener can now use this MCP server."
    } else {
        "screener could not finish the MCP OAuth flow."
    };
    (
        status,
        [("content-type", "text/html; charset=utf-8")],
        render_oauth_result_page("screener MCP OAuth", title, detail, message, ok),
    )
        .into_response()
}
...
fn mcp_oauth_redirect_uri(
    ...
    match app_scheme {
        ...
        Some(scheme @ ("screener" | "screener-enterprise")) => Ok(format!(
            "https://screenpi.pe/api/oauth/mcp/{server_id}/callback?app_scheme={scheme}"
        )),
        Some(_) => Err("app_scheme must be screener or screener-enterprise"),
    }
}
...
mod tests {
    ...
    fn oauth_redirect_allows_only_exact_app_schemes() {
        assert_eq!(
            mcp_oauth_redirect_uri("linear", Some("screener")).unwrap(),
            "https://screenpi.pe/api/oauth/mcp/linear/callback?app_scheme=screener"
        );
        assert_eq!(
            mcp_oauth_redirect_uri("linear", Some("screener-enterprise")).unwrap(),
            "https://screenpi.pe/api/oauth/mcp/linear/callback?app_scheme=screener-enterprise"
        );
        assert!(mcp_oauth_redirect_uri("linear", Some("https")).is_err());
        assert!(mcp_oauth_redirect_uri("linear", Some("screener://evil")).is_err());
    }
