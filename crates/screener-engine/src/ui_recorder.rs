...
//!
//! Integrates screener-a11y capture with the server's recording loop.

use anyhow::Result;
use screener_a11y::{ExtractionThreadPriority, UiCaptureConfig, UiRecorder};
use screener_core::window_pattern::{self, WindowPattern};
use screener_db::{DatabaseManager, InsertUiEvent};
...
fn is_click_context_enrichment(event: &InsertUiEvent) -> bool {
    event.event_type == screener_db::UiEventType::Click
...
fn click_context_merge_candidate(candidate: &InsertUiEvent, enrichment: &InsertUiEvent) -> bool {
    candidate.event_type == screener_db::UiEventType::Click
...
impl Default for UiRecorderConfig {
    fn default() -> Self {
        Self {
            ...
            // (macOS: screener_a11y::scroll; Windows: ScrollAggregator).
...
/// from `screener_a11y::tree::TreeWalkResult` — callers translate their
...
pub async fn start_ui_recording(
    ...
    // patterns (see `screener-core::window_pattern`).
    ...
    let task_handle = tokio::spawn(async move {
        ...
        // captured events (SCREENER-CLI-FZ); this cap stops that retention from
        ...
        // ScrollBuffer::new in screener_a11y::scroll): a long sustained
        ...
        loop {
            ...
            match handle.recv_timeout(recv_timeout) {
                Some(event) => {
                    ...
                    let is_scroll = matches!(db_event.event_type, screener_db::UiEventType::Scroll);
                    let trigger_kind =
...
fn should_record_input_event(
    ...
    match db_event.event_type {
        screener_db::UiEventType::Key | screener_db::UiEventType::Text => record_keyboard_events,
        screener_db::UiEventType::Clipboard => record_clipboard_events,
        ...
        screener_db::UiEventType::Click | screener_db::UiEventType::Scroll => record_click_events,
        _ => true,
    }
}
    ...
    match db.insert_ui_events_batch(&batch.events).await {
        Ok(row_ids) => {
            ...
            *consecutive_failures = 0;
            ...
            // hole in the timeline. See SCREENER-CLI-FZ. Growth across a
        ...
        Err(e) => {
            *consecutive_failures += 1;
            ...
            // as SCREENER-CLI-SQ. A genuinely stuck coordinator is escalated
            // by the write queue itself, not by counting log lines here.
            if screener_db::is_retryable_write_stall(&e) {
...
fn capture_trigger_kind_with_ignored(
    ...
    match &db_event.event_type {
        screener_db::UiEventType::AppSwitch => {
            if is_ignored {
                None
            } else {
        ...
        screener_db::UiEventType::WindowFocus => {
            if is_ignored {
                None
            } else {
        ...
        screener_db::UiEventType::Click => Some(
            target
        ...
        screener_db::UiEventType::Clipboard => Some(CaptureTrigger::Clipboard),
        ...
        screener_db::UiEventType::Text => Some(CaptureTrigger::TypingPause),
        ...
        screener_db::UiEventType::Scroll => None,
        // Key events fire a KeyPress trigger even when privacy settings
        // suppress storing the key row.
        screener_db::UiEventType::Key => Some(CaptureTrigger::KeyPress),
...
mod event_batch_tests {
    ...
    fn click_at(x: i32, y: i32) -> InsertUiEvent {
        ...
        e.window_title = Some("screener".to_string());
        e
    }
    ...
    fn enriched_click_merges_into_pending_raw_click_and_keeps_corr_id() {
        ...
        enriched.element_ancestors = Some(r#"[{"role":"AXWindow","name":"screener"}]"#.to_string());
        ...
        assert_eq!(merged.window_title.as_deref(), Some("screener"));
        ...
        assert_eq!(
            ...
            Some(r#"[{"role":"AXWindow","name":"screener"}]"#)
        );
    }
    ...
    fn enriched_click_with_missing_context_keeps_raw_app_window() {
        ...
        assert_eq!(b.events[0].window_title.as_deref(), Some("screener"));
    ...
    fn enriched_click_replaces_approximate_context_without_losing_linkage() {
        ...
        enriched.element_ancestors = Some(r#"[{"role":"Window","name":"screener"}]"#.to_string());
        ...
        assert_eq!(
            ...
            Some(r#"[{"role":"Window","name":"screener"}]"#)
        );
    }
    ...
    /// Regression for SCREENER-CLI-FZ: a failed insert must NOT discard the
    ...
    /// SCREENER-CLI-SQ: a batch that lost a race for the SQLite write path
    ...
    fn write_path_contention_is_classified_as_a_retryable_stall() {
        let contention = sqlx::Error::Protocol(screener_db::WRITE_LOCK_HELD_MESSAGE.to_string());
        let starvation = sqlx::Error::Protocol(screener_db::WRITE_POOL_STARVED_MESSAGE.to_string());
        assert!(screener_db::is_retryable_write_stall(&contention));
        assert!(screener_db::is_retryable_write_stall(&starvation));
        ...
        assert!(
            !screener_db::is_retryable_write_stall(&real_fault),
            "a real schema/IO fault must not be treated as contention"
        );
        assert!(!screener_db::is_retryable_write_stall(
