...
//!
//! Escape hatch: `SCREENER_COMPACTION_ENCODER=software` forces libx264;
...
async fn select_uncached(ffmpeg: &Path) -> CompactionEncoder {
    ...
    if let Ok(v) = std::env::var("SCREENER_COMPACTION_ENCODER") {
        match CompactionEncoder::from_env_name(v.trim()) {
            Some(CompactionEncoder::X264) => {
                debug!("SCREENER_COMPACTION_ENCODER={v}: forcing software encode");
                return CompactionEncoder::X264;
            }
            Some(enc) => {
                debug!("SCREENER_COMPACTION_ENCODER={v}: probing it first");
                candidates.push(enc);
            }
            None => {
                warn!("SCREENER_COMPACTION_ENCODER={v} not recognized; using default order");
            }
        }
    }
...
async fn probe_encoder(ffmpeg: &Path, enc: CompactionEncoder) -> bool {
    let mut cmd = screener_core::ffmpeg_cmd_async(PathBuf::from(ffmpeg));
    cmd.args([
        "-hide_banner",
        "-loglevel",
        "error",
...
mod tests {
    ...
    async fn unspawnable_ffmpeg_falls_back_to_software() {
        let enc = select_from_candidates(
            Path::new("C:/screener-does-not-exist/ffmpeg.exe"),
