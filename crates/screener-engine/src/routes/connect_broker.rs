...
//!
//! A harness reaches screener only through an MCP server that speaks HTTP to
...
//!
//!   1. The `screener_connect_app` MCP tool POSTs `/v1/connect-request`.
...
pub struct ConnectRequestBody {
    ...
    /// The chat/conversation id (SCREENER_CHAT_SESSION_ID) so the desktop can
...
async fn next_response(
    responses: &mut (impl StreamExt<Item = screener_events::Event<Value>> + Unpin),
    request_id: &str,
    terminal_only: bool,
) -> Option<String> {
...
async fn run_connect_request(body: ConnectRequestBody, grace: Duration, wait: Duration) -> Value {
    ...
    let mut responses = screener_events::subscribe_to_event::<Value>(RESPONSE_EVENT);
...
mod tests {
    ...
    fn answer_next_request(
        ...
        let mut requests = screener_events::subscribe_to_event::<Value>(REQUEST_EVENT);
