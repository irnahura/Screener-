...
use screener_db::{Speaker, TagAutocompleteItem, TagContentType};
    ...
    if let Some(ocr_results) = &frame.ocr_results {
        for ocr in ocr_results {
            db.insert_ocr_text(
                ...
                Arc::new(OcrEngine::default().into()), // Ideally could pass any str as ocr_engine since can be run outside of screener
            )
            .await?;
        }
    }
    ...
    match payload.content.content_type.as_str() {
        "frames" => {
            if let ContentData::Frames(frames) = &payload.content.data {
                if !frames.is_empty() {
                    let output_dir = state.screener_dir.join("data");
                    let time = Utc::now();
...
pub(crate) async fn merge_frames_handler(
    ...
    let output_dir = state.screener_dir.join("videos");
