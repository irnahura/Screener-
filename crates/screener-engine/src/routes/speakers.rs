...
use screener_db::{ReassignScope, Speaker};
...
pub(crate) async fn get_speaker_sample_handler(
    ...
    let (samples, sample_rate) = screener_audio::utils::ffmpeg::read_audio_from_file(Path::new(
        &chunk.file_path,
    ))
    .map_err(|e| {
        (
...
mod tests {
    ...
    fn local_audio_path_validation_rejects_empty_placeholders() {
        ...
        assert!(!is_valid_local_audio_path(" . "));
        assert!(is_valid_local_audio_path(
            r"C:\Users\me\.screener\data\audio.mp4"
        ));
    }
