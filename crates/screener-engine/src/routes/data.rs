...
/// Screener is running — the backup is a consistent snapshot.
#[oasgen]
...
pub(crate) async fn compact_handler(
    ...
    let bytes_before: u64 = database_files
        .iter()
        .map(|file_name| size_of(&state.screener_dir.join(file_name)))
        .sum();
    let required_free_space = compact_required_free_space(bytes_before);
    let available_free_space = available_space_for_path(&state.screener_dir);
