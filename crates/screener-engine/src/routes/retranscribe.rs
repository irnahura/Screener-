...
use screener_audio::transcription::engine::TranscriptionEngine;
use screener_audio::transcription::VocabularyEntry;
use screener_db::{AudioChunkInfo, NewMeetingTranscriptSegment};
...
pub struct MeetingRetranscribeRequest {
    /// Optional engine override: "deepgram", "screener-cloud", "parakeet", etc.
    pub engine: Option<String>,
    /// Custom vocabulary for this re-transcription.
...
pub async fn retranscribe_handler(
    ...
    for chunk in &chunks {
        ...
        let (samples, sample_rate) =
            match screener_audio::utils::ffmpeg::read_audio_from_file(file_path) {
                Ok(result) => result,
                Err(e) => {
...
pub async fn retranscribe_meeting_handler(
    ...
    for batch in batches {
        ...
        for chunk in batch {
            ...
            match tokio::task::spawn_blocking(move || {
                screener_audio::utils::ffmpeg::read_audio_from_file(StdPath::new(&path_owned))
            })
            .await
            {
                Ok(Ok((samples, sr))) => {
    ...
    let summary_refresh_event_emitted = match screener_events::send_event(
