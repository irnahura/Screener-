...
pub(crate) async fn list_live_views_handler(
    ...
    list_live_views(&state.screener_dir)
        .map(Json)
        .map_err(api_error)
}
...
pub(crate) async fn live_view_catalog_handler(
    ...
    reject_pipe_token(&permissions)?;
    list_live_view_templates(&state.screener_dir)
        .map(|templates| {
            templates
                .into_iter()
...
pub(crate) async fn get_live_view_template_handler(
    ...
    reject_pipe_token(&permissions)?;
    get_live_view_template(&state.screener_dir, &id)
        .map(Json)
        .map_err(api_error)
}
...
pub(crate) async fn save_live_view_handler(
    ...
    save_live_view(&state.screener_dir, request)
        .map(Json)
        .map_err(api_error)
}
...
pub(crate) async fn delete_live_view_handler(
    ...
    reject_pipe_token(&permissions)?;
    delete_live_view(&state.screener_dir, &id).map_err(api_error)?;
