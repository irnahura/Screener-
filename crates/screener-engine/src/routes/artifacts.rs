...
// (see screener-db OutputRecord); everything HTTP-facing says "artifacts".
...
fn build_output_path(
    ...
    Some(screener_dir.join("outputs").join(st).join(src).join(fname))
}
...
async fn mark_output_search_processed(
    db: &screener_db::DatabaseManager,
    output_id: i64,
    title: &str,
    source: &str,
    source_type: &str,
...
async fn index_output_record_for_search(db: &screener_db::DatabaseManager, record: &OutputRecord) {
...
pub fn spawn_artifact_search_backfill(db: Arc<screener_db::DatabaseManager>) {
...
// ---------------------------------------------------------------------------
// SAF (Screener Artifact Format) detection
//
...
// apps/screener-app-tauri/lib/saf.ts — keep them in sync.
// ---------------------------------------------------------------------------
...
/// file under `~/.screener/outputs/`. Filesystem-derived pipe artifacts
...
/// `~/.screener/outputs/`) with artifacts derived from pipe directories
...
/// centralized `~/.screener/outputs/` directory and the DB.
///
...
pub async fn auto_register_pipe_artifacts(
    db: &screener_db::DatabaseManager,
    items: Vec<(
        screener_core::pipes::ArtifactDeclaration,
        std::path::PathBuf,
    )>,
    pipe_name: &str,
...
mod tests {
    ...
    fn build_output_path_rejects_traversal() {
        let root = std::path::Path::new("/fake/screener");
    ...
    fn build_output_path_normal() {
        ...
        assert_eq!(
            ...
            Some(std::path::PathBuf::from(
                "/fake/screener/outputs/pipe/my-pipe/report.txt"
            ))
        );
    }
    ...
    fn artifact_query_matches_path_and_original_path() {
        let item = ArtifactItem {
            ...
            kind: "html".to_string(),
            path: "/Users/test/.screener/outputs/pipe/focus-pipe/relatorio-foco-offline.html"
                .to_string(),
            original_path: Some(
                "/Users/test/.screener/pipes/focus-pipe/output/relatorio-foco.html".to_string(),
            ),
            size_bytes: 1024,
    ...
    // Mirrored by apps/screener-app-tauri/lib/saf.ts (parseSafArtifact);
    ...
    async fn setup_db() -> screener_db::DatabaseManager {
        let db = screener_db::DatabaseManager::new("sqlite::memory:", Default::default())
            .await
            .unwrap();
        sqlx::migrate!("../../crates/screener-db/src/migrations")
            .run(&db.pool)
            .await
            .unwrap();
        db
    ...
    fn decl(path: &str) -> screener_core::pipes::ArtifactDeclaration {
        screener_core::pipes::ArtifactDeclaration {
            path: path.to_string(),
            title: None,
            kind: None,
