...
use screener_audio::core::device::DeviceType;
use screener_events::{send_event, subscribe_to_all_events, Event as ScreenerEvent};
    ...
    let incoming = lifecycle.spawn(async move {
        while let Some(Ok(msg)) = receiver.next().await {
            if let Message::Text(t) = msg {
                if let Ok(event) = serde_json::from_str::<ScreenerEvent>(&t) {
...
async fn handle_meeting_overlay_socket(
    ...
    let mut status_stream =
        screener_events::subscribe_to_event::<MeetingStatusResponse>("meeting_status_changed");
    let mut delta_stream =
        screener_events::subscribe_to_event::<serde_json::Value>("meeting_transcript_delta");
    let mut final_stream =
        screener_events::subscribe_to_event::<serde_json::Value>("meeting_transcript_final");
