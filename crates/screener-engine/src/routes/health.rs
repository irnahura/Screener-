...
impl VisionStallCause {
    ...
    /// [`screener_screen::monitor::secs_since_capture_backend_fallback`].
...
/// screener itself turned them off was sent to macOS System Settings to fix
    ...
    pub(crate) fn instruction(self) -> Option<&'static str> {
        match self {
            ...
            Self::DisabledBySetting => {
                Some("Screen capture is turned off in screener. Turn it back on in Settings → Recording.")
            }
            ...
            Self::ScreenshotsDisabledByConfig => {
                Some("Screenshots are turned off in screener, so only on-screen text is being captured. Re-enable them in Settings → Recording. This is not a permission problem.")
            }
            ...
            Self::PermissionDenied => {
                Some("macOS is blocking screen capture. Grant Screen Recording to screener in System Settings → Privacy & Security, then restart the app.")
            }
            Self::CaptureStalled => {
                Some("Screen capture stopped producing frames. Restarting screener usually clears it; if it returns, please send logs from the Help section.")
            }
            Self::NotStarted => {
...
pub(crate) fn classify_vision_reason(
    ...
    screenshot_state: screener_screen::ScreenshotCaptureState,
    permission_granted: bool,
    frame_status: &str,
) -> VisionReason {
    use screener_screen::ScreenshotCaptureState as S;
...
pub struct HealthCheckResponse {
    ...
    ///
    /// `frame_status` alone collapses "screener turned pixels off" and "the
...
async fn health_check_inner(state: &Arc<AppState>) -> HealthCheckResponse {
    ...
    for device in &audio_devices {
        ...
        let last_capture = screener_audio::core::get_device_capture_time(&device_name);
        let exact_last_capture =
            screener_audio::core::get_device_capture_time_exact(&device_name).unwrap_or(0);
    ...
    if audio_devices.is_empty() {
        let last_capture = screener_audio::core::LAST_AUDIO_CAPTURE.load(Ordering::Relaxed);
    ...
    let threshold_secs = {
        #[cfg(debug_assertions)]
        {
            if crate::event_driven_capture::e2e_capture_loop_silent_fault_started() {
                ...
                match std::env::var("SCREENER_E2E_SEED").ok() {
    ...
    let audio_paused_for_screen_lock =
        screener_config::should_pause_audio_for_lock() && audio_devices.is_empty();
...
) -> JsonResponse<screener_screen::MetricsSnapshot> {
...
pub(crate) async fn audio_metrics_handler(
    State(state): State<Arc<AppState>>,
) -> JsonResponse<screener_audio::metrics::AudioMetricsSnapshot> {
...
pub(crate) fn get_verbose_instructions(
    ...
    if unhealthy_systems.contains(&"vision") {
        ...
        // System Settings even when screener had turned pixels off itself.
...
/// state, including the ones screener itself caused. These pin the reason
...
mod vision_reason_tests {
    ...
    /// The reported case: permission is fine, screener disabled screenshots
    ...
    fn verbose_instructions_name_the_cause_instead_of_permission() {
        ...
        assert!(config.contains("Screenshots are turned off in screener"));
