...
use screener_db::{ContentType, Order, SearchResult, SemanticContextQuery, SemanticFrameContext};
use screener_semantic::{IdentityQuality, SemanticKind};
...
/// because it belongs to screener itself.
pub fn is_screener_app(app_name: &str) -> bool {
    const SCREENER_APP_NEEDLE: &[u8] = b"screener";
    app_name
        .as_bytes()
        .windows(SCREENER_APP_NEEDLE.len())
        .any(|window| window.eq_ignore_ascii_case(SCREENER_APP_NEEDLE))
}
...
/// text-bearing fields.
///
/// Shared with the `screener search` CLI so terminal output matches the
...
fn classified_search_database_response(error: &sqlx::Error) -> Option<Response<Body>> {
    ...
    } else if screener_db::is_sqlite_interrupt(error) {
...
pub(crate) async fn search(
    ...
    let mut content_items: Vec<ContentItem> = match results {
        SearchPage::Standard(results) => results
            .iter()
            // Filter out screener results at display time
            .filter(|result| match result {
                SearchResult::OCR(ocr) => !is_screener_app(&ocr.app_name),
                SearchResult::Audio(_) => true, // Audio doesn't have app_name
                SearchResult::UI(ui) => !is_screener_app(&ui.app_name),
                SearchResult::Input(input) => input
                    .app_name
                    .as_ref()
                    .is_none_or(|app| !is_screener_app(app)),
        ...
        SearchPage::Parsed(contexts) => contexts
            .into_iter()
            .filter(|context| !is_screener_app(&context.app_name))
            .map(|context| {
...
pub(crate) async fn keyword_search_handler(
    ...
    if query.group {
        ...
        let filtered: Vec<_> = groups
            .into_iter()
            .filter(|group| !is_screener_app(&group.representative.app_name))
            .collect();
        ...
        let filtered: Vec<_> = matches
            .into_iter()
            .filter(|m| !is_screener_app(&m.app_name))
            .collect();
...
mod tests {
    use super::*;
    use screener_core::pipes::permissions::PermissionRule;
    ...
    fn ocr_text_json_is_not_observable_in_search_response() {
        ...
        let make_result = |text_json: &str| {
            SearchResult::OCR(screener_db::OCRResult {
                frame_id: 42,
    ...
    fn accessibility_event_source_is_observable_in_search_response() {
        let result = SearchResult::UI(screener_db::UiContent {
            id: 42,
    ...
    fn test_group_related_tags_buckets_by_namespace() {
        ...
        let rows = vec![
            ...
            ("project:screener".to_string(), 7),
            ("person:connor".to_string(), 5),
            ("workflow:ai-coding".to_string(), 4),
            ("project:screener-finance".to_string(), 3),
    ...
    #[test]
    fn test_is_screener_app_case_insensitive() {
        assert!(is_screener_app("screener"));
        assert!(is_screener_app("Screener Desktop"));
        assert!(is_screener_app("com.screener.capture"));
        assert!(!is_screener_app("pipe viewer"));
    }
