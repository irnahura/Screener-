...
use screener_db::FeedbackRecord as DbFeedbackRecord;
...
pub(crate) async fn assigned_targets_handler(
    ...
    let targets = targets_for_pipe(&state.screener_dir, pipe_name).map_err(api_error)?;
...
pub(crate) async fn submit_structured_output_handler(
    ...
    let values_dir = state.screener_dir.join("structured-outputs").join("values");
    ...
    let target =
        commit_output_submission(&state.screener_dir, &prepared, artifact.id).map_err(api_error)?;
...
pub(crate) async fn set_structured_output_feedback_handler(
    ...
    if let Some(rating) = rating {
        let target = crate::structured_outputs::list_output_targets(&state.screener_dir)
            .map_err(api_error)?
            .into_iter()
