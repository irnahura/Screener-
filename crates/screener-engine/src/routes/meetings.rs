...
use screener_db::{MeetingRecord, MeetingTranscriptSegment, MEETING_END_REASON_EXPLICIT_STOP};
...
pub fn emit_meeting_status_changed(status: &MeetingStatusResponse) {
    ...
    if let Err(e) = screener_events::send_event("meeting_status_changed", status.clone()) {
...
/// Same `meeting_id` XOR `start`/`end` contract as the `screener export` CLI
...
/// the `screener export` CLI — same `meeting_id` XOR `start`/`end` contract.
#[oasgen]
pub(crate) async fn export_handler(
    ...
    // meeting_id XOR start/end, same contract as the `screener export` CLI.
...
mod tests {
    ...
    async fn claim_test_db() -> (tempfile::TempDir, DatabaseManager) {
        ...
        sqlx::migrate!("../screener-db/src/migrations")
            .run(writer.pool())
            .await
            .unwrap();
        drop(writer);
