...
//! `unset SCREENER_API_KEY` (see screener-core::agents::bash_env) so the
...
//! api.screener.com with the user's cloud JWT held in AppState.
...
const CLOUD_BASE_URL: &str = "https://api.screener.com";
...
pub async fn chat_completions(
    ...
    let Some(token) = (**token).clone().filter(|t| !t.is_empty()) else {
        return (
            ...
            r#"{"error":"cloud_token_missing","message":"sign in to screener to use cloud media analysis"}"#,
        )
            .into_response();
    };
