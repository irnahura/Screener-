...
use screener_db::{OcrTextBlock, TextPosition};
...
fn thumbnail_response(
    ...
    Response::builder()
        ...
        .header("x-screener-thumbnail-cache", result.cache_status)
        .header("x-screener-thumbnail-width", result.value.width)
        .header("x-screener-thumbnail-height", result.value.height)
...
/// Get frame metadata (timestamp) for deep link navigation. screener://frame/123 → resolve to timestamp.
#[oasgen]
    ...
    if text_positions.is_empty() {
        if let Ok((_, Some(tree_json))) = state.db.get_frame_accessibility_data(frame_id).await {
            if let Some(query) = &params.query {
                ...
                text_positions = screener_db::find_matching_a11y_positions(&tree_json, query);
...
pub async fn run_frame_ocr(
    ...
    let ocr_permit = screener_capture::ocr_semaphore()
        .acquire()
        .await
    ...
    let ocr_result = tokio::task::spawn_blocking(move || {
        ...
        let (text, json, _confidence) = screener_screen::perform_ocr_apple(&image, &[]);
        (text, json)
    })
    .await
    ...
    let ocr_result = {
        ...
        let _permit = ocr_permit;
        let _watch = OcrInflightGuard::enter(frame_id);
        match screener_screen::perform_ocr_windows(&image, &[]).await {
    ...
    let ocr_result = tokio::task::spawn_blocking(move || {
        let _permit = ocr_permit;
        let _watch = OcrInflightGuard::enter(frame_id);
        let (text, json, _confidence) = screener_screen::perform_ocr_tesseract(&image, vec![]);
        (text, json)
    })
    .await
    .unwrap_or_else(|_| (String::new(), "[]".to_string()));
    ...
    let text_positions = screener_db::parse_all_text_positions(&blocks);
...
async fn persist_on_demand_ocr(
    ...
    #[cfg(target_os = "macos")]
    let engine = Arc::new(screener_db::OcrEngine::AppleNative);
    #[cfg(target_os = "windows")]
    let engine = Arc::new(screener_db::OcrEngine::WindowsNative);
    #[cfg(not(any(target_os = "macos", target_os = "windows")))]
    let engine = Arc::new(screener_db::OcrEngine::Tesseract);
    if let Err(e) = db
...
mod thumbnail_tests {
        ...
        let responses = join_all(uris.into_iter().map(|uri| {
            ...
            async move {
                ...
                let cache_hit = response
                    .headers()
                    .get("x-screener-thumbnail-cache")
    ...
    ///
    /// Run with:
    /// SCREENER_BENCH_DB=/path/to/db.sqlite cargo test -p screener-engine
    ...
    async fn benchmark_real_frame_thumbnail_workload() {
        let source_db = PathBuf::from(
            std::env::var_os("SCREENER_BENCH_DB")
                .expect("set SCREENER_BENCH_DB to a local screener db.sqlite"),
        );
