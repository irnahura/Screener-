...
pub async fn vault_lock(
    ...
    Err((
        StatusCode::CONFLICT,
        JsonResponse(json!({
            "error": "online vault locking is disabled because the running server owns live SQLite connections; stop screener and run `screener vault lock`"
        })),
    ))
}
...
pub async fn vault_unlock(
    ...
    match state.vault.unlock(&body.password).await {
        ...
        Err(e) => {
            let status = match &e {
                screener_vault::VaultError::WrongPassword => StatusCode::FORBIDDEN,
