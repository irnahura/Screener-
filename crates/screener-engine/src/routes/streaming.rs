...
use screener_db::{DatabaseManager, FrameData, Order};
...
pub(crate) fn create_time_series_frame(chunk: FrameData) -> TimeSeriesFrame {
    ...
    let mut device_frames: Vec<DeviceFrame> = chunk
        .ocr_entries
        .into_iter()
        // Filter out screener frames at display time
        .filter(|device_data| !is_screener_app(&device_data.app_name))
                ...
                result = frame_rx_cache.recv() => {
                    match result {
                        Ok(hot_frame) => {
                            ...
                            // Skip screener's own frames
                            if is_screener_app(&hot_frame.app_name) {
                                continue;
                            }
...
mod tests {
    use super::*;
    use screener_db::{AudioEntry as DbAudioEntry, FrameData, OCREntry, OcrEngine};
    ...
    #[test]
    fn test_create_time_series_frame_filters_screener_app_names() {
        let mut frame_data = create_test_frame_data(2, 0);
        frame_data.ocr_entries[0].app_name = "SCREENER Desktop".to_string();
