...
/// Also updates the shared flag in screener-config so other crates (e.g. audio) can read it.
pub fn set_screen_locked(locked: bool) {
    SCREEN_IS_LOCKED.store(locked, Ordering::SeqCst);
    screener_config::set_screen_locked(locked);
}
...
fn handle_screen_unlock_transition() {
    screener_screen::stream_invalidation::request();
    // Displays are routinely attached, detached or rearranged while locked.
    screener_screen::monitor::invalidate_monitor_lookup_cache();
    crate::permission_monitor::notify_wake();
...
pub fn start_sleep_monitor() {
    ...
    std::thread::spawn(|| {
        ...
        unsafe extern "C" fn on_display_reconfigured(
            ...
            #[cfg(target_os = "macos")]
            screener_screen::monitor::invalidate_monitor_lookup_cache();
            screener_audio::stream_invalidation::request();
...
fn on_will_sleep() {
    ...
    screener_db::request_write_pause();
...
fn on_did_wake(handle: &tokio::runtime::Handle) {
    ...
    handle.spawn(async {
        ...
        screener_db::request_write_resume();
...
mod tests {
    ...
    async fn test_on_did_wake_sets_audio_invalidation() {
        let _guard = RECENTLY_WOKE_TEST_LOCK
            ...
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        // Clear stale flags
        let _ = screener_audio::stream_invalidation::take();
        let _ = screener_screen::stream_invalidation::take();
        reset_recently_woke();
        ...
        assert!(
            screener_audio::stream_invalidation::take(),
            "Audio stream invalidation flag must be set after wake"
        );
        assert!(
            screener_screen::stream_invalidation::take(),
    ...
    /// Run with: cargo test -p screener-engine --lib -- test_screen_lock_unlock --ignored --nocapture
    #[cfg(target_os = "macos")]
    #[tokio::test]
