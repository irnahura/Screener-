...
//! (screener-audio `SttPriorityDip`), and the snapshot-compaction ffmpeg
...
//! threads are pinned HIGHEST in screener-a11y so the system-wide input
...
//!
//! Escape hatch: set `SCREENER_KEEP_NORMAL_PRIORITY=1` to skip the drop —
//! e.g. an embedder that spawns `screener record` as a child and manages
//! priority itself (our call would otherwise override its choice).

/// True when `SCREENER_KEEP_NORMAL_PRIORITY` opts out of the priority drop
...
pub fn set_background_priority() {
    ...
    let env_value = std::env::var("SCREENER_KEEP_NORMAL_PRIORITY").ok();
    if keep_normal_requested(env_value.as_deref()) {
        info!("keeping Normal process priority (SCREENER_KEEP_NORMAL_PRIORITY set)");
        return;
    }
        ...
        match SetPriorityClass(GetCurrentProcess(), BELOW_NORMAL_PRIORITY_CLASS) {
            Ok(()) => info!(
                "process priority set to BelowNormal — engine yields CPU to foreground apps \
                 (opt out with SCREENER_KEEP_NORMAL_PRIORITY=1)"
            ),
...
mod tests {
    ...
    fn drops_process_to_below_normal() {
        ...
        let saved = std::env::var("SCREENER_KEEP_NORMAL_PRIORITY").ok();
        std::env::remove_var("SCREENER_KEEP_NORMAL_PRIORITY");
        ...
        if let Some(v) = saved {
            std::env::set_var("SCREENER_KEEP_NORMAL_PRIORITY", v);
        }
    }
}
