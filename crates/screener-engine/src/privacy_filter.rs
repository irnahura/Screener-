...
/// `SCREENER_PRIVACY_FILTER_ENCLAVE` / `SCREENER_PRIVACY_FILTER_REPO`,
/// defaulting to the screener-published enclave + its source repo so that
...
pub fn global() -> Arc<PrivacyFilter> {
    INSTANCE
        .get_or_init(|| {
            let enclave = std::env::var("SCREENER_PRIVACY_FILTER_ENCLAVE")
                .unwrap_or_else(|_| DEFAULT_ENCLAVE.to_string());
            let repo = std::env::var("SCREENER_PRIVACY_FILTER_REPO")
