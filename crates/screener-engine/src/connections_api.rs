...
use screener_connect::connections::{bee, build_default_client, telegram, ConnectionManager};
use screener_connect::oauth::{self as oauth_store, OAuthCallbackResult, PENDING_OAUTH};
use screener_connect::whatsapp::WhatsAppGateway;
use screener_secrets::SecretStore;
...
use screener_connect::connections::browser::{BrowserRegistry, BrowserSummary, EvalError};
...
/// `apps/screener-app-tauri/components/settings/connections-section.tsx`.
///
...
async fn mcp_oauth_connected_ids(
    screener_dir: &std::path::Path,
    secret_store: Option<Arc<SecretStore>>,
) -> std::collections::HashSet<String> {
    use screener_connect::mcp_servers::McpServerStore;
    let mut connected = std::collections::HashSet::new();
    let store = McpServerStore::new(screener_dir.to_path_buf(), secret_store);
...
async fn list_connections(State(state): State<ConnectionsState>) -> Json<Value> {
    ...
    let wa_connected = matches!(
        wa_status,
        screener_connect::whatsapp::WhatsAppStatus::Connected { .. }
    );
    ...
    let mcp_connected_ids =
        mcp_oauth_connected_ids(&state.screener_dir, state.secret_store.clone()).await;
    ...
    if let Some(arr) = data.as_array_mut() {
        ...
        let (ics_feed_count, ics_enabled_count, ics_error) =
            match screener_connect::ics_calendar::load_ics_calendar_settings_from_store(
                &state.screener_dir,
            ) {
                Ok(settings) => {
...
async fn get_connection(
    ...
    let (status, body) = match mgr.get_credentials(&id).await {
        Ok(Some(creds)) => {
            let safe = mgr
                .find_def(&id)
                .map(|def| screener_connect::connections::safe_connection_config(def, &creds))
                .unwrap_or_default();
            (
                StatusCode::OK,
...
async fn list_instances(
    ...
    let mgr = state.cm.lock().await;
    let is_oauth = screener_connect::connections::all_integrations()
        .iter()
        .find(|i| i.def().id == id)
        ...
        Ok(instances) => {
            if let Some(def) = mgr.find_def(&id) {
                for (inst, conn) in instances {
                    let safe = screener_connect::connections::safe_connection_config(
                        def,
                        &conn.credentials,
                    );
...
enum NativeCalendarError {
    ...
    /// Calendar exists but screener lacks permission (macOS TCC).
...
fn get_native_calendar_events(
    ...
    use screener_connect::calendar::{CalendarError, ScreenerCalendar};
    let cal = ScreenerCalendar::new();
    let events = cal
        .get_events(hours_back, hours_ahead)
        .map_err(|e| match e {
            CalendarError::AuthorizationDenied => NativeCalendarError::AuthRequired(
                "calendar access not granted — allow screener under System Settings > \
...
fn get_native_calendar_events(
    ...
    use screener_connect::calendar_windows::ScreenerCalendar;
    ...
    let cal = ScreenerCalendar::new().map_err(NativeCalendarError::StoreUnavailable)?;
    let events = cal
        .get_events(hours_back, hours_ahead)
...
fn is_native_calendar_available() -> bool {
    use screener_connect::calendar::ScreenerCalendar;
...
#[cfg(target_os = "windows")]
fn is_native_calendar_available() -> bool {
    screener_connect::calendar_windows::ScreenerCalendar::is_available()
}
...
fn ics_feed_summaries(
    settings: &screener_connect::ics_calendar::IcsCalendarSettings,
) -> Vec<Value> {
    settings
        .entries
        .iter()
...
async fn ics_calendar_status(State(state): State<ConnectionsState>) -> (StatusCode, Json<Value>) {
    match screener_connect::ics_calendar::load_ics_calendar_settings_from_store(&state.screener_dir)
    {
        Ok(settings) => {
            let enabled_count = settings
                .entries
...
async fn ics_calendar_events(
    ...
    let settings = match screener_connect::ics_calendar::load_ics_calendar_settings_from_store(
        &state.screener_dir,
    ) {
    ...
    let events = screener_connect::ics_calendar::fetch_ics_calendar_events(
        &client,
        &enabled,
...
async fn imap_messages(
    ...
    match screener_connect::connections::imap::list_messages(
        &creds,
        mailbox,
        limit,
        q.query.as_deref(),
...
async fn imap_message(
    ...
    let mailbox = q.mailbox.as_deref().unwrap_or("INBOX");
    match screener_connect::connections::imap::get_message(&creds, mailbox, uid).await {
...
async fn imap_mailboxes(State(state): State<ConnectionsState>) -> (StatusCode, Json<Value>) {
    ...
    match screener_connect::connections::imap::list_mailboxes(&creds).await {
...
async fn oauth_callback(Query(params): Query<OAuthCallbackQuery>) -> (StatusCode, Html<String>) {
    if let Some(err) = params.error {
        ...
        return oauth_callback_page(
            StatusCode::BAD_REQUEST,
            "Connection failed",
            "screener could not finish the OAuth flow.",
            &err,
        );
    }
    ...
    let (code, state) = match (params.code, params.state) {
        ...
        _ => {
            ...
            return oauth_callback_page(
                StatusCode::BAD_REQUEST,
                "Invalid callback",
                "screener could not verify this authorization response.",
        ...
        Some(pending) => {
            ...
            if delivered {
                ...
                oauth_callback_page(
                    StatusCode::OK,
                    "Connected",
                    "screener can now use this connection.",
                    "You can close this tab and return to screener.",
                )
            } else {
                ...
                oauth_callback_page(
                    StatusCode::BAD_REQUEST,
                    "Sign-in expired",
                    "This sign-in took a while, so screener stopped waiting.",
                    "Open screener and click connect again — a fresh sign-in stays valid for 10 minutes.",
                )
            }
        }
        None => {
            ...
            oauth_callback_page(
                ...
                "If screener already shows the connection, you can close this tab. Otherwise click connect in the app to try again.",
            )
        }
    }
}
...
fn resolve_auth(
    proxy_auth: &screener_connect::connections::ProxyAuth,
    creds: Option<&Map<String, Value>>,
    oauth_token: Option<&str>,
    oauth_extras: Option<&Value>,
) -> ResolvedAuth {
    use screener_connect::connections::ProxyAuth;
    match proxy_auth {
...
async fn connection_proxy(
    ...
    let creds = mgr
        ...
        .flatten();
    let oauth_json =
        screener_connect::oauth::load_oauth_json(state.secret_store.as_deref(), &id, instance_ref)
            .await;
    ...
    let oauth_token = screener_connect::oauth::get_valid_token_instance(
        state.secret_store.as_deref(),
        &http_client,
        &id,
        instance_ref,
    ...
    if matches!(auth, ResolvedAuth::None)
        && !matches!(
            proxy_cfg.auth,
            screener_connect::connections::ProxyAuth::None
        )
    {
        tracing::warn!(
        ...
        let has_oauth_state =
            !screener_connect::oauth::list_oauth_instances(state.secret_store.as_deref(), &id)
                .await
                .is_empty();
        let error = if has_oauth_state {
            let display_name = mgr.find_def(&id).map(|d| d.name).unwrap_or(id.as_str());
            screener_connect::oauth::describe_oauth_error(
                state.secret_store.as_deref(),
                &id,
                display_name,
                instance_ref,
...
async fn browser_run_navigate(
    ...
    // `screener-core::agents::bash_env`) as `x-screener-session`. It rides
...
/// `x-screener-session` owner tag as the dedicated `/navigate` endpoint, or
...
/// `browser_run_navigate` and `screener-core::agents::bash_env`.
...
// (protocol lives in screener_connect::connections::bee)
// ---------------------------------------------------------------------------
...
mod tests {
    use super::*;
    use screener_connect::connections::{lexi::Lexi, mochi::Mochi, Integration, ProxyAuth};
    use serde_json::json;

    use axum::body::{to_bytes, Body};
    use axum::http::{header, Request};
    use screener_connect::connections::ConnectionManager;
    use screener_connect::whatsapp::WhatsAppGateway;
    ...
    fn ics_test_router(dir: &TempDir) -> Router<()> {
        let screener_dir = dir.path().to_path_buf();
        let cm = Arc::new(Mutex::new(ConnectionManager::new(
            screener_dir.clone(),
            None,
        )));
        let wa = Arc::new(Mutex::new(WhatsAppGateway::new(screener_dir.clone())));
        router(
            cm,
            wa,
            screener_dir,
    ...
    fn ics_feed_summaries_never_include_urls() {
        let settings = screener_connect::ics_calendar::IcsCalendarSettings {
            entries: vec![screener_connect::ics_calendar::IcsCalendarEntry {
                name: "Work".to_string(),
    ...
    fn test_all_proxy_configs_have_valid_base_urls() {
        use screener_connect::connections::all_integrations;
    ...
    /// can prove the `x-screener-session` header reaches the browser.
    struct OwnerRecordingBrowser {
        last_owner: Arc<Mutex<Option<String>>>,
    }

    #[async_trait::async_trait]
    impl screener_connect::connections::browser::Browser for OwnerRecordingBrowser {
        fn id(&self) -> &str {
            "owned-default"
        }
        fn name(&self) -> &str {
        ...
        async fn eval(
            ...
            screener_connect::connections::browser::EvalResult,
            screener_connect::connections::browser::EvalError,
        > {
            ...
            *self.last_owner.lock().await = None;
            Ok(screener_connect::connections::browser::EvalResult {
                ok: true,
    ...
    /// the `x-screener-session` owner header the agent's curl shim injects.
        ...
        let response = app
            .oneshot(
                Request::builder()
                    ...
                    .header(header::CONTENT_TYPE, "application/json")
                    .header("x-screener-session", "pipe:reddit-poster")
                    .body(Body::from(
    ...
    use screener_connect::oauth::PendingOAuth;
