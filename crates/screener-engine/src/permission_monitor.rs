...
//! `screener-events` bus when the OS permission state changes.
//!
//! ## Design
//!
...
use screener_core::permissions::{check_permissions, PermissionStatus};
...
fn keychain_accessible() -> bool {
    use screener_secrets::keychain::{get_key, is_keychain_available, KeyResult};
    ...
    if !screener_secrets::is_encryption_requested(&screener_core::paths::default_screener_data_dir())
    {
        return true;
    }
    match get_key() {
