...
//! the `screener` CLI, `screener auth token`, the MCP bridge — must go
...
pub async fn resolve_api_auth_key(data_dir: &Path, settings_key: Option<&str>) -> Result<String> {
    ...
    let (key, source) = if let Ok(k) = std::env::var("SCREENER_API_KEY") {
        if !k.is_empty() {
            (k, "SCREENER_API_KEY env")
        } else {
    ...
    // reader (running server, MCP, `screener auth token` CLI) agrees on
    ...
    // rotating the user's API key (SCREENER-APP-9Z: 25 events / 18 users,
...
async fn open_secret_store(data_dir: &Path) -> Result<screener_secrets::SecretStore> {
    ...
    screener_secrets::SecretStore::open_for_data_dir_with_vault_key(data_dir).await
}

/// Path to the pre-SecretStore `~/.screener/auth.json`, but **only** when this
/// process is actually running against `~/.screener`.
///
/// The legacy file belongs to whichever instance owns the legacy data dir. An
/// instance pointed elsewhere by `SCREENER_DATA_DIR` (a dev build, a relocated
...
fn legacy_auth_json_path() -> Option<PathBuf> {
    let legacy_dir = dirs::home_dir()?.join(".screener");
...
/// be tested without mutating process-global `SCREENER_DATA_DIR`.
fn legacy_auth_json_path_for(legacy_dir: &Path, active_dir: &Path) -> Option<PathBuf> {
    // Compare canonically where possible so `~/.screener` and a symlinked or
...
pub async fn find_api_auth_key() -> Option<String> {
    if let Ok(k) = std::env::var("SCREENER_API_KEY") {
        if !k.is_empty() {
            return Some(k);
        }
    }
    ...
    // was right there.
    for var in ["SCREENER_LOCAL_API_KEY", "SCREENER_API_AUTH_KEY"] {
...
// ---------------------------------------------------------------------------
// Cloud auth token (Clerk JWT — the Bearer for api.screener.com).
//
...
// `db.sqlite`) must read it there too — otherwise a standalone `screener`
// run would lose cloud auth (STT, screener-cloud pipes) once the app has
// migrated the token out of store.bin.
// ---------------------------------------------------------------------------

/// SecretStore key for the cloud auth token. MUST match the desktop app's
/// `auth_token` module (`apps/screener-app-tauri/src-tauri/src/auth_token.rs`).
const CLOUD_AUTH_TOKEN_KEY: &str = "cloud.auth_token";

/// Clerk cloud-session tokens are JWTs. The local HTTP bearer returned by
/// `screener auth token` is deliberately not accepted here: both credentials
/// historically used `SCREENER_API_KEY`, and treating the local `sp-*` key as
...
pub(crate) fn is_cloud_session_token(value: &str) -> bool {
    (value.starts_with("eyJ") && value.matches('.').count() == 2)
        || (std::env::var_os("SCREENER_E2E_SEED").is_some() && value.starts_with("e2e-fake-token-"))
}
...
/// app. This is the durable write path for `screener login`; writing the JWT
...
pub async fn set_cloud_token(data_dir: &Path, token: &str) -> Result<()> {
    anyhow::ensure!(
        ...
        "screener cloud login returned an invalid session token"
    );

    let key = if screener_secrets::is_encryption_requested(data_dir) {
        match screener_secrets::keychain::get_key() {
            screener_secrets::keychain::KeyResult::Found(key) => Some(key),
            screener_secrets::keychain::KeyResult::AccessDenied => {
                anyhow::bail!(
                    "keychain access denied; refusing to persist the cloud session unencrypted"
                )
            }
            screener_secrets::keychain::KeyResult::NotFound
            | screener_secrets::keychain::KeyResult::Unavailable => None,
        }
    } else {
        None
    };
    ...
    std::fs::create_dir_all(data_dir).map_err(|error| {
        anyhow::anyhow!(
            "could not create screener data directory {}: {error}",
            data_dir.display()
        )
    })?;
    let store = screener_secrets::SecretStore::open_for_data_dir(data_dir, key)
        .await
...
pub async fn clear_cloud_token(data_dir: &Path) -> Result<()> {
    if !data_dir.join("db.sqlite").exists()
        && !screener_secrets::secrets_database_path(data_dir).exists()
    {
        return Ok(());
    }
    let store = open_secret_store(data_dir)
...
///      versions of `screener login` persisted the token. Current CLI and
...
/// older `screener login` versions wrote). Skips app-managed encrypted
...
mod cloud_token_tests {
    ...
    async fn resolve_cloud_token_reads_desktop_secret_store() {
        ...
        let store = screener_secrets::SecretStore::open(&db_path.to_string_lossy(), None)
            .await
            .unwrap();
        store
    ...
    fn legacy_auth_json_is_scoped_to_the_instance_that_owns_it() {
        ...
        let legacy = root.join(".screener");
        let other = root.join(".screener-dev");
