...
use screener_db::{DatabaseManager, FrameData, OCREntry};
...
impl From<screener_db::AudioEntry> for AudioEntry {
    fn from(db_entry: screener_db::AudioEntry) -> Self {
        Self {
...
impl FrameCache {
    pub async fn new(screener_dir: PathBuf, db: Arc<DatabaseManager>) -> Result<Self> {
        let cache_config = CacheConfig {
            cache_dir: cache_dir().unwrap().join("screener").join("frames"),
            ..Default::default()
        };
...
async fn extract_frame(
    ...
    let mut cmd = screener_core::ffmpeg_cmd_async(&ffmpeg);
    cmd.args([
        "-i",
        &video_file_path,
        "-vf",
...
async fn is_video_file_complete(ffmpeg_path: &PathBuf, file_path: &str) -> Result<bool> {
    ...
    let mut cmd = screener_core::ffmpeg_cmd_async(ffmpeg_path);
