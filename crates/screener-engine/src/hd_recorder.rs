...
use screener_screen::monitor::SafeMonitor;
...
mod macos {
    ...
    async fn e2e_stall_hd_writer_once() {
        let armed = std::env::var("SCREENER_E2E_SEED")
            .ok()
    ...
    async fn record_one_chunk(
        ...
        use screener_screen::capture_screenshot_by_window::{
    ...
    fn start_hd_ffmpeg(out: &Path, fps: u32) -> Result<tokio::process::Child> {
        let ffmpeg = screener_core::find_ffmpeg_path().context("ffmpeg not found")?;
