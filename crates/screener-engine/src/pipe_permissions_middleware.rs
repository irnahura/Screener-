...
//! extension (`screener-permissions.ts`). The middleware validates pipe
...
use dashmap::DashMap;
use screener_core::pipes::permissions::{PipePermissions, PipeTokenRegistry};
use std::sync::Arc;

use crate::server::AppState;

/// Wrapper that implements PipeTokenRegistry for the server's DashMap.
/// Passed to PipeManager so it can register/remove tokens from screener-core.
