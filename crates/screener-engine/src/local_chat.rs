...
use axum::Json;
use screener_core::pipes::ResolvedPreset;
...
pub fn base_url_for(preset: &ResolvedPreset) -> Option<String> {
    ...
    let provider = preset.provider.as_deref().unwrap_or("screener-cloud");
    let base = match provider {
        "openai" | "openai-chatgpt" => "https://api.openai.com/v1",
        "native-ollama" | "ollama" => "http://localhost:11434/v1",
        // screener-cloud and anything unknown have no local default: the
...
pub type SharedManager = Arc<Mutex<screener_core::pipes::PipeManager>>;
...
mod tests {
    ...
    fn unknown_provider_without_url_is_refused_rather_than_guessed() {
        ...
        assert!(base_url_for(&preset(Some("screener-cloud"), None)).is_none());
