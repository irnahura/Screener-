...
use screener_screen::monitor::{list_monitors_detailed, MonitorListError};
...
struct DetectedVisionStall {
    ...
    last_capture_loop_heartbeat_ts: u64,
    last_db_write_ts: u64,
    loop_stage: screener_screen::CaptureLoopStage,
...
fn classify_empty_enumeration(counter: &mut u32, denied_logged: &mut bool, site: &str) -> bool {
    ...
    let capture_ready = if cfg!(target_os = "macos") {
        screener_core::display_topology::capture_ready_display_ids()
    } else {
        None
    };
...
pub(crate) fn canonical_display_layout_json(
    displays: &mut [(u32, String, screener_screen::monitor::MonitorData)],
) -> String {
...
pub async fn start_monitor_watcher(
    ...
    audio_manager: Option<screener_audio::audio_manager::AudioManager>,
) -> anyhow::Result<()> {
    ...
    let handle = tokio::spawn(async move {
        ...
        loop {
            ...
            {
                let mut geo: Vec<(u32, String, screener_screen::monitor::MonitorData)> =
                    current_monitors
                        .iter()
...
mod tests {
    use super::*;
    use screener_screen::monitor::MonitorData;
    ...
    fn detected_stall_reports_the_stage_the_loop_froze_in() {
        let snapshots = [MonitorLivenessSnapshot {
            ...
            loop_stage: screener_screen::CaptureLoopStage::VisualProbe,
    ...
    fn one_healthy_monitor_cannot_mask_a_silent_sibling() {
        let snapshots = [
            MonitorLivenessSnapshot {
                ...
                loop_stage: screener_screen::CaptureLoopStage::TriggerWait,
