...
use screener_screen::monitor::{get_monitor_by_id, list_monitors};
...
pub struct VisionManagerConfig {
    ...
    /// snapshot max width via `screener_core::video::*`. Values: "low",
...
pub(crate) struct MonitorLivenessSnapshot {
    ...
    /// ago" (see [`screener_screen::CaptureLoopStage`]).
    pub loop_stage: screener_screen::CaptureLoopStage,
...
impl VisionManager {
    ...
    pub fn is_monitor_allowed(&self, monitor: &screener_screen::monitor::SafeMonitor) -> bool {
        if self
    ...
    pub async fn start_monitor(&self, monitor_id: u32) -> Result<()> {
        ...
        // enumeration admission budget in screener-screen.
    ...
    pub(crate) async fn start_monitor_handle(
        &self,
        monitor: screener_screen::monitor::SafeMonitor,
    ) -> Result<()> {
        let monitor_id = monitor.id();
    ...
    async fn start_event_driven_monitor(
        ...
        // single `videoQuality` setting via `screener_core::video` — that
        ...
        #[cfg(target_os = "macos")]
        screener_screen::monitor::set_sck_capture_max_width(max_snapshot_width);
