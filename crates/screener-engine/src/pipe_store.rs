...
use screener_core::pipes::{PipeExecution, PipeExecutionActivity, PipeStore, SchedulerState};
use screener_db::DatabaseManager;
use std::sync::Arc;

/// SQLite-backed pipe store using the main screener database.
...
impl PipeStore for SqlitePipeStore {
    ...
    async fn set_execution_running(&self, id: i64, pid: Option<u32>) -> Result<()> {
        use screener_db::write_queue::PipeBindValue;
...
mod tests {
    ...
    async fn test_create_execution() {
        let (store, _tmp) = setup_test_store().await;
        let id = store
            .create_execution("my-pipe", "manual", "haiku", Some("screener"))
            .await
            .unwrap();
        assert!(id > 0);
    }
    ...
    async fn test_full_execution_lifecycle() {
        ...
        // 1. Create
        let id = store
            .create_execution("lifecycle-pipe", "manual", "claude-haiku", Some("screener"))
            .await
            .unwrap();
