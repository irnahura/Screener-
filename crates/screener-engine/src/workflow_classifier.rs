...
//! screener cloud classifier. When a high-confidence workflow event is detected,
...
use reqwest::Client;
use screener_events::{ActivityEntry, ClassifierResult, WorkflowEvent};
use serde_json::json;
...
/// Default endpoint for the event classifier — routes through the screener
/// AI gateway, which forwards the `screener-event-classifier` model to the
...
/// Override with the `SCREENER_EVENT_CLASSIFIER_URL` env var for self-host.
pub const DEFAULT_CLASSIFIER_URL: &str = "https://api.screener.com";
...
/// Query the local screener `/search` API for recent activity across all content types.
...
/// Call the classifier through the screener gateway (default) or directly
/// against a self-hosted vLLM (`SCREENER_EVENT_CLASSIFIER_URL` override).
/// The model name `screener-event-classifier` is recognized by both: vLLM
...
async fn classify(
    ...
    let body = json!({
        "model": "screener-event-classifier",
        "messages": [
