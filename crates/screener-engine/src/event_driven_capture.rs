...
use anyhow::Result;
use chrono::Utc;
use screener_a11y::tree::TreeWalkerConfig;
use screener_a11y::ActivityFeed;
use screener_capture::ocr_gate::OcrGate;
use screener_capture::paired_capture::{paired_capture, CaptureContext, PairedCaptureResult};
use screener_core::window_pattern::{self, WindowPattern};
use screener_db::DatabaseManager;
use screener_screen::capture_screenshot_by_window::WindowFilters;
use screener_screen::frame_comparison::{FrameComparer, FrameComparisonConfig};
use screener_screen::monitor::{list_monitors, SafeMonitor};
use screener_screen::snapshot_writer::SnapshotWriter;
use screener_screen::utils::capture_monitor_image;
...
fn e2e_force_focus_cold(monitor_id: u32) -> bool {
    let enabled = std::env::var("SCREENER_E2E_SEED")
        .ok()
            ...
            .is_ok()
    {
        ...
        if let Ok(dir) = std::env::var("SCREENER_DATA_DIR") {
            let _ = std::fs::write(
...
async fn e2e_park_capture_loop_once(
    vision_metrics: &screener_screen::PipelineMetrics,
    monitor_id: u32,
) {
...
pub(crate) struct CaptureParams<'a> {
    ...
    pub languages: &'a [screener_core::Language],
...
fn record_capture_attempt(
    aggregate: &screener_screen::PipelineMetrics,
    monitor: &screener_screen::PipelineMetrics,
) {
...
/// [`screener_screen::CaptureLoopStage`]). Per-monitor is what the watchdog
/// reads: the aggregate is overwritten by whichever display iterated last.
fn record_loop_stage(
    aggregate: &screener_screen::PipelineMetrics,
    monitor: &screener_screen::PipelineMetrics,
    stage: screener_screen::CaptureLoopStage,
) {
...
fn record_screenshot_capture_state(
    aggregate: &screener_screen::PipelineMetrics,
    monitor: &screener_screen::PipelineMetrics,
    state: screener_screen::ScreenshotCaptureState,
) {
...
pub(crate) async fn event_driven_capture_loop(
    ...
    vision_metrics: Arc<screener_screen::PipelineMetrics>,
    monitor_liveness: Arc<screener_screen::PipelineMetrics>,
    hot_frame_cache: Option<Arc<HotFrameCache>>,
    use_pii_removal: bool,
    pause_on_drm_content: bool,
    languages: Vec<screener_core::Language>,
    ...
    record_screenshot_capture_state(
        &vision_metrics,
        &monitor_liveness,
        screener_screen::ScreenshotCaptureState::resolve(screenshots_disabled_by_config, false),
    );
    ...
    let mut walk_budget = screener_a11y::budget::AppWalkBudget::new();
    ...
    loop {
        ...
        record_loop_stage(
            &vision_metrics,
            &monitor_liveness,
            screener_screen::CaptureLoopStage::Heartbeat,
        );
        ...
        record_loop_stage(
            &vision_metrics,
            &monitor_liveness,
            screener_screen::CaptureLoopStage::FocusGate,
        );
        let mut warm_trigger_override: Option<CaptureTrigger> = None;
        {
            ...
            if is_cold && !was_cold {
                record_loop_stage(
                    &vision_metrics,
                    &monitor_liveness,
                    screener_screen::CaptureLoopStage::ReleaseStream,
                );
            ...
            match capture_state {
                ...
                CaptureState::Cold => {
                    ...
                    record_loop_stage(
                        &vision_metrics,
                        &monitor_liveness,
                        screener_screen::CaptureLoopStage::ColdWait,
                    );
                    let notify = focus_controller.notify_for(monitor_id);
                    tokio::select! {
                        _ = notify.notified() => {}
        ...
        record_loop_stage(
            &vision_metrics,
            &monitor_liveness,
            screener_screen::CaptureLoopStage::PauseGate,
        );
        ...
        #[cfg(target_os = "macos")]
        if screener_screen::stream_invalidation::take() {
            info!(
            ...
            let invalidate_result = tokio::time::timeout(
                ...
                tokio::task::spawn_blocking(|| {
                    screener_screen::stream_invalidation::invalidate_streams();
                }),
            )
            .await;
        ...
        if let Some(ref mut rx) = power_profile_rx {
            if rx.has_changed().unwrap_or(false) {
                ...
                record_screenshot_capture_state(
                    &vision_metrics,
                    &monitor_liveness,
                    screener_screen::ScreenshotCaptureState::resolve(
        ...
        if let Some(warm) = warm_trigger_override.take() {
            ...
            record_loop_stage(
                &vision_metrics,
                &monitor_liveness,
                screener_screen::CaptureLoopStage::TriggerWait,
            );
            ...
            record_loop_stage(
                &vision_metrics,
                &monitor_liveness,
                screener_screen::CaptureLoopStage::TriggerDrain,
            );
        ...
        ) || force_visual_check_for_e2e
        {
            ...
            record_loop_stage(
                &vision_metrics,
                &monitor_liveness,
                screener_screen::CaptureLoopStage::ExclusionProbe,
            );
            ...
            if let Some(ref mut comparer) = frame_comparer {
                record_loop_stage(
                    &vision_metrics,
                    &monitor_liveness,
                    screener_screen::CaptureLoopStage::VisualProbe,
                );
        ...
        if let Some(trigger) = trigger {
            ...
            if can_capture || debounce_exempt {
                ...
                record_loop_stage(
                    &vision_metrics,
                    &monitor_liveness,
                    screener_screen::CaptureLoopStage::Capture,
                );
                        ...
                        if let Some(ref result) = output.result {
                            ...
                            if let Some(ref cache) = hot_frame_cache {
                                record_loop_stage(
                                    &vision_metrics,
                                    &monitor_liveness,
                                    screener_screen::CaptureLoopStage::HotCachePush,
                                );
...
fn resolve_capture_metadata(
    tree_snapshot: Option<&screener_a11y::tree::TreeSnapshot>,
    trigger: &CaptureTrigger,
...
fn resolved_window_matches_privacy_filters(
    ...
    if resolved_window_hosts_focus
        ...
        && window_name.is_some_and(screener_a11y::incognito::is_title_private)
    {
        return true;
    }
...
async fn do_capture(
    ...
    walk_budget: &mut screener_a11y::budget::AppWalkBudget,
    ...
    use screener_a11y::tree::TreeWalkResult;
    if let Some(ref app) = trigger_app {
    ...
    let tree_walk_result = if monitor_hosts_focus {
        Some(
            tokio::task::spawn_blocking(move || {
                screener_capture::paired_capture::walk_accessibility_tree(&config)
            })
            .await?,
        )
    } else {
    ...
    match tree_walk_result.as_ref() {
        Some(TreeWalkResult::Found(snap)) => {
            ...
            use screener_a11y::tree::TruncationReason;
    ...
    let focused_window_bounds = {
        ...
        tree_snapshot
            ...
            .map(|b| screener_capture::paired_capture::FocusedWindowBounds {
...
fn get_focused_metadata_lightweight() -> Option<LightweightFocusedMetadata> {
    let (app_name, window_name, _) =
        screener_a11y::platform::linux::get_active_window_info_fresh()?;
...
fn get_focused_metadata_lightweight() -> Option<LightweightFocusedMetadata> {
    ...
    let fresh = screener_a11y::platform::windows::get_focused_app_window_lightweight().map(
...
mod tests {
    ...
    fn aggregate_and_per_monitor_liveness_advance_together() {
        let aggregate = screener_screen::PipelineMetrics::new();
        let monitor = screener_screen::PipelineMetrics::new();
    ...
    fn resolve_capture_metadata_uses_lightweight_metadata_when_tree_values_are_blank() {
        let snapshot = screener_a11y::tree::TreeSnapshot {
            app_name: "  ".into(),
            app_id: None,
            executable: None,
            app_version: None,
            ...
            simhash: 0,
            truncated: false,
            truncation_reason: screener_a11y::tree::TruncationReason::None,
            max_depth_reached: 0,
            window_bounds: None,
        };
        let metadata = LightweightFocusedMetadata {
    ...
    fn resolve_capture_metadata_prefers_lightweight_metadata_over_stale_tree_values() {
        ...
        let metadata = LightweightFocusedMetadata {
            ...
            window_name: Some("screener-wayland-fix".into()),
        };
        ...
        assert_eq!(window_name.as_deref(), Some("screener-wayland-fix"));
    }
