...
/// automation can set `SCREENER_DISABLE_TELEMETRY` to opt out explicitly.
/// This is the single chokepoint that keeps CI runs — the `screener` CLI
...
pub fn init(_telemetry_enabled: bool) {
    // RIOM is local-first: the reduced MVP has no implicit analytics or
    // telemetry transport. Keep this initialization chokepoint so upstream
    // callers remain source-compatible, but never enable outbound reporting.
...
pub async fn capture_event(event: &str, properties: Value) {
    ...
    if let Some(obj) = props.as_object_mut() {
        ...
        obj.insert("$lib".to_string(), json!("screener-engine"));
...
mod tests {
    ...
    const VARS: &[&str] = &["SCREENER_DISABLE_TELEMETRY", "GITHUB_ACTIONS", "CI"];
    ...
    fn telemetry_disabled_by_explicit_optout() {
        with_env(&[("SCREENER_DISABLE_TELEMETRY", "1")], || {
            assert!(telemetry_disabled_by_env())
        });
    }
