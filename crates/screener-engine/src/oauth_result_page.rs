...
pub(crate) fn render_oauth_result_page(
    ...
    let hint = if ok {
        "This tab will try to close automatically. You can return to screener."
    } else {
        "Return to screener and try connecting again."
    };
...
<body data-tone="{tone}" data-auto-close="{auto_close}">
  <main class="stage">
    <div class="brand"><span class="brand-mark" aria-hidden="true">{brand_icon}</span><span>screener</span></div>
  ...
  <script>
    if (document.body.dataset.autoClose === "true") {{
      setTimeout(() => window.close(), 1800);
        ...
        brand_icon = screener_brand_icon(),
...
fn screener_brand_icon() -> &'static str {
