...
fn explicit_cloud_token() -> Option<String> {
    std::env::var("SCREENER_API_KEY").ok()
}
...
/// Handle `screener login` — opens browser, polls server for token.
pub async fn handle_login_command() -> anyhow::Result<()> {
    ...
    if current_cloud_token(&data_dir).await.is_some() {
        println!("already logged in. use `screener whoami` to check status.");
        return Ok(());
    }
...
/// Handle `screener logout` — clear cloud auth from the shared SecretStore.
///
...
pub async fn handle_logout_command() -> anyhow::Result<()> {
    ...
    println!();
    println!("  run `screener login` to sign back in");
    println!();

    Ok(())
}

/// Handle `screener whoami` — show current auth status.
    ...
    match token {
        Some(t) if !t.is_empty() => {
            ...
            let source = if explicit_is_cloud {
                "SCREENER_API_KEY env var"
            } else {
                "screener cloud session store"
            };
        ...
        _ => {
            ...
            println!();
            println!("  run `screener login` to authenticate");
            println!();
        }
    }
