...
pub async fn handle_connection_command(command: &ConnectionCommand) -> anyhow::Result<()> {
    let screener_dir = screener_core::paths::default_screener_data_dir();
    ...
    let legacy_or_dedicated_exists = screener_dir.join("db.sqlite").exists()
        || screener_secrets::secrets_database_path(&screener_dir).exists();
    let secret_store = if legacy_or_dedicated_exists {
        match screener_secrets::SecretStore::open_for_data_dir_with_vault_key(&screener_dir).await {
    ...
    let cm = ConnectionManager::new(screener_dir.clone(), secret_store);
    ...
    match command {
        ...
        ConnectionCommand::Get { id, json: use_json } => {
            ...
            if let Some(creds) = has_creds {
                ...
                println!("\nhint: screener connection set {} key=value ...", id);
            }
        }
