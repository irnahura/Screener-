...
use screener_connect::remote_sync;
...
pub async fn start_sync_service(
    ...
    let token = args
        .sync_token
        .as_ref()
        .ok_or_else(|| anyhow::anyhow!("--sync-token or SCREENER_SYNC_TOKEN required for sync"))?;

    let password = args.sync_password.as_ref().ok_or_else(|| {
        anyhow::anyhow!("--sync-password or SCREENER_SYNC_PASSWORD required for sync")
    })?;
    ...
    let provider = Arc::new(ScreenerSyncProvider::new(db, machine_id));
...
/// Load extra exclude globs from `<data-dir>/.screenerignore`
/// (one glob per line; blank lines and `#` comments ignored).
fn load_screenerignore(dir: &std::path::Path) -> Vec<String> {
    match std::fs::read_to_string(dir.join(".screenerignore")) {
        Ok(content) => content
            .lines()
            .map(|l| l.trim())
...
/// Handle `screener sync remote ...` — SFTP push of ~/.screener to an SSH server.
async fn handle_remote_sync_command(command: &RemoteSyncCommand) -> anyhow::Result<()> {
    match command {
        ...
        RemoteSyncCommand::Now { cfg, data_dir } => {
            ...
            excludes.extend(load_screenerignore(&dir));
