...
pub fn store_path() -> PathBuf {
    screener_core::paths::default_screener_data_dir().join("store.bin")
}
...
fn write_store_to_with_key(
    ...
    let bytes = if encrypt {
        ...
        let ciphertext = screener_vault::crypto::encrypt_small(&serialized, &key)
...
fn read_plain_store_bytes_with_key(
    ...
    let key = key_provider()?;
    screener_vault::crypto::decrypt_small(&data[STORE_MAGIC.len()..], &key)
...
fn store_encryption_key() -> Result<[u8; 32]> {
    match screener_secrets::keychain::get_key() {
        screener_secrets::keychain::KeyResult::Found(key) => Ok(key),
        screener_secrets::keychain::KeyResult::AccessDenied => bail!(
            "store.bin is encrypted, but keychain access was denied. Grant screener keychain access or disable store encryption in the app, then try again."
        ),
        screener_secrets::keychain::KeyResult::NotFound => bail!(
            "store.bin is encrypted, but the screener keychain key was not found. Open the app once or disable store encryption, then try again."
        ),
        screener_secrets::keychain::KeyResult::Unavailable => bail!(
...
mod tests {
    ...
    fn encrypted_store_bytes(value: &Value, key: &[u8; 32]) -> Vec<u8> {
        ...
        let ciphertext = screener_vault::crypto::encrypt_small(&plaintext, key).unwrap();
