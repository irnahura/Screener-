...
//!
//! Reads/writes presets in `~/.screener/store.bin` (plain JSON or the
...
//! at the file level — same trade-off as `screener login`.
...
/// stored on disk and consumed by `resolve_preset` in `screener-core`.
...
impl Provider {
    pub fn parse(s: &str) -> Result<Self> {
        match s {
            ...
            "screener-cloud" | "pi" => Ok(Provider::ScreenerCloud),
            "openai-chatgpt" => Ok(Provider::OpenAiChatGpt),
            other => bail!(
                "unknown provider '{}'. Valid: openai, anthropic, native-ollama, custom, screener-cloud",
                other
            ),
        }
    }
    ...
    pub fn as_canonical(self) -> &'static str {
        match self {
            ...
            Provider::ScreenerCloud => "screener-cloud",
    ...
    fn forbids_url(self) -> bool {
        matches!(self, Provider::ScreenerCloud)
    }

    fn forbids_api_key(self) -> bool {
        matches!(
            self,
            Provider::NativeOllama | Provider::ScreenerCloud | Provider::OpenAiChatGpt
        )
    }
}
...
fn validate_provider_combo(
    ...
    if provider.forbids_api_key() && api_key.map(|s| !s.is_empty()).unwrap_or(false) {
        let reason = match provider {
            ...
            Provider::ScreenerCloud => "cloud uses your login token (run `screener login`)",
...
/// Scan `~/.screener/pipes/*/pipe.md` and return the names of pipes whose
...
mod tests {
    ...
    fn provider_combos() {
        ...
        assert!(validate_provider_combo(Provider::ScreenerCloud, None, None).is_ok());
        assert!(validate_provider_combo(Provider::ScreenerCloud, Some("http://x"), None).is_err());
        assert!(validate_provider_combo(Provider::ScreenerCloud, None, Some("k")).is_err());
