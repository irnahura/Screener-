...
fn default_audio_engine() -> CliAudioTranscriptionEngine {
    let tier = screener_config::detect_tier();
    if matches!(tier, screener_config::DeviceTier::Low) {
...
impl CliAppContext {
    fn resolved(self) -> (bool, screener_config::SemanticContextMode) {
        use screener_config::SemanticContextMode;
...
#[command(
    author,
    version,
    about = "screener: power AI by everything you've seen, said or heard",
    long_about = None,
    name = "screener"
)]
pub struct Cli {
...
pub enum Command {
    ...
    /// Show screener status (running state, data stats)
    Status {
        /// Output format
        #[arg(long, default_value_t = false)]
        json: bool,
        /// Data directory. Default to $HOME/.screener
    ...
    /// (no daemon required — opens `~/.screener/db.sqlite` read-side
    ...
    /// Wire an external agent (OpenClaw, Hermes, Claude) to screener
    Agent {
        #[command(subcommand)]
        subcommand: agent::AgentCommand,
    },

    /// Connect screener to every supported AI tool detected on this computer
    Setup {
        /// screener REST API base URL the skills + MCP should target.
        #[arg(long, default_value = "http://localhost:3030")]
        api_url: String,
    },

    /// Run screener as a background service that starts at boot + stays up
    ...
    /// Vault: encrypt/lock all screener data at rest
    Vault {
        #[command(subcommand)]
    ...
    Install {
        /// Manifest URL (HTTPS, JSON). Defaults to the screener starter bundle.
        #[arg(default_value = "https://screener.com/start.json")]
        url: String,
    ...
    /// Authenticate with screener cloud
    Login,

    /// Sign out of screener cloud
    Logout,

    /// Show current auth status
    Whoami,

    /// Open the screener survey in your browser
    Survey,

    /// Check system readiness (permissions, ffmpeg, etc.)
    Doctor,

    /// Collect recent logs + system info and send them to screener support,
    ...
    /// `~/.screener/db.sqlite` directly — no daemon required.
...
pub struct ExportArgs {
    ...
    /// (see `screener search` or the app's meetings list). Mutually
...
pub struct RecordArgs {
    ...
    /// Request Screener's software Acoustic Echo Cancellation (via sonora WebRTC AEC3).
    #[arg(long, default_value_t = false)]
    pub screener_aec_enabled: bool,

    /// Data directory. Default to $HOME/.screener
    #[arg(long, value_hint = ValueHint::DirPath)]
    pub data_dir: Option<String>,

    /// Enable debug logging for screener modules
    #[arg(long)]
    pub debug: bool,
    ...
    /// `rfdetr_v11.onnx` at `~/.screener/models/` and the binary
    ...
    /// `tinfoil` (screener-hosted confidential-compute enclave on
    ...
    #[arg(
        long,
        env = "SCREENER_ENABLE_SEMANTIC_CONTEXT",
    ...
    /// Keep the computer awake while screener is running.
    #[arg(long, default_value_t = false)]
    ...
    #[arg(long, env = "SCREENER_ENABLE_MDNS", default_value_t = false)]
    pub enable_mdns: bool,

    /// API token for cloud sync
    #[arg(long, env = "SCREENER_SYNC_TOKEN")]
    pub sync_token: Option<String>,

    /// Password for encrypting synced data
    #[arg(long, env = "SCREENER_SYNC_PASSWORD")]
    ...
    /// ~/.screener data into a remote LLM (passwords, keys, secrets often
    ...
    /// capture on-screen text. Useful when piping ~/.screener data into a
    ...
    /// requests must include Authorization: Bearer <SCREENER_API_KEY>.
    ...
    pub schedule_rules: Vec<screener_config::ScheduleRule>,
}
...
impl RecordArgSources {
    pub fn from_cli_matches(matches: &ArgMatches) -> Self {
        ...
        Self {
            ...
            screener_aec_enabled: from_command_line(record, "screener_aec_enabled"),
...
fn parse_schedule_rule(s: &str) -> Result<screener_config::ScheduleRule, String> {
    ...
    Ok(screener_config::ScheduleRule {
        day_of_week,
        start_time,
        end_time,
        record_mode,
...
impl RecordArgs {
    ...
    pub fn to_recording_settings(&self) -> screener_config::RecordingSettings {
        ...
        let aec_mode = if self.screener_aec_enabled {
            screener_config::AecMode::Screener
        } else if self.windows_input_aec_enabled {
            screener_config::AecMode::Windows
        } else if self.macos_input_vpio_enabled {
            screener_config::AecMode::Macos
        } else {
            screener_config::AecMode::Off
        };
        let (enable_semantic_context, semantic_context_mode) =
            self.app_context.map(CliAppContext::resolved).unwrap_or((
                self.enable_semantic_context,
                screener_config::SemanticContextMode::Memory,
            ));

        screener_config::RecordingSettings {
            ...
            windows_input_aec_enabled: aec_mode == screener_config::AecMode::Windows,
            macos_input_vpio_enabled: aec_mode == screener_config::AecMode::Macos,
            screener_aec_enabled: aec_mode == screener_config::AecMode::Screener,
            aec_mode,
            ...
            ..screener_config::RecordingSettings::default()
        }
    }
    ...
    pub async fn into_recording_config(
        ...
        let cli_encryption_ready = if self.encrypt_secrets {
            match screener_secrets::keychain::get_or_create_key() {
                Some(_) => {
                    screener_secrets::mark_encryption_enabled(&data_dir)?;
                    true
                }
                None => {
                    tracing::warn!(
        ...
        // or cloud features (STT, screener-cloud pipes) silently lose auth
        ...
        if settings.device_tier.is_none() {
            ...
            let existing = screener_config::load_toml(&config_path).ok();
            ...
            if has_tier {
                ...
                if is_fresh {
                    screener_config::apply_tier_defaults(&mut settings, tier);
        ...
        let tier = settings
            .device_tier
            .as_deref()
            .and_then(screener_config::DeviceTier::from_str_loose)
            .unwrap_or_else(screener_config::detect_tier);
        if screener_config::is_engine_unsafe(&settings.audio_transcription_engine, tier) {
            let safe = screener_config::best_engine_for_platform(tier);
            eprintln!(
        ...
        if self.listen_on_lan && !self.api_auth {
            tracing::warn!(
                "--listen-on-lan was set but --api-auth=false — forcing api_auth on for safety. Use `screener auth token` to view your key."
            );
        }
        if config.api_auth {
    ...
    fn apply_explicit_overrides(
        &self,
        settings: &mut screener_config::RecordingSettings,
        sources: &RecordArgSources,
    ) {
        ...
        if sources.screener_aec_enabled && self.screener_aec_enabled {
            settings.aec_mode = screener_config::AecMode::Screener;
            settings.screener_aec_enabled = true;
            ...
            settings.aec_mode = screener_config::AecMode::Windows;
            settings.screener_aec_enabled = false;
            settings.windows_input_aec_enabled = true;
            settings.macos_input_vpio_enabled = false;
        } else if sources.macos_input_vpio_enabled && self.macos_input_vpio_enabled {
            settings.aec_mode = screener_config::AecMode::Macos;
            settings.screener_aec_enabled = false;
            settings.windows_input_aec_enabled = false;
...
fn load_recording_settings_from_store(
    data_dir: &Path,
) -> anyhow::Result<Option<screener_config::RecordingSettings>> {
...
fn persist_recording_settings_to_store(
    data_dir: &Path,
    settings: &screener_config::RecordingSettings,
...
pub enum PipeCommand {
    ...
    Publish {
        /// Pipe name (directory name under ~/.screener/pipes/)
        name: String,
    },
...
pub enum ModelCommand {
    ...
    Create {
        ...
        id: String,
        /// Provider: openai | anthropic | native-ollama | custom | screener-cloud
        #[arg(long)]
        provider: String,
...
pub enum VaultCommand {
    ...
    Setup {},
    /// Lock the vault (encrypt all data; stop screener first)
    Lock {
        /// Port screener listens on. `vault lock` refuses to run while a
        /// daemon is detected here — locking encrypts the DB in place and would
        /// corrupt it underneath a live daemon, so stop screener first.
        #[arg(short = 'p', long, default_value_t = 3030)]
        port: u16,
    },
...
pub enum DbCommand {
    ...
    /// Refuses to run while screener is open.
    Recover {
        /// Deprecated compatibility flag. Recovery still refuses to race a
        /// reachable screener server because that cannot preserve one exact
    ...
    Cleanup {
        ...
        #[arg(long)]
        apply: bool,
        /// Run even if the screener HTTP server is reachable.
        #[arg(long)]
        force: bool,
    },
    /// Force-clear the .db_recovery.lock file (escape hatch if a previous
    /// `screener db ...` run was killed without releasing the lock).
    Unlock {
...
/// shape the API returns — `screener search` and `curl /search` are
...
pub struct SearchArgs {
    ...
    /// Data directory. Default `$HOME/.screener`.
    #[arg(long, value_hint = ValueHint::DirPath)]
    pub data_dir: Option<String>,
...
/// Mirrors the `screener-team` skill — same endpoints, same vocabulary.
/// All variants hit `https://screenpi.pe/api/enterprise/v1/*` directly
/// with the admin's `team_api_token` from `~/.screener/enterprise.json`
/// (or `SCREENER_TEAM_API_TOKEN` env override). No daemon needed.
...
pub struct TeamSearchArgs {
    ...
    /// Restrict to one device — get the id from `screener team devices`.
    #[arg(long)]
...
pub enum McpCommand {
    ...
    Setup {
        /// Directory to save MCP files (default: $HOME/.screener/mcp)
        #[arg(long, value_hint = ValueHint::DirPath)]
...
pub enum SyncCommand {
    ...
    /// Sync ~/.screener to a remote SSH server (SFTP, no cloud account)
    Remote {
        #[command(subcommand)]
...
/// SSH/SFTP-based sync of `~/.screener` to a remote server.
///
/// No cloud account or screener-cloud dependency — pushes the entire data
...
pub enum RemoteSyncCommand {
    ...
    /// Push `~/.screener/` to the remote once
    Now {
        #[command(flatten)]
        cfg: RemoteSyncArgs,
        /// Override the local data directory (default: $HOME/.screener)
        #[arg(long)]
        data_dir: Option<String>,
    },
...
/// Shared connection flags for `screener sync remote {test,now}`.
///
/// All values can also come from env vars: SCREENER_REMOTE_HOST,
/// SCREENER_REMOTE_USER, SCREENER_REMOTE_KEY, SCREENER_REMOTE_PATH.
#[derive(clap::Args, Debug)]
pub struct RemoteSyncArgs {
    /// Remote host (IP or DNS, e.g. "myserver.tail-scale.ts.net")
    #[arg(long, env = "SCREENER_REMOTE_HOST")]
    pub host: String,
    /// SSH user
    #[arg(long, env = "SCREENER_REMOTE_USER")]
    pub user: String,
    /// Path to SSH private key (e.g. ~/.ssh/id_ed25519)
    #[arg(long, env = "SCREENER_REMOTE_KEY")]
    pub key_path: String,
    /// Absolute path on the remote where ~/.screener/ should land
    #[arg(long, env = "SCREENER_REMOTE_PATH")]
    ...
    /// way to put a huge ~/.screener on a small remote.
    #[arg(long, default_value_t = false)]
    pub no_media: bool,
    /// Exclude paths matching a glob (repeatable, `*` wildcards). Also reads
    /// `<data-dir>/.screenerignore` (one glob per line, `#` comments).
    #[arg(long = "exclude")]
...
pub fn get_or_create_machine_id(override_id: Option<String>) -> String {
    ...
    screener_core::sync::get_or_create_machine_id()
}
...
mod tests {
    ...
    fn test_incognito_flags_default_to_basic_filtering() {
        let cli = Cli::try_parse_from(["screener", "record"]).unwrap();
        match cli.command {
            Command::Record(args) => {
    ...
    fn test_incognito_filter_can_be_disabled() {
        let cli = Cli::try_parse_from(["screener", "record", "--ignore-incognito-windows=false"])
            .unwrap();
        match cli.command {
            Command::Record(args) => {
                ...
                assert!(
                    record_sources(["screener", "record", "--ignore-incognito-windows=false",])
    ...
    fn test_enhanced_incognito_detection_is_explicit() {
        let cli =
            Cli::try_parse_from(["screener", "record", "--enhanced-incognito-detection"]).unwrap();
        match cli.command {
            Command::Record(args) => {
                let settings = args.to_recording_settings();
                assert!(settings.ignore_incognito_windows);
    ...
    fn test_enable_semantic_context_flag_flows_to_recording_settings() {
        let cli = Cli::try_parse_from(["screener", "record", "--enable-semantic-context"]).unwrap();
        match cli.command {
            Command::Record(args) => {
    ...
    fn test_app_context_modes_flow_to_recording_settings() {
        ...
        for (value, enabled, mode) in cases {
            let cli = Cli::try_parse_from(["screener", "record", "--app-context", value]).unwrap();
            match cli.command {
                Command::Record(args) => {
                    let settings = args.to_recording_settings();
                    ...
                    assert!(
                        record_sources(["screener", "record", "--app-context", value,]).app_context,
    ...
    fn test_pause_on_drm_content_flag_sets_true() {
        let cli = Cli::try_parse_from(["screener", "record", "--pause-on-drm-content"]).unwrap();
        match cli.command {
            Command::Record(args) => {
    ...
    fn test_keep_computer_awake_flag_flows_to_recording_settings() {
        let cli = Cli::try_parse_from(["screener", "record", "--keep-computer-awake"]).unwrap();
        match cli.command {
            Command::Record(args) => {
                let settings = args.to_recording_settings();
                assert!(
    ...
    fn test_schedule_rule_rejects_bad_input() {
        ...
        assert!(
            Cli::try_parse_from(["screener", "record", "--schedule-rule", "9,09:00,18:00,all"])
                .is_err(),
            "day out of range should fail to parse"
        );
        assert!(
            Cli::try_parse_from(["screener", "record", "--schedule-rule", "0,9am,18:00,all"])
                .is_err(),
    ...
    fn test_survey_command_parses() {
        let cli = Cli::try_parse_from(["screener", "survey"]).unwrap();
        match cli.command {
            Command::Survey => {}
    ...
    fn test_setup_command_parses_with_default_api_url() {
        let cli = Cli::try_parse_from(["screener", "setup"]).unwrap();
        match cli.command {
    ...
    fn test_setup_command_accepts_remote_api_url() {
        let cli =
            Cli::try_parse_from(["screener", "setup", "--api-url", "https://screener.example"])
                .unwrap();
        match cli.command {
            Command::Setup { api_url } => {
                assert_eq!(api_url, "https://screener.example");
            }
            _ => panic!("expected Setup command"),
        }
    }
    ...
    fn test_profile_command_parses_with_defaults() {
        let cli = Cli::try_parse_from(["screener", "profile"]).unwrap();
        match cli.command {
    ...
    fn test_profile_command_parses_port_and_json() {
        let cli = Cli::try_parse_from(["screener", "profile", "--port", "4040", "--json"]).unwrap();
        match cli.command {
            Command::Profile { json, port } => {
                assert!(json);
                assert_eq!(port, 4040);
    ...
    fn test_record_arg_sources_only_mark_explicit_cli_args() {
        let matches = Cli::command()
            .try_get_matches_from(["screener", "record", "--port", "4040"])
            .unwrap();
    ...
    fn test_explicit_cli_overrides_preserve_app_settings_for_absent_defaults() {
        let cli = Cli::try_parse_from(["screener", "record", "--port", "4040", "--disable-audio"])
            .unwrap();
        let sources = record_sources(["screener", "record", "--port", "4040", "--disable-audio"]);
        let mut settings = screener_config::RecordingSettings {
            port: 3030,
            disable_audio: false,
    ...
    fn test_app_context_off_overrides_persisted_enabled_mode() {
        let args = ["screener", "record", "--app-context", "off"];
        let cli = Cli::try_parse_from(args).unwrap();
        let sources = record_sources(args);
        let mut settings = screener_config::RecordingSettings {
            enable_semantic_context: true,
            semantic_context_mode: screener_config::SemanticContextMode::Both,
            ..Default::default()
        };
    ...
    fn test_app_context_automation_overrides_persisted_disabled_mode() {
        let args = ["screener", "record", "--app-context", "automation"];
        let cli = Cli::try_parse_from(args).unwrap();
        let sources = record_sources(args);
        let mut settings = screener_config::RecordingSettings {
            enable_semantic_context: false,
            semantic_context_mode: screener_config::SemanticContextMode::Memory,
            ..Default::default()
        };
        ...
        assert_eq!(
            settings.semantic_context_mode,
            screener_config::SemanticContextMode::ComputerUse
        );
    }

    #[test]
    fn test_absent_app_context_arguments_preserve_persisted_settings() {
        let args = ["screener", "record", "--port", "4040"];
        let cli = Cli::try_parse_from(args).unwrap();
        let sources = record_sources(args);
        let mut settings = screener_config::RecordingSettings {
            enable_semantic_context: true,
        ...
        assert_eq!(
            settings.semantic_context_mode,
            screener_config::SemanticContextMode::Both
        );
    ...
    fn test_monitor_id_flag_enables_vision_over_persisted_disable() {
        ...
        let args = ["screener", "record", "--monitor-id", "5"];
        let cli = Cli::try_parse_from(args).unwrap();
        let sources = record_sources(args);
        let mut settings = screener_config::RecordingSettings {
            disable_vision: true,
    ...
    fn test_default_true_bools_default_to_true_without_flag() {
        let args = record_args(["screener", "record"]);
    ...
    fn test_default_true_bools_accept_space_separated_value() {
        let args = record_args(["screener", "record", "--use-pii-removal", "false"]);
        assert!(
            !args.use_pii_removal,
            "--use-pii-removal false must disable"
        );
    ...
    fn test_eq_false_registers_as_cli_source_and_overrides_persisted_settings() {
        let args = ["screener", "record", "--use-pii-removal=false"];
        let cli = Cli::try_parse_from(args).unwrap();
        let sources = record_sources(args);
        assert!(
