...
//! `screener profile` — per-stage timing for the capture pipeline.
//!
...
pub async fn handle_profile_command(json: bool, port: u16) -> anyhow::Result<()> {
    ...
    let resp = match resp {
        Ok(r) => r,
        Err(_) => {
            eprintln!("screener does not appear to be running on port {port}.");
            eprintln!(
                "start it (`screener`) — or pass --port — then run `screener profile` again."
            );
            std::process::exit(1);
        }
    };
...
fn format_profile_report(health: &Value) -> String {
    ...
    out.push_str("screener profile — per-stage timing (since server start)\n\n");
