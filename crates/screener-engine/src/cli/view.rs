    ...
    match command {
        ViewCommand::List { json, data_dir } => {
            ...
            if *json {
                ...
                println!("hint: create one in Brain, or import a Template with `screener view apply <file>`");
            } else {
        ...
        ViewCommand::Export {
            ...
            let screener_dir = resolve_data_dir(data_dir);
            let view = get_live_view(&screener_dir, id)?;
...
fn resolve_data_dir(data_dir: &Option<PathBuf>) -> PathBuf {
    data_dir
        .clone()
        .unwrap_or_else(screener_core::paths::default_screener_data_dir)
}
...
fn render_html(view: &LiveView) -> String {
    ...
    format!(
        "<!doctype html><html lang=\"en\"><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>{title}</title><style>{style}</style></head><body><main><header><p>screener Live View</p><h1>{title}</h1><small>{time_range} · updated {updated}</small></header><div class=\"grid\">{blocks}</div></main></body></html>",
