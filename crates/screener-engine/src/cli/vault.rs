    ...
    match command {
        ...
        VaultCommand::Lock { port } => {
            ...
            if daemon_running(*port).await {
                anyhow::bail!(
                    "screener is running on port {port} — stop it first (quit the app, \
                     or kill the `screener` process), then run `screener vault lock` \
...
/// running screener daemon so we refuse to lock the vault underneath it.
