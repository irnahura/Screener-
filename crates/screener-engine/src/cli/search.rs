...
//! `screener search` — query the local SQLite history without the daemon.
//!
//! Mirrors `GET /search` so AI / shell / pipe callers don't have to choose
//! between two vocabularies. Opens `~/.screener/db.sqlite` directly; WAL
...
use screener_db::{ContentType, DatabaseManager, SearchResult};
use std::path::PathBuf;

use crate::cli::SearchArgs;
use crate::routes::content::ContentItem;
use crate::routes::search::{is_screener_app, search_result_to_content_item};
...
pub async fn handle_search_command(args: &SearchArgs) -> anyhow::Result<()> {
    let base_dir = match &args.data_dir {
        ...
        None => paths::default_screener_data_dir(),
    };
    let db_path = base_dir.join("db.sqlite");
    if !db_path.exists() {
        anyhow::bail!(
            "no screener database at {}. run `screener record` first.",
            db_path.display()
        );
    }
...
mod tests {
    ...
    fn empty_query_is_allowed_for_time_only_browse() {
        let cli = Cli::try_parse_from(["screener", "search", "--start", "30m ago"]).unwrap();
        match cli.command {
            Command::Search(args) => {
