...
//! `screener diagnose` — collect recent logs + system info and ship them to
//! screener support, returning a ticket id. This is the headless/CLI twin of
//! the desktop app's "send logs & feedback" button
//! (apps/screener-app-tauri/components/share-logs-button.tsx). It hits the
...
/// Handle `screener diagnose`.
///
/// * `data_dir`  — resolved screener data dir (`~/.screener` by default).
...
pub async fn handle_diagnose_command(
    ...
    if log_files.is_empty() {
        anyhow::bail!(
            "no .log files found in {} — has screener recorded anything yet?\n\
             hint: run `screener record` (or start the app) at least once first.",
            data_dir.display()
        );
    }
    ...
    if dry_run {
        ...
        println!("  drop it in the screener discord. drop --dry-run to send it for you.");
        return Ok(());
    }

    let (identifier, kind) = resolve_identifier(data_dir);
    let screener_id = read_pointer(
        data_dir,
    ...
    if let Some(id) = screener_id.filter(|s| !s.is_empty()) {
        confirm_body["screener_id"] = json!(id);
    }
    ...
    println!();
    println!("  ✓ logs sent to screener support");
    if let Some(id) = ticket {
        println!("  ticket: #{id}");
        match follow_up {
            "email" => println!("  we emailed you a receipt and will reply there."),
            _ => println!("  mention #{id} in the screener discord if you need an update."),
        }
    } else {
...
async fn build_bundle(
    ...
    out.push_str("=== screener diagnostics ===\n");
    out.push_str(&format!(
        "generated: {}\n",
...
async fn probe_health(port: u16) -> String {
    ...
    match client.get(&url).send().await {
        ...
        Err(_) => format!("[server not reachable on :{port} — screener may not be running]\n"),
    }
}
