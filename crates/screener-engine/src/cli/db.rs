...
//! `screener db ...` — corruption recovery + storage cleanup.
//!
//! ## Safety model
//!
//! These commands rewrite or delete files inside `~/.screener/`. A racing
...
//!
//!   * `~/.screener/.db_recovery.lock` — JSON `{pid, host, started_at, op}`
...
//!     (`apps/screener-app-tauri/src-tauri/src/main.rs`); env var
//!     `SCREENER_IGNORE_DB_LOCK=1` is the escape hatch.
//!
...
//! 3. Same host, PID alive: refuse, point at `screener db unlock`.
//! 4. Same host, PID unknown, mtime > 1 h: clear (heartbeat would have kept it
//!    fresh; older means the heartbeat thread is dead too).
//!
//! ## Recovery pre-flight
//!
//!  * SQLite's official recovery extension is compiled into Screener
...
struct RecoveryManifest {
    ...
    phase: RecoveryPhase,
    live_path: PathBuf,
    original_identity: screener_db::SqliteFileIdentity,
    candidate_identity: Option<screener_db::SqliteFileIdentity>,
...
if you're sure no `screener db ...` is running, run `screener db unlock --force`.",
                        path.display(),
                    );
                }
                let _ = fs::remove_file(&path);
            ...
            LockState::HeldHere { pid, op: other } => {
                bail!(
                    "another db op is running: {} (pid {}). wait for it, or `screener db unlock --force` if you're sure it's stuck.",
                    other,
                    pid,
                );
            }
...
will not auto-clear cross-host locks. resolve manually if needed.",
        ...
        let mut file = OpenOptions::new()
            ...
            .with_context(|| {
                format!(
                    "couldn't acquire lock at {}. another `screener db ...` may have raced us.",
                    path.display()
                )
            })?;
    ...
    fn install_signal_handlers(path: &Path, stop: Arc<AtomicBool>) {
        ...
        tokio::spawn(async move {
            ...
            let _ = fs::remove_file(&path);
            eprintln!("\nscreener: caught signal, lock released. exiting.");
...
fn ensure_app_quit(force: bool) -> Result<()> {
    if !force && server_is_running() {
        bail!(
            "screener is currently running (port 3030 is open). quit the app from the menubar first, or pass --force to override (only safe if you know nothing is writing to the db)."
        );
    }
    Ok(())
}
...
fn ensure_disk_space(data_dir: &Path, source_size: u64) -> Result<()> {
    ...
    if let Some(avail) = available {
        if avail < needed {
            bail!(
                "not enough disk space: need ~{} MB free, have {} MB. free space outside the active database recovery evidence; `screener db cleanup` can reclaim old artifacts only when quarantine is not active.",
                needed / 1_048_576,
                avail / 1_048_576,
            );
        }
...
fn integrity_check(db_path: &Path) -> Result<()> {
    ...
    if screener_db::sqlite_quarantine_exists(db_path) {
        bail!(
            "database is durably quarantined; refusing to open or checkpoint its DB/WAL/SHM generation. quit screener and run `screener db recover`"
        );
    }
    ...
    if results.as_slice() == ["ok"] {
        ...
        bail!(
            "corruption detected:\n{}\n\nnext step: quit the screener app, then run `screener db recover`",
            results.join("\n")
        )
    }
}
...
struct GenerationComponentFingerprint {
    name: String,
    identity: screener_db::SqliteFileIdentity,
    length: u64,
    ...
    for path in [
        ...
        fingerprint.push(GenerationComponentFingerprint {
            name: path
                ...
                .to_string_lossy()
                .into_owned(),
            identity: screener_db::sqlite_file_identity(&path)
...
fn restore_interrupted_swap(data_dir: &Path, live: &Path) -> Result<()> {
    for directory in newest_recovery_directories(data_dir)? {
        ...
        let live_identity = screener_db::sqlite_file_identity(live).ok();
...
pub async fn recover_quarantined_database(data_dir: &Path) -> Result<()> {
    ...
    if !screener_db::sqlite_quarantine_exists(&live) {
        bail!(
...
async fn recover_offline(data_dir: &Path) -> Result<()> {
    ...
    let original_identity = screener_db::sqlite_file_identity(&live)
    ...
    let marker = match screener_db::read_sqlite_quarantine(&live) {
        Ok(marker) => marker,
        Err(error) => {
            eprintln!(
    ...
    if !screener_db::sqlite_quarantine_exists(&live) {
        screener_db::persist_sqlite_quarantine(
            &live,
            None,
    ...
    screener_sqlite_recovery::recover_database(&work, &candidate)
    ...
    let rebuilt_fts = screener_db::rebuild_recovered_fts5_indexes(&candidate)
        .await
    ...
    let candidate_verification =
        screener_db::verify_fresh_sqlite_recovery_candidate(&candidate, &forbidden_identities)
            .await
            .context(
    ...
    let installed_verification =
        screener_db::verify_fresh_sqlite_recovery_candidate(&live, &forbidden_identities).await;
    ...
    screener_db::archive_resolved_sqlite_quarantine(
        &live,
        &resolved_marker,
        &installed_verification.file_identity,
    ...
    println!();
    println!("    next: start screener — it will rebuild WAL/SHM on first open.");
    println!("    once you've confirmed everything works, run:");
    println!("        screener db cleanup --apply");
...
async fn cleanup(data_dir: &Path, apply: bool, force: bool) -> Result<()> {
    ...
    let live = data_dir.join("db.sqlite");
    if apply && screener_db::sqlite_quarantine_exists(&live) {
        bail!(
...
fn unlock(data_dir: &Path, force: bool) -> Result<()> {
    ...
    if !safe && !force {
        bail!(
            "lock looks live ({state:?}). pass --force to remove anyway. you should only do this if you're certain no `screener db ...` is actually running."
        );
    }
...
mod recovery_tests {
    ...
    fn test_manifest(live: &Path) -> RecoveryManifest {
        RecoveryManifest {
            ...
            original_identity: screener_db::sqlite_file_identity(live).expect("original identity"),
            candidate_identity: None,
    ...
    async fn end_to_end_recovery_salvages_corruption_installs_fresh_inode_and_archives_original() {
        ...
        let original_identity = screener_db::sqlite_file_identity(&live).expect("old identity");
        screener_db::prepare_sqlite_quarantine_reserve(&live).expect("prepare marker reserve");
        screener_db::persist_sqlite_quarantine(&live, Some(11), "test corruption")
        ...
        let replacement_identity =
            screener_db::sqlite_file_identity(&live).expect("replacement identity");
        assert_ne!(replacement_identity, original_identity);
        assert!(!screener_db::sqlite_quarantine_exists(&live));
    ...
    async fn cleanup_cannot_delete_evidence_while_quarantine_is_active() {
        ...
        screener_db::prepare_sqlite_quarantine_reserve(&live).expect("prepare reserve");
        screener_db::persist_sqlite_quarantine(&live, Some(522), "test I/O fault")
            .expect("persist quarantine");
        let check_error =
