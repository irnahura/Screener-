...
//! `screener service {install,uninstall,status}` — run screener as a
//! background service that starts at boot and stays up (Restart=always). This
//! is what makes a headless box (a VPS, a Mac mini) keep screener running
...
pub enum ServiceCommand {
    /// Install + enable a service that runs screener at boot, always-on.
    Install {
        /// Args appended to `screener record`. Default is serve-only (no
...
const SERVICE_NAME: &str = "screener";
const LAUNCHD_LABEL: &str = "com.screener.agent";
...
fn systemd_unit(exec_start: &str, path_env: &str) -> String {
    format!(
        "[Unit]\n\
         Description=screener — always-on capture/serve\n\
...
fn screener_exe() -> Result<String> {
    Ok(std::env::current_exe()
        .context("could not resolve the screener binary path")?
        .to_string_lossy()
        .into_owned())
}
...
fn install(record_args: &str) -> Result<()> {
    ...
    println!("  ✓ screener service enabled + started — runs at boot, restarts on crash");
...
fn install(record_args: &str) -> Result<()> {
    ...
    println!("  ✓ screener launch agent loaded — runs at login, restarts on crash");
    Ok(())
}
...
fn uninstall() -> Result<()> {
    ...
    println!("  ✓ screener service removed");
    Ok(())
}
...
fn uninstall() -> Result<()> {
    ...
    println!("  ✓ screener launch agent removed");
    Ok(())
}
...
mod tests {
    ...
    fn test_systemd_unit() {
        let u = systemd_unit(
            "/usr/local/bin/screener record --disable-vision",
            "/usr/bin",
        );
        assert!(u.contains("ExecStart=/usr/local/bin/screener record --disable-vision"));
    ...
    fn test_launchd_plist() {
        let p = launchd_plist(&[
            "/Applications/x/screener".to_string(),
            "record".to_string(),
            "--disable-audio".to_string(),
        ]);
        assert!(p.contains("<key>Label</key>"));
        assert!(p.contains("com.screener.agent"));
