...
//! `screener debug` — one-shot diagnostic subcommands used to reproduce bugs
...
async fn a11y_walk(app_filter: Option<&str>, settle_ms: u64) -> anyhow::Result<()> {
    ...
    #[cfg(target_os = "macos")]
    {
        use screener_a11y::tree::{create_tree_walker, TreeWalkResult, TreeWalkerConfig};
                ...
                TreeWalkResult::Found(snap) => {
                    ...
                    for node in &snap.nodes {
                        ...
                        if let Some(b) = &node.bounds {
                            ...
                            if node.on_screen == Some(true)
                                && screener_a11y::tree::node_looks_multiline(&node.text, b, 1.5)
                            {
