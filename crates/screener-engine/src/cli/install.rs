...
//! `screener install <url>` — fetch a JSON manifest and install every pipe
//! it lists from the screener registry.
//!
...
//! ```
//!
//! Hosts other than `screener.com` or `screenpi.pe` are rejected unless
//! `--allow-untrusted` is passed or `SCREENER_TRUSTED_HOSTS` lists them
...
pub async fn handle_install(url: &str, allow_untrusted: bool) -> Result<()> {
    ...
    if !allow_untrusted && !is_trusted_host(url)? {
        return Err(anyhow!(
            "manifest host is not in the trusted list — pass --allow-untrusted to override or set SCREENER_TRUSTED_HOSTS"
        ));
    }
    ...
    let pipes_dir = screener_core::paths::default_screener_data_dir().join("pipes");
    ...
    let api_base = std::env::var("SCREENER_API_BASE_URL")
        .unwrap_or_else(|_| "https://screener.com".to_string());
...
fn is_trusted_host(url: &str) -> Result<bool> {
    ...
    let env_hosts = std::env::var("SCREENER_TRUSTED_HOSTS").unwrap_or_default();
...
mod tests {
    ...
    fn is_trusted_host_rejects_unknown() {
        // Note: this test reads SCREENER_TRUSTED_HOSTS — we don't set it, so default list applies.
        std::env::remove_var("SCREENER_TRUSTED_HOSTS");
