...
use screener_core::pipes::PipeManager;
...
pub async fn handle_pipe_command(command: &PipeCommand) -> anyhow::Result<()> {
    ...
    // SCREENER_API_KEY is also the local HTTP bearer returned by `auth token`.
    ...
    let user_token = if matches!(command, PipeCommand::Run { .. }) {
        crate::auth_key::resolve_cloud_token(&data_dir, std::env::var("SCREENER_API_KEY").ok())
            .await
    } else {
        None
    };
    ...
    match command {
        PipeCommand::List { json } => {
            ...
            if *json {
                ...
                println!("\nhint: create one at ~/.screener/pipes/my-pipe/pipe.md");
            } else {
                println!(
                    "{:<20} {:<10} {:<12} {:<20}",
...
pub fn api_base_url() -> String {
    std::env::var("SCREENER_API_BASE_URL").unwrap_or_else(|_| "https://screener.com".to_string())
}

/// Resolve the cloud session from a JWT override or the shared SecretStore.
pub async fn get_cloud_auth_token() -> Option<String> {
    let data_dir = screener_core::paths::default_screener_data_dir();
    crate::auth_key::resolve_cloud_token(&data_dir, std::env::var("SCREENER_API_KEY").ok()).await
}
...
async fn handle_publish_command(name: &str, pipes_dir: &std::path::Path) -> anyhow::Result<()> {
    ...
    if !pipe_md_path.exists() {
        anyhow::bail!(
            "pipe.md not found at {}\nhint: make sure the pipe '{}' exists in ~/.screener/pipes/",
    ...
    let token = get_cloud_auth_token()
        .await
        .ok_or_else(|| anyhow::anyhow!("no cloud session found. run `screener login`"))?;
...
async fn handle_info_command(slug: &str) -> anyhow::Result<()> {
    ...
    println!();
    println!("install with: screener pipe install {}", slug);
...
fn model_list(json: bool) -> anyhow::Result<()> {
    ...
    if presets.is_empty() {
        ...
        println!(
            "\nhint: create one with `screener pipe models create <id> --provider … --model …`"
        );
        return Ok(());
    }
...
async fn handle_set_preset_command(
    ...
    if manager.get_pipe(name).await.is_none() {
        anyhow::bail!(
            "pipe '{}' not found — install it first with `screener pipe install <source>`",
            name
        );
    }
