...
//! `screener team` — enterprise admin queries against the org's team
...
//! `screener-team` skill at
//! `crates/screener-core/assets/skills/screener-team/SKILL.md` — this
...
//!
//! Auth: `team_api_token` from `~/.screener/enterprise.json` (admin
//! mints it once at <https://screenpi.pe/enterprise?tab=tokens>). Override
//! via `SCREENER_TEAM_API_TOKEN` env var for scripts/CI. Base URL
//! resolution (same contract as `packages/screener-mcp`):
//! `SCREENER_TEAM_API_URL` env (full v1 base) → legacy
//! `SCREENER_CLOUD_BASE_URL` env (origin) → `gateway_url` in
...
//! laptop, a server), not just one running screener locally.
//!
...
/// file used to say screener.com while every other client said
/// screenpi.pe; the drift is fixed on the screenpi.pe side.
const DEFAULT_V1_BASE: &str = "https://screenpi.pe/api/enterprise/v1";
const ENV_TOKEN: &str = "SCREENER_TEAM_API_TOKEN";
const ENV_TEAM_API_URL: &str = "SCREENER_TEAM_API_URL";
const ENV_BASE_URL: &str = "SCREENER_CLOUD_BASE_URL";
...
`read:devices`, `read:search`, `read:records`, then either:
  ...
  - paste it into desktop Settings → Privacy → Admin Team API Token
    (writes ~/.screener/enterprise.json), or
  - export SCREENER_TEAM_API_TOKEN=<token> for this shell.";
...
/// Base resolution, same contract as packages/screener-mcp: explicit
...
fn read_enterprise_json() -> Option<Value> {
    ...
    let path: PathBuf = home.join(".screener").join("enterprise.json");
...
mod tests {
    ...
    fn parse_duration_units() {
        ...
        // this mirrors the screener-api skill's lenient parsing.
    ...
    fn parses_team_devices() {
        let cli = Cli::try_parse_from(["screener", "team", "devices", "--raw"]).unwrap();
        match cli.command {
            Command::Team {
    ...
    fn parses_team_records_requires_device_id() {
        ...
        let res = Cli::try_parse_from(["screener", "team", "records"]);
    ...
    fn v1_base_resolution_covers_env_gateway_and_default() {
        // Default: hosted screenpi.pe (screener.com drift fixed).
        assert_eq!(
