...
//! `screener agent setup` — make an external agent (OpenClaw, Hermes, Claude)
//! aware of screener in one command: install the screener skill(s) into the
//! agent's skills directory and register the screener MCP server in its config.
//!
//! The skills are host-aware: pass `--api-url` (e.g. when the agent runs on a
//! VPS that holds a synced copy of your screener data) and the written
...
// Canonical skill sources — single source of truth lives in screener-core
...
const API_SKILL_MD: &str =
    include_str!("../../../screener-core/assets/skills/screener-api/SKILL.md");
const CLI_SKILL_MD: &str =
    include_str!("../../../screener-core/assets/skills/screener-cli/SKILL.md");

#[derive(clap::Subcommand, Debug)]
pub enum AgentCommand {
    /// Install the screener skills + MCP server into one agent or every
    ...
    Setup {
        ...
        all: bool,
        /// screener REST API base URL the skill + MCP should target.
    ...
    /// Remove the screener skills + MCP server entry from an agent.
...
pub fn maybe_prompt_connect_detected(data_dir: &Path) {
    if std::env::var("SCREENER_NO_REMINDERS").is_ok()
        || std::env::var("SCREENER_NO_AGENT_SETUP_PROMPT").is_ok()
    ...
    eprintln!(
        "  add screener MCP + supported skills to every detected tool? existing settings are preserved."
    );
    ...
    if accepted {
        ...
        if let Err(error) = setup_all_detected("http://localhost:3030") {
            ...
            eprintln!("  retry with {}", "screener setup".green().bold());
        }
    } else {
        eprintln!(
            "  not now — run {} whenever you're ready.\n",
            "screener setup".green().bold()
        );
    }
}
...
fn is_agent_setup_in(target: &str, home: &Path) -> bool {
    ...
    skills_ready(&layout) && has_screener_mcp(&layout)
}

fn has_screener_mcp(layout: &AgentLayout) -> bool {
    ...
    match layout.mcp_format {
        McpFormat::Json => serde_json::from_str::<serde_json::Value>(&existing)
            .ok()
            .and_then(|root| root.get("mcpServers")?.get("screener").cloned())
            .is_some_and(|entry| {
                !entry.is_null()
                    && (layout.name != "Runner"
        ...
        McpFormat::Toml => existing.lines().any(|line| {
            line.trim() == "[mcp_servers.screener]" || line.trim() == "[mcp_servers.\"screener\"]"
        }),
        McpFormat::Yaml => existing.lines().any(|line| {
            let line = line.trim_start();
            !line.starts_with('#')
                && (line.starts_with("screener:") || line.contains("screener-mcp"))
        }),
    }
}
...
pub fn setup_all_detected(api_url: &str) -> Result<()> {
    ...
    let pending = detected
        ...
        .collect::<Vec<_>>();
    if pending.is_empty() {
        println!("all detected AI tools are already connected to screener");
        return Ok(());
    }
    ...
    if failures.is_empty() {
        println!(
            "\nconnected screener to {} detected AI tool(s)",
            pending.len()
        );
        Ok(())
    } else {
...
fn desktop_launch_config(
    ...
    let mut env = BTreeMap::from([
        ("SCREENER_API_URL".to_string(), api_url.to_string()),
        ("SCREENER_MCP_CLIENT".to_string(), agent.id.to_string()),
    ]);
    if let Some(api_key) = api_key {
        env.insert("SCREENER_LOCAL_API_KEY".to_string(), api_key.to_string());
    }
    McpLaunchConfig {
        command: bun_path.to_string_lossy().to_string(),
        args: vec!["x".to_string(), "screener-mcp@latest".to_string()],
        env,
...
fn install_missing_desktop_skills(layout: &AgentLayout) -> Result<Vec<DesktopSkillChange>> {
    ...
    for (name, markdown) in [
        ("screener-api", API_SKILL_MD),
        ("screener-cli", CLI_SKILL_MD),
    ] {
...
/// How many `.screener-backup-*` siblings to keep per config file.
const MAX_CONFIG_BACKUPS: usize = 2;

fn config_lock_path(path: &Path) -> PathBuf {
    PathBuf::from(format!("{}.screener.lock", path.display()))
}
...
fn replace_config(path: &Path, expected: Option<&str>, contents: &str) -> Result<()> {
    ...
    anyhow::ensure!(
        ...
        "{} changed while screener was preparing its MCP entry; left the newer file untouched",
        path.display()
    );
    ...
    if current.is_some() {
        let ts = std::time::SystemTime::now()
            ...
            .map(|d| d.as_secs())
            .unwrap_or(0);
        let backup = PathBuf::from(format!("{}.screener-backup-{ts}", path.display()));
...
fn prune_config_backups(path: &Path) {
    ...
    let prefix = format!("{name}.screener-backup-");
...
fn write_skill(skills_dir: &Path, name: &str, md: &str, api_url: &str) -> Result<PathBuf> {
    ...
    // target host so an off-box agent hits the right screener.
...
fn setup(target: &str, api_url: &str) -> Result<()> {
    ...
    println!("wiring screener → {} (api: {})", l.name, api_url);
    ...
    println!(
        "\ndone — restart {} so it loads the screener {}, then ask it:\n  \"what was i doing yesterday afternoon?\"",
        l.name,
        if has_skills { "skills + MCP" } else { "MCP" }
    );
    if remote {
        println!(
            "\nnote: the agent will query screener at {api_url}. make sure that host\n      is reachable from the agent (run `screener record` there over your\n      synced data, or expose the API via a tunnel)."
        );
    }
    Ok(())
}

/// Install the canonical screener API and CLI skills for an external agent.
///
...
/// the exact same skill installation behavior as `screener agent setup`.
...
fn install_skills_in(target: &str, api_url: &str, home: &Path) -> Result<Vec<PathBuf>> {
    ...
    Ok(vec![
        write_skill(skills_dir, "screener-api", API_SKILL_MD, api_url)?,
        write_skill(skills_dir, "screener-cli", CLI_SKILL_MD, api_url)?,
    ])
}

/// Remove the two built-in screener skills from an external agent.
///
/// Mirror of [`install_skills`]: deletes only `<skills_dir>/screener-api`
/// and `<skills_dir>/screener-cli`, never the parent skills directory or any
...
fn remove_skills_from(skills_dir: &Path) -> Result<Vec<PathBuf>> {
    ...
    for name in ["screener-api", "screener-cli"] {
        let dir = skills_dir.join(name);
        if dir.exists() {
...
/// `screener agent remove <target>` — undo `setup`. Removes the two
/// screener skills and the screener MCP entry; idempotent, missing
/// files/entries are a no-op.
fn remove(target: &str) -> Result<()> {
    let l = layout(target)?;
    println!("removing screener from {}", l.name);

    let removed = remove_skills(target)?;
    if l.skills_dir.is_none() {
        println!("  · {} is MCP-only (no skills dir)", l.name);
    } else if removed.is_empty() {
        println!("  · no screener skills installed");
    } else {
        for path in removed {
    ...
    println!(
        "\ndone — restart {} so it drops the screener tools.",
        l.name
    );
    Ok(())
}

/// Remove `mcpServers.screener` from a JSON config, preserving everything
...
fn remove_mcp_json(path: &Path) -> Result<()> {
    ...
    let existing = match read_config_text(path)? {
        ...
        _ => {
            println!("  · no screener mcp entry in {}", path.display());
            return Ok(());
        }
    };
...
/// Strip the `[mcp_servers.screener]` table and its `.env` subtable from a
...
fn remove_mcp_toml(path: &Path) -> Result<()> {
    ...
    if !existing.contains("[mcp_servers.screener]") {
        println!("  · no screener mcp entry in {}", path.display());
        return Ok(());
    }
    let mut out: Vec<&str> = Vec::new();
    let mut in_screener = false;
    for line in existing.lines() {
        let trimmed = line.trim();
        if trimmed == "[mcp_servers.screener]" || trimmed == "[mcp_servers.screener.env]" {
            in_screener = true;
            continue;
        }
        if in_screener && trimmed.starts_with('[') {
            in_screener = false;
        }
        if !in_screener {
            out.push(line);
...
/// Remove only the exact `screener:` child `merge_mcp_yaml` writes. Other
...
fn remove_mcp_yaml(path: &Path) -> Result<()> {
    ...
    // Comment-aware: a commented `# screener` mention isn't an entry.
    let has_uncommented_screener = existing
        .lines()
        .any(|l| !l.trim_start().starts_with('#') && l.contains("screener"));
    if !has_uncommented_screener {
        println!("  · no screener mcp entry in {}", path.display());
        return Ok(());
    }
    ...
    let Some(block_start) = lines.iter().position(|line| line == "mcp_servers:") else {
        println!(
            "  • {} references screener outside an mcp_servers block — remove it manually",
            path.display()
        );
        return Ok(());
    };
    ...
    let Some(server_start) = (block_start + 1..block_end)
        .find(|index| yaml_indent(&lines[*index]) == 2 && lines[*index].trim() == "screener:")
    else {
        println!(
            "  • {} has a customized screener MCP entry — remove it manually",
            path.display()
        );
        return Ok(());
    };
...
fn cli_launch_config(remote: bool, api_url: &str) -> McpLaunchConfig {
    let env = if remote {
        BTreeMap::from([("SCREENER_API_URL".to_string(), api_url.to_string())])
    } else {
        BTreeMap::new()
    };
    McpLaunchConfig {
        command: "npx".to_string(),
        args: vec!["-y".to_string(), "screener-mcp@latest".to_string()],
        env,
        transport: None,
        server_type: None,
    }
...
/// Idempotently add the `screener` server to a JSON MCP config (OpenClaw,
...
fn merge_mcp_json_launch(path: &Path, launch: &McpLaunchConfig) -> Result<()> {
    ...
    servers.insert("screener".to_string(), entry);
    replace_config(
        path,
        existing.as_deref(),
        &(serde_json::to_string_pretty(&root)? + "\n"),
...
/// Add the `screener` server to a YAML MCP config (Hermes). We don't pull a
...
fn merge_mcp_yaml_launch(path: &Path, launch: &McpLaunchConfig) -> Result<()> {
    ...
    if uncommented("screener-mcp") {
        anyhow::bail!(
            "{} references screener-mcp outside a standard mcp_servers block; left it untouched",
            path.display()
        );
    }
...
/// Add or repair the `screener` server in a TOML MCP config (Codex),
...
fn strip_screener_toml_tables(existing: &str) -> String {
    let mut out = Vec::new();
    let mut in_screener = false;
    for line in existing.lines() {
        let trimmed = line.trim();
        let screener_header = matches!(
            trimmed,
            "[mcp_servers.screener]"
                | "[mcp_servers.screener.env]"
                | "[mcp_servers.\"screener\"]"
...
fn merge_mcp_toml_launch(path: &Path, launch: &McpLaunchConfig) -> Result<()> {
    ...
    let existing_value = read_config_text(path)?;
    let mut out = strip_screener_toml_tables(existing_value.as_deref().unwrap_or_default());
    if !out.is_empty() && !out.ends_with('\n') {
        out.push('\n');
    }
    if !out.is_empty() {
...
mod tests {
    ...
    fn test_api_skill_routes_learning_to_user_owned_skills() {
        ...
        assert!(API_SKILL_MD.contains("Never modify this `screener-api` skill"));
    ...
    #[test]
    fn test_remove_skills_deletes_only_screener_dirs() {
        let dir = std::env::temp_dir().join(format!("sp-agent-remove-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);

        // Two screener skills plus a user skill that must survive.
        write_skill(&dir, "screener-api", "api", "http://localhost:3030").unwrap();
        write_skill(&dir, "screener-cli", "cli", "http://localhost:3030").unwrap();
        write_skill(&dir, "my-own-skill", "mine", "http://localhost:3030").unwrap();

        let removed = remove_skills_from(&dir).unwrap();
        assert_eq!(
            removed,
            vec![dir.join("screener-api"), dir.join("screener-cli")]
        );
        assert!(!dir.join("screener-api").exists());
        assert!(!dir.join("screener-cli").exists());
    ...
    fn test_merge_mcp_json_fresh_and_idempotent() {
        ...
        assert_eq!(v["mcpServers"]["screener"]["command"], "npx");
        assert!(v["mcpServers"]["screener"]["env"].is_null());
        ...
        assert_eq!(
            v["mcpServers"]["screener"]["env"]["SCREENER_API_URL"],
            "http://box:3030"
        );
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn test_replace_config_backs_up_prunes_and_leaves_no_tmp() {
        ...
        let backups = names
            .iter()
            .filter(|n| n.contains(".screener-backup-"))
            .count();
        assert!(
    ...
    fn test_replace_config_rejects_a_concurrent_edit() {
        ...
        let error = replace_config(&path, Some("stale edit"), "screener edit").unwrap_err();
        assert!(error.to_string().contains("changed while screener"));
    ...
    fn test_authenticated_config_replacement_is_owner_only() {
        ...
        let launch = McpLaunchConfig {
            ...
            args: vec!["x".to_string(), "screener-mcp@latest".to_string()],
            env: BTreeMap::from([(
                "SCREENER_LOCAL_API_KEY".to_string(),
                "sp-secret".to_string(),
            )]),
            transport: None,
            server_type: None,
    ...
    fn test_remove_mcp_json_preserves_other_servers() {
        ...
        std::fs::write(
            &path,
            serde_json::json!({
                "mcpServers": {"other": {"command": "x"}, "screener": {"command": "bun"}},
                "theme": "dark"
            })
            .to_string(),
        )
        ...
        assert!(v["mcpServers"]["screener"].is_null());
    ...
    fn test_remove_mcp_toml_strips_table_and_env() {
        ...
        assert!(!s.contains("mcp_servers.screener"));
        assert!(!s.contains("SCREENER_API_URL"));
    ...
    fn test_merge_mcp_yaml_ignores_commented_example_block() {
        ...
        let s = std::fs::read_to_string(&path).unwrap();
        assert!(s.contains("\nmcp_servers:\n"));
        assert!(s.contains("screener-mcp"));
    ...
    fn test_merge_and_remove_mcp_yaml_preserves_existing_servers() {
        ...
        assert!(merged.contains("  screener:\n    command: \"npx\""));
        assert!(merged.contains("SCREENER_API_URL: \"http://box:3030\""));
    ...
    fn test_remove_mcp_yaml_leaves_foreign_block() {
        ...
        // Hand-authored: screener alongside another server — must not touch.
        let content =
            "mcp_servers:\n  screener:\n    url: http://custom\n  other:\n    command: x\n";
    ...
    fn test_merge_mcp_toml() {
        ...
        assert!(s.contains("model = \"o3\""));
        assert!(s.contains("[mcp_servers.screener]"));
        assert!(s.contains("SCREENER_API_URL = \"http://box:3030\""));

        // idempotent: no duplicate table.
        merge_mcp_toml(&path, true, "http://box:3030").unwrap();
        let s2 = std::fs::read_to_string(&path).unwrap();
        assert_eq!(s2.matches("[mcp_servers.screener]").count(), 1);
        let _ = std::fs::remove_dir_all(&dir);
    }
    ...
    fn test_claude_skills_directory_alone_does_not_invent_claude_code() {
        let dir = tempfile::tempdir().unwrap();
        std::fs::create_dir_all(dir.path().join(".claude/skills/screener-api")).unwrap();
    ...
    fn test_runner_is_connected_only_with_stdio_type() {
        ...
        std::fs::write(
            ...
            serde_json::json!({"mcpServers": {"screener": {"command": "npx"}}}).to_string(),
        )
        .unwrap();
        ...
        std::fs::write(
            home.join(".runner/mcp.json"),
            serde_json::json!({
                "mcpServers": {"screener": {"type": "stdio", "command": "npx"}}
            })
            .to_string(),
        )
        .unwrap();
    ...
    fn test_desktop_background_setup_connects_detected_tools_end_to_end() {
        ...
        let bun = home.join("screener-runtime/bun");
        ...
        assert!(codex.contains("SCREENER_LOCAL_API_KEY = \"sp-test-key\""));
        assert!(codex.contains("SCREENER_MCP_CLIENT = \"codex\""));
        assert!(codex.contains("SCREENER_API_URL = \"http://localhost:31337\""));
        assert!(home.join(".codex/skills/screener-api/SKILL.md").is_file());
        assert!(home.join(".codex/skills/screener-cli/SKILL.md").is_file());
        ...
        assert_eq!(
            cursor["mcpServers"]["screener"]["command"],
            bun.to_string_lossy().as_ref()
        );
        assert_eq!(
            cursor["mcpServers"]["screener"]["env"]["SCREENER_LOCAL_API_KEY"],
            "sp-test-key"
        );
        ...
        assert_eq!(openclaw["mcpServers"]["screener"]["transport"], "stdio");

        let hermes = std::fs::read_to_string(home.join(".hermes/config.yaml")).unwrap();
        assert!(hermes.contains("model: test"));
        assert!(hermes.contains("SCREENER_MCP_CLIENT: \"hermes\""));
        assert!(hermes.contains("SCREENER_LOCAL_API_KEY: \"sp-test-key\""));
        ...
        assert_eq!(runner["mcpServers"]["screener"]["type"], "stdio");
        assert_eq!(
            runner["mcpServers"]["screener"]["env"]["SCREENER_MCP_CLIENT"],
            "runner"
        );
    ...
    fn test_desktop_background_setup_repairs_a_stale_mcp_entry() {
        ...
        std::fs::write(
            home.join(".codex/config.toml"),
            "model = \"gpt-5\"\n\n[mcp_servers.screener]\ncommand = \"npx\"\nargs = [\"stale-package\"]\n\n[mcp_servers.screener.env]\nSCREENER_LOCAL_API_KEY = \"stale-key\"\n",
        )
        .unwrap();
        write_skill(
            &home.join(".codex/skills"),
        ...
        assert!(config.contains("SCREENER_LOCAL_API_KEY = \"current-key\""));
        assert!(config.contains("SCREENER_API_URL = \"http://localhost:4242\""));
        assert!(!config.contains("stale-package"));
        assert!(!config.contains("stale-key"));
        assert_eq!(config.matches("[mcp_servers.screener]").count(), 1);
    }
    ...
    fn test_desktop_background_setup_removes_stale_key_when_auth_is_disabled() {
        ...
        assert!(!config.contains("SCREENER_LOCAL_API_KEY"));
        assert!(config.contains("SCREENER_API_URL = \"http://localhost:3030\""));
        assert_eq!(config.matches("[mcp_servers.screener]").count(), 1);
    }
    ...
    fn test_desktop_background_setup_isolates_invalid_config_failure() {
        ...
        std::fs::create_dir_all(home.join(".cursor/skills/screener-api")).unwrap();
        std::fs::write(
            home.join(".cursor/skills/screener-api/SKILL.md"),
        ...
        assert_eq!(
            std::fs::read_to_string(home.join(".cursor/skills/screener-api/SKILL.md")).unwrap(),
            "custom pre-existing skill"
        );
        assert!(!home.join(".cursor/skills/screener-cli").exists());
        assert!(home.join(".codex/skills/screener-api/SKILL.md").is_file());
    }
    ...
    fn test_setup_all_cli_flag_is_explicit_and_conflicts_with_target() {
        ...
        let cli = crate::cli::Cli::try_parse_from(["screener", "agent", "setup", "--all"]).unwrap();
        assert!(matches!(
            cli.command,
        ...
        assert!(crate::cli::Cli::try_parse_from(["screener", "agent", "setup"]).is_err());
        assert!(
            crate::cli::Cli::try_parse_from(["screener", "agent", "setup", "codex", "--all"])
                .is_err()
        );
    }
}
