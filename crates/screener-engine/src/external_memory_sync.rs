...
//! (`<vault>/<folder>/screener-memories.md`).
//!
//! ## Layering
//!
//! - The *rendering* + *file write* layer lives in
//!   `screener-core::memories::external_sync`. Pure, no DB, easy to
//!   unit-test.
//! - The two *destination definitions* (Claude Code, Codex) live in
//!   `screener-connect::connections::{claude_code, codex}`. They're
...
//!
//! Mirrors the shape of `screener_connect::oauth_refresh_scheduler`
...
use anyhow::Result;
use screener_connect::connections::{load_connection, SavedConnection};
use screener_core::memories::external_sync::{
...
impl ExternalMemorySyncScheduler {
    ...
    pub fn start(
        ...
        let dir_clone = screener_dir.clone();
...
async fn sync_destination(
    ...
    let conn = load_connection(secret_store, screener_dir, dest.id).await;
...
fn apply(
    ...
    // Destinations screener owns end-to-end (e.g. an Obsidian vault note)
    ...
    if dest.owns_target {
        ...
        let entries_used = entries
            .len()
            .min(screener_core::memories::external_sync::MAX_ENTRIES_PER_DIGEST);
    ...
    // screener-owned — no hand-edited content to preserve — so we
    // rewrite it whole.
...
fn resolve_claude_code_path(creds: &serde_json::Map<String, Value>) -> Result<PathBuf> {
    screener_connect::connections::claude_code::resolve_home_path(creds)
}

fn resolve_codex_path(creds: &serde_json::Map<String, Value>) -> Result<PathBuf> {
    screener_connect::connections::codex::resolve_home_path(creds)
}
...
fn resolve_obsidian_path(creds: &serde_json::Map<String, Value>) -> Result<PathBuf> {
    screener_connect::connections::obsidian_memories::resolve_target_dir(creds)
}
...
mod tests {
    use super::*;
    use screener_core::memories::external_sync::{marker_end, marker_start};
    use serde_json::json;
    ...
    fn apply_writes_block_into_destination_file() {
        ...
        match outcome {
            SyncOutcome::Wrote { path, entries: n } => {
                ...
                assert!(
                    outer.contains("@screener-memories.md"),
                ...
                let sidecar = dir.path().join("screener-memories.md");
    ...
    fn apply_reports_change_when_entries_shift() {
        ...
        let sidecar = std::fs::read_to_string(dir.path().join("screener-memories.md")).unwrap();
    ...
    fn apply_writes_owned_note_for_obsidian() {
        ...
        match outcome {
            SyncOutcome::Wrote { path, entries: n } => {
                assert_eq!(n, 1);
                assert!(path.ends_with("screener-memories.md"));
                ...
                assert!(!dir.path().join("screener-memories.md.sidecar").exists());
            }
            other => panic!("expected Wrote, got {:?}", other),
        }
    }
    ...
    async fn run_once_covers_obsidian_destination() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let db = screener_db::DatabaseManager::new("sqlite::memory:", Default::default())
            .await
            .unwrap();
    ...
    async fn sync_destination_writes_when_enabled() {
        ...
        match result.outcome {
            Ok(SyncOutcome::Wrote { path, entries: n }) => {
                ...
                let outer = std::fs::read_to_string(&path).unwrap();
                assert!(outer.contains("@screener-memories.md"));
                assert!(!outer.contains("first fact"));
                // The memory bodies land in the sidecar.
                let sidecar =
                    std::fs::read_to_string(target_dir.join("screener-memories.md")).unwrap();
