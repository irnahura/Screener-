...
fn store_path(screener_dir: &Path) -> PathBuf {
    screener_dir.join("structured-outputs").join("targets.json")
}
...
fn load_unlocked(screener_dir: &Path) -> Result<OutputTargetStore, StructuredOutputError> {
...
fn with_store<T>(
    ...
    let mut store = load_unlocked(screener_dir)?;
    let (result, changed) = operation(&mut store)?;
    if changed {
        write_unlocked(screener_dir, &store)?;
    }
    Ok(result)
}
...
pub fn list_output_targets(
    ...
    with_store(screener_dir, |store| Ok((store.targets.clone(), false)))
}
