...
//! The local daemon: the `screener` binary, the localhost HTTP API, and the
...
//! - `cli/` — argument parsing and startup for the `screener` binary.
...
//! - All database writes go through `screener-db`, which serializes through
//!   `screener-sqlite-coordinator`. Do not open your own pool.
//!
...
pub mod ui_recorder;
// Exposed publicly so the commercial `screener-sdk` (screener/sdk repo)
...
pub use screener_core::Language;
