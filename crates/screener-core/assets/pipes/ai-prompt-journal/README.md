...
- **Falls back gracefully** — no Obsidian? Saves to `~/.screener/ai-prompts/` as plain markdown
...
1. Install this pipe from the screener store
...
4. Check your journal at `{vault}/screener/ai-prompts/YYYY-MM-DD.md`

## How it works

The pipe uses screener's accessibility tree and OCR data to read what's on your screen. It:
...
- Only reads screen data already captured by screener
- Prompts are saved to your local filesystem only
- You control the output location (Obsidian vault or ~/.screener/)
