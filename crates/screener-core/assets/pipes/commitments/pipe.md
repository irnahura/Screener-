...
Read the screener skill first. Then call `structured_output` with `get_targets` before searching. The returned targets include exact schemas, the prior payload, user feedback, and per-item state. Treat the requested target time range as authoritative.

Search the API only. Use at most 5 bounded searches with `limit=10`, covering messages, meetings, email, documents, and issue trackers visible in Screener. Prefer a recent delta, but use enough overlap to reconcile open items. Never run code or probe files in other apps.
