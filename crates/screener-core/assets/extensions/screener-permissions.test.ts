...
import extension, { __testing } from "./screener-permissions";
...
describe("screener permissions curl guard", () => {
  ...
  it("fails closed when a loopback command cannot be parsed safely", () => {
    ...
    ).toBe("could not safely parse the Screener API URL");
  ...
  it("rejects URLs that disguise a remote host as loopback user info", () => {
    ...
    ).toBe("Screener API URL must use an exact loopback hostname");
  });
});

const PIPE_DIR = "/home/u/.screener/pipes/daily-report";
...
describe("screener permissions filesystem sandbox", () => {
...
describe("screener permissions file-tool sandbox", () => {
  beforeEach(() => __testing.setPermissions(sandboxPermissions));
  afterEach(() => __testing.setPermissions(null));
...
describe("screener permissions tool_call hook", () => {
