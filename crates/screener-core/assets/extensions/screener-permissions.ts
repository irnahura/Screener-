...
try {
  ...
  const filePath = path.join(process.cwd(), ".screener-permissions.json");
...
function hitsScreenerApi(cmd: string): boolean {
...
function checkCurlCommand(cmd: string): string | null {
  ...
  if (!url) {
    return "could not safely parse the Screener API URL";
  }
  let pathname: string;
  ...
  try {
    ...
    if (parsed.hostname !== "localhost" && parsed.hostname !== "127.0.0.1") {
      return "Screener API URL must use an exact loopback hostname";
    }
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("tool_call", async (event: any) => {
    ...
    if (hitsScreenerApi(cmd) && !isParsableCurl(cmd)) {
      return {
        block: true,
        reason: "Use curl to access the Screener API. Other HTTP clients are not supported with permissions enabled.",
      };
    }
    if (isParsableCurl(cmd) && hitsScreenerApi(cmd)) {
