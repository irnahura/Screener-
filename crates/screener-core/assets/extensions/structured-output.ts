...
function pipeToken(): string {
  try {
    const raw = readFileSync(
      join(process.cwd(), ".screener-permissions.json"),
      "utf8",
    );
...
function apiHeaders(): Record<string, string> {
  ...
  const authKey = pipeToken() || process.env.SCREENER_LOCAL_API_KEY || "";
...
export default function (pi: ExtensionAPI) {
  const pipeName = process.env.SCREENER_PIPE_NAME;
  if (!pipeName) return;

  const apiBase =
    process.env.SCREENER_LOCAL_API_URL ||
    `http://localhost:${process.env.SCREENER_PORT || "3030"}`;
  ...
  pi.registerTool({
    ...
    description:
      "Fill typed output targets explicitly assigned to this pipe. Call get_targets first. Each target instruction and x-screener-time-range overrides generic pipe defaults for that target. Use feedback history and rated payloads to improve the result, then submit evidence-backed payloads matching the returned JSON schema.",
    parameters,
