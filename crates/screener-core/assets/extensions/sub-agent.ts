...
// - No nesting (SCREENER_SUBAGENT env blocks recursive spawning)
// - Sub-agents inherit pipe token for screener API auth
// - All children killed on parent exit

import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";

// Prevent nesting — sub-agents must not spawn further sub-agents
if (process.env.SCREENER_SUBAGENT === "1") {
...
try {
  ...
  const permPath = path.join(process.cwd(), ".screener-permissions.json");
...
async function spawnSubAgent(prompt: string): Promise<string> {
  ...
  // Inject auth token so sub-agent can call screener API
  let authHint = "";
  if (pipeToken) {
    authHint = `\n\nWhen calling the screener API (localhost:3030), include: -H "Authorization: Bearer ${pipeToken}"`;
  }
  ...
  return new Promise((resolve, reject) => {
    const child = spawn("bun", [
      ...
      "--mode", "print",
      "--no-session",
      "--provider", process.env.PI_PROVIDER || "screener",
      ...
      env: {
        ...process.env,
        SCREENER_SUBAGENT: "1",
        SCREENER_PARENT_PID: String(parentPid),
      },
    });
...
- Sub-agents can use bash (curl screener API at localhost:3030)
...
**Use for:** parallel screener queries, independent research, breaking analysis into pieces
