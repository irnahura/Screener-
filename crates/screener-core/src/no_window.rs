...
//!
//! Screener's UI process is a GUI-subsystem binary with no console of its
...
mod tests {
    ...
    fn a_no_window_command_still_runs_the_program() {
        let output = no_window_command("cmd")
            .args(["/C", "echo screener"])
            .output()
            .expect("spawn cmd with CREATE_NO_WINDOW");
        assert!(output.status.success());
        assert!(String::from_utf8_lossy(&output.stdout).contains("screener"));
    }
