...
lazy_static! {
    static ref PII_PATTERNS: Vec<(Regex, &'static str)> = vec![
        ...
        // shows the full secret in plaintext — common screener failure
...
mod tests {
    ...
    fn test_slack_token_detection() {
        ...
        assert!(contains_pii(&format!(
            "xapp-1-A{}-{}-{}",
            "12345678", "1234567890123", "abcdef"
        )));

        let result = remove_pii(&format!(
            "SLACK_TOKEN=xoxb-{}-{}-{}",
            "123456789012", "1234567890123", "abc"
        ));
