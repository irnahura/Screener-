...
//! screener-engine's `process_priority`: the desktop app embeds the engine
...
//!   screener-audio's `transcribe_sync` for the pattern).
...
//!
//! `SCREENER_KEEP_NORMAL_PRIORITY=1` disables all dips process-wide — the
...
/// Mirrors screener-engine's `analytics::env_value_truthy`.
fn env_value_truthy(value: &str) -> bool {
    let v = value.trim().to_ascii_lowercase();
    !(v.is_empty() || v == "0" || v == "false" || v == "no" || v == "off")
}

/// True when `SCREENER_KEEP_NORMAL_PRIORITY` opts the process out of
...
pub fn keep_normal_priority() -> bool {
    ...
    *KEEP_NORMAL.get_or_init(|| {
        std::env::var("SCREENER_KEEP_NORMAL_PRIORITY")
