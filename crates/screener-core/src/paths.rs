...
//! Path resolution for Screener data directory.

use std::path::Path;
use std::{env, path::PathBuf};

/// Default Screener data directory. When `SCREENER_DATA_DIR` is set (e.g. for E2E tests),
/// that path is used; otherwise `~/.screener`.
pub fn default_screener_data_dir() -> PathBuf {
    env::var("SCREENER_DATA_DIR")
        .ok()
        .filter(|s| !s.is_empty())
        .map(PathBuf::from)
        .unwrap_or_else(|| {
...
/// copy a skill folder into the screener skills store and to mirror that
...
mod best_effort_tests {
    ...
    fn best_effort_exclusion_never_fails_startup() {
        let missing = PathBuf::from("/nonexistent-screener-spotlight-probe/data");
