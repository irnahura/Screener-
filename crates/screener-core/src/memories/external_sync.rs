...
//! External memory sync — write a screener-owned digest of memories
...
//! file, it produces a markdown digest, wraps it in a screener-owned
//! marker block, and writes it atomically.
//!
//! The scheduler that decides *when* to call us lives in
//! `screener-engine`, next to the DB it queries. Splitting the two
...
//! out a region screener fully owns; everything outside is preserved
...
/// inside the block changes incompatibly so older screener builds can
/// detect a newer block and refuse to rewrite it.
pub const RENDER_SCHEMA: u32 = 1;

/// Start sentinel for the screener-owned region. Both halves include
/// the schema version so a future format change is visible to anyone
/// reading the file.
pub fn marker_start() -> String {
    format!("<!-- screener-memories:start v{} -->", RENDER_SCHEMA)
}

pub fn marker_end() -> String {
    "<!-- screener-memories:end -->".to_string()
}
...
/// `screener-core` from needing to depend on `screener-db`.
...
pub struct Destination {
    ...
    /// keeps the outer file small and hand-editable while screener owns
    /// the sidecar end-to-end.
    pub sidecar_filename: Option<&'static str>,
    /// When true, screener owns the *entire* target file — there is no
    ...
    /// into existing content. Used for dedicated, screener-only notes
...
impl Destination {
    pub const CLAUDE_CODE: Destination = Destination {
        ...
        filename: "CLAUDE.md",
        sidecar_filename: Some("screener-memories.md"),
        owns_target: false,
    };
    ...
    /// entirely by screener, so it carries the full digest with no marker
...
pub fn render_block_body(entries: &[MemoryEntry], dest: &Destination) -> String {
    if let Some(sidecar) = dest.sidecar_filename {
        ...
        return format!(
            "## screener memories\n\n\
            screener writes durable memory facts to the sidecar below. \
            Manage them in the screener app — the file is rewritten \
            on every sync.\n\n\
            @{}\n",
            sidecar
        );
...
pub fn render_digest(entries: &[MemoryEntry], dest: &Destination) -> String {
    ...
    out.push_str(&format!(
        "## screener memories\n\n\
        Auto-synced by screener from this user's local memory store. \
    ...
    if sorted.is_empty() {
        out.push_str("_no memories yet — screener will populate this on the next sync._\n");
        return out;
    }
...
/// Render the full note screener writes into a [`Destination::owns_target`]
/// file (today: an Obsidian vault). Because screener owns the whole file
...
/// contents. Used for screener-owned sidecar files where there's no
...
mod tests {
    ...
    fn block_body_for_claude_is_import_reference_not_full_digest() {
        ...
        assert!(
            body.contains("@screener-memories.md"),
    ...
    fn obsidian_destination_uses_expected_id_and_filename() {
        ...
        assert_eq!(Destination::OBSIDIAN.filename, "screener-memories.md");
    ...
    fn owned_note_leads_with_frontmatter_and_inlines_digest() {
        ...
        assert!(!note.contains("@screener-memories.md"));
    ...
    fn sidecar_path_set_for_claude_unset_for_codex() {
        ...
        assert_eq!(
            ...
            Some(home.join("screener-memories.md"))
        );
    ...
    fn write_atomic_full_creates_new_file() {
        let dir = tempfile::tempdir().unwrap();
        let target = dir.path().join("screener-memories.md");
    ...
    fn write_atomic_full_overwrites_entire_file() {
        // Sidecar files are screener-owned end-to-end — no splice.
    ...
    fn write_atomic_leaves_no_temp_sibling_after_success() {
        ...
        assert!(
            entries.iter().all(|n| !n.contains(".screener-tmp")),
