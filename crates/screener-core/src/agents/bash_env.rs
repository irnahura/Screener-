...
//! then reads the skill file, learns about `$SCREENER_LOCAL_API_KEY`, and
...
//! screener API (localhost:3030) or local app server (localhost:11435).
//! Non-screener calls pass through untouched — the token never leaks.
//!
...
//!
//! Reads `$SCREENER_LOCAL_API_KEY` only — every spawn path (Tauri chat,
//! core pipe-executor) is now contractually required to export it.
//! `SCREENER_API_AUTH_KEY` was a historical second name from when the two
...
///
/// Only matches literal localhost/loopback screener API or app-server
/// substrings in command arguments. If none match, `curl` runs unchanged.
pub const WRAPPER_SCRIPT: &str = r#"# screener — auto-injected by pi-agent bash subshells (do not edit by hand)
# Transparently adds Authorization: Bearer to curl calls that target the
# local screener API/app server, tags them with x-screener-session
# (the chat/pipe that owns this agent, from SCREENER_SESSION_ID) so
# owned-browser and notification source links route to the right chat, and — when
# SCREENER_FILTER_PII=1 — rewrites any /search URL to include filter_pii=1
...
#
# Regenerated on every pi-agent spawn from screener-core::agents::bash_env.

# Hide the cloud-LLM JWT (SCREENER_API_KEY) from the agent's bash. The
...
# 2026-05-05): the agent saw an env var named "SCREENER_API_KEY", used
...
# the agent can see is SCREENER_LOCAL_API_KEY, which is correct.
unset SCREENER_API_KEY

_sp_auth_key() {
  # spawn paths guarantee SCREENER_LOCAL_API_KEY is set (see pi.rs).
  printf '%s' "${SCREENER_LOCAL_API_KEY:-}"
}
...
curl() {
  ...
  # (x-screener-session); the navigate handler rides it to the frontend so a
  ...
  sid="${SCREENER_SESSION_ID:-}"
  if [ "${SCREENER_FILTER_PII:-}" = "1" ]; then
    add_filter=1
  fi
  ...
  if [ "$has_local" = "1" ]; then
    ...
    [ -n "$sid" ] && hdrs+=(-H "x-screener-session: $sid")
...
/// Path to the bash wrapper under `~/.screener/<WRAPPER_RELATIVE_PATH>`,
/// relative to the supplied screener data dir.
pub fn wrapper_path_under(screener_dir: &Path) -> PathBuf {
    screener_dir.join(WRAPPER_RELATIVE_PATH)
}

/// Write the wrapper into the given screener data dir if the on-disk
...
/// ```no_run
/// # use std::path::Path;
/// let sp_dir = Path::new("/Users/me/.screener");
/// let p = screener_core::agents::bash_env::ensure_wrapper(sp_dir).unwrap();
/// // cmd.env("BASH_ENV", p);
/// ```
pub fn ensure_wrapper(screener_dir: &Path) -> Result<PathBuf> {
    let path = wrapper_path_under(screener_dir);
...
/// Resolve `~/.screener` using the same logic the rest of the app uses,
...
pub fn ensure_wrapper_in_default_dir() -> Result<PathBuf> {
    ...
    ensure_wrapper(&home.join(".screener"))
}
...
mod tests {
    ...
    fn wrapper_script_reads_canonical_env_var_name() {
        assert!(WRAPPER_SCRIPT.contains("SCREENER_LOCAL_API_KEY"));
        ...
        assert!(
            !WRAPPER_SCRIPT.contains("SCREENER_API_AUTH_KEY"),
    ...
    fn wrapper_script_unsets_cloud_token_env_var() {
        // The agent must not see SCREENER_API_KEY (cloud JWT) — it's
        // distinct from SCREENER_LOCAL_API_KEY (local sp-<uuid>) and
        ...
        assert!(
            WRAPPER_SCRIPT.contains("unset SCREENER_API_KEY"),
            "wrapper must unset SCREENER_API_KEY in pi-agent subshells"
        );
    }
    ...
    fn wrapper_script_references_filter_pii_env() {
        assert!(
            WRAPPER_SCRIPT.contains("SCREENER_FILTER_PII"),
    ...
    fn wrapper_script_tags_session_owner() {
        assert!(
            WRAPPER_SCRIPT.contains("SCREENER_SESSION_ID"),
            "wrapper must read the session id env var"
        );
        assert!(
            WRAPPER_SCRIPT.contains("x-screener-session"),
            "wrapper must send the owner header so navigations route to the right chat"
        );
    }

    /// End-to-end: the shim adds `x-screener-session: <id>` to local API
    /// calls when `SCREENER_SESSION_ID` is set, and never leaks it to
    ...
    fn shim_tags_session_header_for_local_only() {
        ...
        let status = Command::new("bash")
            ...
            .env("SCREENER_LOCAL_API_KEY", "sp-test")
            .env("SCREENER_SESSION_ID", "conv-abc-123")
            .arg("-c")
        ...
        assert!(
            local.contains("x-screener-session: conv-abc-123"),
        ...
        let status = Command::new("bash")
            ...
            .env("SCREENER_LOCAL_API_KEY", "sp-test")
            .env("SCREENER_SESSION_ID", "pipe:daily:7")
            .arg("-c")
        ...
        assert!(
            notify.contains("x-screener-session: pipe:daily:7"),
        ...
        assert!(
            !ext.contains("x-screener-session"),
