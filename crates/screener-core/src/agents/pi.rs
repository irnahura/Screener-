...
pub const SCREENER_API_URL: &str = "https://api.screener.com/v1";
...
const CUSTOM_PROVIDER_USER_AGENT: &str = "screener";
...
pub async fn screener_cloud_models(api_url: &str, token: Option<&str>) -> serde_json::Value {
...
pub struct PiExecutor {
    /// Screener cloud token (for LLM calls via screener proxy).
    ///
    ...
    /// webview doesn't restart the screener sidecar.
    pub user_token: Arc<ArcSwap<Option<String>>>,
    /// Screener API base URL (default: `https://api.screener.com/v1`).
    pub api_url: String,
    /// Bearer token for the *local* screener-server API (localhost:3030).
    /// Exposed to the Pi subprocess as `SCREENER_LOCAL_API_KEY` so bash/TS
    /// pipe code can authenticate against the local server. `SCREENER_API_AUTH_KEY`
...
impl PiExecutor {
    pub fn new(user_token: Option<String>) -> Self {
        Self {
            ...
            api_url: SCREENER_API_URL.to_string(),
            api_auth_key: None,
        }
    }
    ...
    fn cloud_media_analysis_enabled() -> bool {
        // Data-dir scoped, not `~/.screener` scoped: a dev or relocated
        // instance must read the marker its own Settings toggle wrote, not
        // another install's. Identical path for a default install.
        !crate::paths::default_screener_data_dir()
            .join("cloud_media_analysis.disabled")
            .exists()
    }

    fn render_screener_api_skill() -> String {
        let mut s = String::from(include_str!("../../assets/skills/screener-api/SKILL.md"));
        if Self::cloud_media_analysis_enabled() {
            ...
            s.push('\n');
            s.push_str(
                include_str!("../../assets/skills/screener-api/cloud_media_analysis_block.md")
                    .trim_end(),
            );
            s.push('\n');
        }
    ...
    /// Install or wipe the `screener-team` enterprise-admin skill under an
    ...
    ///
    /// Source of truth: `~/.screener/enterprise.json`. The Tauri host
    ...
    pub fn ensure_screener_team_skill(skill_root: &Path) -> Result<Option<PathBuf>> {
        if !cfg!(feature = "enterprise-build") {
            return Ok(None);
        }

        let skill_dir = skill_root.join("screener-team");
        ...
        if should_install {
            ...
            std::fs::write(
                &skill_path,
                include_str!("../../assets/skills/screener-team/SKILL.md"),
            )?;
            debug!("screener-team skill installed at {:?}", skill_path);
            ...
            info!(
                "screener-team skill removed (no longer an enterprise admin or license inactive)"
            );
        }
    ...
    /// True only in the Enterprise app when `~/.screener/enterprise.json`
    ...
    /// Enterprise apps share `~/.screener` on one machine.
    ///
    ...
    fn is_enterprise_admin() -> bool {
        ...
        let ent_path = home.join(".screener").join("enterprise.json");
    ...
    /// Ensure screener skills exist in `project_dir/.pi/skills/`.
    pub fn ensure_screener_skill(project_dir: &Path) -> Result<()> {
        // Always-on baseline skills (every pi-agent session needs these).
        let api_skill = Self::render_screener_api_skill();
        let skills: &[(&str, &str)] = &[
            ("screener-api", api_skill.as_str()),
            (
                "screener-cli",
                include_str!("../../assets/skills/screener-cli/SKILL.md"),
            ),
            (
                "render-html-report",
        ...
        let deprecated = [
            ...
            "screener-elements",
            "screener-media",
            "screener-pipe-creator",
            "screener-pipes",
            "screener-retranscribe",
            "screener-search",
            "screener-qa",
        ];
        ...
        let legacy_team_skill = skills_root.join("screener-team");
    ...
    /// baseline (`screener-api`/`-cli`/`-team`) and hand-authored skills and
    /// safely remove ones the user has since deleted from the store.
    const USER_SKILL_MARKER: &'static str = ".screener-managed";

    /// Baseline skills screener writes into every session itself
    /// ([`Self::ensure_screener_skill`]).
    ...
    pub fn sync_user_skills(project_dir: &Path) -> Result<()> {
        let store = crate::paths::default_screener_data_dir().join("skills");
    ...
    fn sync_user_skills_from(store: &Path, project_dir: &Path) -> Result<()> {
        ...
        if let Ok(entries) = std::fs::read_dir(store) {
            for entry in entries.flatten() {
                ...
                // Never let a store entry shadow a baseline skill screener
                ...
                let copy = (|| -> std::io::Result<()> {
                    ...
                    std::fs::write(
                        ...
                        b"mirrored from <data>/skills by screener\n",
                    )?;
                    Ok(())
                })();
                match copy {
    ...
    /// Install or remove the screener-permissions extension based on config.
    ...
    pub fn ensure_permissions_extension(
        ...
        let ext_path = ext_dir.join("screener-permissions.ts");

        if perms.has_any_restrictions() {
            std::fs::create_dir_all(&ext_dir)?;
            let ext_content = include_str!("../../assets/extensions/screener-permissions.ts");
            std::fs::write(&ext_path, ext_content)?;
            debug!("screener-permissions extension installed at {:?}", ext_path);
        } else if ext_path.exists() {
            std::fs::remove_file(&ext_path)?;
            info!("screener-permissions extension removed (no restrictions configured)");
        }
    ...
    pub fn ensure_screener_skill_filtered(
        project_dir: &Path,
        config: &crate::pipes::PipeConfig,
    ) -> Result<()> {
        use crate::pipes::permissions::PipePermissions;
        ...
        let legacy_team_skill = project_dir.join(".pi").join("skills").join("screener-team");
        if legacy_team_skill.exists() {
            std::fs::remove_dir_all(&legacy_team_skill)?;
            debug!("removed legacy auto-discovered team skill");
        }
    ...
    /// If PipeManager already wrote `.screener-permissions.json`, use filtered.
    /// Otherwise install all skills (CLI / direct executor usage).
    pub fn ensure_screener_skill_auto(project_dir: &Path) -> Result<()> {
        let perms_path = project_dir.join(".screener-permissions.json");
        if perms_path.exists() {
            ...
            Self::ensure_screener_skill(project_dir)
        }
    }
    ...
    /// Install orphan guard extension — kills bun if the parent (screener) dies.
    ...
    /// Web search uses the screener cloud backend, so we only enable it
    /// for screener-cloud to avoid sending data to our backend when the
    /// user chose a local/custom provider.
    pub fn ensure_web_search_extension(project_dir: &Path, provider: Option<&str>) -> Result<()> {
        Self::ensure_web_search_extension_with_api_url(project_dir, provider, SCREENER_API_URL)
    }
    ...
    fn ensure_web_search_extension_with_api_url(
        ...
        let is_screener_cloud = matches!(
            provider,
            None | Some("screener") | Some("screener-cloud") | Some("pi")
        );
    ...
    /// Merge screener provider (and optionally the pipe's own provider) into
    ...
    ///
    /// When a pipe uses a non-screener provider (e.g. ollama, openai), pass
    ...
    pub async fn ensure_pi_config(
        ...
        // Only add screener cloud provider if it's the intended provider
        // (or no provider specified). If the user explicitly chose ollama/openai/custom,
        // do NOT write screener into models.json to avoid silent credit drain via fallback.
        let should_add_screener = match provider {
            None => true,
            Some("screener") | Some("screener-cloud") | Some("pi") => true,
            Some(_) => false,
        };

        if should_add_screener {
            // Use actual token value in apiKey — Pi doesn't resolve bare env var
            // names, so writing the literal string "SCREENER_API_KEY" causes
            ...
            let api_key_value = user_token
                .map(|t| t.to_string())
                .or_else(|| std::env::var("SCREENER_API_KEY").ok())
                .unwrap_or_else(|| "$SCREENER_API_KEY".to_string());
            ...
            let screener_provider = json!({
                ...
                "authHeader": true,
                "headers": {
                    "x-screener-latency": "background",
                    "x-screener-workload": "pipe"
                },
                "models": models
            });
            ...
            {
                providers.insert("screener".to_string(), screener_provider);
            }
        }
        ...
        if let (Some(prov), Some(mdl)) = (provider, model) {
            if prov != "screener" {
                ...
                // providers (including screener cloud). Skip the entry instead.
        ...
        // models.json embeds the raw cloud JWT as the screener provider's
        ...
        // -- auth.json: merge/remove screener token, preserve other providers --
        // Only manage screener auth when screener provider is actually being used.
        ...
        if should_add_screener {
            if let Some(token) = user_token.filter(|token| !token.is_empty()) {
                ...
                if let Some(obj) = auth.as_object_mut() {
                    obj.insert("screener".to_string(), api_key_credential(token));
                }
            ...
            // BYOK-only users never reach the screener branch, but their
    ...
    /// Passthrough for non-screener providers.
    ...
    /// Resolve a screener-cloud model AND validate it against the tier's
    ...
    ///
    /// For non-screener providers (ollama / openai-byok / anthropic-byok /
    ...
    async fn resolve_screener_model(
        &self,
        requested: &str,
        provider: &str,
        ...
        let fetch_allowed = |token: Option<String>| {
            ...
            async move {
                let models = screener_cloud_models(&api_url, token.as_deref()).await;
                models
                    .as_array()
                    .map(|arr| {
                        arr.iter()
    ...
    /// Pure validation step for [`resolve_screener_model`] (network-free so
    /// it's unit-testable).
    ///
    ...
    async fn spawn_pi(
        ...
        if let Some(key) = provider_api_key {
            if !key.is_empty() {
                match resolved_provider {
                    ...
                    // Ensure screener API key is set as env var fallback
                    "screener" if cloud_token.is_none() => {
                        cmd.env("SCREENER_API_KEY", key);
                    }
                    _ => {}
                }
            }
        ...
        // Canonical name: SCREENER_LOCAL_API_KEY. The AUTH_KEY alias is
        ...
        // overwrite). TODO(remove next release): drop SCREENER_API_AUTH_KEY.
    ...
    async fn spawn_pi_streaming(
        ...
        if let Some(ids) = mcp_server_allowlist {
            cmd.env("SCREENER_MCP_SERVER_ALLOWLIST", ids.join(","));
        }
        ...
        // bash shim reads SCREENER_SESSION_ID and adds x-screener-session;
        ...
        if let Some(owner) = session_owner {
            cmd.env("SCREENER_SESSION_ID", owner);
            ...
            if let Some(rest) = owner.strip_prefix("pipe:") {
                ...
                cmd.env("SCREENER_PIPE_NAME", name);
            }
        }
...
impl AgentExecutor for PiExecutor {
    async fn run(
        ...
        // 2. No provider specified → screener cloud (default)
        let resolved_provider = provider.unwrap_or("screener").to_string();

        let (resolved_model, fell_back_from) =
            self.resolve_screener_model(model, &resolved_provider).await;
        ...
        Self::ensure_screener_skill_auto(working_dir)?;
        ...
        let pi_path = find_pi_executable().ok_or_else(|| {
            anyhow!(
                "pi not found. try restarting the app or delete ~/.screener/pi-agent and restart"
            )
        })?;
    ...
    async fn run_streaming(
        ...
        // Re-read after resolution: resolve_screener_model may have refreshed
    ...
    fn screener_api_url(&self) -> &str {
        &self.api_url
    }
}

// ---------------------------------------------------------------------------
// Helpers (extracted from apps/screener-app-tauri/src-tauri/src/pi.rs)
// ---------------------------------------------------------------------------

/// Screener's private pi agent dir (models.json, auth.json, sessions, …).
///
/// Isolated from the user's global `~/.pi/agent` so screener never rewrites
/// config a standalone pi install owns
/// (https://github.com/screenpipe/screener/issues/4002) and never loads the user's
/// global pi packages — whose tools can collide with ours and abort the run —
/// into pipe/chat runs (https://github.com/screenpipe/screener/issues/3812).
/// Every pi spawn must pass this dir via the
/// `PI_CODING_AGENT_DIR` env var (see [`apply_pi_isolation_env`]).
///
/// Escape hatch: `SCREENER_PI_AGENT_DIR` overrides the location; setting it
/// to `~/.pi/agent` restores the old shared-config behavior.
pub fn pi_config_dir() -> Result<PathBuf> {
    let dir = match std::env::var("SCREENER_PI_AGENT_DIR") {
        ...
        _ => crate::paths::default_screener_data_dir().join("pi-config"),
    };
...
/// Return whether a Pi npm package is enabled in screener's isolated config.
///
...
/// Set the env vars that scope a pi subprocess to screener's private agent
...
pub fn apply_pi_isolation_env(apply: &mut dyn FnMut(&str, &str)) {
    ...
    // screener CLI, and `bun x screener@latest` spends ~1.5s resolving the
    ...
    // `default_screener_data_dir` is the one authority for this path on every
    // platform — including Windows, where `%LOCALAPPDATA%\screener` holds
    // bundled tooling (git-portable, bun) but *not* the data dir.
    let dir = crate::paths::default_screener_data_dir();
...
///
/// Earlier releases wrote screener's provider/auth into the global config
...
///   https://github.com/screenpipe/screener/issues/3812.
/// - `sessions/<encoded-cwd>/`: copied only for cwds under the screener
...
fn seed_pi_config_from_global(dest: &Path) {
    ...
    let data_dir = crate::paths::default_screener_data_dir();
...
fn seed_from_global(global: &Path, dest: &Path, data_dir: &Path) -> bool {
    ...
    //   defaults. Screener passes --provider/--model on every spawn, so
    //   they'd never be read — except by a future flagless spawn, which must
    //   not silently land on the user's BYOK provider. Pin screener's own
    //   safe fallback instead ("screener"/"auto": the gateway picks a
    ...
    if let Some(obj) = settings.as_object_mut() {
        obj.remove("packages");
        obj.insert("defaultProvider".to_string(), json!("screener"));
...
/// Screener wrote the cloud token as a bare string, which pi <=0.80 accepted,
/// so after the 0.83 bump every signed-in user's hosted AI (chat *and* every
/// background pipe) fails with "Provider is not configured: screener".
...
fn upgrade_legacy_pi_credential(value: &serde_json::Value) -> Option<serde_json::Value> {
    match value {
        // pi <=0.80 screener format: the bare token string.
...
fn remove_screener_auth_from_path(auth_path: &Path) -> Result<()> {
    if !auth_path.exists() {
        return Ok(());
    }
...
/// Returns the screener-managed pi install directory (`~/.screener/pi-agent/` or SCREENER_DATA_DIR/pi-agent).
fn pi_local_install_dir() -> Option<PathBuf> {
    Some(crate::paths::default_screener_data_dir().join("pi-agent"))
}
...
/// nested `pi` invocations resolve to screener's pinned Pi package, not an
...
fn seed_pi_package_json(install_dir: &Path) {
    ...
    if pkg_path.exists() {
        ...
        // in package.json (SCREENER-APP-AR — bun then errors at SyntaxError).
...
///
/// Pi's shebang is `#!/usr/bin/env node`, but screener only bundles bun
...
fn download_portable_git() -> std::result::Result<String, String> {
    ...
    let screener_dir = PathBuf::from(&local_app_data).join("screener");
    let git_dir = screener_dir.join("git-portable");
    ...
    std::fs::create_dir_all(&screener_dir)
        .map_err(|e| format!("Failed to create screener data dir: {}", e))?;
    ...
    let extract_temp = screener_dir.join("git-portable-extracting");
...
/// no AVX2 required) to `%LOCALAPPDATA%\screener\bun-baseline\bun.exe`.
...
fn download_baseline_bun() -> std::result::Result<String, String> {
    ...
    // apps/screener-app-tauri/scripts/pre_build.js (bunVersion) — bump both
    ...
    let local_app_data =
        std::env::var("LOCALAPPDATA").map_err(|_| "LOCALAPPDATA env var not set".to_string())?;
    let screener_dir = PathBuf::from(&local_app_data).join("screener");
    let bun_dir = screener_dir.join("bun-baseline");
    ...
    let extract_temp = screener_dir.join("bun-baseline-extracting");
    let _ = std::fs::remove_dir_all(&extract_temp);
...
mod tests {
    ...
    fn managed_pipe_guidance_only_ships_in_enterprise_team_skill() {
        let consumer_skill = include_str!("../../assets/skills/screener-cli/SKILL.md");
        let enterprise_skill = include_str!("../../assets/skills/screener-team/SKILL.md");

        assert!(!consumer_skill.contains("Enterprise Team Pipes"));
        assert!(!consumer_skill.contains("screener team pipes"));
        assert!(enterprise_skill.contains("screener team pipes list"));
        assert!(enterprise_skill.contains("screener team pipes schedule"));
    ...
    fn consumer_build_never_enables_enterprise_team_skill() {
        ...
        let sentinel = root.path().join("screener-team").join("KEEP");
        ...
        let installed = PiExecutor::ensure_screener_team_skill(root.path()).unwrap();
    ...
    fn web_search_extension_uses_executor_gateway_url() {
        ...
        PiExecutor::ensure_web_search_extension_with_api_url(dir.path(), Some("screener"), api_url)
        ...
        assert!(!content.contains(SCREENER_API_URL));
    }
    ...
    #[test]
    fn pi_child_path_prefers_screener_local_pi() {
    ...
    ///
    /// Run with:
    /// SCREENER_E2E_CLOUD_TOKEN=... cargo test -p screener-core \
    ///   pi_subagents_parent_child_e2e -- --ignored --nocapture
    #[tokio::test]
    #[ignore]
    async fn pi_subagents_parent_child_e2e() {
        let token = std::env::var("SCREENER_E2E_CLOUD_TOKEN")
            .expect("SCREENER_E2E_CLOUD_TOKEN is required for this live test");
        assert!(
    ...
    fn clear_screener_auth_preserves_other_provider_tokens() {
        ...
        remove_screener_auth_from_path(&auth_path).expect("clear screener auth");
    ...
    /// "Provider is not configured: screener".
    #[test]
    ...
    fn upgrade_at_path_rewrites_legacy_file_and_skips_clean_one() {
        let dir = tempfile::tempdir().expect("tempdir");
        let auth_path = dir.path().join("auth.json");
        std::fs::write(&auth_path, r#"{"screener":"bare-jwt"}"#).expect("write auth");
    ...
    fn clear_screener_auth_missing_file_is_noop() {
        ...
        remove_screener_auth_from_path(&auth_path).expect("missing auth is ok");
    ...
    fn sync_user_skills_mirrors_and_self_cleans() {
        ...
        // A baseline skill already written by screener (no marker) must survive.
        std::fs::create_dir_all(skills.join("screener-api")).unwrap();
        std::fs::write(skills.join("screener-api").join("SKILL.md"), "base").unwrap();
        // A store entry colliding with a baseline name must be ignored, never
        // mirrored — otherwise it would clobber the baseline above.
        std::fs::create_dir_all(store.join("screener-api")).unwrap();
        std::fs::write(store.join("screener-api").join("SKILL.md"), "evil").unwrap();
        ...
        assert_eq!(
            std::fs::read_to_string(skills.join("screener-api").join("SKILL.md")).unwrap(),
            "base"
        );
        assert!(!skills
            .join("screener-api")
        ...
        assert!(skills.join("screener-api").join("SKILL.md").exists());
    ...
    /// First-run seed copies config + screener-owned sessions from the
    ...
    #[test]
    fn seed_from_global_copies_config_and_screener_sessions() {
        let tmp = tempfile::tempdir().expect("tempdir");
        let global = tmp.path().join("global");
        let dest = tmp.path().join("isolated");
        let data_dir = tmp.path().join("home").join(".screener");
        ...
        .unwrap();
        std::fs::write(global.join("auth.json"), r#"{"screener":"tok"}"#).unwrap();
        std::fs::write(
            global.join("settings.json"),
            // The user's personal pi defaults must NOT leak into screener's
        ...
        // Session dirs: one for a screener cwd (copied), one for an
        ...
        // Personal defaults replaced by screener's safe fallback: a future
        // flagless spawn must never silently run on the user's BYOK provider.
        assert_eq!(settings["defaultProvider"], "screener");
        assert_eq!(settings["defaultModel"], "auto");

        // Only the screener-owned session dir came over.
        assert!(dest
            .join("sessions")
            .join(format!("--{}-pi-chat--", encoded))
            .join("s1.jsonl")
    ...
    fn seed_from_global_handles_missing_global_and_self_target() {
        ...
        let dest = tmp.path().join("isolated");
        let data_dir = tmp.path().join(".screener");

        assert!(seed_from_global(&global, &dest, &data_dir));
        assert!(dest.join(PI_MIGRATION_MARKER).exists());

        // dest == global (SCREENER_PI_AGENT_DIR=~/.pi/agent escape hatch):
    ...
    async fn gateway_output_budget_flows_from_http_to_pi_catalog() {
        ...
        let models = screener_cloud_models(&server.uri(), None).await;
    ...
    fn custom_provider_compat_repairs_ai_genesis_and_overrides_sdk_user_agent() {
        for base_url in ["https://ai.ai-genesis.app", "https://api.ai-genesis.app/"] {
            ...
            assert_eq!(provider["headers"]["User-Agent"], "screener");
        }
    }
    ...
    async fn test_ensure_pi_config_adds_ollama_provider() {
        ...
        // the developer's real `~/.screener/pi-chat/.pi/models.json`, so it
        // mutates live config on every `cargo test -p screener-core` and then
        ...
        std::env::set_var("SCREENER_PI_AGENT_DIR", temp.path());
        ...
        std::env::remove_var("SCREENER_PI_AGENT_DIR");
    }
    ...
    /// Regression guard for SCREENER-APP-AR: a corrupted package.json
