...
//! Freshness-bounded launcher for the screener CLI.
//!
//! Both bundled skills tell agents to run `cd "$(mktemp -d)" && bun x
//! screener@latest …`. Measured on macOS 26 / bun 1.x, 5 trials each:
//!
//! | invocation                      | per call |
//! |---------------------------------|----------|
...
//!   `<screener>/cli/<version>/`, so publishing a new one never rewrites a
...
//! `SCREENER_CLI`, and the skills invoke
//! `${SCREENER_CLI:-bun x screener@latest}` so the slow path still works
...
//! `screener@0.4.37…0.4.40` copies in bun's cache on a developer machine are
...
pub const CLI_ENV_VAR: &str = "SCREENER_CLI";

/// Platform package that ships the native binary, for an explicit host.
///
/// Mirrors `PLATFORMS` in `packages/cli/screener/lib/index.js` — keep the two
...
pub fn platform_package_for(os: &str, arch: &str) -> Option<&'static str> {
    Some(match (os, arch) {
        ("macos", "aarch64") => "@screener/cli-darwin-arm64",
        ("macos", "x86_64") => "@screener/cli-darwin-x64",
        ("linux", "x86_64") => "@screener/cli-linux-x64",
        ("windows", "x86_64") => "@screener/cli-win32-x64",
        _ => return None,
    })
}
...
pub fn store_dir(screener_dir: &Path) -> PathBuf {
    screener_dir.join("cli")
}

/// Immutable per-version directory. Never reused for a different build.
pub fn version_dir(screener_dir: &Path, version: &str) -> PathBuf {
    store_dir(screener_dir).join(version)
}

/// Path the receipt points at for `version`.
pub fn binary_path_for(screener_dir: &Path, version: &str) -> PathBuf {
    version_dir(screener_dir, version).join(binary_file_name())
}

fn receipt_path(screener_dir: &Path) -> PathBuf {
    store_dir(screener_dir).join("receipt.json")
}
...
pub fn read_receipt(screener_dir: &Path) -> Option<Receipt> {
    let raw = std::fs::read_to_string(receipt_path(screener_dir)).ok()?;
    Receipt::from_json(&raw)
}

/// Atomic tmp-then-rename, matching `pi.rs` and `preset_fallback.rs`.
pub fn write_receipt(screener_dir: &Path, receipt: &Receipt) -> std::io::Result<()> {
    let dir = store_dir(screener_dir);
    std::fs::create_dir_all(&dir)?;
    let tmp = dir.join(format!("receipt.json.tmp-{}", std::process::id()));
    std::fs::write(&tmp, receipt.to_json())?;
    if let Err(e) = std::fs::rename(&tmp, receipt_path(screener_dir)) {
...
pub fn resolved_cli_binary(screener_dir: &Path) -> Option<PathBuf> {
    let receipt = read_receipt(screener_dir)?;
    let path = binary_path_for(screener_dir, &receipt.version);
...
pub fn needs_refresh(screener_dir: &Path, now: SystemTime, interval: Duration) -> bool {
    match read_receipt(screener_dir) {
        None => true,
        Some(r) => {
            is_stale(&r, now, interval)
                || !is_executable_file(&binary_path_for(screener_dir, &r.version))
        }
    }
}
...
pub fn publish(
    ...
    let dest_dir = version_dir(screener_dir, version);
    ...
    if !is_executable_file(&dest) {
        ...
        // Matches `getBinaryPath` in packages/cli/screener/lib/index.js: a
...
/// Mirrors `getBinaryPath`: `<root>/node_modules/<platform pkg>/bin/screener`.
...
/// Read the installed `screener` package version out of an install root.
...
pub fn refresh_blocking(screener_dir: &Path, bun: &Path) -> std::io::Result<PathBuf> {
    let work = store_dir(screener_dir).join(format!("resolve-{}", std::process::id()));
        ...
        match child.try_wait() {
            ...
            Ok(None) if std::time::Instant::now() >= deadline => {
                ...
                return Err(std::io::Error::new(
                    ...
                    "bun install screener@latest exceeded 180s",
                ));
            }
    ...
    let result = (|| {
        ...
        publish(screener_dir, &version, &binary, SystemTime::now())
    })();
...
pub fn maybe_refresh_in_background(screener_dir: &Path, bun: &Path) {
    use std::sync::atomic::{AtomicBool, Ordering};
    static IN_FLIGHT: AtomicBool = AtomicBool::new(false);

    if !needs_refresh(screener_dir, SystemTime::now(), REFRESH_INTERVAL) {
        return;
    }
    ...
    let dir = screener_dir.to_path_buf();
    let bun = bun.to_path_buf();
    let spawned = std::thread::Builder::new()
        .name("screener-cli-refresh".to_string())
        .spawn(move || {
            match refresh_blocking(&dir, &bun) {
                Ok(p) => tracing::info!("resolved screener cli launcher: {}", p.display()),
                Err(e) => tracing::debug!("screener cli launcher refresh skipped: {e}"),
            }
...
mod tests {
    ...
    fn every_published_target_maps_and_unpublished_ones_do_not() {
        ...
        // `optionalDependencies` in packages/cli/screener/package.json.
        assert_eq!(
            platform_package_for("macos", "aarch64"),
            Some("@screener/cli-darwin-arm64")
        );
        assert_eq!(
            platform_package_for("macos", "x86_64"),
            Some("@screener/cli-darwin-x64")
        );
        assert_eq!(
            platform_package_for("linux", "x86_64"),
            Some("@screener/cli-linux-x64")
        );
        assert_eq!(
            platform_package_for("windows", "x86_64"),
            Some("@screener/cli-win32-x64")
        );
    ...
    fn binary_name_carries_the_exe_suffix_only_on_windows() {
        if cfg!(windows) {
            assert_eq!(binary_file_name(), "screener.exe");
        } else {
            assert_eq!(binary_file_name(), "screener");
        }
    }
    ...
    fn platform_package_matches_the_published_set() {
        ...
        if let Some(pkg) = platform_package() {
            assert!(pkg.starts_with("@screener/cli-"), "unexpected pkg {pkg}");
        }
    }
    ...
    fn installed_version_reads_the_package_and_rejects_junk() {
        let dir = tmpdir();
        let pkg = dir.join("node_modules").join("screener");
        std::fs::create_dir_all(&pkg).unwrap();

        std::fs::write(
            pkg.join("package.json"),
            br#"{"name":"screener","version":"0.4.40"}"#,
        )
        .unwrap();
    ...
    fn installed_version_parses_the_real_published_package_json() {
        // Verbatim head of `node_modules/screener/package.json` as bun
  ...
  "name": "screener",
  "version": "0.4.40",
  "description": "screener CLI - AI that knows everything you've seen, said, or heard",
  "main": "lib/index.js",
  "bin": {
    "screener": "lib/cli.js"
  },
  "optionalDependencies": {
    "@screener/cli-darwin-arm64": "0.4.40",
    "@screener/cli-linux-x64": "0.4.40"
  }
}
"#,
        )
