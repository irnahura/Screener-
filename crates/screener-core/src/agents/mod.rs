...
pub trait AgentExecutor: Send + Sync {
    ...
    /// If `None`, uses screener cloud as default.
    ...
    async fn run_streaming(
        ...
        // agent subprocess as `SCREENER_SESSION_ID` so its local API calls are
    ...
    /// Optional cloud auth token for screener provider proxy.
