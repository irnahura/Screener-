...
const FAST_PATH_ANCHOR: &str =
    "read the screener skill first so you know the meetings + search endpoints.";
    ...
    let mut swaps = vec![
        ...
        FragmentSwap {
            ...
            new: concat!(
                ...
                "\n",
                "  curl -s -H \"Authorization: Bearer $SCREENER_LOCAL_API_KEY\" \\\n",
        ...
        FragmentSwap {
            ...
            old: "keep the wording of this prompt in sync with `buildMeetingSummarizeInstructions` in `apps/screener-app-tauri/lib/utils/meeting-context.ts` (used by the in-app \"summarize with AI\" button) — the two surfaces should produce the same behavior.",
            new: "the instructions below are complete. screener API search is required: use the meeting id and exact meeting time window with the named local HTTP endpoints below. do not inspect app source or recursively search the filesystem; never run recursive `find` or `grep` over the user's home or `~/.screener`.",
        },
        FragmentSwap {
            ...
            old: "the instructions below are complete. do not inspect app source or search outside this pipe folder. never run recursive `find` or `grep` over the user's home or `~/.screener`; use only the named local files and bounded HTTP endpoints below.",
            new: "the instructions below are complete. screener API search is required: use the meeting id and exact meeting time window with the named local HTTP endpoints below. do not inspect app source or recursively search the filesystem; never run recursive `find` or `grep` over the user's home or `~/.screener`.",
        },
        FragmentSwap {
...
mod tests {
    ...
    fn migrate_builtin_pipe_points_meeting_summary_at_trigger_context() {
        ...
        assert!(fixed.starts_with("read the screener skill first."));
    ...
    fn migrate_builtin_pipe_removes_source_search_instruction() {
        let stale = concat!(
            ...
            "`apps/screener-app-tauri/lib/utils/meeting-context.ts` (used by the in-app ",
        ...
        assert!(fixed.contains("screener API search is required"));
        assert!(fixed.contains("never run recursive `find` or `grep`"));
        assert!(fixed.ends_with("read the screener skill first.\n"));
        assert!(migrate_builtin_pipe_text("meeting-summary", &fixed).is_none());
    }
    ...
    fn migrate_builtin_pipe_gives_meeting_summary_a_tool_call_budget() {
        ...
        assert!(!fixed.contains("read the screener skill first"));
    ...
    fn bundled_meeting_summary_needs_no_migration() {
        ...
        assert!(body.contains("screener API search is required"));
    ...
    fn bundled_meeting_summary_batches_its_fetches() {
        ...
        assert!(!body.contains("read the screener skill first"));
