...
//!
//! A pipe is a `pipe.md` file inside `~/.screener/pipes/{name}/`.  The
...
pub struct PipeConfig {
    ...
    pub model: String,
    /// LLM provider override.  Default: none (uses screener cloud).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub provider: Option<String>,
    /// AI preset id(s) from `~/.screener/store.bin` → `settings.aiPresets`.
    ...
    /// When true, the pipe's agent has `SCREENER_FILTER_PII=1` in its
    ...
    ///
    /// Paths are relative to the pipe directory (`~/.screener/pipes/<name>/`).
    /// Example:
    /// ```yaml
    /// artifacts:
    ///   - path: "output/profile.md"
...
// Stored in `~/.screener/pipes/.local-overrides.json`.
// ---------------------------------------------------------------------------
...
/// Implemented in `screener-engine` using SQLite.
...
fn read_chatgpt_token_from_legacy_file() -> Option<String> {
    let path = crate::paths::default_screener_data_dir().join("chatgpt-oauth.json");
...
fn read_chatgpt_token_from_secrets() -> Option<String> {
    ...
    if !db_path.exists() && !screener_secrets::secrets_database_path(&data_dir).exists() {
        return None;
    }
    ...
    let result = tokio::task::block_in_place(|| {
        tokio::runtime::Handle::current().block_on(async {
            let store = screener_secrets::SecretStore::open_for_data_dir_with_vault_key(&data_dir)
                .await
                .ok()?;
...
/// Matches `STORE_MAGIC` in `screener-engine/src/cli/store_file.rs`.
...
fn read_store_bin(path: &Path) -> Option<serde_json::Value> {
    ...
    if data.len() >= STORE_MAGIC.len() && &data[..STORE_MAGIC.len()] == STORE_MAGIC {
        #[cfg(feature = "secrets")]
        {
            use screener_secrets::keychain::{get_key, KeyResult};
            ...
            let plaintext =
                screener_vault::crypto::decrypt_small(&data[STORE_MAGIC.len()..], &key).ok()?;
...
/// Read `~/.screener/store.bin` and find the preset by id.
...
fn resolve_preset(pipes_dir: &Path, preset_id: &str) -> Option<ResolvedPreset> {
    // store.bin lives at ~/.screener/store.bin (sibling of pipes/)
    let store_path = pipes_dir.parent()?.join("store.bin");

    if !store_path.exists() {
        // Bootstrap for CLI users who don't have the app.
        // Default to screener cloud — user needs SCREENER_API_KEY env var.
    ...
    let provider = preset
        ...
        .and_then(|v| v.as_str())
        .and_then(|p| match p {
            "screener-cloud" | "pi" => Some("screener"),
...
///
/// Injected from the engine layer (which owns the SecretStore + screener
/// dir) to keep the scheduler in `screener-core` free of a
/// `screener-connect` dep — that crate already depends on us.
...
async fn setup_pipe_permissions(
    ...
    if let Err(e) = PiExecutor::ensure_screener_skill_filtered(pipe_dir, config) {
    ...
    if perms.has_any_restrictions() || force_write {
        ...
        let perms_path = pipe_dir.join(".screener-permissions.json");
        ...
        let _ = std::fs::remove_file(pipe_dir.join(".screener-permissions.json"));
        None
    }
}
...
pub struct PipeManager {
    ...
    /// Local API auth key — injected into pipe subprocesses as SCREENER_LOCAL_API_KEY
...
impl PipeManager {
    pub fn new(
        ...
        // Initialize fallback registry from the screener data dir
        let screener_dir = pipes_dir.parent().unwrap_or(&pipes_dir);
        let registry = Arc::new(preset_fallback::PresetFallbackRegistry::new(screener_dir));
    ...
    /// Returns the pipes directory (e.g. `~/.screener/pipes/`).
    ...
    pub fn set_local_api_key(&mut self, key: Option<String>) {
        ...
        if let Some(ref k) = key {
            ...
            unsafe { std::env::set_var("SCREENER_LOCAL_API_KEY", k) };
        }
    }
    ...
    fn pipe_not_found_error(&self, name: &str) -> anyhow::Error {
        ...
        if !pipe_dir.exists() {
            return anyhow!(
                "pipe '{}' not found — directory does not exist: {}\nhint: install it first with `screener pipe install <source>`",
                name,
                pipe_dir.display()
            );
        }
            ...
            if running.contains_key(name) {
                return Err(anyhow!(
                    "pipe '{}' is already running — you may already be executing inside this pipe. \
                     Do NOT run `screener pipe run` from within a pipe.",
                    name
                ));
            }
    ...
    fn run_pipe_with_trigger_inner<'a>(
        ...
        Box::pin(async move {
            ...
            let (
                    ...
                    None => {
                        ...
                        return Err(anyhow!(
                            "pipe '{}': preset '{}' not found in settings — {}. \
                             ...
                             create a new preset with `screener pipe models create {} --provider … --model …`, \
                ...
                // falling through to screener cloud.
                match resolve_preset(&self.pipes_dir, "default") {
                    Some(resolved) => {
                        info!(
    ...
    pub async fn start_scheduler(&mut self) -> Result<()> {
        ...
        let handle = tokio::spawn(async move {
            ...
            let mut meeting_summary_refresh_rx = screener_events::subscribe_to_event::<
                serde_json::Value,
            >("meeting_summary_refresh_requested");
            let mut workflow_rx =
                screener_events::subscribe_to_event::<serde_json::Value>("workflow_event");
            // pipe_completed:* uses subscribe_to_all with prefix filtering below
            let mut pipe_completed_rx = screener_events::subscribe_to_all_events();
            ...
            let mut connection_trigger_rx =
                screener_events::subscribe_to_event::<serde_json::Value>("connection_trigger");
            ...
            loop {
                ...
                for (name, config, body) in &pipe_snapshot {
                        ...
                        config.preset.first()
                    {
                        ...
                        // of silently falling through to screener cloud.
                    ...
                    tokio::spawn(async move {
                        ...
                        let _ = screener_events::send_event(
                            &event_name,
                            screener_events::PipeCompletedEvent {
    ...
    fn spawn_connection_trigger_watcher(&self) {
        ...
        tokio::spawn(async move {
            ...
            let mut completed_rx = screener_events::subscribe_to_all_events();
            info!(
...
fn render_pipe_system_prompt(
    ...
    let api_auth_note = if local_api_key.is_some() {
        "\nAPI Authentication: REQUIRED. Add `-H \"Authorization: Bearer $SCREENER_LOCAL_API_KEY\"` to ALL curl requests to the Screener API. The env var is already set in your environment.\n"
    } else {
        ""
    };

    sys.push_str(&format!(
        "CRITICAL: You ARE this pipe. You are already running inside it. NEVER run `screener pipe run` — that would create a recursive duplicate. Execute the task directly using the tools available to you (bash, file I/O, HTTP requests, etc.).\n\nOS: {os}\nOutput directory: ./output/\nScreener API: http://localhost:{api_port}{api_auth_note}\nPrefer bun/TypeScript for scripts. Python may not be installed.\nUser ratings and written corrections for this Pipe are available from authenticated GET http://localhost:{api_port}/feedback. Check them before regenerating recurring output: preserve what earned up ratings and correct what earned down ratings.\nSend notifications via POST http://localhost:11435/notify with {{\"title\": \"...\", \"body\": \"...\"}}. Body supports markdown. File links MUST use absolute paths (e.g. [View log](/Users/me/file.md)), never relative paths like ./output/file.md — relative paths break the notification link handler.\nNotifications support action buttons (`\"actions\": [...]`) so you can ASK the user instead of sending a passive FYI — when a human decision or follow-up would help (send/share/draft/fix/dig deeper), attach actions rather than doing nothing or acting unilaterally. Schema + examples: screener-api skill, Notifications section.\n\n"
    ));
...
/// Example: `/Users/me/.screener/pipes/foo/` → `--Users-me-.screener-pipes-foo--`
#[cfg(test)]
...
mod tests {
    ...
    fn test_pipe_manager() -> PipeManager {
        let dir = std::env::temp_dir().join(format!("screener-test-pipes-{}", std::process::id()));
    ...
    fn acp_presets_are_not_exposed_to_raw_pi_pipes() {
        ...
        assert_eq!(default.provider.as_deref(), Some("screener"));
        ...
        assert_eq!(raw.provider.as_deref(), Some("screener"));
    ...
    fn test_system_prompt_contains_anti_recursion_warning() {
        ...
        assert!(sys.contains("NEVER run `screener pipe run`"));
    ...
    fn test_system_prompt_emits_auth_note_when_local_api_key_present() {
        ...
        assert!(sys.contains("SCREENER_LOCAL_API_KEY"));
    }
    ...
    fn test_encode_pi_session_dir_unix_path() {
        ...
        // /Users/me/.screener/pipes/foo → --Users-me-.screener-pipes-foo--
        let dir = Path::new("/Users/me/.screener/pipes/foo");
        ...
        if let Some(path) = result {
            ...
            assert_eq!(dir_name, "--Users-me-.screener-pipes-foo--");
        }
    }
    ...
    fn test_encode_pi_session_dir_uses_isolated_agent_base() {
        let result = encode_pi_session_dir_from_base(
            Path::new("/Users/me/.screener/pipes/foo"),
            Path::new("/tmp/screener-pi-agent"),
        );
        assert_eq!(
            result,
            Path::new("/tmp/screener-pi-agent/sessions/--Users-me-.screener-pipes-foo--"),
        );
    }
    ...
    fn test_add_on_output_line_preserves_existing_listener() {
        let mut manager = PipeManager::new(
            std::path::PathBuf::from("/tmp/screener-output-listener-test"),
            HashMap::new(),
            None,
            3030,
        );
