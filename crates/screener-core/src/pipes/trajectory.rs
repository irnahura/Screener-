...
//! `<trajectories_dir>` is `$SCREENER_TRAJECTORIES_DIR` if set, else a
//! `trajectories/` sibling of the pipes dir (i.e. `~/.screener/trajectories`).
//!
//! File layout:
...
/// screener data dir (and the enterprise worker can sync one folder).
fn trajectories_root(pipes_dir: &Path) -> PathBuf {
    if let Ok(dir) = std::env::var("SCREENER_TRAJECTORIES_DIR") {
        let dir = dir.trim();
        if !dir.is_empty() {
...
fn max_runs_per_pipe() -> usize {
    std::env::var("SCREENER_TRAJECTORIES_MAX_RUNS")
        .ok()
...
mod tests {
    ...
    fn env_override_and_prune() {
        ...
        std::env::set_var("SCREENER_TRAJECTORIES_DIR", &custom);
        std::env::set_var("SCREENER_TRAJECTORIES_MAX_RUNS", "2");
        for i in 0..4 {
            let w = TrajectoryWriter::create(&pipes_dir, "p", i, "m", "q").unwrap();
            w.finish();
            std::thread::sleep(std::time::Duration::from_millis(5));
        }
        std::env::remove_var("SCREENER_TRAJECTORIES_DIR");
        std::env::remove_var("SCREENER_TRAJECTORIES_MAX_RUNS");
