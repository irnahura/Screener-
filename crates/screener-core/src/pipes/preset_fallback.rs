...
//!
//! State is persisted to `~/.screener/ai_preset_fallback.json` so it survives
//! restarts.
...
impl PresetFallbackRegistry {
    ...
    pub fn new(screener_dir: &Path) -> Self {
        let persist_path = screener_dir.join("ai_preset_fallback.json");
