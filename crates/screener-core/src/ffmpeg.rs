...
// See SCREENER-CLI-V8 (337×/day on Windows) and SCREENER-CLI-VC.
...
// start another download (SCREENER-CLI-WB).
...
fn handle_ffmpeg_installation() -> Result<(), anyhow::Error> {
    ...
    // every launch (SCREENER-CLI-W3). Trust the file if it's on disk.
...
mod tests {
    ...
    fn missing_absolute_path_is_not_usable() {
        ...
        // This is the SCREENER-CLI-V8 condition: a once-resolved ffmpeg whose
        ...
        #[cfg(windows)]
        let missing = PathBuf::from(r"C:\screener-does-not-exist\nope-7f3a\ffmpeg.exe");
        #[cfg(not(windows))]
        let missing = PathBuf::from("/screener-does-not-exist/nope-7f3a/ffmpeg");
        assert!(!ffmpeg_path_is_usable(&missing));
    }

    // SCREENER-CLI-W3: the macOS auto-install lands ffmpeg in ~/.local/bin and
