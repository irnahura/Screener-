// screener — AI that knows everything you've seen, heard, or remembered
// https://screenpi.pe
...
/// See [`remote_is_newer`]. `screener-connect`'s OAuth writer stamps it.
pub const OAUTH_REFRESH_GEN_FIELD: &str = "screener_refresh_gen";
...
pub fn read_connection_tombstones(screener_dir: &Path) -> HashMap<String, String> {
    let path = screener_dir.join(CONNECTION_TOMBSTONES_FILE);
...
pub fn record_connection_tombstone(screener_dir: &Path, key: &str) {
    let mut tombstones = read_connection_tombstones(screener_dir);
    tombstones.insert(key.to_string(), now_rfc3339());
    if let Err(e) = write_connection_tombstones(screener_dir, &tombstones) {
        warn!(
...
pub fn clear_connection_tombstone(screener_dir: &Path, key: &str) {
    let mut tombstones = read_connection_tombstones(screener_dir);
...
pub fn clear_connection_tombstones(screener_dir: &Path, actions: &[ConnectionSyncAction]) {
...
fn read_oauth_gen_highwater_map(screener_dir: &Path) -> HashMap<String, u64> {
    let path = screener_dir.join(OAUTH_GEN_HIGHWATER_FILE);
...
pub fn read_oauth_gen_highwater(screener_dir: &Path, key: &str) -> u64 {
    read_oauth_gen_highwater_map(screener_dir)
        .get(key)
        .copied()
        .unwrap_or(0)
}
...
pub fn bump_oauth_gen_highwater(screener_dir: &Path, key: &str, gen: u64) {
    if gen == 0 {
        return;
    }
    let mut map = read_oauth_gen_highwater_map(screener_dir);
...
fn read_connection_file(screener_dir: &Path) -> HashMap<String, SavedConnection> {
    let path = screener_dir.join("connections.json");
    match std::fs::read_to_string(path) {
...
fn connection_oauth_file_path(screener_dir: &Path, manifest_key: &str) -> std::path::PathBuf {
    screener_dir.join(format!("{}{}", manifest_key, OAUTH_SUFFIX))
}
...
pub async fn build_local_manifest(
    ...
    let source_ts = source_timestamp(&screener_dir.join("connections.json"));

    // 1) File-backed credentials first.
    if let Ok(raw) = std::fs::read_to_string(screener_dir.join("connections.json")) {
        if let Ok(file_store) = serde_json::from_str::<HashMap<String, SavedConnection>>(&raw) {
            ...
            debug!(
                ...
                screener_dir.join("connections.json")
            );
        }
    }
    ...
    if let Ok(entries) = std::fs::read_dir(screener_dir) {
        for entry in entries.flatten() {
            ...
            let path = screener_dir.join(&name);
    ...
    for (key, ts) in read_connection_tombstones(screener_dir) {
...
pub async fn build_local_manifest(screener_dir: &Path, machine_id: &str) -> ConnectionSyncManifest {
    let mut manifest = ConnectionSyncManifest::empty(machine_id);
    let source_ts = source_timestamp(&screener_dir.join("connections.json"));
...
fn remove_oauth_file(screener_dir: &Path, manifest_key: &str) {
    let path = connection_oauth_file_path(screener_dir, manifest_key);
...
pub async fn apply_manifest_to_disk(
    ...
    let mut connection_file = read_connection_file(screener_dir);
        ...
        match action {
            ConnectionSyncAction::Imported(key) | ConnectionSyncAction::Updated(key) => {
                ...
                if conn.is_oauth {
                    ...
                    } else if let Err(e) = write_oauth_token_file(screener_dir, key, token) {
    ...
    if connection_file_dirty {
        if let Err(e) = write_connection_file(screener_dir, &connection_file) {
        ...
        match action {
            ConnectionSyncAction::Imported(key) | ConnectionSyncAction::Updated(key) => {
                ...
                if conn.is_oauth {
                    ...
                    if let Err(e) = write_oauth_token_file(screener_dir, key, token) {
                        errors.push(format!("failed to write oauth token {}: {}", key, e));
                    }
                    connection_file.remove(key);
                    connection_file_dirty = true;
...
mod tests {
    ...
    async fn build_local_manifest_reads_connection_file() {
        ...
        // this test compiles under both `cargo test -p screener-core` (no
