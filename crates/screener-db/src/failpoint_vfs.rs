...
mod tests {
    ...
    async fn write_queue_quarantines_ioerr_across_manager_generations() {
        ...
        let write_pool = SqlitePoolOptions::new()
            ...
            .await
            .unwrap();
        let sem = screener_sqlite_coordinator::sqlite_write_lock(&db);
        ...
        assert!(
            screener_sqlite_coordinator::sqlite_write_lock(&db).is_closed(),
        ...
        let replacement_error = match crate::DatabaseManager::new(
            ...
            screener_config::DbConfig::for_tier(screener_config::DeviceTier::Low),
        )
        .await
        {
            Ok(database) => {
    ...
    async fn write_queue_quarantines_sqlite_full_without_tail_flush() {
        ...
        let queue = spawn_write_drain_with(
            pool.clone(),
            screener_sqlite_coordinator::sqlite_write_lock(&db),
        ...
        assert_eq!(
            screener_sqlite_coordinator::registered_sqlite_hard_fault(&db),
            Some(13)
        );
        assert!(screener_sqlite_coordinator::sqlite_quarantine_exists(&db));
    ...
    async fn direct_transaction_statement_ioerr_quarantines_before_rollback() {
        ...
        let writer_gate = screener_sqlite_coordinator::sqlite_write_lock(&db);
