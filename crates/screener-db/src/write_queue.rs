...
//
//   * SCREENER-CLI-SQ conflated "another writer holds the lock" (expected
...
pub const WRITE_LOCK_HELD_MESSAGE: &str =
    "screener write queue: another writer held the SQLite write lock past this batch's budget";
...
pub const WRITE_POOL_STARVED_MESSAGE: &str =
    "screener write queue: no write-pool connection became available inside this batch's budget";
...
impl WriteQueueHealth {
    ...
    pub fn is_hard_faulted(&self) -> bool {
        self.inner.hard_faulted.load(Ordering::SeqCst)
            || self.inner.hard_fault_path.as_ref().is_some_and(|path| {
                screener_sqlite_coordinator::registered_sqlite_hard_fault(path.as_ref()).is_some()
            })
    }
    ...
    pub(crate) fn latch_hard_fault_code(&self, code: i32) -> bool {
        ...
        if let Some(path) = self.inner.hard_fault_path.as_ref() {
            screener_sqlite_coordinator::latch_sqlite_hard_fault(path.as_ref(), code);
        }
        first_for_manager
    }
    fn note_reopen(&self) {
...
async fn execute_batch(
    ...
    for attempt in 1..=MAX_BUSY_RETRIES {
        ...
        // SCREENER-CLI-RC / CLI-SR. It never discards committed data, only an
    ...
    let mut conn = match conn_opt {
        ...
        None => {
            ...
            // SCREENER-CLI-RC) — escalate to FatalConnection so the drain
...
fn send_error_to_all(batch: &mut Vec<PendingWrite>, error: sqlx::Error) {
    ...
    // checkpoint raise a Sentry issue (SCREENER-CLI-SQ). The drain loop still
...
mod tests {
    ...
    /// Regression for SCREENER-CLI-HA: after the data dir/file vanishes
    ...
    /// the same string. SCREENER-CLI-SQ was therefore untriageable: one
    ...
    /// transport variants. SCREENER-CLI-SN's collateral-abort signal
