...
fn quarantine_startup_error(database_path: &Path, error: SqlxError) -> SqlxError {
    if let Some(code) = crate::sqlite_error::sqlite_hard_fault_code(&error) {
        screener_sqlite_coordinator::latch_sqlite_hard_fault(database_path, code);
    }
    error
}
...
impl DatabaseManager {
    ...
    pub fn coordinated_writer(&self) -> screener_sqlite_coordinator::SqliteWritePool {
        screener_sqlite_coordinator::SqliteWritePool::new(
    ...
    pub async fn new(database_path: &str, config: DbConfig) -> Result<Self, sqlx::Error> {
        screener_sqlite_coordinator::verify_sqlite_runtime().map_err(SqlxError::Protocol)?;
        debug!(
        ...
        screener_sqlite_coordinator::prepare_sqlite_quarantine_reserve(database_file).map_err(
            |error| {
                SqlxError::Protocol(format!(
        ...
        if let Some(code) = screener_sqlite_coordinator::registered_sqlite_hard_fault(database_path)
        {
            return Err(SqlxError::Protocol(format!(
                "SQLite database remains durably quarantined after a hard fault (code: {code}); run `screener db recover` while the app is closed"
            )));
        }
        ...
        if database_file.is_file() {
            if let Err(error) = preflight_existing_database_header(database_file) {
                if let Some(code) = crate::sqlite_error::sqlite_hard_fault_code(&error) {
                    screener_sqlite_coordinator::latch_sqlite_hard_fault(database_file, code);
                }
                return Err(error);
            }
        }
        ...
        // Every screener-controlled capture writer and checkpoint resolves
        // this process-wide coordinator for the physical database path.
        let write_semaphore = screener_sqlite_coordinator::sqlite_write_lock(database_path);
        ...
        // inode. Screener already requires exclusive ownership for live
        ...
        for (pragma, value) in screener_config::WAL_SAFETY_PRAGMAS {
        ...
        // started it (only the standalone `screener-engine` CLI did), so app
...
mod shutdown_tests {
    ...
    async fn startup_rebuilds_missing_speaker_tables_with_current_schema() {
        ...
        let database = DatabaseManager::new(
            ...
            DbConfig::for_tier(screener_config::DeviceTier::Low),
        )
        .await
    ...
    fn early_startup_hard_fault_closes_the_process_writer_gate() {
        ...
        let writer_gate = screener_sqlite_coordinator::sqlite_write_lock(&db_path);
        ...
        assert_eq!(
            screener_sqlite_coordinator::registered_sqlite_hard_fault(&db_path),
            Some(522)
        );
        assert!(
