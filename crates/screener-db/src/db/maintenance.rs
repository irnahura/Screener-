...
impl DatabaseManager {
    ...
    pub async fn repair_database(&self) -> Result<(), anyhow::Error> {
        anyhow::bail!(
            "online SQLite repair is disabled; stop every screener process and run `screener db recover` so recovery operates on a preserved offline generation"
        )
    }
    ...
    /// command so the user can self-heal via the existing `screener db
                ...
                Ok(detail) => {
                    ...
                    error!(
                        ...
                        "DATABASE CORRUPTION DETECTED at startup. Recording is stopping to \
                         protect the database. Quit screener and run \
                         `screener db recover` to rebuild the database (it backs up the \
                         original first)."
                    );
                ...
                Err(e) => {
                    ...
                    error!(
                        ...
                        "startup integrity check could not run (database may be corrupt). \
                         If problems persist, quit screener and run `screener db recover`."
                    );
                }
            }
        });
...
mod wal_maintenance_tests {
    ...
    use screener_config::{DbConfig, DeviceTier};
