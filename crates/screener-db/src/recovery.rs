...
use screener_sqlite_coordinator::{sqlite_file_identity, SqliteFileIdentity};
...
/// `screener db check` already reports as "database is healthy". Callers
...
async fn write_and_reopen_canary(path: &Path) -> Result<()> {
    ...
    sqlx::query(
        "CREATE TABLE IF NOT EXISTS __screener_recovery_write_canary (token TEXT PRIMARY KEY NOT NULL)",
    )
    .execute(&mut connection)
    .await
    .context("creating recovery write canary table")?;
    sqlx::query("DELETE FROM __screener_recovery_write_canary")
        .execute(&mut connection)
        .await
        .context("resetting recovery write canary")?;
    sqlx::query("INSERT INTO __screener_recovery_write_canary (token) VALUES (?)")
        .bind(&token)
        .execute(&mut connection)
        .await
    ...
    let readback: String =
        sqlx::query_scalar("SELECT token FROM __screener_recovery_write_canary LIMIT 1")
    ...
    sqlx::query("DROP TABLE __screener_recovery_write_canary")
...
mod tests {
    ...
    async fn fresh_candidate_passes_integrity_fk_and_reopen_canary() {
        ...
        .bind("__screener_recovery_write_canary")
        .fetch_one(&mut connection)
        .await
