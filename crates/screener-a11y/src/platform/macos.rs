...
// Tauri app (tao's event loop), but NOT for the headless `screener` CLI
...
mod tests {
    ...
    fn test_decode_process_name_rejects_empty_results() {
        assert_eq!(decode_process_name(b"screener", 0), None);
    ...
    fn test_decode_process_name_bounds_and_sanitizes_results() {
        assert_eq!(
            decode_process_name(b"screener\0ignored", 18),
            Some("screener".to_string())
        );
        assert_eq!(
            decode_process_name(b" screener ", 12),
            Some("screener".to_string())
        );
        assert_eq!(
    ...
    fn test_get_clipboard_set_and_read() {
        ...
        let test_text = "screener_clipboard_test_12345";
        {
    ...
    /// verbatim in the focused app's text field after screener quit.
    #[test]
