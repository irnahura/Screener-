...
mod tests {
    ...
    fn test_parse_sway_tree_focused_wayland_window_json() {
            ...
            "nodes": [{
                ...
                "nodes": [{
                    ...
                    "nodes": [{
                        "type": "con",
                        "app_id": "foot",
                        "name": "screener-wayland-ci-active",
                        "pid": 4242,
                        "focused": true,
                        "nodes": [],
                        "floating_nodes": []
        ...
        assert_eq!(window_title, "screener-wayland-ci-active");
        assert_eq!(pid, 4242);
    }
    ...
    fn linux_live_sway_active_window_info_reads_focused_window() {
        if std::env::var("SCREENER_RUN_LIVE_SWAY_TEST").ok().as_deref() != Some("1") {
            eprintln!("skipping live Sway test; set SCREENER_RUN_LIVE_SWAY_TEST=1");
            return;
        }

        let expected_title = std::env::var("SCREENER_WAYLAND_TEST_TITLE")
            .unwrap_or_else(|_| "screener-wayland-ci-active".to_string());
