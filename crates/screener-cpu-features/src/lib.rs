...
//!
//! This is a dependency-free leaf crate because both `screener-core` and
//! `screener-config` need it and config cannot depend on core (package
//! cycle via screener-secrets). `screener-core` re-exports it as
//! `screener_core::cpu_features`, which is the path most callers use.
