...
use screener_sqlite_coordinator::{
...
mod tests {
    ...
    async fn first_migration_refuses_a_quarantined_capture_generation() {
        ...
        let legacy = SqlitePoolOptions::new()
            ...
            .unwrap();
        legacy.close().await;
        screener_sqlite_coordinator::prepare_sqlite_quarantine_reserve(&legacy_path).unwrap();
        assert!(screener_sqlite_coordinator::latch_sqlite_hard_fault(
            &legacy_path,
            13
        ));
