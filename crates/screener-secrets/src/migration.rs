...
/// Migrate legacy secret files from `~/.screener` into the unified SecretStore.
///
...
pub async fn migrate_legacy_secrets(
    ...
    let fixed = fix_secret_file_permissions(screener_dir);
    if fixed > 0 {
    ...
    let connections_path = screener_dir.join("connections.json");
...
/// Walk `screener_dir` and set 0o600 on any file matching secret-like patterns.
/// Returns the count of files whose permissions were fixed.
pub fn fix_secret_file_permissions(screener_dir: &Path) -> usize {
    let mut count = 0;
    ...
    let whatsapp_dir = screener_dir.join("whatsapp-session");
    let whatsapp_marker = screener_dir.join(".whatsapp-perms-fixed");
