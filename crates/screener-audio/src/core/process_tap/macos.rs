...
/// ```
///
/// Path defaults to `$HOME/.screener/audio-exclusions.json` and can be
/// overridden with the `SCREENER_AUDIO_EXCLUSIONS_PATH` environment
...
mod exclusions {
    ...
    pub const ENV_OVERRIDE: &str = "SCREENER_AUDIO_EXCLUSIONS_PATH";
    pub const DEFAULT_RELATIVE_PATH: &str = ".screener/audio-exclusions.json";
    ...
    /// `$HOME/.screener/audio-exclusions.json`.
    ...
    pub fn resolve_to_audio_object_ids(bundle_ids: &[String]) -> Vec<u32> {
        ...
        // in meeting_processes.rs and get_frontmost_pid in screener-screen.
        cidre::objc::ar_pool(|| {
            let mut out = Vec::new();
    ...
    mod tests {
        ...
        fn missing_file_returns_empty() {
            let (ids, mt) = read_bundle_ids(Path::new(
                "/nonexistent/screener-audio-exclusion-test/path.json",
            ));
            assert!(ids.is_empty());
            assert!(mt.is_none());
        }
        ...
        fn resolved_path_default_uses_home() {
            ...
            assert_eq!(
                p,
                PathBuf::from("/Users/anyone/.screener/audio-exclusions.json")
            );
        }
    }
}
...
///
/// Never put Screener's own CoreAudio process object in the exclusion list.
...
/// that opens and clocks the real speaker for the lifetime of screener, which
...
/// screener's behalf and can block Zoom/Slack startup.
