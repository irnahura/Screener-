...
impl From<screener_db::DeviceType> for DeviceType {
    fn from(device_type: screener_db::DeviceType) -> Self {
        match device_type {
            screener_db::DeviceType::Input => DeviceType::Input,
            screener_db::DeviceType::Output => DeviceType::Output,
        }
    }
}

impl From<DeviceType> for screener_db::DeviceType {
    fn from(device_type: DeviceType) -> Self {
        match device_type {
            DeviceType::Input => screener_db::DeviceType::Input,
            DeviceType::Output => screener_db::DeviceType::Output,
        }
    }
}

impl From<DeviceType> for Arc<screener_db::DeviceType> {
    fn from(device_type: DeviceType) -> Self {
...
impl From<screener_db::AudioDevice> for AudioDevice {
    fn from(device: screener_db::AudioDevice) -> Self {
        AudioDevice {
            name: device.name,
...
///
/// Resolves the device by name once (unavoidable — screener's device model
...
pub async fn get_cpal_device_and_config(
    ...
    let config = if is_output_device && !is_display {
        ...
        // live: a meeting app's own mic stream stalling right as screener
...
/// True for screener's own private tap aggregate (macOS Process Tap).
...
mod windows_com_audio {
    ...
    unsafe fn endpoint_has_other_session(
        ...
        for i in 0..count {
            ...
            // screener never chases audio it plays itself. GetProcessId
    ...
    mod tests {
        ...
        ///
        /// Run manually with:
        ///   cargo test -p screener-audio --lib \
...
mod tap_aggregate_filter_tests {
    ...
    fn own_tap_aggregate_is_filtered() {
        assert!(is_own_tap_aggregate("ScreenerProcessTap"));
