...
const SCREENER_CLOUD_REALTIME_URL: &str = "wss://api.screener.com/v1/realtime";
const DEEPGRAM_LIVE_URL: &str = "wss://api.deepgram.com/v1/listen";
const SCREENER_CLOUD_REALTIME_PATH: &str = "/v1/realtime";
...
impl MeetingStreamingProvider {
    pub fn as_str(&self) -> &'static str {
        match self {
            ...
            Self::ScreenerCloud => "screener-cloud",
...
impl Default for MeetingStreamingConfig {
    fn default() -> Self {
        let provider = env::var("SCREENER_MEETING_STREAMING_PROVIDER")
            .ok()
            .as_deref()
        ...
        let endpoint = match provider {
            MeetingStreamingProvider::DeepgramLive => endpoint_from_env(
                &["SCREENER_MEETING_DEEPGRAM_LIVE_URL"],
                DEEPGRAM_LIVE_URL,
                DEEPGRAM_LIVE_PATH,
            ),
            _ => endpoint_from_env(
                &["SCREENER_MEETING_REALTIME_URL"],
                SCREENER_CLOUD_REALTIME_URL,
                SCREENER_CLOUD_REALTIME_PATH,
            ),
        };
        let default_model = match provider {
            MeetingStreamingProvider::SelectedEngine => "selected transcription engine",
            MeetingStreamingProvider::Disabled
            | MeetingStreamingProvider::ScreenerCloud
        ...
        Self {
            enabled: true,
            provider,
            auth_token: env::var("SCREENER_MEETING_CLOUD_TOKEN")
                .ok()
                .filter(|s| !s.trim().is_empty()),
            api_key,
            endpoint,
            model: Some(
                env_non_empty("SCREENER_MEETING_TRANSCRIPTION_MODEL")
                    .unwrap_or_else(|| default_model.to_string()),
            ),
            language: env::var("SCREENER_MEETING_TRANSCRIPTION_LANGUAGE")
                .ok()
                .filter(|s| !s.trim().is_empty()),
            local_speaker_name: env_non_empty("SCREENER_MEETING_LOCAL_SPEAKER_NAME"),
            persist_finals: true,
            keyterms: Vec::new(),
        }
    }
...
impl FromStr for MeetingStreamingProvider {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        match value.trim().to_ascii_lowercase().as_str() {
            ...
            "screener-cloud" | "screener_cloud" | "screener" | "cloud" => Ok(Self::ScreenerCloud),
...
impl MeetingStreamingConfig {
    ...
    pub fn from_settings(
        ...
        } else if config.provider == MeetingStreamingProvider::ScreenerCloud {
            config.endpoint = endpoint_from_env(
                &["SCREENER_MEETING_REALTIME_URL"],
                SCREENER_CLOUD_REALTIME_URL,
                SCREENER_CLOUD_REALTIME_PATH,
    ...
    pub fn live_transcription_ready(&self) -> bool {
        match self.provider {
            ...
            MeetingStreamingProvider::ScreenerCloud => self
                .auth_token
                .as_deref()
...
fn provider_api_key(provider: &MeetingStreamingProvider) -> Option<String> {
    let keys: &[&str] = match provider {
        MeetingStreamingProvider::DeepgramLive => {
            &["SCREENER_MEETING_DEEPGRAM_API_KEY", "DEEPGRAM_API_KEY"]
        }
        MeetingStreamingProvider::Disabled
        | MeetingStreamingProvider::SelectedEngine
        | MeetingStreamingProvider::ScreenerCloud => &[],
...
mod tests {
    ...
    #[test]
    fn screener_cloud_uses_cloud_token_for_readiness() {
        ...
        assert_eq!(config.provider, MeetingStreamingProvider::ScreenerCloud);
        assert!(config.live_transcription_ready());
        assert_eq!(config.local_speaker_name.as_deref(), Some("Alice"));
    }

    #[test]
    fn screener_cloud_is_not_ready_without_cloud_login() {
        let config =
    ...
    fn realtime_endpoint_normalization_rejects_hostless_urls() {
        assert_eq!(
            normalize_realtime_endpoint("wss://", SCREENER_CLOUD_REALTIME_PATH),
            None
        );
        assert_eq!(
            normalize_realtime_endpoint("https://", SCREENER_CLOUD_REALTIME_PATH),
            None
        );
        assert_eq!(
            normalize_realtime_endpoint("", SCREENER_CLOUD_REALTIME_PATH),
            None
        );
    }
    ...
    fn realtime_endpoint_normalization_accepts_https_base_url() {
        assert_eq!(
            normalize_realtime_endpoint("https://api.screenpi.pe", SCREENER_CLOUD_REALTIME_PATH)
                .as_deref(),
