...
async fn run_stream(
    ...
    let credential = match config.provider {
        MeetingStreamingProvider::ScreenerCloud => config
            .auth_token
            .as_deref()
            .filter(|s| !s.trim().is_empty())
            .context("Screener Cloud login is required for live meeting transcription")?,
    ...
    request.headers_mut().insert(
        USER_AGENT,
        HeaderValue::from_static("screener-meeting-streaming-deepgram"),
    );
...
fn auth_header(provider: &MeetingStreamingProvider, credential: &str) -> String {
    match provider {
        MeetingStreamingProvider::ScreenerCloud => format!("Bearer {credential}"),
...
fn handle_results_event(
    ...
    if is_final {
        ...
        let _ = screener_events::send_event("meeting_transcript_final", event);
    } else {
        ...
        let _ = screener_events::send_event("meeting_transcript_delta", event);
    }
}
...
mod tests {
    ...
    fn keyterms_are_appended_to_live_query() {
        ...
        assert!(q.contains("keyterm=Screener"), "got: {q}");
    ...
    async fn stream_outcome(api_key: &str, language: &str) -> (bool, Option<String>) {
        ...
        request.headers_mut().insert(
            USER_AGENT,
            HeaderValue::from_static("screener-meeting-streaming-deepgram-test"),
        );
    ...
    async fn deepgram_live_streaming_accepts_language_params() {
        let Some(api_key) = std::env::var("SCREENER_MEETING_DEEPGRAM_API_KEY")
            .ok()
            ...
            eprintln!(
                "skipping deepgram live smoke test: set SCREENER_MEETING_DEEPGRAM_API_KEY (or DEEPGRAM_API_KEY) to run it"
            );
            return;
        };
