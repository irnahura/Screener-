...
pub fn start_meeting_streaming_loop(
    ...
    tokio::spawn(async move {
        ...
        let mut started_sub =
            screener_events::subscribe_to_event::<MeetingLifecycleEvent>("meeting_started");
        let mut ended_sub =
            screener_events::subscribe_to_event::<MeetingLifecycleEvent>("meeting_ended");
        let mut delta_sub = screener_events::subscribe_to_event::<MeetingTranscriptDelta>(
            "meeting_transcript_delta",
        );
        let mut final_sub = screener_events::subscribe_to_event::<MeetingTranscriptFinal>(
            "meeting_transcript_final",
        );
        let mut error_sub =
            screener_events::subscribe_to_event::<MeetingStreamingError>("meeting_streaming_error");
        let mut reattach_sub = screener_events::subscribe_to_event::<MeetingLifecycleEvent>(
...
async fn start_streaming_session(
    ...
    *active = Some(ActiveMeetingStream {
    ...
    let _ = screener_events::send_event("meeting_streaming_session_started", started);
}
...
fn emit_session_ended(session: ActiveMeetingStream) {
    ...
    let _ = screener_events::send_event("meeting_streaming_session_ended", ended);
}
...
fn emit_status(
    ...
    let _ = screener_events::send_event("meeting_streaming_status_changed", status);
}
...
fn route_frame_to_provider(
    ...
    if !session.device_senders.contains_key(&key) {
        ...
        match config.provider {
            ...
            MeetingStreamingProvider::ScreenerCloud | MeetingStreamingProvider::DeepgramLive => {
...
async fn effective_streaming_config(
    ...
    if background.is_screener_cloud() {
        let mut cloud_config = config
            .clone()
            .with_provider(MeetingStreamingProvider::ScreenerCloud);
        ...
        if cloud_config.live_transcription_ready() {
            info!(
                "meeting streaming: selected-engine resolved to screener-cloud live because the selected transcription engine is screener cloud"
            );
            return cloud_config;
        }
    ...
    match config.provider {
        ...
        MeetingStreamingProvider::SelectedEngine => match transcription_engine.read().await.as_ref()
        {
            Some(engine) if engine.config() == AudioTranscriptionEngine::Disabled => Some(
                "Choose an audio transcription engine to use live meeting notes without Screener Cloud"
                    .to_string(),
            ),
            Some(_) => None,
            None => Some("Selected transcription engine is still loading".to_string()),
        },
        MeetingStreamingProvider::ScreenerCloud if config.live_transcription_ready() => None,
        MeetingStreamingProvider::ScreenerCloud => Some(
            "Log in to Screener Cloud to enable live meeting transcription".to_string(),
        ),
        MeetingStreamingProvider::DeepgramLive if config.live_transcription_ready() => None,
        MeetingStreamingProvider::DeepgramLive => Some(
            "Direct Deepgram live transcription needs a Deepgram API key; Screener Cloud does not"
                .to_string(),
        ),
    }
}
...
fn emit_error(
    ...
    let _ = screener_events::send_event("meeting_streaming_error", event);
}
...
mod tests {
    ...
    // `check_and_emit_stall_notifications` calls `screener_events::send_event`,
    ...
    async fn live_not_ready_session_keeps_background_recording_enabled() {
        ...
        let mut active = None;
        let config =
            MeetingStreamingConfig::from_settings(true, "screener-cloud", None, None, None, None);
    ...
    #[tokio::test]
    async fn selected_screener_cloud_uses_cloud_live() {
        let engine = TranscriptionEngine::new(
            Arc::new(AudioTranscriptionEngine::Deepgram),
            Some(DeepgramTranscriptionConfig::screener_cloud(
        ...
        assert_eq!(effective.provider, MeetingStreamingProvider::ScreenerCloud);
