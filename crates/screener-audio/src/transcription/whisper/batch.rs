...
// in screener-core (`BackgroundWorkDip`) and is shared with the vision
...
fn transcribe_sync(
    ...
    let _priority_dip = screener_core::thread_priority::BackgroundWorkDip::new();
...
mod tests {
    ...
    fn stt_priority_dip_lowers_and_restores() {
        use screener_core::thread_priority::BackgroundWorkDip;
    ...
    fn joins_entries_within_budget() {
        let p = build_initial_prompt(&vocab(&["Arvind", "Vercel", "Screener"]), 800);
        assert_eq!(p, "Arvind, Vercel, Screener");
