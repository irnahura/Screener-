...
fn find_reset_in(value: &Value) -> Option<f64> {
    match value {
        ...
        // Screener Cloud currently wraps Deepgram's JSON object in the
    ...
    match response {
                ...
                || body_text.starts_with("<html")
            {
                error!(
                    "device: {}, deepgram request blocked — received HTML instead of JSON (status {}). \
                     ...
                     requests to api.screener.com. body preview: {}",
                    device,
                    status,
                ...
                return Err(anyhow::anyhow!(
                    "Audio transcription blocked by network (firewall/proxy). \
                     Please check that api.screener.com is accessible from your network."
                ));
            }
        ...
        Err(e) => {
            ...
            if err_str.contains("timed out") || err_str.contains("timeout") {
                error!(
                    "device: {}, deepgram request timed out — possible firewall blocking api.screener.com: {:?}",
                    device, e
                );
                return Err(anyhow::anyhow!(
                    "Audio transcription request timed out. Check if api.screener.com is accessible from your network."
                ));
            }
            ...
            if err_str.contains("connection refused") || err_str.contains("Connection refused") {
                ...
                return Err(anyhow::anyhow!(
                    "Connection refused to audio transcription server. A firewall may be blocking api.screener.com."
                ));
            }
...
mod tests {
    ...
    fn vocabulary_becomes_plain_keyterms_without_intensifier() {
        ...
        assert!(params.contains("&keyterm=Screener"), "got: {params}");
