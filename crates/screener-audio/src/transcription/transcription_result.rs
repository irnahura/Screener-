...
use screener_db::{DatabaseManager, NewDiarizationSegment, Speaker};
...
pub async fn process_transcription_result(
    ...
    // its log line is the one SCREENER-CLI-SN reports. Requiring the path
    ...
    for retry in 0..CHUNK_TRANSCRIPTION_INSERT_ATTEMPTS {
        match db
            .insert_audio_chunk_and_transcription(
                ...
                &screener_db::AudioDevice {
                    name: result.input.device.name.clone(),
                    device_type: match result.input.device.device_type {
                        crate::core::device::DeviceType::Input => screener_db::DeviceType::Input,
                        crate::core::device::DeviceType::Output => screener_db::DeviceType::Output,
                    },
                },
                speaker_id,
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                // what SCREENER-CLI-SN reports and what makes it look like a
                ...
                if retry + 1 < CHUNK_TRANSCRIPTION_INSERT_ATTEMPTS {
                    ...
                    // Whisper pass). SCREENER-CLI-RC. device is a structured
...
mod tests {
    ...
    // SCREENER-CLI-RC: when the chunk+transcription insert exhausts its
