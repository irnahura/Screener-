...
/// 583 events from the model-missing error alone (Sentry SCREENER-CLI).
...
/// Mirrors the MeetingEvent struct from screener-events for deserialization.
...
impl AudioManager {
    pub async fn new(options: AudioManagerOptions, db: Arc<DatabaseManager>) -> Result<Self> {
        let effective_windows_input_aec_enabled =
            options.windows_input_aec_enabled && !options.screener_aec_enabled;
        let effective_macos_input_vpio_enabled =
            options.macos_input_vpio_enabled && !options.screener_aec_enabled;
        ...
        if effective_windows_input_aec_enabled {
            info!("screener-audio: Windows WASAPI microphone AEC enabled in settings");
        }
        if effective_macos_input_vpio_enabled {
            info!(
                "screener-audio: macOS VoiceProcessingIO (AEC) enabled in settings (default input only)"
            );
        }
    ...
    pub async fn start_device(&self, device: &AudioDevice) -> Result<()> {
        ...
        #[cfg(target_os = "macos")]
        if screener_config::should_pause_audio_for_lock() {
            debug!(
        ...
        // Smart recording is Screener's single capture owner for the
    ...
    async fn record_device(&self, device: &AudioDevice) -> Result<JoinHandle<Result<()>>> {
        ...
        let screener_aec_enabled = options.screener_aec_enabled;
    ...
    /// Whether a detected meeting currently makes piggyback Screener's
    ...
    pub async fn languages(&self) -> Vec<screener_core::Language> {
...
async fn run_meeting_speaker_constraint_loop(
    ...
    let mut meeting_sub =
        screener_events::subscribe_to_event::<MeetingEventData>("meeting_started");
    let mut ended_sub = screener_events::subscribe_to_event::<MeetingEventData>("meeting_ended");
        ...
        let first = get_or_create_engine(
            ...
            move || async move {
                ...
                TranscriptionEngine::new(
                    ...
                    None,
                    None,
                    vec![screener_core::Language::English],
                    Vec::new(),
                )
                .await
            },
