...
//! this is Screener's single capture owner: its normal configured input and
...
/// set. Observed live (2026-07-08, Zoom join): rebuilding screener's own
...
/// grabbing the mic before screener reacts. Cancelled outright (not just
...
pub(crate) struct MicFollowObservation {
    ...
    /// Devices held closed by Screener's meeting ownership gate.
...
pub(crate) async fn run_meeting_piggyback_sweep(
    ...
    use screener_events::AudioCaptureHealthEvent;
    ...
    //    observed) still owns Screener capture. MANUAL meetings
    ...
    //    screener's own), damped by persistence-gated adoption so a flapping holder
                            ...
                            match audio_manager.start_session_device(&device, None).await {
                                ...
                                Err(e) => {
                                    ...
                                    if !state.mic_fail_reported {
                                        ...
                                        let _ = screener_events::send_event(ev.event_name(), ev);
    ...
    if state.last_meeting_seen && !meeting_now && state.telemetry.meeting_seen {
        ...
        let _ = screener_events::send_event("piggyback_meeting_summary", summary);
...
mod tests {
    ...
    fn enabled_resolved_mic_adopts_with_zero_device_actions() {
        // The meeting app uses the mic screener already records (the common
