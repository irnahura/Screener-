    ...
    *DEVICE_MONITOR.lock().await = Some(tokio::spawn(async move {
        ...
        loop {
            ...
            if audio_manager.status().await == AudioManagerStatus::Running {
                #[cfg(target_os = "macos")]
                {
                    let should_suspend = screener_config::should_pause_audio_for_lock();
                ...
                if meetings_only_action == MeetingsOnlyAudioAction::Resume {
                    ...
                    // may have changed while Screener intentionally owned no
                    // streams.
                        ...
                        && !currently_available_devices.contains(&device)
                    {
                        ...
                        match audio_manager.start_device(&device).await {
                            ...
                            Err(e) => {
                                ...
                                // hits Sentry. SCREENER-CLI-S8: 4 users × ~50 events/wk of
...
async fn run_pinned_input_fallback_sweep(
    ...
    use screener_events::AudioDeviceFallbackEvent;
...
mod tests {
    ...
    // disabled their built-in mic from the screener-side enabled set; macOS
    // default input is "MacBook Pro Microphone (input)" (NOT user-disabled —
    // user just didn't pin it in screener).
