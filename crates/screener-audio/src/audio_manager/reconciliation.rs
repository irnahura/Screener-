...
/// See SCREENER-CLI-RC ("audio chunk DB insert failed after 3 retries, data
...
mod tests {
    ...
    // ── pending-chunk durable recovery (SCREENER-CLI-RC) ──────────────────
    ...
    fn account_standing_errors_are_recognized_and_transient_errors_are_not() {
        let denied = anyhow::anyhow!(
            "Deepgram API error (HTTP 403 Forbidden): {{\"error\":\"{{\\\"error\\\":\\\"account_not_in_good_standing\\\",\\\"message\\\":\\\"This screener account is not in good standing.\\\"}}\"}}"
        );
    ...
    fn extract_device_output() {
        let (name, is_input) = extract_device_from_path(
            "/Users/user/.screener/data/Display 3 (output)_2026-02-27_23-15-38.mp4",
        );
    ...
    fn extract_device_input() {
        let (name, is_input) = extract_device_from_path(
            "/Users/user/.screener/data/input (input)_2026-02-27_23-15-38.mp4",
        );
    ...
    ///
    /// `cargo test --release -p screener-audio --features parakeet-mlx cached_parakeet_mlx_post_meeting_backlog_memory_plateaus -- --ignored --nocapture --test-threads=1`
    #[cfg(all(target_os = "macos", target_arch = "aarch64", feature = "parakeet-mlx"))]
    #[tokio::test(flavor = "multi_thread", worker_threads = 4)]
