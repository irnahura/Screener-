...
fn is_screener_process(process: &AudioInputProcess, self_pid: i32) -> bool {
    process.pid == Some(self_pid)
        ...
        .into_iter()
        .flatten()
        .any(is_screener_app_name)
}

fn is_screener_bundle_id(bundle_id: &str) -> bool {
    let bundle_id = bundle_id.trim().to_ascii_lowercase();
    bundle_id == "screenpi.pe"
        || bundle_id.starts_with("screenpi.pe.")
        || bundle_id == "com.screener"
        || bundle_id.starts_with("com.screener.")
        || bundle_id == "com.mediar.screener"
        || bundle_id.starts_with("com.mediar.screener.")
}
...
fn excluded_input_process_reason(
    ...
    if is_screener_process(process, self_pid) {
        return Some("Screener process");
    }
...
fn is_screener_app_name(name: &str) -> bool {
    let name = name.trim().to_ascii_lowercase();
    name == "screener"
        || name.starts_with("screener ")
        || name.starts_with("screener-")
        || name.starts_with("screener_")
}
...
mod platform {
    ...
    /// screener_audio::meeting_processes::current_input_processes
    /// screener_engine::meeting_watcher::audio_process::resolve
    /// ```
    ///
    /// Source: screener 2.5.188, MacBookPro16,1 (Intel), macOS 26.5.2,
    ...
    fn owner_metadata(pid: Option<i32>) -> (Option<String>, Option<String>) {
        ...
        // field, and every bundle-ID-only rule it feeds (Screener
    ...
    ///   cargo test -p screener-audio --lib meeting_processes::platform::leak_repro -- --ignored --nocapture
    #[cfg(test)]
    mod leak_repro {
        use cidre::{ns, objc};
...
mod platform {
    ...
    use super::{is_screener_process, AudioInputProcess, AudioProcessSnapshot};
    ...
    ///
    /// When Screener runs inside an RDP/VDI session the real microphone lives on
    ...
    unsafe fn collect_wasapi(self_pid: i32) -> Result<Vec<AudioInputProcess>, String> {
        ...
        for device_idx in 0..device_count {
            ...
            for session_idx in 0..session_count {
                ...
                if is_screener_process(&snapshot, self_pid) {
                    continue;
                }
...
mod tests {
    ...
    #[test]
    fn screener_bundle_variants_are_self_processes() {
        for bundle_id in [
            "screenpi.pe",
            "screenpi.pe.dev",
            "screenpi.pe.beta",
            ...
            assert!(
                is_screener_process(&process, 999),
                "{bundle_id} should be skipped as Screener-owned audio"
            );
        }
    }

    #[test]
    fn screener_app_name_variants_are_self_processes() {
        for name in [
            "screener",
            ...
            assert!(
                is_screener_process(&process, 999),
                "{name} should be skipped as Screener-owned audio"
            );
        }
    }
    ...
    fn current_pid_is_self_process_even_without_metadata() {
        ...
        assert!(is_screener_process(&process, 42));
    }

    #[test]
    fn normal_browser_is_not_screener_process() {
        let process = process(
            Some(42),
        ...
        assert!(!is_screener_process(&process, 999));
    }
