...
use screener_core::{Language, TESSERACT_LANGUAGES};
...
/// `screener` binary, because — unlike the `.deb` (which `depends` on
...
/// worker (SCREENER-CLI-V3 / CLI-T0: Linux/npx hosts with no system tesseract
...
pub fn perform_ocr_tesseract(
    ...
    // every frame (SCREENER-CLI-V3 / CLI-T0). Returns the same empty sentinel
    ...
    // panicky deps (fbank #4159, ort #3290). SCREENER-CLI-V3 / CLI-T0.
...
mod tests {
    ...
    // SCREENER-CLI-V3 / CLI-T0: rusty_tesseract panics (unwrap on None) when
