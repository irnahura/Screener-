...
/// Coupling these means screener captures exactly the pixels it stores —
...
fn is_clamshell_inactive_builtin(display_id: u32) -> bool {
    screener_core::display_topology::is_clamshell_inactive_builtin(display_id)
}
    ...
    F: FnOnce() -> std::result::Result<T, MonitorListError> + Send + 'static,
{
    ...
    let permit = workers.try_acquire_owned().map_err(|e| match e {
        tokio::sync::TryAcquireError::NoPermits => MonitorListError::Other(
            "ScreenCaptureKit monitor enumeration retry budget exhausted; two Apple callbacks remain blocked, restart screener to recover"
                .to_string(),
        ),
...
fn enumerate_sck_monitors() -> std::result::Result<Vec<SafeMonitor>, MonitorListError> {
    #[cfg(debug_assertions)]
    if std::env::var("SCREENER_E2E_SEED")
        .ok()
        .is_some_and(|seeds| {
            seeds
                .split(',')
...
mod tests {
    ...
    /// (user's screener at 13.2 GB RSS after ~48 h).
    ///
    ...
    /// test — run with:
    ///   cargo test -p screener-screen --lib monitor::macos::tests::repro_list_monitors_autorelease_leak -- --ignored --nocapture
    #[tokio::test(flavor = "multi_thread", worker_threads = 2)]
