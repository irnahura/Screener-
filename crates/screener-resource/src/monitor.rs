...
/// Aggregated resource usage for a screener-related process group.
...
impl ProcessBreakdown {
    ...
    fn mcp_count(&self) -> usize {
        self.group("screener_mcp_child")
            .map(|group| group.process_count)
            .unwrap_or(0)
            + self
                .group("screener_mcp_external")
                .map(|group| group.process_count)
                .unwrap_or(0)
    }
    ...
    pub fn compact_summary(&self) -> String {
        ...
        if groups.is_empty() {
            "no screener-related processes found".to_string()
        } else {
            groups
        }
    }
...
fn collect_snapshot(system: &System) -> ResourceSnapshot {
    ...
    let descendant_ids = screener_descendant_ids(system, current_pid);
...
fn screener_descendant_ids(system: &System, current_pid: sysinfo::Pid) -> HashSet<u32> {
...
fn related_process_group(
    ...
    if pid == current_pid {
        return Some("screener_app");
    }
    if text.contains("screener-mcp") && descendant_ids.contains(&pid.as_u32()) {
        return Some("screener_mcp_child");
    }
    if text.contains("screener-mcp") {
        return Some("screener_mcp_external");
    }
    if descendant_ids.contains(&pid.as_u32()) {
        return Some("screener_app_child");
    }
    text.contains("screener").then_some("screener_other")
}
