...
(`screener-redact::adapters::rfdetr_mlx`) looks for the file at
`~/.screener/models/rfdetr_v9.safetensors` by default.

Until a safetensors sibling is published next to `rfdetr_v9.onnx` on
[huggingface.co/screener/pii-image-redactor][hf], convert from the
existing ONNX yourself with the script kept in the archived
[`screener/rfdetr-mlx`][archive] repo (`convert/onnx_to_mlx.py`).
...
[hf]: https://huggingface.co/screener/pii-image-redactor
