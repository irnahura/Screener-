...
```bash
cargo run -p screener-semantic --example context_eval --locked -- --report
```
...
scripts/eval-semantic-replay.sh \
  ...
  --samples-per-app 25 \
  --max-apps 25 \
  --output /tmp/screener-semantic-replay-report.json
```
...
```bash
cargo run -p screener-semantic --example context_eval --locked -- --prompts \
  > /tmp/screener-semantic-context-prompts.jsonl
```
...
```bash
cargo run --release -p screener-semantic --example context_eval --locked -- \
  --run-pi ollama/screener-gemma4:latest
```
...
```bash
cargo test -p screener-db --test semantic_storage_test -- --nocapture
```
