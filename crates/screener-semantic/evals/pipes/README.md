...
This suite compares how representative Screener pipe tasks perform when the
same captured evidence is supplied as:

- persisted accessibility JSON;
- Screener's current text-bearing element outline;
...
through `screener_core::pipes::PipeManager`, launches Pi, and requires the
...
```bash
cargo run --release -p screener-semantic --example pipe_eval --locked -- \
  --report
```
...
```bash
cargo run --release -p screener-semantic --example pipe_eval --locked -- \
  --run-pi ollama/screener-gemma4:latest 3
```
...
```bash
cargo run --release -p screener-semantic --example pipe_eval --locked -- \
  --run-pi ollama/screener-gemma4:latest 5 \
...
```bash
cargo run --release -p screener-semantic --example pipe_eval --locked -- \
  --run-installed-pipes "$HOME/.screener/pipes" \
  ollama/screener-gemma4:latest 3
```

Hosted Screener models use the same sandboxed installed-prompt path, but
...
```bash
SCREENER_EVAL_PI_AGENT_DIR="$HOME/.screener/pi-config" \
cargo run --release -p screener-semantic --example pipe_eval --locked -- \
  --run-installed-pipes "$HOME/.screener/pipes" \
  screener/gpt-5.6-terra 1
```
...
cargo run --release -p screener-semantic --example pipe_eval --locked -- \
  --run-installed-pipes "$HOME/.screener/pipes" \
  ollama/screener-gemma4:latest 3 sop_release_signing
```
...
Screener APIs, integrations, notifications, extensions, skills, sessions,
...
already-running local Screener pipe endpoint. It uses the configured `auto`
preset by default:

```bash
export SCREENER_LOCAL_API_KEY="$(cd "$(mktemp -d)" && \
  bun x screener@latest auth token)"
cargo run --release -p screener-semantic --example pipe_eval --locked -- \
  --run-live-installed-pipes "$HOME/.screener/pipes" \
  3 sop_release_signing
```

Set `SCREENER_EVAL_PRESET` to the ID of another configured preset to compare
...
```bash
SCREENER_EVAL_PRESET=my-fixed-preset \
SCREENER_EVAL_MODEL=gpt-5.6-luna \
SCREENER_EVAL_PROVIDER=screener \
cargo run --release -p screener-semantic --example pipe_eval --locked -- \
  --run-live-installed-pipes "$HOME/.screener/pipes" 1
```

Live evals are explicitly opt-in and require a healthy authenticated local
Screener API. Each variant is installed under a unique ephemeral pipe name,
uses the real `POST /pipes/:id/run` path and configured model preset, denies all
Screener API calls from inside the pipe, disables subagents, and relies on the
