...
use screener_core::agents::{
    install_spawned_pid, AgentExecutor, AgentOutput, ExecutionHandle, SharedPid,
};
use screener_core::pipes::{parse_frontmatter, PipeManager};
use serde_json::json;
...
pub fn run_installed_pipes(
    ...
    Ok(InstalledPipeReport {
        ...
        cases: cases.len(),
        runtime: "screener_core::pipes::PipeManager with the installed pipe body",
...
pub fn run_live_installed_pipes(
    ...
    if api_key.trim().is_empty() {
        return Err("SCREENER_LOCAL_API_KEY must not be empty".into());
    }
    ...
    Ok(InstalledPipeReport {
        model: format!(
            "requested screener preset:{preset_id}; explicit fallback:{fallback_provider}/{fallback_model}"
        ),
        repeats,
        case_filter: case_filter.map(str::to_owned),
        cases: cases.len(),
        runtime: "running Screener POST /pipes/:id/run endpoint with the installed pipe body",
        safety_boundary: "ephemeral pipe; synthetic evidence only; all Screener API calls denied to the pipe; filesystem writes sandboxed to the ephemeral pipe directory; subagents disabled; directory removed after every run",
...
fn validate_loopback_api_url(api_url: &str) -> Result<()> {
    let url = reqwest::Url::parse(api_url).context("parsing SCREENER_API_URL")?;
    let host = url
        .host_str()
        .ok_or_else(|| anyhow!("SCREENER_API_URL must have a host"))?;
    if !matches!(host, "localhost" | "127.0.0.1" | "::1") {
        return Err(anyhow!(
            "live pipe evals require a loopback SCREENER_API_URL"
        ));
    }
    Ok(())
}
...
fn validate_preset_id(preset_id: &str) -> Result<()> {
    ...
    {
        return Err(anyhow!(
            "SCREENER_EVAL_PRESET must contain only letters, digits, '-' or '_'"
        ));
    }
    Ok(())
}
...
fn validate_model_name(model: &str) -> Result<()> {
    ...
    {
        return Err(anyhow!(
            "SCREENER_EVAL_MODEL contains unsupported characters"
        ));
    }
    Ok(())
}
...
fn validate_provider_name(provider: &str) -> Result<()> {
    ...
    {
        return Err(anyhow!(
            "SCREENER_EVAL_PROVIDER contains unsupported characters"
        ));
    }
    Ok(())
}
...
fn run_one_live_installed_pipe(
    ...
    let initial = loop {
        ...
        if Instant::now() >= discovery_deadline {
            ...
            return Err(anyhow!(
                "active Screener runtime did not discover the ephemeral pipe: {}",
...
fn live_api_json(
    ...
    if !status.is_success() {
        return Err(anyhow!("live Screener API returned HTTP {status}"));
    }
    Ok(value)
}
...
async fn run_one_installed_pipe(
    ...
    let temp = tempfile::Builder::new()
        .prefix("screener-real-pipe-eval-")
        .tempdir()?;
...
fn installed_contexts(
    registry: &screener_semantic::ParserRegistry,
    case: &InstalledPipeCase,
    captures: &[SourceCase],
...
fn configure_eval_agent_dir(working_dir: &Path, model: &str) -> Result<Option<PathBuf>> {
    if model.starts_with("screener/") {
        let agent_dir = std::env::var("SCREENER_EVAL_PI_AGENT_DIR")
            .context("SCREENER_EVAL_PI_AGENT_DIR is required for hosted screener model evals")?;
        let agent_dir = PathBuf::from(agent_dir);
        if !agent_dir.join("models.json").is_file() || !agent_dir.join("auth.json").is_file() {
            return Err(anyhow!(
                "SCREENER_EVAL_PI_AGENT_DIR must contain models.json and auth.json"
            ));
        }
...
mod tests {
    ...
    fn live_pipe_frontmatter_is_parseable_and_restricted() {
        ...
        assert_eq!(config.provider.as_deref(), Some("screener"));
        assert_eq!(parsed_body, body);
        assert!(content.contains("Api(* *)"));
        let permissions = screener_core::pipes::permissions::PipePermissions::from_config(&config);
    ...
    fn live_api_key_cannot_be_sent_off_device() {
        ...
        validate_provider_name("screener").unwrap();
        assert!(validate_model_name("argus\npermissions: []").is_err());
        assert!(validate_provider_name("screener\nmodel: auto").is_err());
    }
}
