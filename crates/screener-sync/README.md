...
Generic data-sync primitives shared across the screener codebase.
...
This crate has no knowledge of screener's database, no knowledge of any specific ingest contract, and no background scheduler. Callers compose the pieces.
...
Today the same patterns are re-implemented in three places: consumer cloud sync (`screener-core::sync`), the enterprise telemetry sync (`apps/screener-app-tauri/src-tauri/src/enterprise/sync.rs`), and the cloud-archive media path (`screener-engine/archive.rs`). Extracting the generic plumbing lets each caller keep its own wire format and crypto policy while sharing the transport, encryption, and retry logic.
