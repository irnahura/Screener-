...
fn database_files(screener_dir: &std::path::Path) -> impl Iterator<Item = PathBuf> + '_ {
    VAULT_DATABASE_FILES
        .iter()
        .map(|name| screener_dir.join(name))
}
...
impl VaultManager {
    pub fn new(screener_dir: PathBuf) -> Self {
        let initial_state = if !screener_dir.join("vault.meta").exists() {
            VaultState::None
        } else if screener_dir.join(".vault_locked").exists() {
            ...
            if database_files(&screener_dir)
    ...
    pub fn is_set_up(&self) -> bool {
        self.screener_dir.join("vault.meta").exists()
    }
    ...
    pub async fn setup(&self, password: &str) -> VaultResult<()> {
        ...
        let meta_path = self.screener_dir.join("vault.meta");
    ...
    pub async fn lock(&self, password: &str) -> VaultResult<watch::Receiver<MigrationProgress>> {
        ...
        let screener_dir = self.screener_dir.clone();
        let data_dir = self.screener_dir.join("data");
        let database_paths: Vec<_> = database_files(&self.screener_dir).collect();
        let sentinel = self.screener_dir.join(".vault_locked");
        ...
        *self.state.write().await = VaultState::Migrating { progress: 0.0 };
        ...
        tokio::spawn(async move {
            ...
            if data_dir.exists() {
                if let Err(e) =
                    migration::encrypt_data_dir(&screener_dir, &data_dir, key_bytes, progress_tx)
                        .await
                {
    ...
    pub async fn unlock(&self, password: &str) -> VaultResult<watch::Receiver<MigrationProgress>> {
        ...
        *self.state.write().await = VaultState::Migrating { progress: 0.0 };
        ...
        tokio::spawn(async move {
            ...
            if data_dir.exists() {
                if let Err(e) =
                    migration::decrypt_data_dir(&screener_dir, &data_dir, key_bytes, progress_tx)
                        .await
                {
