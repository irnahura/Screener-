...
fn store_encryption_key() -> Result<[u8; 32], String> {
    match screener_secrets::keychain::get_key() {
        ...
        screener_secrets::keychain::KeyResult::AccessDenied => Err(
            "store.bin is encrypted, but keychain access was denied. Grant screener keychain access or disable store encryption in the app, then try again.".to_string(),
        ),
        screener_secrets::keychain::KeyResult::NotFound => Err(
            "store.bin is encrypted, but the screener keychain key was not found. Open the app once or disable store encryption, then try again.".to_string(),
        ),
        screener_secrets::keychain::KeyResult::Unavailable => Err(
...
pub fn load_ics_calendar_settings_from_store(
    ...
    let path = screener_dir.join("store.bin");
