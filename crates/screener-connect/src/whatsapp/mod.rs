            ...
            if reason == "logged_out" {
                ...
                *status.lock().await = if let Some(error) = cleanup_error {
                    WhatsAppStatus::Error {
                        message: format!(
                            "WhatsApp signed this device out, but screener could not reset the old session: {}",
                            error
                        ),
                    }
                } else {
...
impl WhatsAppGateway {
    ...
    async fn ensure_deps(&self, bun_path: &str) -> Result<()> {
        let session_dir = self.screener_dir.join("whatsapp-session");
        std::fs::create_dir_all(&session_dir)?;

        let script_path = self.screener_dir.join("whatsapp-gateway.mjs");
        ...
        tokio::spawn(async move {
            loop {
                ...
                *status.lock().await = WhatsAppStatus::Error {
                ...
                // Re-spawn
                let session_dir = screener_dir.join("whatsapp-session");
                let script_path = screener_dir.join("whatsapp-gateway.mjs");
...
///      `screener_core::agents::pi::find_bun_executable` — this is the
...
pub fn resolve_bun_path(hint: Option<&str>) -> Result<String> {
    ...
    if let Some(p) = screener_core::agents::pi::find_bun_executable() {
        return Ok(p);
    }
    ...
    anyhow::bail!(
        "bun binary not found. screener normally ships bun bundled inside the \
