...
//! Pure-Rust SSH/SFTP sync for screener data.
//!
//! Copies the entire ~/.screener directory to a remote server over SFTP.
...
/// everything else is literal. Enough for `.screenerignore` entries like
...
/// Screener skill file dropped on the remote so AI agents know how to query the data.
const SCREENER_SKILL: &str = r#"# Screener Data Reference

This directory contains synced data from [screener](https://screenpi.pe) — an AI that knows everything you've seen, said, or heard.
...
/// Used to wire an agent (e.g. `screener agent setup <target>`) on a VPS
...
async fn sync_dir(
    ...
    while let Ok(Some(entry)) = entries.next_entry().await {
        ...
        // User selectivity (--no-media / --exclude / .screenerignore), on top
...
/// Sync the entire ~/.screener directory to the remote server.
...
/// (`--no-media`, `--exclude`, `.screenerignore`). The plain
...
async fn sync_to_remote_inner(
    ...
    let skill_path = format!("{}/SCREENER.md", remote_base);
    if let Ok(mut f) = sftp.create(&skill_path).await {
        let _ = f.write_all(SCREENER_SKILL.as_bytes()).await;
...
pub async fn discover_ssh_hosts() -> Vec<DiscoveredHost> {
    ...
    // mDNS discovery — find screener instances advertising on LAN
