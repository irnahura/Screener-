...
//!
//! - Secrets live in the OS vault via `screener-secrets`. They must never
...
pub use screener_core::pipes::connections::{
...
pub async fn is_pipe_connection_configured(
    ...
    if parse_mcp_connection_id(conn_id).is_some() {
        return mcp_servers::is_mcp_connection_configured(screener_dir, conn_id).await;
    }

    connections::is_connection_configured(secret_store, screener_dir, conn_id).await
}
...
pub async fn missing_pipe_connections(
    ...
    let mcp_connections = if required.iter().any(|conn_id| is_mcp_connection_id(conn_id)) {
        match mcp_servers::configured_mcp_connection_ids(screener_dir).await {
            Ok(ids) => Some(ids),
            Err(e) => {
                tracing::warn!(
