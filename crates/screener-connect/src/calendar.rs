...
// Re-export so downstream crates (screener-engine's calendar routes) can
...
/// Thin wrapper around `eventkit::EventsManager` with screener-specific additions.
...
impl ScreenerCalendar {
    ...
    ///
    /// screener only reads calendars/events, but EventKit does not offer a
    ...
    fn fetch_events_with_attendees(
        ...
        // here: screener reads existing calendar events, attendees, URLs, and
...
impl Default for ScreenerCalendar {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_list_calendars() {
        if ScreenerCalendar::authorization_status() != AuthorizationStatus::FullAccess {
