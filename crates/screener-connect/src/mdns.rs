...
//! mDNS service advertisement and discovery for screener instances on the LAN.
//!
//! Uses `_screener._tcp.local.` as the service type. Each instance advertises
...
//! screener continues to work. Discovery falls back to SSH/Tailscale.
...
fn get_hostname() -> String {
    ...
    cmd.output()
        ...
        .filter(|s| !s.is_empty())
        .unwrap_or_else(|| "screener".to_string())
}

const SERVICE_TYPE: &str = "_screener._tcp.local.";
...
/// `SCREENER_ENABLE_MDNS`), so screener stays silent on the network unless
...
/// Advertise this screener instance on the local network.
///
...
pub fn advertise(port: u16) -> Result<(), String> {
    ...
    // collisions when the same user runs screener on multiple machines
    ...
    info!("mdns: advertising screener on port {}", port);
    Ok(())
}

/// Discover screener instances on the local network.
///
...
fn browse_blocking() -> Vec<(String, u16)> {
    ...
    while let Ok(event) = receiver.recv_deadline(deadline) {
        if let ServiceEvent::ServiceResolved(info) = event {
            ...
            for addr in info.get_addresses() {
                ...
                debug!("mdns: found screener at {}:{}", host, port);
    ...
    if !results.is_empty() {
        info!("mdns: discovered {} screener instance(s)", results.len());
    }
