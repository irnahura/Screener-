...
//!   `~/.screener/mcp_servers.json` so it survives without the secret
//!   store.
...
pub use screener_core::pipes::connections::{parse_mcp_connection_id, MCP_CONNECTION_PREFIX};
...
fn store_path(screener_dir: &Path) -> PathBuf {
    screener_dir.join("mcp_servers.json")
}

pub async fn is_mcp_connection_configured(screener_dir: &Path, conn_id: &str) -> bool {
    let Some(server_id) = parse_mcp_connection_id(conn_id) else {
        return false;
    };
    let Ok(file) = load_file(screener_dir).await else {
        return false;
    };
    file.servers
        .iter()
...
pub async fn configured_mcp_connection_ids(screener_dir: &Path) -> Result<HashSet<String>> {
    let file = load_file(screener_dir).await?;
    Ok(file
        .servers
        .iter()
...
async fn load_file(screener_dir: &Path) -> Result<McpServersFile> {
...
async fn save_file(screener_dir: &Path, file: &McpServersFile) -> Result<()> {
    let path = store_path(screener_dir);
...
impl McpServerStore {
    ...
    pub async fn list(&self) -> Result<Vec<McpServerConfig>> {
        Ok(load_file(&self.screener_dir).await?.servers)
    }

    pub async fn get(&self, id: &str) -> Result<Option<McpServerConfig>> {
        Ok(load_file(&self.screener_dir)
            .await?
            .servers
            .into_iter()
            .find(|s| s.id == id))
...
fn registry_path() -> &'static str {
    ...
    CACHED.get_or_init(|| {
        ...
        let out = screener_core::no_window_command("powershell")
...
impl Drop for StdioSession {
    fn drop(&mut self) {
        ...
        let _ = screener_core::no_window_command("taskkill")
    ...
    if let Err(e) = session
        .send(
            ...
            json!({
                ...
                "capabilities": {},
                "clientInfo": { "name": "screener", "version": env!("CARGO_PKG_VERSION") },
            }),
            probe_timeout,
        )
        .await
...
pub async fn render_context(screener_dir: &Path, api_port: u16) -> String {
    render_context_for_ids(screener_dir, api_port, None).await
}
...
pub async fn render_context_for_connections(
    ...
    let ids: Vec<String> = connections
        ...
        .collect();
    render_context_for_ids(screener_dir, api_port, Some(&ids)).await
}
...
pub async fn render_context_for_ids(
    ...
    let file = match load_file(screener_dir).await {
        Ok(f) => f,
        Err(e) => {
