...
//! `store: Option<&screener_secrets::SecretStore>` as its first parameter.
//!
...
//! existing screener API server at a fixed path:
//!
...
//!   3. The provider redirects back; the screener server handles
...
// /connections/oauth/callback HTTP handler (screener-engine)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Legacy plaintext file location  (~/.screener/{id}-oauth.json)
//
...
pub fn oauth_token_path_instance(integration_id: &str, instance: Option<&str>) -> PathBuf {
    ...
    screener_core::paths::default_screener_data_dir().join(name)
}
...
/// `screener_core::connections::sync::remote_is_newer`.
fn next_refresh_gen(
...
pub async fn write_oauth_token_instance(
    ...
    // `screener_core::connections::sync::remote_is_newer` immune to the
    // wall-clock skew between two devices refreshing the same connection.
    {
        let gen_field = screener_core::connections::sync::OAUTH_REFRESH_GEN_FIELD;
        ...
        let prior_gen = prior
            ...
            .unwrap_or(0);
        let highwater =
            screener_core::connections::sync::read_oauth_gen_highwater(&data_dir, &manifest_key);
        ...
        screener_core::connections::sync::bump_oauth_gen_highwater(
            &data_dir,
            &manifest_key,
            new_gen,
        );
    ...
    if let Some(s) = store {
        ...
        screener_core::connections::sync::clear_connection_tombstone(
...
pub async fn delete_oauth_token_instance(
    ...
    screener_core::connections::sync::record_connection_tombstone(
        &screener_core::paths::default_screener_data_dir(),
        &manifest_key,
    );
    Ok(())
...
mod tests {
    ...
    // in `load_oauth_json_exact` (which scans `~/.screener/{id}*-oauth.json`)
...
/// Exchange an authorization `code` for tokens via the screener backend
