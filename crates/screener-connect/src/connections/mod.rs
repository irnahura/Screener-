...
use screener_core::connections::sync as core_connections_sync;
...
pub trait Integration: Send + Sync {
    ...
    ///
    /// The proxy handler in screener-engine and the integration's own
...
// ---------------------------------------------------------------------------
// Credential store (~/.screener/connections.json)
// ---------------------------------------------------------------------------
...
fn store_path(screener_dir: &Path) -> PathBuf {
    screener_dir.join("connections.json")
}

pub fn load_store(screener_dir: &Path) -> HashMap<String, SavedConnection> {
    let path = store_path(screener_dir);
...
pub fn save_store(screener_dir: &Path, data: &HashMap<String, SavedConnection>) -> Result<()> {
    let path = store_path(screener_dir);
...
pub async fn load_connection(
    ...
    let file_store = load_store(screener_dir);
...
pub async fn is_connection_configured(
    ...
    if integration.oauth_config().is_some() {
        ...
        load_connection(secret_store, screener_dir, conn_id)
            .await
...
async fn save_connection(
    ...
    let mut file_store = load_store(screener_dir);
    file_store.insert(key.to_string(), conn.clone());
    save_store(screener_dir, &file_store)
}
...
impl ConnectionManager {
    pub fn new(screener_dir: PathBuf, secret_store: Option<Arc<SecretStore>>) -> Self {
        Self {
    ...
    pub async fn connect(&self, id: &str, creds: Map<String, Value>) -> Result<()> {
        ...
        let result =
            save_connection(self.secret_store.as_deref(), &self.screener_dir, id, &conn).await;

        if result.is_ok() {
            core_connections_sync::clear_connection_tombstone(&self.screener_dir, id);
        }
    ...
    pub async fn get_credentials_instance(
        ...
        Ok(
            load_connection(self.secret_store.as_deref(), &self.screener_dir, &key)
                .await
                .map(|c| c.credentials),
        )
    }
    ...
    pub async fn disconnect(&self, id: &str) -> Result<()> {
        let result = remove_connection(self.secret_store.as_deref(), &self.screener_dir, id).await;
        if result.is_ok() {
            core_connections_sync::record_connection_tombstone(&self.screener_dir, id);
        }
        result
    }
    ...
    pub async fn connect_instance(
        ...
        if result.is_ok() {
            core_connections_sync::clear_connection_tombstone(&self.screener_dir, &key);
        }
    ...
    pub async fn get_all_instances(
        ...
        self.find(id)?;
        Ok(load_all_instances(self.secret_store.as_deref(), &self.screener_dir, id).await)
    }
    ...
    pub async fn disconnect_instance(&self, id: &str, instance: Option<&str>) -> Result<()> {
        let key = make_key(id, instance);
        let result =
            remove_connection(self.secret_store.as_deref(), &self.screener_dir, &key).await;
        if result.is_ok() {
            core_connections_sync::record_connection_tombstone(&self.screener_dir, &key);
        }
        result
    }
...
pub async fn render_context(
    ...
    for integration in &integrations {
        ...
        for (instance, conn) in load_all_instances(secret_store, screener_dir, def.id).await {
    ...
    let ics_settings =
        crate::ics_calendar::load_ics_calendar_settings_from_store(screener_dir).ok();
...
async fn load_all_instances(
    ...
    let store = load_store(screener_dir);
...
mod tests {
    ...
    fn temp_screener_dir() -> PathBuf {
        let dir = std::env::temp_dir().join(format!(
            "screener-connect-test-{}-{}",
            std::process::id(),
