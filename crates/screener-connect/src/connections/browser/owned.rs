...
//! user browsers. Keeping the seam here means `screener-connect` never
...
/// types don't leak into `screener-connect`.
#[async_trait]
    ...
    pub fn default_instance() -> Arc<Self> {
        Self::new(
            ...
            "An isolated app-managed webview with its own persistent cookie \
             ...
             Screener asks the user before copying that browser session into \
             this webview. Use this for: scraping, signups under screener's \
