...
static SCOPE_VARIANTS: &[ScopeVariant] = &[
    ScopeVariant {
        id: "send",
        label: "Send only",
        description: "Post messages as you. Screener can't read your Slack.",
    ...
    async fn test(
        ...
        if let Some(user_token) = token_json["authed_user"]["access_token"].as_str() {
            ...
            let resp: Value = client
                ...
                .json(&serde_json::json!({"channel": channel, "text": "screener connected"}))
                .send()
                .await?
                .json()
                .await?;
        ...
        client
            .post(url)
            .json(&serde_json::json!({"text": "screener connected"}))
            .send()
            .await?
            .error_for_status()?;
