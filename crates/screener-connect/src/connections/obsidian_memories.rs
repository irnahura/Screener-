...
//! `screener-engine::external_memory_sync`). It mirrors how `claude_code`
...
/// avoids surprising the user with a screener file next to their notes and
/// makes it trivial to exclude from search if they want.
const DEFAULT_FOLDER: &str = "screener";
...
static DEF: IntegrationDef = IntegrationDef {
    ...
    description: "screener continuously writes a digest of your durable memories into this Obsidian vault as a single screener-owned note (<vault>/<memories_folder>/screener-memories.md). The note is rewritten end-to-end on each sync and carries YAML frontmatter + #tags so it lives in Obsidian's graph. Managed by screener — no action needed.",
    fields: &[
        FieldDef {
            key: "vault_path",
            label: "Vault Path",
...
impl Integration for ObsidianMemories {
    ...
    async fn test(
        ...
        let probe = target_dir.join(".screener-write-probe");
        std::fs::write(&probe, "ok")
            .map_err(|e| anyhow::anyhow!("{} is not writable: {}", target_dir.display(), e))?;
        let _ = std::fs::remove_file(&probe);

        let note = target_dir.join("screener-memories.md");
...
/// `<vault>/<memories_folder>` (folder defaults to `screener`).
///
...
mod tests {
    ...
    #[test]
    fn defaults_folder_to_screener() {
        let dir = resolve_target_dir(&creds(Some("/tmp/vault"), None)).unwrap();
        assert_eq!(dir, PathBuf::from("/tmp/vault/screener"));
    }
    ...
    fn honors_custom_nested_folder() {
        let dir = resolve_target_dir(&creds(Some("/tmp/vault"), Some("inbox/screener"))).unwrap();
        assert_eq!(dir, PathBuf::from("/tmp/vault/inbox/screener"));
    }
    ...
    fn cannot_escape_vault_with_dotdot() {
        ...
        assert_eq!(only_dots, PathBuf::from("/tmp/vault/screener"));
    }
    ...
    async fn test_creates_target_folder_and_reports_note_path() {
        ...
        assert!(vault.path().join("screener").is_dir());
        assert!(msg.contains("screener-memories.md"));
        // Probe must be cleaned up so the vault isn't littered with dotfiles.
        assert!(!vault.path().join("screener/.screener-write-probe").exists());
    }
