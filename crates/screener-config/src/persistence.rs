...
//!
//! Used by the CLI binary to persist settings in `~/.screener/config.toml`.
...
/// Default config file path: `~/.screener/config.toml`.
...
/// Resolve the screener config directory: `~/.screener/`
fn dirs_next() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".screener"))
}
