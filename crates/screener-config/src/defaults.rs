...
/// `Default` returns bounded High-tier values; explicit task-level concurrency
/// controls still decide when extra work may run.
...
impl Default for DbConfig {
    /// High-tier defaults — bounded for continuous capture workloads.
    fn default() -> Self {
        Self {
            // mmap disabled (0): memory-mapped I/O maps the DB file *writably*
            // into screener's address space, where a stray write from any
...
/// `Default` returns bounded High-tier values for continuous capture workloads.
#[derive(Clone, Debug, PartialEq, Eq)]
...
impl ChannelConfig {
    ...
    pub fn for_tier(tier: DeviceTier) -> Self {
        match tier {
            DeviceTier::High => Self::default(),
            DeviceTier::Mid => Self {
                recording_capacity: 250,
                transcription_capacity: 250,
            },
            DeviceTier::Low => Self {
...
impl Default for ChannelConfig {
    /// High-tier defaults — bounded so queued audio cannot grow into a large
    /// memory reservation during a slow transcription/model-load period.
    fn default() -> Self {
        Self {
...
/// `screener_core::cpu_features`), so Windows/Linux fall back to
...
pub fn best_engine_for_platform(tier: DeviceTier) -> &'static str {
    best_engine_for_platform_for_cpu(tier, screener_cpu_features::has_avx2())
}
...
pub fn best_engine_for_platform_for_cpu(tier: DeviceTier, has_avx2: bool) -> &'static str {
    ...
    // screener-audio disables it on EVERY x86-64 target — this picker must
...
pub fn is_engine_unsafe(engine: &str, tier: DeviceTier) -> bool {
    is_engine_unsafe_for_cpu(engine, tier, screener_cpu_features::has_avx2())
}
...
pub fn is_engine_unsafe_for_cpu(engine: &str, tier: DeviceTier, has_avx2: bool) -> bool {
    ...
    // gate in screener-audio's TranscriptionEngine::new exactly, so the
