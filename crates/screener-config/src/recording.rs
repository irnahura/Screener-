...
//! The core recording settings type shared across all screener components.
...
/// - **CLI**: built from command-line args or loaded from `~/.screener/config.toml`
...
pub struct RecordingSettings {
    ...
    #[serde(rename = "screenerAecEnabled", default)]
    ...
    /// `screener-redact` for the full design.
    ...
    /// model-backed text path). Off by default. See `screener-redact`.
    ...
    /// `true`, the `screener_redact::image::worker` runs alongside
    ...
    /// independently togglable. Requires the `screener-redact`
    /// crate to be built with one of the `onnx-*` cargo features and
    /// the `rfdetr_v8.onnx` model present at `~/.screener/models/`.
    ...
    ///   per frame).
    /// - `"tinfoil"`: send to the screener Tinfoil enclave (H200,
    ...
    ///   `SCREENER_PRIVACY_FILTER_API_KEY` (or the cloud auth key).
    ///
    ...
    /// what's stored (see screener-redact `parse_allow_list`). The
    ...
    /// screener-redact). Default = the clear, lighter capture surfaces ON,
    ...
    /// tokenize). Off by default. See issue #4206 and `screener-redact`'s
    /// `Pseudonymizer`.
    #[serde(rename = "piiRedactionPseudonyms", default)]
    pub pii_redaction_pseudonyms: bool,

    // ── Cloud / Auth ───────────────────────────────────────────────────
    /// Screener cloud user ID. Empty string means not logged in.
    /// Kept as String (not Option) to match existing store.bin schema.
    #[serde(rename = "userId")]
    pub user_id: String,
    ...
    // ── System ─────────────────────────────────────────────────────────
    /// HTTP server port for the screener API.
    pub port: u16,
    ...
    /// LAN can reach the screener API. Off by default — the server binds
...
impl RecordingSettings {
    ...
    /// Returns effective AEC booleans as `(screener, windows, macos)`.
    pub fn effective_aec_flags(&self) -> (bool, bool, bool) {
        match self.aec_mode {
            AecMode::Off => (false, false, false),
            AecMode::Screener => (true, false, false),
...
/// screener-redact (this crate can't depend on it) and the
...
mod tests {
    ...
    fn deserializes_from_empty_json() {
        ...
        assert!(!settings.screener_aec_enabled);
    ...
    #[test]
    fn explicit_screener_aec_mode_enables_software_aec() {
        let settings: RecordingSettings = serde_json::from_str(
            r#"{
                "aecMode": "screener",
                "screenerAecEnabled": false,
    ...
    fn deserializes_legacy_extra_fields() {
        ...
        assert_eq!(settings.vocabulary[0].word, "screener");
    ...
    fn deserializes_real_store_bin_shape() {
        ...
        let json = r#"{
            ...
            "aiPresets": [{"name": "test"}],
            "showScreenerShortcut": "Super+Alt+S",
    ...
    fn flatten_simulation() {
        ...
        struct FakeSettingsStore {
            ...
            overlay_mode: String,
            #[serde(rename = "showScreenerShortcut", default)]
            show_screener_shortcut: String,
        ...
        assert_eq!(store.show_screener_shortcut, "Super+Alt+S");
    ...
    fn vocab_entry_accepts_bare_string_and_struct_mixed() {
        // Regression for SCREENER-APP-DN: legacy store.bin stored vocabularyWords
        ...
        let json = r#"{"vocabularyWords": ["せばんぬ", {"word": "screener"}, {"word": "foo", "replace_with": "bar"}]}"#;
        let s: RecordingSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.vocabulary.len(), 3);
        assert_eq!(s.vocabulary[0].word, "せばんぬ");
        assert_eq!(s.vocabulary[0].replace_with, None);
        assert_eq!(s.vocabulary[1].word, "screener");
