...
//! records it in `enterprise_upload_manifests`. Screener Cloud sees
...
pub const DIRECT_UPLOAD_CONTENT_TYPE: &str = "application/vnd.screener.telemetry+jsonl";

/// Manifest `mode` for the strict binding: plaintext JSONL PUT into a
/// bucket no Screener principal can read. Hosted content surfaces are
...
/// deliberately grants Screener read access (cloud pipes keep working).
