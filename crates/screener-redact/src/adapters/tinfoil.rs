...
//! Confidential-compute redactor backed by the screener-hosted
//! Tinfoil enclave running the privacy-filter model.
//!
//! Wire format kept compatible with the existing
//! `screener-engine::privacy_filter` HTTP shape so we can phase the
...
//! 2. The `SCREENER_PRIVACY_FILTER_API_KEY` env var.
//! 3. The `SCREENER_LOCAL_API_KEY` env var (the local-API bearer the
//!    rest of the codebase uses), with `SCREENER_API_AUTH_KEY` honored
...
//! visible. The previous `screener-engine::privacy_filter`
...
pub struct TinfoilConfig {
    ...
    /// `SCREENER_PRIVACY_FILTER_ENCLAVE`, then [`DEFAULT_ENCLAVE`].
    /// `SCREENER_PRIVACY_FILTER_URL` is also honored for backwards
    ...
    /// `SCREENER_PRIVACY_FILTER_REPO`, then [`DEFAULT_REPO`].
    pub repo: Option<String>,
    /// Bearer token for the enclave. Falls back through
    /// `SCREENER_PRIVACY_FILTER_API_KEY`, `SCREENER_LOCAL_API_KEY`,
    /// and `SCREENER_API_AUTH_KEY` (deprecated alias).
...
impl TinfoilRedactor {
    ...
    pub fn new(cfg: TinfoilConfig) -> Self {
        let enclave_raw = cfg
            .enclave
            .or_else(|| std::env::var("SCREENER_PRIVACY_FILTER_ENCLAVE").ok())
            .or_else(|| std::env::var("SCREENER_PRIVACY_FILTER_URL").ok())
            .unwrap_or_else(|| DEFAULT_ENCLAVE.to_string());
        let enclave = strip_scheme_and_path(&enclave_raw).to_string();

        let repo = cfg
            .repo
            .or_else(|| std::env::var("SCREENER_PRIVACY_FILTER_REPO").ok())
            .unwrap_or_else(|| DEFAULT_REPO.to_string());

        let api_key = cfg.api_key.or_else(|| {
            std::env::var("SCREENER_PRIVACY_FILTER_API_KEY")
                .ok()
                .or_else(|| std::env::var("SCREENER_LOCAL_API_KEY").ok())
                .or_else(|| std::env::var("SCREENER_API_AUTH_KEY").ok())
        });
            ...
            None => {
                tracing::info!(
                    "tinfoil redactor has no api key — requests will be un-authenticated. \
                     Set SCREENER_PRIVACY_FILTER_API_KEY or pass via TinfoilConfig.api_key."
                );
                (None, false)
            }
        };
...
mod tests {
    ...
    async fn no_api_key_means_no_auth() {
        ...
        std::env::remove_var("SCREENER_PRIVACY_FILTER_API_KEY");
        std::env::remove_var("SCREENER_LOCAL_API_KEY");
        std::env::remove_var("SCREENER_API_AUTH_KEY");
        let r = TinfoilRedactor::new(TinfoilConfig {
            enclave: Some("example.invalid".into()),
            repo: Some("test/never".into()),
            api_key: None,
