...
//! `screener-pii-redactor-runtime/results/onnx_export_2026-05-08.md`):
...
//! MPS on Metal (41 ms vs 40 ms p50). See `screener/opf-rs` for the
//! full benchmark.
//!
//! ## Status
//!
...
//! `apps/screener-app-tauri/src-tauri/src/server_core.rs`); this
...
//! translate to screener-redact's `RedactionOutput` (same shape, but
...
/// canonical screener install layout.
#[derive(Debug, Clone)]
pub struct OpfConfig {
    /// Directory containing `config.json` + `model.safetensors`.
    /// Default: `~/.screener/models/opf-v6`. First-run is created
...
const HF_BASE_URL: &str = "https://huggingface.co/screener/pii-text-redactor/resolve/v6";
