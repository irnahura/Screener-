...
pub struct TinfoilImageConfig {
    ...
    /// `SCREENER_PRIVACY_FILTER_ENCLAVE`, then `SCREENER_PRIVACY_FILTER_URL`,
    ...
    /// to `SCREENER_PRIVACY_FILTER_REPO`, then [`DEFAULT_REPO`].
    pub repo: Option<String>,
    /// Bearer token. Falls back through `SCREENER_PRIVACY_FILTER_API_KEY`,
    /// `SCREENER_LOCAL_API_KEY`, then `SCREENER_API_AUTH_KEY` (deprecated alias).
            ...
            None => {
                tracing::info!(
                    "tinfoil_image redactor has no api key — requests will be un-authenticated. \
                     Set SCREENER_PRIVACY_FILTER_API_KEY or pass via TinfoilImageConfig.api_key."
                );
                (None, false)
            }
        };
