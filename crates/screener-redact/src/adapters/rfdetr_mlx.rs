...
//! RF-DETR-Nano image-PII detector via [`screener_rfdetr_mlx`] —
...
//! `huggingface.co/screener/pii-image-redactor` on first run
//! (~108 MB), verifies SHA-256, atomic-renames into
//! `~/.screener/models/rfdetr_v9.safetensors`. Subsequent starts
//! are instant.
...
///
/// Mirrors `screener_config::PARAKEET_MIN_MACOS_MAJOR` — mlx-sys's
...
/// `screener_config::macos_major_version`, inlined here so this
/// crate doesn't pick up screener-config just for one fn. On any
...
pub struct RfdetrMlxConfig {
    ...
    /// `~/.screener/models/rfdetr_v9.safetensors`.
...
impl RfdetrMlxConfig {
    ...
    pub const HF_DOWNLOAD_URL: &'static str =
        "https://huggingface.co/screener/pii-image-redactor/resolve/main/rfdetr_v9.safetensors";
...
mod imp {
    ...
    /// pipeline (`screener-pii-bench-image/src/score.py`).
    ...
    type ModelHandle = Arc<StdMutex<screener_rfdetr_mlx::RfDetr>>;
    ...
    impl RfdetrMlxRedactor {
        ...
        async fn ensure_loaded(&self) -> Result<ModelHandle, RedactError> {
            ...
            if state.model.is_none() {
                ...
                let model = tokio::task::block_in_place(|| {
                    let mut m = screener_rfdetr_mlx::RfDetr::load(&path)
        ...
        async fn infer(&self, image_path: &Path) -> Result<Vec<ImageRegion>, RedactError> {
            ...
            tokio::task::block_in_place(move || {
                ...
                // screener-rfdetr-mlx already excludes it.
...
mod tests {
    ...
    #[test]
    fn default_path_under_screener_dir() {
        let p = RfdetrMlxConfig::default_model_path();
        let s = p.to_string_lossy();
        assert!(s.contains(".screener/models/rfdetr_v9.safetensors"));
    }
