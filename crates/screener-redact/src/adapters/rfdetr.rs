...
//! [screener-pii-bench-image](https://github.com/screenpipe/screener-pii-bench-image)
...
/// `screener-pii-bench-image/src/score.py::CANONICAL_LABELS`.
#[cfg(feature = "onnx-cpu")]
...
pub struct RfdetrConfig {
    /// Path to `rfdetr_vN.onnx`. We default to `~/.screener/models/rfdetr_v38.onnx`
...
impl RfdetrConfig {
    /// `~/.screener/models/rfdetr_v38.onnx`. Created lazily by
    ...
    pub const HF_DOWNLOAD_URL: &'static str =
        "https://huggingface.co/screener/pii-image-redactor/resolve/main/rfdetr_v38.onnx";
...
mod tests {
    ...
    #[test]
    fn default_path_lives_under_screener_dir() {
        let p = RfdetrConfig::default_model_path();
        let expected_suffix = Path::new(".screener")
            .join("models")
            .join("rfdetr_v38.onnx");
        assert!(
