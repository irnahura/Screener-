...
export default function RootLayout({
  ...
  useEffect(() => {
    ...
    // Write immediately so the stack lands in ~/.screener/screener-app.
          ...
          dangerouslySetInnerHTML={{
            __html: `
              (function() {
                ...
                try {
                  var theme = localStorage.getItem('screener-ui-theme');
                ...
                try {
                  var savedFs = localStorage.getItem('screener-font-size');
                  if (savedFs) {
      ...
      <body className={`${inter.className} scrollbar-hide ${usesTransparentWindowBackground ? "bg-transparent" : ""}`}>
        ...
        <Providers>
          {/* DeeplinkHandler is mounted in Providers (outside the entitlement
              gate) so the screener:// login callback is always caught, even
