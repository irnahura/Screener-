...
fn generate_and_validate_tauri_commands() {
    ...
    for required in [
        "screener_app::config::get_screener_base_dir",
        "screener_app::commands::get_cloud_token",
        "screener_app::commands::is_enterprise_build_cmd",
    ] {
        assert!(
...
fn main() {
    ...
    let build_time = std::time::SystemTime::now()
        ...
        .map(|d| d.as_secs())
        .unwrap_or(0);
    println!("cargo:rustc-env=SCREENER_BUILD_UNIX_TIME={}", build_time);
...
/// Build the `screener-wer-dump-helper` cdylib with a nested cargo
...
fn build_wer_dump_helper() {
    ...
    match status {
        ...
        other => panic!("failed to build screener-wer-dump-helper: {other:?}"),
    }

    let dll_name = "screener_wer_dump_helper.dll";
...
fn stage_mlx_metallib() {
    ...
    if needs_download {
        ...
        let url =
            "https://github.com/screenpipe/screener/releases/download/mlx-metallib-v0.2.0/mlx.metallib";
