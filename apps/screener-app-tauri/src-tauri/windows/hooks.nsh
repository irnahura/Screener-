...
; Kills all screener processes by name and by install-directory path.
...
!macro _SP_KillProcesses
  ...
  DetailPrint "Stopping screener processes..."
  nsExec::ExecToLog 'taskkill /F /T /IM screener.exe'
  Pop $0
  DetailPrint "taskkill screener.exe: $0"
  nsExec::ExecToLog 'taskkill /F /T /IM screener-app.exe'
  Pop $0
  DetailPrint "taskkill screener-app.exe: $0"
  ...
  ClearErrors
  FileOpen $1 "$PLUGINSDIR\screener-kill.ps1" w
  IfErrors ${_SP_SKIP}
    ...
    ; Screener targets 64-bit Windows 10/11 only. RemoteSigned is sufficient --
    ...
    nsExec::ExecToLog /TIMEOUT=30000 '"$WINDIR\System32\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -NonInteractive -inputformat none -ExecutionPolicy RemoteSigned -File "$PLUGINSDIR\screener-kill.ps1"'
    Pop $0
...
!macro NSIS_HOOK_PREINSTALL
  ...
  !insertmacro _SP_ClearLockedFile "screener.exe"
  !insertmacro _SP_ClearLockedFile "screener-app.exe"
...
!macro NSIS_HOOK_POSTUNINSTALL
  ...
  RMDir /r "$LOCALAPPDATA\screener\git-portable"
  ; Clean up runtime-downloaded baseline bun (non-AVX2 CPU fallback)
  RMDir /r "$LOCALAPPDATA\screener\bun-baseline"
  ; Remove parent dir only if empty (preserves other screener data)
  RMDir "$LOCALAPPDATA\screener"
!macroend
