...
use screener_audio::core::device::resolve_audio_devices_for_capture;
use screener_audio::core::engine::AudioTranscriptionEngine;
use screener_audio::transcription::stt::{
...
pub struct ServerCore {
    ...
    pub audio_manager: Arc<screener_audio::audio_manager::AudioManager>,
    pub hot_frame_cache: Arc<HotFrameCache>,
    pub vision_metrics: Arc<screener_screen::PipelineMetrics>,
    pub power_manager: Arc<PowerManagerHandle>,
    pub pipe_manager: Arc<tokio::sync::Mutex<screener_core::pipes::PipeManager>>,
    ...
    pub high_fps_controller: Arc<screener_engine::high_fps_controller::HighFpsController>,
    ...
    pub vision_manager_handle:
        Arc<arc_swap::ArcSwap<Option<Arc<screener_engine::vision_manager::VisionManager>>>>,
    ...
    oauth_refresher:
        Option<Arc<screener_connect::oauth_refresh_scheduler::OAuthRefreshScheduler>>,
    external_memory_sync:
        Option<Arc<screener_engine::external_memory_sync::ExternalMemorySyncScheduler>>,
...
fn monitor_openai_compatible_transcription_failures(
    metrics: Arc<screener_audio::metrics::AudioPipelineMetrics>,
        ...
        loop {
            ...
            if should_notify_openai_compatible_failure(
                ...
                crate::notifications::client::send_typed_with_priority(
                    ...
                    "Screener is still recording audio, but the endpoint is failing or returning empty transcripts, so new audio may not be searchable. Check Settings and run Test and enable again.",
                    "system",
                    Some(20_000),
...
async fn identify_port_holder(port: u16) -> Option<String> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let mut netstat = screener_core::no_window_command_async("netstat");
        ...
        let mut tasklist = screener_core::no_window_command_async("tasklist");
        tasklist
...
impl ServerCore {
    ...
    pub async fn start(
        ...
        on_pipe_output: Option<screener_core::pipes::OnPipeOutputLine>,
        owned_browser: Option<
            std::sync::Arc<screener_connect::connections::browser::OwnedBrowser>,
        >,
        ...
        let ai_gateway_url = crate::config::screener_ai_gateway_url()?;

        // --- Environment setup ---
        std::env::set_var("SCREENER_FD_LIMIT", "8192");
        if !config.analytics_id.is_empty() {
            std::env::set_var("SCREENER_ANALYTICS_ID", &config.analytics_id);
        }
        ...
        if std::env::var("SCREENER_DISTRIBUTION").is_err() {
            std::env::set_var("SCREENER_DISTRIBUTION", "desktop-app");
        }
        ...
        // sync, or a stale advisory lock from a crashed prior screener
        ...
        if !config.disable_timeline {
            ...
            audio_manager.set_on_transcription_insert(Arc::new(move |info| {
                ...
                rt.spawn(async move {
                    cache
                        .push_audio(screener_engine::hot_frame_cache::HotAudio {
        ...
        let initial_power_pref = config
            ...
            .as_deref()
            .and_then(|s| {
                serde_json::from_value::<screener_engine::power::PowerMode>(
        ...
        {
            match screener_secrets::SecretStore::open_for_data_dir_with_vault_key(
                &config.data_dir,
            )
            .await
            {
                Ok(store) => {
                    let fixed = screener_secrets::fix_secret_file_permissions(&config.data_dir);
                    if fixed > 0 {
                        info!("fixed permissions on {} credential files", fixed);
                    }
                    match screener_secrets::migrate_legacy_secrets(&store, &config.data_dir).await
                    {
                        Ok(report) => {
                    ...
                    match screener_connect::oauth::sweep_shadowed_default_slots(&store).await {
                        Ok(n) if n > 0 => {
                    ...
                    let refresher = Arc::new(
                        screener_connect::oauth_refresh_scheduler::OAuthRefreshScheduler::new(),
                    );
                    ...
                    let memory_sync = Arc::new(
                        screener_engine::external_memory_sync::ExternalMemorySyncScheduler::new(),
                    );
        ...
        let pi_executor = Arc::new(
            screener_core::agents::pi::PiExecutor::with_shared_user_token(
        ...
        let mut agent_executors: std::collections::HashMap<
            String,
            Arc<dyn screener_core::agents::AgentExecutor>,
        > = std::collections::HashMap::new();
        agent_executors.insert("pi".to_string(), pi_executor.clone());

        let pipe_store: Option<Arc<dyn screener_core::pipes::PipeStore>> = Some(Arc::new(
            screener_engine::pipe_store::SqlitePipeStore::new(db.clone()),
        ));

        let mut pipe_manager = screener_core::pipes::PipeManager::new(
            pipes_dir,
            agent_executors,
            pipe_store,
            config.port,
        ...
        let mcp_session_access =
            screener_core::pipes::mcp_access::McpSessionAccessRegistry::new();
        ...
        // manual click.
        {
            let screener_dir_for_live_views = config.data_dir.clone();
        ...
        }
        {
            ...
            let screener_dir_for_check = config.data_dir.clone();
            pipe_manager.set_connection_check(Arc::new(move |required| {
                ...
                Box::pin(async move {
                    screener_connect::missing_pipe_connections(ss.as_deref(), &dir, &required)
                        .await
                })
            }));
        }
        ...
        let shared_pipe_manager = Arc::new(tokio::sync::Mutex::new(pipe_manager));
        {
            ...
            let screener_dir_for_cb = config.data_dir.clone();
            ...
            shared_pipe_manager.lock().await.set_on_run_complete(Arc::new(
                move |pipe_name, execution_id, trigger_type, success, duration_secs, error_type| {
                    ...
                    // Auto-register pipe artifacts to ~/.screener/outputs/
                    if success {
                        let db = db_for_cb.clone();
                        let dir = screener_dir_for_cb.clone();
                        ...
                        tokio::spawn(async move {
                            ...
                            let items = {
                                ...
                                let all = mgr
                                    .list_artifact_declarations(
                                        screener_engine::routes::artifacts::ARTIFACT_FALLBACK_CAP,
                                    )
                                    .await;
                                all.into_iter()
                                    .find(|(n, _)| n == &name)
                            ...
                            if !items.is_empty() {
                                screener_engine::routes::artifacts::auto_register_pipe_artifacts(
                                    &db,
                                    items,
                                    &name,
                                    execution_id,
        ...
        let high_fps_controller = Arc::new(
            screener_engine::high_fps_controller::HighFpsController::new(
                None,
        ...
        // ends. Without this the only safety net is the 4-hour hard cap.
        {
            ...
            owned_tasks.push(tokio::spawn(async move {
                ...
                let mut sub =
                    screener_events::subscribe_to_event::<serde_json::Value>("meeting_ended");
            ...
            Err(e) => {
                let msg = if e.kind() == std::io::ErrorKind::AddrInUse {
                    match identify_port_holder(config.port).await {
                        Some(proc) => format!(
                            "port {} is already in use by {}. close that process or set SCREENER_PORT to a different value",
                            config.port, proc
                        ),
                        None => format!(
                            "port {} is already in use by another process. close that process or set SCREENER_PORT to a different value",
                            config.port
                        ),
                    }
                } else {
        ...
        // wants multi-device discovery. Enable with SCREENER_ENABLE_MDNS=1.
        let mdns_enabled = std::env::var("SCREENER_ENABLE_MDNS")
            .map(|v| matches!(v.trim(), "1" | "true" | "TRUE"))
            .unwrap_or(false);
        screener_connect::mdns::set_enabled(mdns_enabled);
        if let Err(e) = screener_connect::mdns::advertise(config.port) {
        ...
        if config.async_pii_redaction {
            use screener_redact::adapters::onnx::{OnnxConfig, OnnxRedactor};
            use screener_redact::adapters::opf::{OpfAdapter, OpfConfig};
            use screener_redact::adapters::tinfoil::{TinfoilConfig, TinfoilRedactor};
            use screener_redact::pipeline::{Pipeline, PipelineConfig};
            use screener_redact::worker::{Worker, WorkerConfig, ALL_TARGET_TABLES};
            use screener_redact::Pseudonymizer;
            use screener_redact::Redactor;
            use screener_redact::TextRedactionPolicy;
            ...
            //                 huggingface.co/screener/pii-text-redactor
        ...
        if config.async_image_pii_redaction {
            ...
            use screener_redact::adapters::tinfoil_image::{
                TinfoilImageConfig, TinfoilImageRedactor,
            };
            use screener_redact::image::worker::{ImageWorker, ImageWorkerConfig};
            use screener_redact::ImageRedactionPolicy;
            use screener_redact::ImageRedactor;
            ...
            if use_tinfoil {
                ...
                // from huggingface.co/screener/pii-image-redactor and
                // verifies SHA-256 before landing in ~/.screener/models/.
    ...
    pub async fn shutdown(mut self) {
        ...
        screener_connect::mdns::shutdown();
        ...
        // Windows "Screener isn't reachable" zombie (2026-08-04 report).
        ...
        self.db.close().await;
        screener_secrets::close_all_secret_pools().await;
...
mod tests {
    ...
    fn process_lookup_parsers_fall_back_cleanly() {
        ...
        assert_eq!(
            parse_tasklist_process_name("\"screener.exe\",\"42\""),
            Some("screener.exe")
        );
