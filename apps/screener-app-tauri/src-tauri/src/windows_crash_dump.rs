...
//! On the next launch this module moves those dumps into the screener data
//! directory and prunes old ones. Screener never uploads them automatically.
...
const HELPER_DLL_NAME: &str = "screener_wer_dump_helper.dll";
...
pub fn install() {
    match register_helper() {
        Ok(dll) => eprintln!(
            "screener: fail-fast crash minidumps enabled via {}",
            dll.display()
        ),
        Err(reason) => {
            eprintln!("screener: fail-fast crash minidumps unavailable: {reason}")
        }
    }
    ...
    let _ = std::thread::Builder::new()
        .name("screener-wer-dump-harvest".to_string())
        .spawn(harvest_dumps);
}
...
/// Move dumps the helper DLL wrote next to the executable into the screener
...
fn harvest_dumps() {
    ...
    let output_dir = screener_core::paths::default_screener_data_dir().join("crash-dumps");

    let moved = move_dumps(&source_dir, &output_dir);
    if moved > 0 {
        eprintln!(
            "screener: recovered {moved} crash minidump(s) into {}",
            output_dir.display()
        );
    }
