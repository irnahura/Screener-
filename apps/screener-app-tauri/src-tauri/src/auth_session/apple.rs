...
//!
//! Opens a system-managed browser sheet, intercepts the `screener://` redirect,
...
define_class!(
    ...
    #[name = "ScreenerAuthPresentationProvider"]
    #[ivars = ProviderIvars]
    ...
    dispatch2::DispatchQueue::main().exec_async(move || {
        ...
        if get_key_window_as_anchor().is_none() {
            if let Some(tx) = tx.lock().unwrap().take() {
                let _ = tx.send(Err(
                    "No window available to present login sheet — open screener first"
                        .to_string(),
                ));
            }
            return;
