...
//! hidden mode of the signed Screener executable so no second sidecar or
...
pub const RUNTIME_ARG: &str = "--screener-acp-runtime";
const PROCESS_GUARD_ARG: &str = "--screener-acp-process-guard";
pub(crate) const CLOUD_API_KEY_ENV: &str = "SCREENER_API_KEY";

/// The core screener MCP server (activity-summary, search-content,
...
/// SQL. `pi::prewarm_screener_mcp` seeds the cache at install. Bump alongside
/// packages/screener-mcp.
pub(crate) const SCREENER_MCP_PKG: &str = "screener-mcp@0.19.2";
...
/// provider API keys (`SCREENER_ACP_ENV_JSON`), third-party MCP secret headers
/// (`SCREENER_ACP_USER_MCP_JSON`), the system prompt, and session config.
...
const RUNTIME_ONLY_ENV: &[&str] = &[
    ...
    "SCREENER_ACP_USER_MCP_JSON",
    "SCREENER_ACP_SESSION_CONFIG_JSON",
    "SCREENER_ACP_SYSTEM_PROMPT",
    "SCREENER_ACP_AUTH_METHOD",
    "SCREENER_ACP_COMMAND",
    "SCREENER_ACP_ARGS_JSON",
    "SCREENER_ACP_CWD",
    "SCREENER_ACP_ID",
    "SCREENER_ACP_RESUME_SESSION_ID",
];
...
#[cfg(windows)]
const WRAPPED_COMMAND_ENV: &str = "SCREENER_INTERNAL_ACP_WRAPPED_COMMAND";
#[cfg(windows)]
const WRAPPED_ARGS_ENV: &str = "SCREENER_INTERNAL_ACP_WRAPPED_ARGS_JSON";
...
struct RuntimeConfig {
    ...
    /// session/new alongside the screener server.
...
impl RuntimeConfig {
    fn from_env() -> Result<Self, String> {
        let agent_id = env_nonempty("SCREENER_ACP_ID").unwrap_or_else(|| "custom".into());
        let bun_path =
            env_nonempty("SCREENER_BUN_PATH").ok_or("SCREENER_BUN_PATH is required for ACP")?;
        let configured_args =
            parse_json_env::<Vec<String>>("SCREENER_ACP_ARGS_JSON")?.unwrap_or_default();
        let mut env = parse_json_env::<HashMap<String, String>>("SCREENER_ACP_ENV_JSON")?
            .unwrap_or_default();
        ...
        // (see `build_first_turn_context`), and the always-on `screener-cli`
        // skill invokes `${SCREENER_CLI:-bun x screener@latest}`. Without
        ...
        // inbound `SCREENER_ACP_ENV_JSON` entry, and inherited from here by
        // the adapter's own terminals (ACP never calls `env_clear`, and
        // `SCREENER_CLI` is not on the `RUNTIME_ONLY_ENV` scrub list).
        let data_dir = screener_core::paths::default_screener_data_dir();
        if let Some(cli) = screener_core::agents::cli_runtime::resolved_cli_binary(&data_dir) {
            env.insert(
                screener_core::agents::cli_runtime::CLI_ENV_VAR.to_string(),
        ...
        // the critical path.
        screener_core::agents::cli_runtime::maybe_refresh_in_background(
            &data_dir,
            Path::new(&bun_path),
        );
        let configured_command = env_nonempty("SCREENER_ACP_COMMAND");
        ...
        let project_dir = env_nonempty("SCREENER_ACP_CWD")
            .map(PathBuf::from)
        ...
        Ok(Self {
            ...
            project_dir,
            bun_path,
            preferred_auth_method: env_nonempty("SCREENER_ACP_AUTH_METHOD"),
            ...
            .unwrap_or_default(),
            resume_session_id: env_nonempty("SCREENER_ACP_RESUME_SESSION_ID"),
        })
    }
}

/// A short, harness-agnostic note about the screener tools every ACP session
...
/// the bundled server registers (see assets/mcp/screener-tools.mjs) plus the
/// core screener search server, and points at the seeded `.pi/skills` guides
/// that third-party agents otherwise only find by chance.
const SCREENER_TOOLS_HINT: &str = "\
You are running inside screener. Prefer its MCP tools over shell/curl (this is your usage guide). Tool names below are written with hyphens; some agents expose the same tools with underscores (activity_summary, search_content) or a query_recordings tool for read-only SQL — use whatever your own tool list shows, and never fall back to curl or /raw_sql just because a name here doesn't match exactly:
- the `screener` server searches and summarizes the user's screen, audio, and UI history.
...
- `list_connections` shows the user's connected apps; `screener_connect_app` connects one and waits for the user when a task needs it.
...
- screener seeds task guides in `.pi/skills/*/SKILL.md` under your working directory. Before starting a task, list that folder and read the SKILL.md that matches the request; it names the exact tools and steps to use.
...
fn build_first_turn_context(user_prompt: Option<String>) -> String {
    match user_prompt {
        Some(prompt) => format!("{SCREENER_TOOLS_HINT}\n\n{prompt}"),
        None => SCREENER_TOOLS_HINT.to_string(),
    }
}
...
pub(crate) fn is_forbidden_acp_env(name: &str) -> bool {
    // This is the signed-in user's Screener cloud JWT, not the local API
...
/// How an agent can be pointed at Screener Cloud instead of the user's own
...
struct CatalogAgent {
    ...
    /// Serve screener's MCP tools over loopback http instead of client stdio,
    /// for agents that reject stdio MCP servers (Cursor, GitHub Copilot). JSON
    /// key is `httpMcp`.
    #[serde(default)]
    http_mcp: bool,
    /// Present only for agents that can be pointed at Screener Cloud.
...
/// The env that routes an agent through Screener Cloud, plus the names to
...
async fn run_terminal_login(
    ...
    let mut child = screener_core::no_window_command_async(program)
        .args(args)
...
/// Whether `id` names an agent screener can launch (any agent in the static
...
fn windows_target_command(program: &str, args: &[String]) -> Result<Command, String> {
    ...
    let mut command = screener_core::no_window_command(resolved);
    command.args(args);
    Ok(command)
}
...
fn tool_name(update: &Value) -> String {
    // The title is the human-readable name ("Grep", "mcp__screener__search");
...
fn ensure_tools_mcp_server(config: &RuntimeConfig) -> Option<PathBuf> {
    const SOURCE: &str = include_str!("../assets/mcp/screener-tools.mjs");
    let dir = config.project_dir.join(".screener");
    if let Err(error) = std::fs::create_dir_all(&dir) {
        eprintln!("[acp-runtime] could not create MCP tools dir: {error}");
        return None;
    }
    let path = dir.join("screener-tools.mjs");
...
/// screener's tools over loopback Streamable-HTTP instead (see
...
fn agent_needs_http_mcp(agent_id: &str) -> bool {
    ...
    // Copilot set it today; without it they get no screener tools at all.
    agent_catalog()
        .into_iter()
        .find(|agent| agent.id == agent_id)
...
/// The local screener engine base url, from the runtime's env.
fn engine_api_url() -> Option<String> {
    env_nonempty("SCREENER_LOCAL_API_URL").or_else(|| {
        env_nonempty("SCREENER_LOCAL_API_PORT").map(|port| format!("http://localhost:{port}"))
    })
}
...
fn mcp_servers(config: &RuntimeConfig) -> Vec<McpServer> {
    ...
    if let Some(http) = HTTP_MCP_URLS.get().filter(|urls| !urls.is_empty()) {
        ...
        // up loopback http servers for screener's tools (see run_from_env),
        ...
        let mut args = vec!["x".into(), SCREENER_MCP_PKG.into()];
        let mut env = Vec::new();
        if let Some(url) = engine_api_url() {
            args.extend(["--screener-url".into(), url.clone()]);
            env.push(EnvVariable::new("SCREENER_API_URL", url));
        }
        if let Some(key) = env_nonempty("SCREENER_LOCAL_API_KEY") {
            env.push(EnvVariable::new("SCREENER_LOCAL_API_KEY", key));
        }
        servers.push(McpServer::Stdio(
            McpServerStdio::new("screener", &config.bun_path)
                .args(args)
                .env(env),
        ));
        ...
        if let Some(tools_server) = ensure_tools_mcp_server(config) {
            ...
            if let Some(url) = engine_api_url() {
                tools_env.push(EnvVariable::new("SCREENER_API_URL", url));
            }
            if let Some(key) = env_nonempty("SCREENER_LOCAL_API_KEY") {
                tools_env.push(EnvVariable::new("SCREENER_LOCAL_API_KEY", key));
            }
            if let Some(chat_id) = env_nonempty("SCREENER_CHAT_SESSION_ID") {
                tools_env.push(EnvVariable::new("SCREENER_CHAT_SESSION_ID", chat_id));
            }
            servers.push(McpServer::Stdio(
                McpServerStdio::new("screener-tools", &config.bun_path)
...
fn spawn_http_mcp_servers(config: &RuntimeConfig) -> Vec<std::process::Child> {
    ...
    let api_key = env_nonempty("SCREENER_LOCAL_API_KEY");
    ...
    if let (Some(tools_path), Some(port)) = (ensure_tools_mcp_server(config), free_loopback_port())
    {
        ...
        cmd.arg(&tools_path)
            .env("SCREENER_TOOLS_HTTP_PORT", port.to_string())
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::inherit());
        if let Some(url) = &engine_url {
            cmd.env("SCREENER_API_URL", url);
        }
        if let Some(key) = &api_key {
            cmd.env("SCREENER_LOCAL_API_KEY", key);
        }
        if let Some(chat_id) = env_nonempty("SCREENER_CHAT_SESSION_ID") {
            cmd.env("SCREENER_CHAT_SESSION_ID", chat_id);
        }
        match cmd.spawn() {
            Ok(child) => {
                children.push(child);
                urls.push(("screener-tools".into(), format!("http://127.0.0.1:{port}/mcp")));
    ...
    // Core search over http. `screener-mcp --http` dispatches to the package's
    // Streamable-HTTP server (there is no separate screener-mcp-http package,
    ...
    if let Some(port) = free_loopback_port() {
        ...
        if let Some(engine_port) =
            env_nonempty("SCREENER_LOCAL_API_PORT").or_else(|| env_nonempty("SCREENER_PORT"))
        {
            cmd.arg("--screener-port").arg(engine_port);
        }
        if let Some(url) = &engine_url {
            cmd.env("SCREENER_API_URL", url);
        }
        ...
        match cmd.spawn() {
            Ok(child) => {
                children.push(child);
                urls.push(("screener".into(), format!("http://127.0.0.1:{port}/mcp")));
            }
...
/// Screener's own local, read-only screen-data tools (the `screener` MCP
...
/// `screener-tools` server (mcp__screener-tools__*), which includes writes,
/// app-connect, and the user-MCP bridge and must keep prompting.
fn is_screener_read_tool(tool_title: &str) -> bool {
    // The screener-mcp read server (search-content, activity-summary, ...) is
    // entirely read-only.
    if tool_title.starts_with("mcp__screener__") {
        return true;
    }
    // Specific read-only tools from the bundled screener-tools server.
    ...
    // recordings, so auto-approved exactly like their mcp__screener__*
    // equivalents on stdio. The write/bridge tools (save_artifact, sp_mcp_call,
    // screener_connect_app, live_view, sp_web_search) stay NOT auto-approved.
    matches!(
        tool_title,
        "mcp__screener-tools__query_recordings"
            | "mcp__screener-tools__list_connections"
            | "mcp__screener-tools__activity_summary"
            | "mcp__screener-tools__keyword_search"
            | "mcp__screener-tools__search_elements"
            | "mcp__screener-tools__frame_context"
            | "mcp__screener-tools__get_frame_elements"
            | "mcp__screener-tools__list_meetings"
            | "mcp__screener-tools__get_meeting"
            | "mcp__screener-tools__health_check"
    )
}
...
/// Consumer Claude subscription sign-in, which Screener must never offer.
///
            ...
            let init = connection
                .send_request(
                    InitializeRequest::new(ProtocolVersion::V1)
                        ...
                        .client_info(
                            Implementation::new("screener", env!("CARGO_PKG_VERSION"))
                                .title("Screener".to_owned()),
                        ),
                )
                .block_task()
                .await?;
...
pub async fn run_from_env() -> Result<(), String> {
    ...
    // Agents that ignore client stdio MCP servers (Cursor) get screener's
...
mod tests {
    ...
    #[test]
    fn screener_mcp_pin_matches_workspace_package_version() {
        let package: Value = serde_json::from_str(include_str!(
            "../../../../packages/screener-mcp/package.json"
        ))
        .expect("parse screener-mcp package.json");
        let version = package["version"]
            .as_str()
            .expect("screener-mcp package version");
        assert_eq!(SCREENER_MCP_PKG, format!("screener-mcp@{version}"));
    }
    ...
    fn cloud_token_is_forbidden_from_acp_processes() {
        assert!(is_forbidden_acp_env("SCREENER_API_KEY"));
        assert!(is_forbidden_acp_env("screener_api_key"));
        assert!(!is_forbidden_acp_env("SCREENER_LOCAL_API_KEY"));
    ...
    /// Routing an agent at Screener Cloud is what lets someone use a coding
    ...
    #[test]
    fn bundled_tools_mcp_server_is_registered_alongside_screener() {
        ...
        let names: Vec<String> = servers
            ...
            .collect();
        assert!(names.iter().any(|n| n == "screener"));
        assert!(
            names.iter().any(|n| n == "screener-tools"),
    ...
    fn first_turn_context_always_includes_the_tools_hint() {
        ...
        assert!(none.contains("screener_connect_app"));
    ...
    #[test]
    fn user_mcp_servers_are_forwarded_alongside_screener() {
        let mut config = runtime_config("claude-acp");
        ...
        let servers = mcp_servers(&config);
        // The built-in screener server is always present; match user
        ...
        assert!(servers
            .iter()
            .any(|s| matches!(s, McpServer::Stdio(stdio) if stdio.name == "screener")));
    ...
    #[test]
    fn screener_read_tools_auto_approve_prefer_always() {
        // Only the read-only screen-data server auto-approves; writes/bridge
        // (screener-tools) and everything else still prompt.
        assert!(is_screener_read_tool("mcp__screener__search-content"));
        assert!(is_screener_read_tool("mcp__screener-tools__query_recordings"));
        assert!(is_screener_read_tool("mcp__screener-tools__list_connections"));
        // Core read tools mirrored on screener-tools for http-only agents.
        assert!(is_screener_read_tool("mcp__screener-tools__activity_summary"));
        assert!(is_screener_read_tool("mcp__screener-tools__keyword_search"));
        assert!(is_screener_read_tool("mcp__screener-tools__get_meeting"));
        assert!(is_screener_read_tool("mcp__screener-tools__health_check"));
        assert!(!is_screener_read_tool("mcp__screener-tools__sp_mcp_call"));
        assert!(!is_screener_read_tool("mcp__screener-tools__save_artifact"));
        assert!(!is_screener_read_tool("bash"));
