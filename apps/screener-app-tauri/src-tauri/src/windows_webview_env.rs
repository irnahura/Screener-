...
pub fn install_user_data_dir() {
    ...
    eprintln!(
        "screener: WebView2 user data dir source={} writable={} path={}{}",
        diagnostics.source,
...
fn ensure_writable_dir(path: &Path) -> Result<(), String> {
    ...
    let test_path = path.join(format!(".screener-write-test-{}", std::process::id()));
