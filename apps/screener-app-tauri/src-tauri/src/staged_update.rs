...
/// `…/screener.app/Contents/MacOS/screener-app` → `…/screener.app`.
...
pub fn install_staged_if_any<R: tauri::Runtime>(app: &tauri::AppHandle<R>) -> bool {
    ...
    if let (Some(extracted), Some(bundle)) = (extracted_app.as_deref(), bundle_root.as_deref()) {
        let scratch = path
            ...
            .unwrap_or_else(|| std::env::temp_dir().join("screener-replaced"));
...
/// bundles in a temp dir. No screener process, no real bundle, no network.
#[cfg(test)]
mod tests {
    use super::*;
    use base64::Engine;
    ...
    fn bundle_root_requires_macos_bundle_shape() {
        assert_eq!(
            bundle_root_from_exe(Path::new(
                "/Applications/screener.app/Contents/MacOS/screener-app"
            )),
            Some(PathBuf::from("/Applications/screener.app"))
        );
        // dev binary → no bundle → fast path disabled
        assert_eq!(
            bundle_root_from_exe(Path::new("/tmp/target/debug/screener-app")),
            None
        );
