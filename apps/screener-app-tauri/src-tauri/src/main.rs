...
pub use recording::spawn_screener;
pub use recording::stop_screener;
...
use log_files::{get_log_files, get_screener_data_dir};
use shortcuts::{
...
/// Returns true when SCREENER_SKIP_ONBOARDING is set to a truthy value
...
fn should_skip_onboarding() -> bool {
    std::env::var("SCREENER_SKIP_ONBOARDING")
        .ok()
...
impl PendingPlainLoginDuplicate {
    ...
    fn consume_if_plain<S: AsRef<str>>(&mut self, args: &[S], now: std::time::Instant) -> bool {
        ...
        if args.len() != 1 || args[0].as_ref().starts_with("screener://") {
            return false;
        }
        self.expires_at = None;
        true
...
async fn main() {
    ...
    // Refuse to launch while a `screener db recover|cleanup` operation is in
    // progress. The CLI writes ~/.screener/.db_recovery.lock before doing
    ...
    //   1. `screener db unlock` — friendly path
    //   2. SCREENER_IGNORE_DB_LOCK=1 env var — bypass on this launch only
    //   3. `rm ~/.screener/.db_recovery.lock` — manual
    //
    // See `crates/screener-engine/src/cli/db.rs`.
    if std::env::var("SCREENER_IGNORE_DB_LOCK").ok().as_deref() != Some("1") {
        let lock_path =
            screener_core::paths::default_screener_data_dir().join(".db_recovery.lock");
        if let Ok(metadata) = std::fs::metadata(&lock_path) {
            ...
            if stale {
                ...
                eprintln!(
                    "screener: a `screener db ...` operation is in progress.\n\
                     lock: {}\n\
                     content: {}\n\
                     options:\n  • wait for the op to finish, then re-open the app\n  • run `screener db unlock` if you're sure it's stuck\n  • set SCREENER_IGNORE_DB_LOCK=1 and retry to bypass this check",
                    lock_path.display(),
                    body.trim(),
                );
                std::process::exit(2);
    ...
    // runtime in screener-audio. This flag drives the "compatibility mode"
    ...
    {
        let cpu = screener_core::cpu_features::snapshot();
        if !cpu.avx2 {
            eprintln!(
                "screener: cpu lacks AVX2 ({}); running in compatibility mode — local whisper/qwen3 STT disabled",
                cpu.as_log_string()
            );
            health::set_cpu_compat_mode(true);
        }

        // Hidden CI probe: `screener-app.exe --cpu-smoke` exits 0 here,
    ...
    // zbus/tokio conflict) and acts as a fallback on macOS/Windows.
    {
        ...
        let deep_link_url = args
            .iter()
            .find(|a| a.starts_with("screener://"))
            .cloned();

        let focus_port: u16 = std::env::var("SCREENER_FOCUS_PORT")
            .ok()
            ...
            .await
        {
            if resp.status().is_success() {
                eprintln!("screener: another instance is already running — focused existing window, exiting.");
                std::process::exit(0);
            }
        }
    }
    ...
    let store_path = screener_core::paths::default_screener_data_dir().join("store.bin");
    let store_json = std::fs::read(&store_path).ok().and_then(|data| {
        if data.len() >= 8 && &data[..8] == b"SPSTORE1" {
            ...
            let plain = screener_vault::crypto::decrypt_small(&data[8..], &key).ok()?;
    ...
    let sentry_guard = if !telemetry_disabled {
        Some(sentry::init((
            ...
            sentry::ClientOptions {
                release: Some(format!("screener-app@{}", app_version).into()),
                send_default_pii: false,
                server_name: Some("screener-app".into()),
                before_send: Some(std::sync::Arc::new(|mut event| {
                    ...
                    let build_time: u64 = env!("SCREENER_BUILD_UNIX_TIME")
                        .parse()
                        .unwrap_or(0);
                    if build_time > 0 {
    ...
    {
        let log_dir = screener_core::paths::default_screener_data_dir();
    ...
    let app = app.plugin(tauri_plugin_single_instance::init(|app, args, _cwd| {
        ...
        let _ = app.run_on_main_thread(move || {
            ...
            if let Some(url) = args_clone.iter().find(|a| a.starts_with("screener://")) {
    ...
    let sync_scheduler = screener_connect::sync_scheduler::SyncScheduler::new();
    ...
    let app = app.manage(recording_state)
        ...
        .setup(move |app| {
            ...
            #[cfg(target_os = "macos")]
            {
                ...
                let mut app_submenu_builder = SubmenuBuilder::new(app, "screener")
                    .item(&PredefinedMenuItem::about(app, Some("About screener"), None)?)
                    .separator();
                ...
                let app_submenu = app_submenu_builder
                    .item(
                        &MenuItemBuilder::with_id("quit_app", "Quit screener")
            ...
            let log_dir = get_screener_data_dir(app.handle())
                .unwrap_or_else(|_| screener_core::paths::default_screener_data_dir());
            let file_appender = RollingFileAppender::builder()
                .rotation(Rotation::DAILY)
                .filename_prefix("screener-app")
                .filename_suffix("log")
                .max_log_files(5)
                .build(log_dir)?;
            ...
            if !screener_core::cpu_features::has_avx2() {
                warn!(
                    ...
                    screener_core::cpu_features::snapshot().as_log_string()
                );
            }
            ...
            // triggers the "screener is damaged" Gatekeeper popup and can
            ...
            // Resolve data directory from user setting (custom dir or ~/.screener)
            let (data_dir, data_dir_fell_back) = config::resolve_data_dir(&store.data_dir)?;
            info!("Recording data directory: {}", data_dir.display());

            // Pin SCREENER_DATA_DIR to the *resolved* dir so every consumer of
            // `default_screener_data_dir()` agrees with the engine on where the
            ...
            // `~/.screener`. Tokens landed in one data directory and were read
            ...
            // can fire) makes `default_screener_data_dir()` self-consistent and
            // also propagates the correct dir to child processes (the CLI
            // sidecar inherits this env).
            std::env::set_var("SCREENER_DATA_DIR", &data_dir);

            // The fs-plugin scope in capabilities/main.json only whitelists
            // static locations ($HOME/.screener, $APPDATA, …). A custom
            ...
            // the dir is the default ~/.screener.
            {
            ...
            if !telemetry_disabled {
                sentry::configure_scope(|scope| {
                    ...
                    let telemetry_context = screener_engine::telemetry_context::TelemetryContext::from_env();
            ...
            // Escape hatch: SCREENER_SKIP_ONBOARDING=1 marks onboarding complete
            ...
            if should_skip_onboarding() && !onboarding_store.is_completed {
                match store::OnboardingStore::update(&app.handle(), |o| o.complete()) {
                    Ok(_) => {
                        info!("SCREENER_SKIP_ONBOARDING: onboarding marked complete");
                        onboarding_store.is_completed = true;
                    }
                    Err(e) => error!("SCREENER_SKIP_ONBOARDING: failed to complete onboarding: {}", e),
                }
            }
            ...
            //   reduces wait time after the final restart.
            {
                ...
                tauri::async_runtime::spawn(async move {
                    ...
                    let engine = match store_for_download.recording.audio_transcription_engine.as_str() {
                        "deepgram" | "screener-cloud" => None, // Cloud engines don't need local model
                    ...
                    // gate in screener-audio disables it), so don't burn 834MB of
                    // bandwidth/disk on a model that can't load.
                    if let Some(engine) = engine {
                        if screener_core::cpu_features::has_avx2() {
                            let engine_clone = engine.clone();
                            tokio::task::spawn_blocking(move || {
                                match screener_audio::transcription::whisper::model::download_whisper_model(engine_clone) {
                    ...
                    let (_silero_result, _seg_result, _emb_result) = tokio::join!(
                        async {
                            match screener_audio::vad::silero::SileroVad::ensure_model_downloaded().await {
                        ...
                        async {
                            ...
                            match screener_audio::speaker::models::ensure_model_file(
                                screener_audio::speaker::models::PyannoteModel::Segmentation
                            ).await {
                        ...
                        async {
                            match screener_audio::speaker::models::ensure_model_file(
                                screener_audio::speaker::models::PyannoteModel::Embedding
                            ).await {
            ...
            //         && screener_vault::crypto::is_encrypted_file(&data_dir.join("db.sqlite")).unwrap_or(false));
            // if vault_is_locked {
            ...
            let launch_db_quarantined = screener_db::sqlite_quarantine_exists(&launch_db_path);
            if launch_db_quarantined {
                ...
                crate::health::set_boot_error(
                    "database remains quarantined after a SQLite hard fault; run `screener db recover` while screener is closed",
                );
            ...
            'start_server: {
                ...
                if !crate::recording::recording_access_allowed(&store_clone) {
                    info!("Skipping server auto-start: screener account access required");
                ...
                // spawn_screener command. DB-wedge recovery consults this
                ...
                let on_pipe_output: Option<screener_core::pipes::OnPipeOutputLine> = Some(
                ...
                let server_thread = std::thread::Builder::new()
                    .name("screener-server".to_string())
                    .spawn(move || {
                        let server_runtime = match tokio::runtime::Builder::new_multi_thread()
                            .worker_threads(crate::resource_limits::worker_threads())
                            .max_blocking_threads(crate::resource_limits::blocking_threads())
                            .thread_name("screener-worker")
                            .enable_all()
                            .build()
                        {
                            Ok(runtime) => runtime,
                        ...
                        server_runtime.block_on(async move {
                            ...
                            // request would 403. Mirrors the path in `spawn_screener`.
                            if store_clone.recording.api_auth {
                                ...
                                match screener_engine::auth_key::resolve_api_auth_key(
                                    &data_dir_clone,
                            ...
                            let owned_browser =
                                screener_connect::connections::browser::OwnedBrowser::default_instance();
            ...
            // Setup tray
            if let Some(_) = app_handle.tray_by_id("screener_main") {
            ...
            // degraded heuristic — detection now happens in `screener-engine`
            ...
            // which only handles screener-connect integrations.
            ...
            // telemetry builds with SCREENER_ENTERPRISE_LICENSE_KEY env set.
                ...
                tauri::RunEvent::Ready { .. } => {
                    ...
                    tauri::async_runtime::spawn(async move {
                        if let Some(analytics) = app_handle.try_state::<Arc<AnalyticsManager>>() {
                            ...
                            // STT disabled) — see cpu_features in screener-core.
                            let cpu = screener_core::cpu_features::snapshot();
                            let _ = analytics
                                .send_event(
                                    "app_started",
...
mod autostart_arg_tests {
    ...
    fn detects_autostart_flag() {
        assert!(args_contain_autostart(["screener", AUTOSTART_ARG]));
    ...
    fn ignores_unrelated_args() {
        assert!(!args_contain_autostart(["screener"]));
        assert!(!args_contain_autostart([
            "screener",
            "--check-arc-automation"
        ]));
        assert!(!args_contain_autostart(["screener", "--autostarted"]));
        assert!(!should_suppress_startup_handoff(&["screener"]));
    }
    ...
    fn plain_login_duplicate_is_bounded_and_one_shot() {
        ...
        assert!(!pending.consume_if_plain(&["screener", "--manual"], start));
        assert!(!pending.consume_if_plain(&["screener://open"], start));
        assert!(pending.consume_if_plain(&["screener"], start));
        assert!(!pending.consume_if_plain(&["screener"], start));
