...
mod tests {
    ...
    fn team_config_writes_and_updates_gateway_url() {
        ...
        std::env::set_var("SCREENER_DATA_DIR", dir.path());
        ...
        std::env::remove_var("SCREENER_DATA_DIR");
    }
    ...
    fn chat_entries_missing_dir_is_empty() {
        ...
        let res =
            scan_chat_entries_by_mtime("/definitely/not/a/real/screener/chats/path").unwrap();
        assert!(res.is_empty());
    }
    ...
    fn login_callback_accepts_website_fallback_scheme() {
        assert!(is_login_callback_scheme("screener"));
    }

    // Regression for b7dc02415: `get_local_api_config` returned {key: null}
    // during the cold-spawn window between webview load and `spawn_screener`
...
/// (`SCREENER_DISABLE_TELEMETRY` / `GITHUB_ACTIONS` / `CI`).
///
/// The Rust senders already consult
/// [`screener_engine::analytics::telemetry_disabled_by_env`] directly, but the
...
pub fn is_telemetry_disabled_by_env() -> bool {
    screener_engine::analytics::telemetry_disabled_by_env()
}
...
pub fn get_low_disk_guard_config() -> LowDiskGuardConfig {
    LowDiskGuardConfig {
        threshold_bytes: screener_events::LOW_DISK_THRESHOLD_BYTES,
        check_interval_seconds:
            screener_engine::disk_pressure::LOW_DISK_CHECK_INTERVAL_SECS,
    }
}
...
pub async fn get_local_api_config(app_handle: tauri::AppHandle) -> serde_json::Value {
    ...
    // against spawn_screener, or pause window). The webview's
...
pub fn get_app_server_config() -> serde_json::Value {
    let port = std::env::var("SCREENER_FOCUS_PORT")
        .ok()
...
/// the `SCREENER_PORT` E2E/dev override.
...
pub async fn regenerate_api_auth_key(app_handle: tauri::AppHandle) -> Result<String, String> {
    ...
    let key = screener_engine::auth_key::regenerate_api_auth_key(&data_dir)
        .await
...
pub async fn set_api_auth_key(app_handle: tauri::AppHandle, key: String) -> Result<(), String> {
    let data_dir = screener_core::paths::default_screener_data_dir();
    screener_engine::auth_key::set_api_auth_key(&data_dir, &key)
        .await
        .map_err(|e| e.to_string())?;
    persist_api_auth_key_to_settings(&app_handle, &key)
}
...
/// Resolve the chat-conversations directory under the *active* screener data
/// dir (honors `SCREENER_DATA_DIR` / a relocated data dir), creating it if
/// needed. The frontend previously hardcoded `~/.screener/chats` via
...
/// copy conversations from the legacy `~/.screener/chats` so history isn't
/// orphaned. Skipped under e2e (`SCREENER_E2E_SEED` set) and under dev
/// isolation so isolated runs stay empty — otherwise `bun tauri dev` copies the
/// developer's entire production chat history into `~/.screener-dev`, which
...
pub fn get_chats_dir() -> Result<String, String> {
    ...
    let is_isolated = std::env::var("SCREENER_E2E_SEED").is_ok()
        || crate::dev_isolation::is_active();
    if !is_isolated {
        if let Some(home) = dirs::home_dir() {
            let legacy = home.join(".screener").join("chats");
            if legacy != chats
                && legacy.is_dir()
                && !dir_has_conversations(&chats)
                && dir_has_conversations(&legacy)
            {
                match screener_core::paths::copy_dir_all(&legacy, &chats) {
...
/// in the screener-api skill that Pi installs on every run.
///
/// Mechanism: the screener-core `Pi::ensure_screener_skill` reads
...
/// to `<project>/.pi/skills/screener-api/SKILL.md`. Default (no
...
pub fn set_cloud_media_analysis_skill(enabled: bool) -> Result<(), String> {
    // Resolve through the data dir rather than hardcoding ~/.screener, so a
    // dev or relocated instance writes the marker its own Pi will read. Same
    // path as before for a default production install.
    let dir = screener_core::paths::default_screener_data_dir();
    ...
    if enabled {
        ...
        std::fs::write(
            &marker,
            "Opt-out marker — Pi will install the screener-api skill \
...
pub fn get_enterprise_file_config() -> EnterpriseFileConfig {
    ...
    let user_path = screener_core::paths::default_screener_data_dir().join("enterprise.json");
...
/// Merge-write device config fields into `~/.screener/enterprise.json`,
...
/// Save the enterprise license key to `~/.screener/enterprise.json`.
...
/// Persist the resolved "hide app UI" decision into `~/.screener/enterprise.json`
...
fn persist_enterprise_hide_app(hidden: bool) {
    let path = screener_core::paths::default_screener_data_dir().join("enterprise.json");
...
/// `~/.screener/enterprise.json`. Returns None when the file is
...
/// Read the user's screener cloud session JWT.
///
...
/// `~/.screener/auth.json` (the CLI credential file) remains as a fallback
...
pub fn get_cloud_token() -> Option<String> {
    ...
    let path = screener_core::paths::default_screener_data_dir().join("auth.json");
    let raw = std::fs::read_to_string(&path).ok()?;
    let parsed: serde_json::Value = serde_json::from_str(&raw).ok()?;
    parsed
        .get("token")
...
pub async fn set_cloud_token(
    ...
    // Sign-out: scrub the screener token from pi's auth files before the
    ...
    if should_clear_pi_auth {
        if let Err(e) = crate::pi::clear_screener_auth_token_files() {
            warn!("failed to clear pi screener auth token: {}", e);
        }
    }
...
/// decide whether to inject `screener-team`; the native CLI resolves the API
...
pub fn set_tray_unhealth_icon(app_handle: tauri::AppHandle) {
    ...
    let _ = app_handle.run_on_main_thread(move || {
        if let Some(main_tray) = app.tray_by_id("screener_main") {
            match tauri::image::Image::from_path("icons/screener-logo-tray-failed.png") {
                Ok(icon) => {
...
pub fn set_tray_health_icon(app_handle: tauri::AppHandle) {
    ...
    let _ = app_handle.run_on_main_thread(move || {
        if let Some(main_tray) = app.tray_by_id("screener_main") {
            match tauri::image::Image::from_path("icons/screener-logo-tray-black.png") {
                Ok(icon) => {
                    if let Err(e) = crate::safe_icon::safe_set_icon(&main_tray, icon) {
...
#[specta::specta]
pub fn update_show_screener_shortcut(
    app_handle: tauri::AppHandle,
    new_shortcut: String,
    enabled: bool,
) -> Result<(), String> {
...
pub async fn get_disk_usage(
    ...
    let screener_dir_path = match data_dir {
        Some(d) if !d.is_empty() && d != "default" => std::path::PathBuf::from(d),
        _ => screener_core::paths::default_screener_data_dir(),
    };
    ...
    match crate::disk_usage::disk_usage(&screener_dir_path, freshness).await {
...
/// produced with NEXT_PUBLIC_SCREENER_WEB_URL signs in against that
/// deployment, so its session token verifies on the same control plane
/// the rest of the app talks to.
fn login_url() -> String {
    crate::web_base::screener_web_url("/login")
}
...
mod login_url_intent_tests {
    ...
    fn carries_explicit_signup_intent_and_return_scheme() {
        let login_url = login_url_with_intent(Some(LoginMode::SignUp), Some("screener"))
    ...
    fn keeps_neutral_login_urls_free_of_mode() {
        let login_url =
            login_url_with_intent(None, Some("screener-enterprise")).expect("valid login URL");
        let parsed: tauri::Url = login_url.parse().expect("parse generated login URL");
        let pairs = parsed
            .query_pairs()
            .collect::<std::collections::HashMap<_, _>>();
...
/// `screener://` on machines that have both installed (see #3890). Login
...
/// redirects stay on `screener://`, matching the consumer path.
...
fn is_login_callback_scheme(scheme: &str) -> bool {
    scheme == deep_link_scheme() || scheme == "screener"
}
...
/// Open the screener.com login page.
/// macOS: ASWebAuthenticationSession (system-managed sheet, forwards callback).
/// Windows/Linux: in-app WebView that intercepts the screener:// redirect.
///
...
pub async fn open_login_window(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        // scheme routing), so the consumer `screener` scheme cannot collide
    ...
    #[cfg(not(target_os = "macos"))]
    {
        ...
        if !fresh_session {
            ...
            // website deep-link `screener://auth?api_key=…` straight back.
            //
...
mod login_window_tests {
    ...
    fn reused_login_window_returns_to_login_page() {
        ...
        let login_url: tauri::Url = "https://screener.com/login?return_scheme=screener"
            .parse()
            .unwrap();
        let window =
...
/// Same pattern as `open_login_window` — intercepts the screener:// deep-link
...
pub async fn show_window(
    ...
    if !matches!(
        ...
        // transitioning to another screener window so focus should stay
...
pub async fn enable_keychain_encryption() -> Result<KeychainStatus, String> {
    ...
    let data_dir = screener_core::paths::default_screener_data_dir();
    if let Err(e) = screener_secrets::mark_encryption_enabled(&data_dir) {
        tracing::warn!("failed to write .encrypt-store flag: {}", e);
    }

    if let Ok(store) =
        screener_secrets::SecretStore::open_for_data_dir(&data_dir, Some(key)).await
    {
...
pub async fn disable_keychain_encryption() -> Result<KeychainStatus, String> {
    let data_dir = screener_core::paths::default_screener_data_dir();
    let secrets_path = screener_secrets::secrets_database_path(&data_dir);

    if secrets_path.exists() || data_dir.join("db.sqlite").exists() {
        let plain_store = screener_secrets::SecretStore::open_for_data_dir(&data_dir, None)
            .await
        ...
        if encrypted_count > 0 {
            ...
            let encrypted_store =
                screener_secrets::SecretStore::open_for_data_dir(&data_dir, Some(key))
                    .await
    ...
    screener_secrets::mark_encryption_disabled(&data_dir)
...
fn shortcut_reminder_payload(
    ...
    map.insert(
        ...
        serde_json::Value::String(shortcut_reminder_label(
            &settings.show_screener_shortcut,
            "showScreenerShortcut",
...
pub(crate) async fn show_shortcut_reminder_impl(
    ...
    // The screener shortcut only opens the timeline/rewind overlay, so the
...
#[cfg(not(target_os = "macos"))]
pub(crate) fn any_screener_webview_has_focus(app: &tauri::AppHandle) -> bool {
    app.webview_windows()
        .values()
...
pub async fn rollback_to_version(
    ...
    if let Err(e) =
        crate::stop_screener(app_handle.state::<RecordingState>(), app_handle.clone()).await
    {
...
pub async fn perform_ocr_on_image(
    ...
    let languages: Vec<screener_core::Language> = store
        .recording
        .languages
        .iter()
        .filter(|s| s != &"default")
    ...
    let (text, _text_json, _confidence) = match ocr_engine {
        #[cfg(target_os = "macos")]
        OcrEngine::AppleNative => screener_screen::perform_ocr_apple(&img, &languages),
        OcrEngine::Tesseract => screener_screen::perform_ocr_tesseract(&img, languages),
        #[cfg(target_os = "windows")]
        OcrEngine::WindowsNative => screener_screen::perform_ocr_windows(&img, &languages)
            .await
...
/// Copy a frame deeplink (screener://frame/N) to clipboard. Native API only.
#[tauri::command]
#[specta::specta]
pub async fn copy_deeplink_to_clipboard(frame_id: i64) -> Result<(), String> {
    let link = format!("screener://frame/{}", frame_id);
    let mut clipboard = arboard::Clipboard::new().map_err(|e| format!("clipboard error: {}", e))?;
    clipboard
        .set_text(link)
        .map_err(|e| format!("failed to set clipboard: {}", e))?;
...
pub async fn list_cache_files() -> Result<Vec<CacheFile>, String> {
    ...
    // Pi agent node_modules (~/.screener/pi-agent/)
    ...
    // Pi config (~/.screener/pi-config/). Never list the user's global
    ...
    // (https://github.com/screenpipe/screener/issues/4002).
    ...
    // Stale root-level node_modules (~/.screener/node_modules/)
    ...
    for entry in std::fs::read_dir(&data_dir).map_err(|e| e.to_string())? {
        ...
        // Old log files (screener.*.log — legacy CLI format)
        if name.starts_with("screener.") && name.ends_with(".log") {
        ...
        // Empty/stale DB files (data.db, screener.db, store.sqlite)
        if matches!(name.as_str(), "data.db" | "screener.db" | "store.sqlite") {
            let size = path.metadata().map(|m| m.len()).unwrap_or(0);
            if size == 0 {
                files.push(CacheFile {
                    path: path.to_string_lossy().to_string(),
