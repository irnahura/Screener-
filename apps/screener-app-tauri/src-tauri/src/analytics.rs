...
impl AnalyticsManager {
    ...
    pub async fn fetch_attribution(&self) {
        ...
        match self
            .client
            .get("https://screener.com/api/attribution")
    ...
    pub async fn send_event(
        ...
        let mut payload = json!({
            ...
            "properties": {
                ...
                "release": format!("screener-app@{}", app_version),
                "$set": {
                    "app_version": app_version,
                    "os_name": os_name,
                    "os_version": os_version,
    ...
    fn read_feature_config(&self) -> serde_json::Value {
        let store_path = self.screener_dir_path.join("store.bin");
...
pub fn start_analytics(
    ...
    // RIOM is a local-first MVP. Keep the manager as a no-op-compatible state
    // object for existing command paths, but never start outbound attribution,
    // PostHog, or periodic analytics workers.
    let _ = (analytics_enabled, is_debug);
    let should_enable_analytics = false;
    ...
    tokio::spawn({
        ...
        async move {
            ...
            // screener.com regardless — so an opt-out user still leaked their
