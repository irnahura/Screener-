...
/// must never be treated as consent to relaunch screener.
#[tauri::command]
#[specta::specta]
...
/// Intel-Mac CI smoke-test investigation, screener#4978).
#[cfg(target_os = "macos")]
...
/// the boot-phase state machine that `spawn_screener`'s race-prevention
/// path and the auto-update restart gate already use for this exact class of
/// race (found during the Intel-Mac CI smoke-test investigation, screener#4978).
#[cfg(target_os = "macos")]
...
fn check_accessibility_permission() -> OSPermissionStatus {
    core_to_os_status(screener_core::permissions::check_accessibility())
}
...
fn core_to_os_status(status: screener_core::permissions::PermissionStatus) -> OSPermissionStatus {
    match status {
        screener_core::permissions::PermissionStatus::NotNeeded => OSPermissionStatus::NotNeeded,
        screener_core::permissions::PermissionStatus::NotDetermined => OSPermissionStatus::Empty,
        screener_core::permissions::PermissionStatus::Granted => OSPermissionStatus::Granted,
        screener_core::permissions::PermissionStatus::Denied => OSPermissionStatus::Denied,
    }
}
...
pub fn check_microphone_permission() -> OSPermissionStatus {
    core_to_os_status(screener_core::permissions::check_microphone())
}
...
pub fn check_screen_recording_permission() -> OSPermissionStatus {
    #[cfg(target_os = "macos")]
    {
        screen_recording_status(
            ...
            screener_engine::permission_monitor::screen_enumeration_denied(),
            false,
        )
    }
...
pub fn check_accessibility_permission_live_cmd() -> OSPermissionStatus {
    core_to_os_status(screener_core::permissions::check_accessibility_live())
}
...
pub fn check_input_monitoring_permission_cmd() -> OSPermissionStatus {
    #[cfg(target_os = "macos")]
    {
        if screener_a11y::check_input_monitoring() {
...
pub async fn request_input_monitoring_permission() -> OSPermissionStatus {
    #[cfg(target_os = "macos")]
    {
        ...
        if screener_a11y::request_input_monitoring() {
            OSPermissionStatus::Granted
        } else {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if matches!(permission, OSPermission::Calendar) {
            tokio::task::spawn_blocking(|| {
                let cal = screener_connect::calendar::ScreenerCalendar::new();
                cal.reset();
            })
            .await
...
pub fn check_coreaudio_process_tap_available() -> bool {
    ...
    {
        screener_audio::core::process_tap::is_process_tap_available()
    }
...
/// In production: triggers "screener wants to control Arc" prompt via direct FFI.
...
// `screener-engine::permission_monitor` which emits `permission_lost` /
...
mod mic_grant_restart_tests {
    ...
    // the Intel-Mac CI smoke-test investigation, screener#4978).
    #[test]
