...
//!
//! See `crates/screener-audio/src/audio_manager/windows_output_follow.rs`
//! for the watchdog, and
//! `crates/screener-events/src/custom_events/audio_health.rs` for the
//! payload shape.
...
fn show_speaker_silent_notification(app: AppHandle, data: Value) {
    ...
    let body = if rendering.is_empty() {
        "you're in a meeting but screener isn't capturing speaker audio, \
         so the transcript will be missing the other side. check your \
         playback device, or restart recording."
            .to_string()
    } else {
        format!(
            "you're in a meeting and audio is playing on \"{}\", but screener \
...
/// Show the in-app notification panel telling the user screener couldn't
