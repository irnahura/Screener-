...
//!
//! See `crates/screener-audio/src/audio_manager/device_monitor.rs` for the
//! state machine, and `crates/screener-events/src/custom_events/audio_devices.rs`
...
fn show_fallback_engaged(app: AppHandle, data: Value) {
    ...
    let body = format!(
        "\"{pinned}\" is offline, so screener is recording from \"{fallback}\" \
...
fn show_input_unavailable(app: AppHandle, data: Value) {
    ...
    let body = format!(
        "\"{pinned}\" went offline and there's no other microphone to record from, \
         ...
         screener will resume automatically."
    );
    let payload = serde_json::json!({
...
fn show_recovered(app: AppHandle, data: Value) {
    ...
    let body = if key == NO_INPUT_KEY {
        "a microphone is available again — screener resumed recording.".to_string()
    } else {
        format!(
            "\"{}\" is back — screener is recording from your selected mic again.",
            display_name(pinned_raw)
        )
    };
    let payload = serde_json::json!({
