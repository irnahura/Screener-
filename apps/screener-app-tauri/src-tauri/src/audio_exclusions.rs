...
//! (`screener_audio::core::process_tap`) and read from a JSON file on
...
//! ```
//!
//! The path defaults to `$HOME/.screener/audio-exclusions.json` and can
//! be overridden with `SCREENER_AUDIO_EXCLUSIONS_PATH` — both the engine
...
const ENV_OVERRIDE: &str = "SCREENER_AUDIO_EXCLUSIONS_PATH";
const DEFAULT_RELATIVE_PATH: &str = ".screener/audio-exclusions.json";
...
mod tests {
    ...
    fn read_returns_empty_when_missing() {
        with_env(
            "/nonexistent/screener-audio-exclusion-cmd-test.json",
            || {
