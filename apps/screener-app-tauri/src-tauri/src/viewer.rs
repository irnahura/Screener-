...
/// directory* — e.g. `.pi/skills/screener-api/SKILL.md` — so a naive read from
...
pub fn resolve_local_path(path: &str) -> PathBuf {
    resolve_local_path_in(path, &screener_core::paths::default_screener_data_dir())
}
...
fn resolve_local_path_in(path: &str, data_dir: &Path) -> PathBuf {
    ...
    // Resolve relative agent paths ONLY against screener-owned, app-created
...
fn friendly_io_error(e: &std::io::Error) -> String {
    ...
    match e.kind() {
        ...
        ErrorKind::PermissionDenied => {
            "permission denied — screener can't read this file".to_string()
        }
        _ => e.to_string(),
    }
}
...
mod tests {
    ...
    fn resolve_pi_skills_maps_to_store() {
        ...
        let skill = data.join("skills").join("screener-api");
        std::fs::create_dir_all(&skill).unwrap();
        let file = skill.join("SKILL.md");
        std::fs::write(&file, b"# skill").unwrap();

        let got = resolve_local_path_in(".pi/skills/screener-api/SKILL.md", &data);
        assert_eq!(got, file);

        // Same with a leading `./`.
        let got2 = resolve_local_path_in("./.pi/skills/screener-api/SKILL.md", &data);
        assert_eq!(got2, file);
