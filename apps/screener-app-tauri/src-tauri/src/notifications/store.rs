...
//! Disk-backed notification history at `~/.screener/notifications.json`.
...
fn path() -> PathBuf {
    screener_core::paths::default_screener_data_dir().join("notifications.json")
}
...
fn write_all_to(path: &Path, entries: &[NotificationHistoryEntry]) -> Result<String, String> {
    ...
    screener_core::memories::external_sync::write_atomic_full(path, &json)
