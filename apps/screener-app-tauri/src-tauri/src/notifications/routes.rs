...
fn pipe_config_denies_notify(pipe_name: &str) -> bool {
    ...
    let pipe_md = screener_core::paths::default_screener_data_dir()
        .join("pipes")
        .join(pipe_name)
        .join("pipe.md");
...
fn pipe_config_denies_notify_from_content(raw: &str) -> anyhow::Result<bool> {
    let (config, _) = screener_core::pipes::parse_frontmatter(raw)?;
    let perms = screener_core::pipes::permissions::PipePermissions::from_config(&config);
...
pub async fn send_notification(
    ...
    // Rewrite file-path markdown links to screener://view?path=… so they
    ...
    let source_url = payload.source_url.clone().or_else(|| {
        source_session_id.as_ref().map(|session_id| {
            ...
            if let Some(message_id) = &source_message_id {
                format!(
                    "screener://chat/{}?message={}",
                    encoded_session,
                    urlencoding::encode(message_id)
                )
            } else {
                format!("screener://chat/{}", encoded_session)
            }
        })
    });
    ...
    fn derives_source_metadata_from_pipe_session_header() {
        ...
        assert_eq!(
            ...
            NotificationSourceMetadata {
                ...
                source_url: Some(
                    "screener://chat/pipe%3Along-notification-source-test%3A389?message=notification-abc123"
                        .to_string()
                ),
