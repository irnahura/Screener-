...
//!     screener's whole job is to keep recording, so no reduced state hides
...
mod tests {
    ...
    fn a_different_meeting_is_not_a_repeat() {
        ...
        let first = repeat_key(
            ...
            None,
            "meeting detected",
            "screener is saving this meeting for transcription: Google Meet",
        );
        let second = repeat_key(
            Some("meeting"),
            None,
            "meeting detected",
            "screener is saving this meeting for transcription: standup",
        );
