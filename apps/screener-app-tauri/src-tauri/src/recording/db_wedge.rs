...
use super::{
    bounded_teardown, spawn_screener_inner, RecordingState, TeardownOutcome,
};
...
fn db_wedge_recovery_decision(
    signaled_health: &screener_db::WriteQueueHealth,
    signaled_recovery_epoch: u64,
    current_health: Option<&screener_db::WriteQueueHealth>,
...
pub fn make_db_wedge_recovery_hook(
    ...
    health: screener_db::WriteQueueHealth,
) -> screener_db::PersistentFailureHook {
...
async fn recover_from_db_wedge(
    ...
    *recording_state.interrupted_meeting.lock().await = None;
    ...
    if let Err(e) = spawn_screener_inner(&recording_state, app.clone()).await {
        ...
        error!("db wedge auto-recovery: spawn_screener failed: {}", e);
        let evt = screener_events::DbRecoveryEvent::restart_failed();
        let _ = screener_events::send_event(evt.event_name(), evt);
...
mod tests {
    ...
    use screener_db::WriteQueueHealth;
