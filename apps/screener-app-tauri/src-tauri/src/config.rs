...
use screener_core::paths::{
    default_screener_data_dir, ensure_spotlight_excluded, ensure_spotlight_excluded_best_effort,
};
use std::{fs, net::IpAddr, path::PathBuf};
use tracing::warn;

pub const SCREENER_AI_GATEWAY_URL: &str =
    screener_core::agents::pi::SCREENER_API_URL;
const E2E_AI_GATEWAY_URL_ENV: &str = "SCREENER_E2E_AI_GATEWAY_URL";
...
fn resolve_ai_gateway_url(e2e_override: Option<&str>, e2e_mode: bool) -> Result<String, String> {
    if !e2e_mode {
        return Ok(SCREENER_AI_GATEWAY_URL.to_string());
    }

    match e2e_override {
        Some(raw_url) => validate_e2e_ai_gateway_url(raw_url),
        None => Ok(SCREENER_AI_GATEWAY_URL.to_string()),
    }
}
...
pub fn screener_ai_gateway_url() -> Result<String, String> {
    #[cfg(feature = "e2e")]
...
#[tauri::command]
#[specta::specta]
pub async fn get_screener_ai_gateway_url() -> Result<String, String> {
    screener_ai_gateway_url()
}
...
pub fn get_base_dir(
    ...
    let default_path = default_screener_data_dir();
...
/// Tauri command: absolute path of the screener base dir (where store.bin
/// lives). Honors SCREENER_DATA_DIR; the webview must use this instead of
/// hardcoding ~/.screener, or it reads/writes a different settings file
/// than the Rust side whenever the override is set.
#[tauri::command]
#[specta::specta]
pub async fn get_screener_base_dir(app: tauri::AppHandle) -> Result<String, String> {
...
/// custom path was unusable and we silently fell back to default (~/.screener or SCREENER_DATA_DIR).
pub fn resolve_data_dir(data_dir_setting: &str) -> anyhow::Result<(PathBuf, bool)> {
    let default_path = default_screener_data_dir();

    // "default" or empty → use ~/.screener
...
pub async fn validate_data_dir(path: String) -> Result<(), String> {
    ...
    let probe = p.join(".screener_probe");
...
mod tests {
    ...
    fn test_resolve_default() {
        ...
        assert!(!fell_back);
        assert!(path.ends_with(".screener"));
    }
    ...
    fn test_existing_absolute_path_gets_spotlight_migration() {
        ...
        let tmp = root.path().join("existing-screener-data");
        fs::create_dir_all(tmp.join("data")).unwrap();
        fs::write(tmp.join("db.sqlite"), b"existing install").unwrap();

        #[cfg(target_os = "macos")]
        {
            screener_core::paths::set_spotlight_excluded(&tmp, false).unwrap();
            assert!(!screener_core::paths::is_spotlight_excluded(&tmp).unwrap());
        }
    ...
    fn e2e_build_reads_configured_ai_gateway_override() {
        ...
        assert_eq!(screener_ai_gateway_url().unwrap(), configured_url);
    }
}
