...
pub fn setup_dock_menu(app_handle: AppHandle) {
    ...
    unsafe {
        ...
        extern "C" fn show_screener(_this: &Object, _sel: Sel, _sender: id) {
        ...
        extern "C" fn dock_menu(_this: &Object, _sel: Sel, _sender: id) -> id {
            unsafe {
                ...
                // "Show screener"
                let title = NSString::alloc(nil).init_str("Show screener");
                let action = sel!(showScreener:);
        ...
        let mut decl =
            objc::declare::ClassDecl::new("ScreenerDockMenuDelegate", superclass).unwrap();
        decl.add_method(
            sel!(showScreener:),
            show_screener as extern "C" fn(&Object, Sel, id),
        );
        decl.add_method(
            sel!(openSettings:),
        ...
        if !method.is_null() {
            ...
            for sel_name in &[sel!(showScreener:), sel!(openSettings:)] {
