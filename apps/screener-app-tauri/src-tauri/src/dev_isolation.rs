...
//! install's state.
//!
//! Before this existed, running the dev app next to an installed screener
...
//! * `SCREENER_DEV_USE_PROD_DATA=1` opts out entirely, for the rare case of
...
/// Data directory used by isolated debug builds. A sibling of `~/.screener`
...
pub const DEV_DATA_DIR_NAME: &str = ".screener-dev";
...
pub const AI_TOOLS_HOME_ENV: &str = "SCREENER_AI_TOOLS_HOME";

/// Opt-out: run a debug build against the production profile.
// Read only by the debug-only `apply_inner` (and the tests below).
#[cfg_attr(not(any(debug_assertions, test)), allow(dead_code))]
pub const OPT_OUT_ENV: &str = "SCREENER_DEV_USE_PROD_DATA";
...
#[cfg(debug_assertions)]
fn apply_inner() -> bool {
    // The E2E harness sets SCREENER_DATA_DIR / SCREENER_PORT /
    // SCREENER_FOCUS_PORT itself and compiles out the single-instance plugin.
    ...
    if is_truthy(OPT_OUT_ENV) {
        eprintln!(
            "screener[dev]: {OPT_OUT_ENV} set — using the PRODUCTION profile \
             (~/.screener, port 3030). Quit the installed app first; this build \
    ...
    let Some(home) = dirs::home_dir() else {
        ...
        eprintln!("screener[dev]: no home directory — dev isolation NOT applied");
        return false;
    };

    let data_dir: PathBuf = home.join(DEV_DATA_DIR_NAME);

    set_if_unset("SCREENER_DATA_DIR", &data_dir.to_string_lossy());
    set_if_unset("SCREENER_PORT", DEV_API_PORT);
    set_if_unset("SCREENER_FOCUS_PORT", DEV_FOCUS_PORT);
    // Dev sessions are not product usage; keep them out of PostHog/Sentry.
    set_if_unset("SCREENER_DISABLE_TELEMETRY", "1");
    ...
    eprintln!(
        "screener[dev]: isolated from production — data {}, api :{}, focus :{}. \
         Set {OPT_OUT_ENV}=1 to use the production profile instead.",
        env_or("SCREENER_DATA_DIR", &data_dir.to_string_lossy()),
        env_or("SCREENER_PORT", DEV_API_PORT),
        env_or("SCREENER_FOCUS_PORT", DEV_FOCUS_PORT),
    );
...
fn set_if_unset(key: &str, value: &str) {
    ...
    // `default_screener_data_dir()`'s handling of SCREENER_DATA_DIR.
...
fn is_truthy(key: &str) -> bool {
    // Lenient, matching the engine's SCREENER_DISABLE_TELEMETRY handling:
...
mod tests {
    ...
    fn dev_data_dir_is_not_inside_the_production_data_dir() {
        // A child of ~/.screener would be walked by production's disk-usage
        // accounting and retention sweeps.
        assert!(!DEV_DATA_DIR_NAME.starts_with(".screener/"));
        assert_ne!(DEV_DATA_DIR_NAME, ".screener");
    }
    ...
    fn is_truthy_accepts_common_spellings_and_rejects_others() {
        ...
        let key = "SCREENER_DEV_ISOLATION_TRUTHY_TEST";
    ...
    fn set_if_unset_never_overrides_an_explicit_value() {
        ...
        let key = "SCREENER_DEV_ISOLATION_SET_IF_UNSET_TEST";
