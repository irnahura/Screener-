...
fn detect_windows_enterprise_management(metadata: &mut EnterpriseInstallMetadata) {
    ...
    if let Ok(screener_key) = hklm.open_subkey("SOFTWARE\\screener") {
        let install_source: Result<String, _> = screener_key.get_value("InstallSource");
        set_enterprise_metadata_value(&mut metadata.install_source, install_source.ok());

        let update_manager: Result<String, _> = screener_key.get_value("UpdateManager");
        set_enterprise_metadata_value(&mut metadata.update_manager, update_manager.ok());

        metadata.detected_by.push("screener_registry".to_string());
    }
...
fn detect_macos_enterprise_management(metadata: &mut EnterpriseInstallMetadata) {
    for path in [
        "/Library/Preferences/pe.screener.enterprise.plist",
        "/Library/Managed Preferences/pe.screener.enterprise.plist",
    ] {
...
pub fn get_enterprise_install_metadata() -> EnterpriseInstallMetadata {
    ...
    if let Ok(value) = std::env::var("SCREENER_ENTERPRISE_INSTALL_SOURCE") {
        set_enterprise_metadata_value(&mut metadata.install_source, Some(value));
        metadata.detected_by.push("env".to_string());
    }
    if let Ok(value) = std::env::var("SCREENER_ENTERPRISE_UPDATE_MANAGER") {
