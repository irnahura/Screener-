...
//!
//! Hosted ingest sends plaintext JSONL to Screener over TLS. Direct upload
...
//!   Screener `PutObject` only — no Screener principal ever holds
...
//!   grants Screener read access so cloud pipes / Workflow Studio keep
//!   working against their bucket.
//!
//! In both cases Screener Cloud sees checksums and cursors, not the
...
use screener_core::sync::crypto::compute_checksum;
use screener_sync::pipeline::{TicketedConfig, TicketedPipeline};
use screener_sync::SyncError;
...
// `screener-telemetry-wire`, not here.
...
pub enum EnterpriseUploadMode {
    HostedIngest,
    /// Plaintext JSONL PUT into a bucket no Screener principal can read.
...
pub struct DirectUploadConfig {
    ...
    /// (`SCREENER_ENTERPRISE_UPLOAD_PINNED_HOSTS`, comma-separated
...
impl EnterpriseUploadMode {
    ...
    /// customers don't have to set `SCREENER_ENTERPRISE_UPLOAD_MODE`
    ...
    pub async fn resolve(
        ...
        if let Ok(raw) = std::env::var("SCREENER_ENTERPRISE_UPLOAD_MODE") {
            let normalized = raw.trim().to_ascii_lowercase();
            if !normalized.is_empty()
                && normalized != "screener_write"
                ...
                if let Some(mode) = Self::from_env(ingest_url) {
                    tracing::info!(
                        "enterprise sync: upload mode taken from \
                         SCREENER_ENTERPRISE_UPLOAD_MODE env override ({})",
                        normalized
                    );
                    return Ok(Some(mode));
                }
    ...
    pub fn from_env(ingest_url: &str) -> Option<Self> {
        let mode = std::env::var("SCREENER_ENTERPRISE_UPLOAD_MODE")
            .unwrap_or_else(|_| "screener_write".to_string())
            .trim()
            .to_ascii_lowercase();

        match mode.as_str() {
            "" | "screener_write" | "hosted_ingest" => Some(Self::HostedIngest),
...
fn parse_server_mode_hint(raw: &str) -> Result<ServerModeHint, EnterpriseSyncError> {
    match raw.trim().to_ascii_lowercase().as_str() {
        ...
        "hosted_ingest" | "screener_write" | "" => Ok(ServerModeHint::HostedIngest),
...
impl DirectUploadConfig {
    fn from_ingest_url(ingest_url: &str) -> Self {
        let ticket_url = std::env::var("SCREENER_ENTERPRISE_UPLOAD_TICKET_URL")
            .ok()
            .filter(|s| !s.trim().is_empty())
            .unwrap_or_else(|| sibling_enterprise_endpoint(ingest_url, "upload-ticket"));
        let complete_url = std::env::var("SCREENER_ENTERPRISE_UPLOAD_COMPLETE_URL")
            .ok()
            .filter(|s| !s.trim().is_empty())
        ...
        if pinned_hosts.is_empty() {
            tracing::info!(
                "enterprise sync: no upload destination pin configured \
                 (SCREENER_ENTERPRISE_UPLOAD_PINNED_HOSTS unset) — tickets \
...
fn pinned_hosts_from_env() -> Vec<String> {
    std::env::var("SCREENER_ENTERPRISE_UPLOAD_PINNED_HOSTS")
        .ok()
        .map(|raw| {
            raw.split(',')
...
/// Glue between the screener ingest wire format and
/// `screener_sync::TicketedPipeline`. The pipeline does ticket → PUT →
/// complete with backoff; this fn just builds the JSON shapes the
/// screener control plane expects and maps errors back into the
...
mod tests {
    ...
    fn direct_cfg() -> DirectUploadConfig {
        DirectUploadConfig {
            ticket_url: "https://screener.com/api/enterprise/upload-ticket".to_string(),
            complete_url: "https://screener.com/api/enterprise/upload-complete".to_string(),
    ...
    fn sync_cfg() -> EnterpriseSyncConfig {
        EnterpriseSyncConfig {
            ...
            ingest_url: "https://screener.com/api/enterprise/ingest".to_string(),
    ...
    fn sibling_urls_are_derived_from_ingest_url() {
        assert_eq!(
            sibling_enterprise_endpoint(
                "https://screener.com/api/enterprise/ingest",
                "upload-ticket"
            ),
            "https://screener.com/api/enterprise/upload-ticket"
        );
        assert_eq!(
    ...
    fn pinned_hosts_env_parsing_handles_lists_and_absence() {
        let prior = std::env::var("SCREENER_ENTERPRISE_UPLOAD_PINNED_HOSTS").ok();

        std::env::remove_var("SCREENER_ENTERPRISE_UPLOAD_PINNED_HOSTS");
        assert!(pinned_hosts_from_env().is_empty());

        std::env::set_var(
            "SCREENER_ENTERPRISE_UPLOAD_PINNED_HOSTS",
        ...
        match prior {
            Some(v) => std::env::set_var("SCREENER_ENTERPRISE_UPLOAD_PINNED_HOSTS", v),
            None => std::env::remove_var("SCREENER_ENTERPRISE_UPLOAD_PINNED_HOSTS"),
        }
    }
