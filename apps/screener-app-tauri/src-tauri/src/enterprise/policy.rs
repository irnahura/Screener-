...
//!
//! The frontend fetches the policy from the screener API and pushes it here
...
fn env_hides_app_ui() -> bool {
    std::env::var("SCREENER_ENTERPRISE_HIDE_APP")
        .ok()
        .as_deref()
        .map(truthy)
        .unwrap_or(false)
        || std::env::var("SCREENER_ENTERPRISE_UI_MODE")
            .ok()
            .as_deref()
            .map(|mode| {
                matches!(
...
fn user_enterprise_config_path() -> std::path::PathBuf {
    screener_core::paths::default_screener_data_dir().join("enterprise.json")
}
