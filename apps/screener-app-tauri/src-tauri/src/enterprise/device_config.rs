...
//! `~/.screener/enterprise.json` and starts sync — no app restart.
//!
//! What is deliberately NOT here: the MDM destination pin
//! (`SCREENER_ENTERPRISE_UPLOAD_PINNED_HOSTS`). It exists to catch a
...
/// first:
///   1. `SCREENER_ENTERPRISE_DEVICE_CONFIG_URL` (dev/staging override)
...
pub fn device_config_url(file_ingest_url: Option<&str>) -> String {
    if let Ok(url) = std::env::var("SCREENER_ENTERPRISE_DEVICE_CONFIG_URL") {
        let url = url.trim();
        if !url.is_empty() {
...
mod tests {
    ...
    fn device_config_url_layers() {
        ...
        let prior = std::env::var("SCREENER_ENTERPRISE_DEVICE_CONFIG_URL").ok();
        let prior_ingest = std::env::var("SCREENER_ENTERPRISE_INGEST_URL").ok();
        std::env::remove_var("SCREENER_ENTERPRISE_DEVICE_CONFIG_URL");
        std::env::remove_var("SCREENER_ENTERPRISE_INGEST_URL");

        // default = the baked control-plane base (prod unless the build
        // set NEXT_PUBLIC_SCREENER_WEB_URL — assert against the helper
        // so the test holds for dev builds too)
        let baked_default = crate::web_base::screener_web_url("/api/enterprise/device-config");
        ...
        std::env::set_var(
            "SCREENER_ENTERPRISE_INGEST_URL",
        ...
        std::env::set_var(
            "SCREENER_ENTERPRISE_DEVICE_CONFIG_URL",
        ...
        match prior {
            Some(v) => std::env::set_var("SCREENER_ENTERPRISE_DEVICE_CONFIG_URL", v),
            None => std::env::remove_var("SCREENER_ENTERPRISE_DEVICE_CONFIG_URL"),
        }
        match prior_ingest {
