...
//!
//! Periodically pulls new screen + audio + UI activity from the local screener
//! API and POSTs it as JSONL to the screener enterprise ingest endpoint,
...
/// Default endpoint. Overridable via `SCREENER_ENTERPRISE_INGEST_URL` for
/// staging / on-prem.
pub fn default_ingest_url() -> String {
    // Baked control-plane base (crate::web_base): a build produced with
    // NEXT_PUBLIC_SCREENER_WEB_URL targets that deployment app-wide,
    // exactly like a prod build targets screener.com.
    crate::web_base::screener_web_url("/api/enterprise/ingest")
}
...
impl EnterpriseSyncConfig {
    ...
    /// required env (`SCREENER_ENTERPRISE_LICENSE_KEY`) is missing — caller
    ...
    /// old "set `SCREENER_ENTERPRISE_UPLOAD_MODE` on every device" UX —
    ...
    /// URL resolved from somewhere else (`~/.screener/enterprise.json`,
    ...
    pub fn from_env_with_fallback(
        ...
        let license_key = std::env::var("SCREENER_ENTERPRISE_LICENSE_KEY")
            .ok()
            .filter(|s| !s.trim().is_empty())
            .or_else(|| license_key_fallback.filter(|s| !s.trim().is_empty()))?;
        let ingest_url = std::env::var("SCREENER_ENTERPRISE_INGEST_URL")
            .ok()
            .filter(|s| !s.trim().is_empty())
        ...
        let explicit_mode = std::env::var("SCREENER_ENTERPRISE_UPLOAD_MODE")
            .ok()
            .map(|s| s.trim().to_ascii_lowercase())
            .filter(|s| {
                !s.is_empty() && s != "auto" && s != "screener_write" && s != "hosted_ingest"
            });
...
/// Abstraction over the local screener HTTP API — kept generic so tests can
...
// same types — so it lives in `screener-telemetry-wire`. Re-exported here
// so the desktop shim keeps importing everything from `ee_sync::`.
pub use screener_telemetry_wire::{
...
pub enum EnterpriseSyncError {
    ...
    /// MDM-pinned destination list (`SCREENER_ENTERPRISE_UPLOAD_PINNED_HOSTS`).
...
pub use screener_telemetry_wire::{build_jsonl, build_jsonl_with_parsed};
...
async fn run_one_sync_inner(
    ...
    body.extend_from_slice(&screener_telemetry_wire::build_feedback_jsonl(
        &cfg.device_id,
        &cfg.device_label,
        &feedback,
    ));
...
/// Derive the control-plane base (e.g. `https://screener.com`) from the
/// configured ingest URL, so staging / on-prem `SCREENER_ENTERPRISE_INGEST_URL`
...
async fn fulfill_log_requests(
    ...
    // Existing readable customer-storage orgs deliberately grant Screener
...
mod tests {
    ...
    async fn enterprise_log_confirmation_uses_shared_device_metadata() {
        ...
        tokio::fs::write(
            dir.path().join("screener.2026-07-10.log"),
    ...
    fn from_env_handles_all_cases() {
        ...
        let prior_license = std::env::var("SCREENER_ENTERPRISE_LICENSE_KEY").ok();
        let prior_url = std::env::var("SCREENER_ENTERPRISE_INGEST_URL").ok();
        let prior_mode = std::env::var("SCREENER_ENTERPRISE_UPLOAD_MODE").ok();

        // Case 1: no license env → None.
        std::env::remove_var("SCREENER_ENTERPRISE_LICENSE_KEY");
        std::env::remove_var("SCREENER_ENTERPRISE_UPLOAD_MODE");
        let dir = TempDir::new().unwrap();
        assert!(
        ...
        std::env::set_var("SCREENER_ENTERPRISE_LICENSE_KEY", "   ");
        let dir = TempDir::new().unwrap();
        assert!(
            EnterpriseSyncConfig::from_env(dir.path().to_path_buf(), "dev".into(), "host".into())
                .is_none(),
        ...
        std::env::set_var("SCREENER_ENTERPRISE_LICENSE_KEY", "sek_test");
        std::env::remove_var("SCREENER_ENTERPRISE_INGEST_URL");
        let dir = TempDir::new().unwrap();
        let cfg =
            EnterpriseSyncConfig::from_env(dir.path().to_path_buf(), "dev".into(), "host".into())
        ...
        std::env::set_var("SCREENER_ENTERPRISE_INGEST_URL", "https://staging/ingest");
        let dir = TempDir::new().unwrap();
        let cfg =
            EnterpriseSyncConfig::from_env(dir.path().to_path_buf(), "dev".into(), "host".into())
                .unwrap();
        ...
        std::env::set_var(
            "SCREENER_ENTERPRISE_UPLOAD_MODE",
        ...
        std::env::set_var("SCREENER_ENTERPRISE_UPLOAD_MODE", "carrier_pigeon");
        let dir = TempDir::new().unwrap();
        ...
        match prior_license {
            Some(v) => std::env::set_var("SCREENER_ENTERPRISE_LICENSE_KEY", v),
            None => std::env::remove_var("SCREENER_ENTERPRISE_LICENSE_KEY"),
        }
        match prior_url {
            Some(v) => std::env::set_var("SCREENER_ENTERPRISE_INGEST_URL", v),
            None => std::env::remove_var("SCREENER_ENTERPRISE_INGEST_URL"),
        }
        match prior_mode {
            Some(v) => std::env::set_var("SCREENER_ENTERPRISE_UPLOAD_MODE", v),
            None => std::env::remove_var("SCREENER_ENTERPRISE_UPLOAD_MODE"),
        }
    }
    ...
    impl UploadModeEnvGuard {
        fn clear() -> Self {
            let prior = std::env::var("SCREENER_ENTERPRISE_UPLOAD_MODE").ok();
            std::env::remove_var("SCREENER_ENTERPRISE_UPLOAD_MODE");
            Self(prior)
        }
    }
    ...
    impl Drop for UploadModeEnvGuard {
        fn drop(&mut self) {
            match self.0.take() {
                Some(value) => std::env::set_var("SCREENER_ENTERPRISE_UPLOAD_MODE", value),
                None => std::env::remove_var("SCREENER_ENTERPRISE_UPLOAD_MODE"),
            }
        }
    }
    ...
    async fn explicit_mdm_mode_resolves_without_calling_the_control_plane() {
        let _env_lock = ENV_LOCK.lock().unwrap();
        let _mode_env = UploadModeEnvGuard(std::env::var("SCREENER_ENTERPRISE_UPLOAD_MODE").ok());
        std::env::set_var(
            "SCREENER_ENTERPRISE_UPLOAD_MODE",
            "direct_upload_write_only",
        );
    ...
    async fn parsed_rows_upload_as_a_separate_gated_stream_end_to_end() {
        ...
        let batch = screener_telemetry_wire::parse_jsonl(body);
    ...
    async fn direct_upload_success_advances_cursor_after_complete() {
        ...
        // IAM (no Screener read principal), not ciphertext. The PUT body
    ...
    fn control_plane_base_derives_from_ingest_url() {
        assert_eq!(
            control_plane_base("https://screener.com/api/enterprise/ingest").as_deref(),
            Some("https://screener.com")
        );
        assert_eq!(
