...
/// recording start via `screener_engine::auth_key::resolve_api_auth_key`,
...
fn decrypt_store_file(path: &Path) -> DecryptOutcome {
    ...
    match screener_vault::crypto::decrypt_small(&data[8..], &key) {
        Ok(plaintext) => {
...
///
/// To opt in: create ~/.screener/.encrypt-store or set SCREENER_ENCRYPT_STORE=1.
fn encrypt_store_file(path: &Path) {
    // Check opt-in flag
    let opted_in = std::env::var("SCREENER_ENCRYPT_STORE")
        .map(|v| v == "1")
        .unwrap_or(false)
        || path
            .parent()
    ...
    match screener_vault::crypto::encrypt_small(&data, &key) {
        Ok(ciphertext) => {
...
pub struct SettingsStore {
    ...
    #[serde(flatten)]
    pub recording: screener_config::RecordingSettings,
    ...
    pub user: User,
    #[serde(rename = "showScreenerShortcut")]
    pub show_screener_shortcut: String,
    ...
    /// Use screener cloud for AI-powered features like suggestions.
    ...
    /// Disabled by default so the overlay doesn't appear in screener's own recordings.
    #[serde(rename = "showOverlayInScreenRecording", default)]
    pub show_overlay_in_screen_recording: bool,

    /// Hide screener windows from screenshots and screen-sharing viewers
...
pub enum AIProviderType {
    ...
    #[serde(rename = "custom")]
    Custom,
    #[serde(rename = "screener-cloud", alias = "claude-code")]
    ScreenerCloud,
    #[serde(rename = "acp")]
    Acp,
...
impl Default for AIPreset {
    fn default() -> Self {
        Self {
            id: String::new(),
            prompt: String::new(),
            provider: AIProviderType::ScreenerCloud,
            acp_agent: None,
            url: "https://api.screener.com/v1".to_string(),
...
impl AudioEngineFallbackReason {
    pub fn notification_title(&self) -> &'static str {
        match self {
            Self::NotLoggedIn => "Screener Cloud unavailable",
    ...
    pub fn notification_body(&self) -> &'static str {
        match self {
            Self::NotLoggedIn => {
                "You are not logged in, so audio is being transcribed locally with Whisper Turbo (fast). Log in to use Screener Cloud."
            }
...
impl Default for SettingsStore {
    fn default() -> Self {
        ...
        let default_free_preset = AIPreset {
            id: "screener-cloud".to_string(),
            prompt: r#"IMPORTANT: At the start of every conversation, read the files in .pi/skills/ directory (e.g. .pi/skills/screener-api/SKILL.md and .pi/skills/screener-cli/SKILL.md) before responding.
Rules:
...
- Always wrap local file paths in angle brackets because screener paths often contain spaces or parentheses
...
"#.to_string(),
        ...
        Self {
            ...
            recording: screener_config::RecordingSettings {
            ...
            user: User::default(),
            #[cfg(target_os = "windows")]
            show_screener_shortcut: "Alt+S".to_string(),
            #[cfg(not(target_os = "windows"))]
            show_screener_shortcut: "Super+Ctrl+S".to_string(),
            #[cfg(target_os = "windows")]
...
impl SettingsStore {
    ...
    fn sanitize_legacy_fields(mut val: Value) -> Value {
        if let Some(obj) = val.as_object_mut() {
            ...
            // Migrate unscoped "screener" ignore-pattern to app-scoped "screener::"
            // so browser tabs whose title contains "screener" are no longer falsely
            ...
            if let Some(Value::Array(windows)) = obj.get_mut("ignoredWindows") {
                for entry in windows.iter_mut() {
                    if entry.as_str() == Some("screener") {
                        *entry = Value::String("screener::".to_string());
                    }
                }
            }
    ...
    /// Screener Business". One downgrade, or one older build opening the
    /// store, was enough.
    ///
    ...
    pub fn to_recording_settings(&self) -> screener_config::RecordingSettings {
        ...
        // This token is used as the Bearer credential for screener cloud
            ...
            .filter(|policy| policy.get("schemaVersion").and_then(Value::as_u64) == Some(1))
        {
            ...
            if policy
                ...
                .unwrap_or(false)
            {
                settings.aec_mode = screener_config::AecMode::Off;
    ...
    pub fn to_recording_config(
    ...
    ) -> screener_engine::RecordingConfig {
        let resolved_engine = self.audio_engine_resolution().active;
        let settings = self.to_recording_settings();
        let mut config = screener_engine::RecordingConfig::from_settings(
            &settings,
            data_dir,
            Some(&resolved_engine),
        );
        // Resolve the API auth key from the seeded cache. The cache is populated
        // asynchronously by `recording::spawn_screener` via the shared helper
        // (`screener_engine::auth_key::resolve_api_auth_key`) — which is the
        ...
        // throwaway instance can run alongside another screener already on
        // :3030 instead of piggy-backing on it.
        if let Some(p) = std::env::var("SCREENER_PORT")
            .ok()
    ...
    fn audio_engine_resolution_with_cloud_auth(
        ...
        match engine.as_str() {
            ...
            "screener-cloud" if !has_cloud_auth => {
                tracing::warn!("screener-cloud selected but user not logged in, falling back to whisper-large-v3-turbo-quantized");
...
pub fn init_store(app: &AppHandle) -> Result<SettingsStore, String> {
    ...
    {
        let detected = screener_config::detect_tier();
        ...
        if stored_tier != Some(detected) {
            ...
            if is_new_store || store.recording.device_tier.is_none() {
                screener_config::apply_tier_defaults(&mut store.recording, detected);
            }
        ...
        if screener_config::is_engine_unsafe(
            &store.recording.audio_transcription_engine,
            detected,
        ) {
            let safe = screener_config::best_engine_for_platform(detected);
            tracing::warn!(
                "engine {} is unsafe on this platform (tier={:?}, macOS={:?}) — switching to {}",
                store.recording.audio_transcription_engine,
                detected,
                screener_config::macos_major_version(),
                safe,
            );
    ...
    // recordings, and machine migrations often copy ~/.screener without the
    ...
    if is_new_store && !store.extra.contains_key("localRetentionEnabled") {
        let (data_dir, _) = crate::config::resolve_data_dir(&store.data_dir)
            .map_err(|error| format!("failed to prepare Screener data directory: {error}"))?;
...
pub fn locked_store_alert_message(detail: &str) -> String {
    format!(
        "screener could not open your settings, so it stopped before starting.\n\n\
         Your settings file is encrypted and screener needs the system keychain \
         ...
         Unlock your login keychain, then open screener again. If it keeps \
...
pub fn show_fatal_startup_alert(title: &str, message: &str) {
    ...
    let spawned = if cfg!(target_os = "macos") {
        ...
        screener_core::no_window_command("powershell")
            .args([
                "-NoProfile",
                "-WindowStyle",
                "Hidden",
...
mod tests {
    ...
    fn fresh_explicit_free_plan_applies_pipe_limit() {
        ...
        let config = store.to_recording_config(std::path::PathBuf::from("/tmp/screener"));
    ...
    fn verified_free_policy_persists_offline_but_paid_plan_stays_unlimited() {
        ...
        let config = lifetime.to_recording_config(std::path::PathBuf::from("/tmp/screener"));
    ...
    #[test]
    fn screener_cloud_falls_back_when_not_logged_in() {
        let mut store = SettingsStore::default();
        store.recording.audio_transcription_engine = "screener-cloud".to_string();
        store.user.id = None;
        ...
        assert_eq!(resolution.requested, "screener-cloud");
    ...
    #[test]
    fn screener_cloud_stays_active_for_signed_in_free_users() {
        ...
        assert_eq!(resolution.active, "screener-cloud");
        assert_eq!(resolution.fallback_reason, None);
    }

    #[test]
    fn screener_cloud_stays_active_for_app_entitled_users() {
        let mut store = SettingsStore::default();
        store.recording.audio_transcription_engine = "screener-cloud".to_string();
        store.user.id = Some("user_paid".to_string());
        store.user.token = Some("token".to_string());
    ...
    #[test]
    fn screener_cloud_stays_active_for_stale_legacy_cloud_subscribed_without_entitlement() {
    ...
    fn presets_n(n: usize) -> Vec<Value> {
        (0..n)
            .map(|i| json!({"id": format!("p{}", i), "model": "x", "provider": "screener-cloud"}))
            .collect()
    }
    ...
    /// and shows the 403 as "upgrade to Screener Business".
    #[test]
    ...
    fn deliberate_non_acp_presets_are_left_alone() {
        ...
        assert_eq!(presets[0]["provider"].as_str(), Some("screener-cloud"));
    ...
    fn remote_force_offs_win_after_recording_settings() {
        ...
        store.recording.aec_mode = screener_config::AecMode::Macos;
        ...
        assert_eq!(effective.aec_mode, screener_config::AecMode::Off);
        assert!(!effective.screener_aec_enabled);
