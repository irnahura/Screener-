...
async fn open_secret_store() -> Option<screener_secrets::SecretStore> {
...
/// Activate screener after an external OAuth flow completes.
///
/// macOS can activate the application as a whole and let AppKit restore the
/// user's last active Screener window. Windows and Linux require a concrete
...
pub(crate) fn activate_app_after_oauth_impl(app_handle: &AppHandle, window: &WebviewWindow) {
    #[cfg(target_os = "macos")]
    {
        ...
        if let Err(e) = app_handle.run_on_main_thread(|| {
            ...
            warn!("OAuth connected but could not activate screener: {e}");
        } else {
            info!("OAuth connected; activated screener");
        }
    }
...
pub async fn oauth_connect(
    ...
    if config.redirect_uri_override.is_none() {
        let effective_port = std::env::var("SCREENER_PORT")
            .ok()
            .and_then(|v| v.parse::<u16>().ok())
            .unwrap_or(settings.recording.port);
        if effective_port != 3030 {
            return Err(format!(
                "OAuth callbacks require the screener API on port 3030, but it is configured for port {}. Reset the port in settings to use connections.",
                effective_port
            ));
        }
    }
    ...
    let callback_timeout = std::env::var("SCREENER_OAUTH_TIMEOUT_SECS")
        .ok()
...
mod tests {
    ...
    use screener_connect::oauth::{OAuthCallbackResult, PendingOAuth, PENDING_OAUTH};
    use serde_json::json;
    ...
    async fn stale_pending_entries_are_swept_fresh_ones_kept() {
        use screener_connect::oauth::OAUTH_CALLBACK_TIMEOUT;
