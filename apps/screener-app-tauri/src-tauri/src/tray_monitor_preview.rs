...
fn preview_capture_expected() -> bool {
    preview_capture_expected_from(
        screener_engine::sleep_monitor::screen_is_locked(),
        screener_engine::sleep_monitor::recently_woke_from_sleep(),
        screener_engine::drm_detector::drm_content_paused(),
        screener_engine::schedule_monitor::schedule_paused(),
    )
}
...
async fn poll_sck_frames(app: &AppHandle) {
    ...
    for monitor_id in active_monitor_ids() {
        ...
        if screener_screen::stream_invalidation::monitor_frame_seq(monitor_id).unwrap_or(0) == 0 {
...
fn refresh_monitor_from_sck(monitor_id: u32) -> PreviewUpdate {
    let Some(frame) = screener_screen::stream_invalidation::peek_monitor_frame(monitor_id) else {
...
async fn bootstrap_sck_stream(monitor_id: u32) {
    ...
    if screener_screen::stream_invalidation::peek_monitor_frame(monitor_id).is_some() {
    ...
    let Some(monitor) = screener_screen::monitor::get_monitor_by_id(monitor_id).await else {
    ...
    if screener_screen::stream_invalidation::ensure_monitor_stream(
        monitor_id,
        monitor.width(),
        monitor.height(),
        &[],
