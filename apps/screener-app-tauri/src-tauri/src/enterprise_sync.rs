...
//! `apps/screener-app-tauri/src-tauri/src/enterprise/sync.rs`.
//! This shim only:
//!   - implements the `LocalApiClient` trait against the running screener
...
mod imp {
    ...
    // ─── Local API client over the running screener HTTP server ──────

    pub(super) struct ScreenerLocalClient {
        api_url_base: String,
    ...
    impl LocalApiClient for ScreenerLocalClient {
    ...
    fn license_key_from_env_or_config() -> Option<String> {
        std::env::var("SCREENER_ENTERPRISE_LICENSE_KEY")
            .ok()
            .filter(|s| !s.trim().is_empty())
    ...
    /// (crate::web_base — `NEXT_PUBLIC_SCREENER_WEB_URL` at build time).
    fn default_policy_url() -> String {
        crate::web_base::screener_web_url("/api/enterprise/policy")
    }
    ...
    fn spawn_hidden_ui_policy_watcher(app: &tauri::AppHandle) {
        ...
        tauri::async_runtime::spawn(async move {
            let policy_url = std::env::var("SCREENER_ENTERPRISE_POLICY_URL")
                .ok()
    ...
    pub fn configure_telemetry_context(app: &tauri::AppHandle) {
        ...
        set_env_default("SCREENER_ENTERPRISE_LICENSE_HASH", &org_key);
        set_env_default("SCREENER_CUSTOMER_ID", &org_key);
        set_env_default("SCREENER_ORG_ID", &org_key);

        if let Some(device_id) = device_id.as_deref() {
            set_env_default("SCREENER_ENTERPRISE_DEVICE_ID", device_id);
            set_env_default("SCREENER_DEPLOYMENT_ID", device_id);
            set_env_default("SCREENER_SUPPORT_ID", &format!("{org_key}:{device_id}"));
        }
    ...
    /// required env (`SCREENER_ENTERPRISE_LICENSE_KEY`) is missing — this is
    ...
    pub fn spawn(app: &tauri::AppHandle) -> Option<tokio::sync::watch::Sender<bool>> {
        ...
        // live in the screener data dir (RollingFileAppender), NOT Tauri's
        // app_data_dir — so the from_env default would miss them.
        let mut log_dirs = Vec::new();
        if let Ok(d) = crate::log_files::get_screener_data_dir(app) {
            log_dirs.push(d);
        }
        ...
        // usually `None`. ScreenerLocalClient re-reads it from
        // RecordingState on every request via the app handle.
        let local: Arc<dyn LocalApiClient> =
            Arc::new(ScreenerLocalClient::new(api_url_base, app.clone()));
        ...
        tauri::async_runtime::spawn(async move {
            ...
            // Small startup delay so the local screener server is up before
