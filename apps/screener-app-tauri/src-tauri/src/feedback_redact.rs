...
//! in the `screener-redact` crate — the frontend ships raw text and does no
...
mod tests {
    ...
    use screener_redact::{RedactError, RedactionOutput, Redactor};
    use serde_json::json;
    use std::sync::Arc;
    use std::time::Duration;
    ...
    async fn unattended_diagnostics_use_fail_closed_local_redaction() {
        let raw = concat!(
            ...
            "screener://auth?api_key=eyJhbGciOiJIUzI1NiJ9.fake-signature&source=email"
        )
        .to_string();
