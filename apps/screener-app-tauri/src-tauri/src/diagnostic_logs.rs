...
fn owned_log_files(files: Vec<LogFile>) -> Vec<LogFile> {
    files
        .into_iter()
        .filter(|file| is_screener_owned_log_name(&file.name))
        .collect()
}

fn is_screener_owned_log_name(name: &str) -> bool {
    matches!(
        name,
        "screener.log"
            | "screener-app.log"
            ...
            | "recover.stderr.log"
            | ".db_cleanup.log"
    ) || is_dated_rolling_log(name, "screener.")
        || is_dated_rolling_log(name, "screener-app.")
}
...
mod tests {
    ...
    async fn symlinks_are_never_followed() {
        ...
        let link = dir.path().join("screener.log");
    ...
    #[test]
    fn unattended_collection_only_accepts_known_screener_log_names() {
        for name in [
            "screener.log",
            "screener.2026-07-10.log",
            "screener.2026-07-10.2.log",
            "screener-app.2026-07-10.log",
            "last-panic.log",
            "recover.stderr.log",
            ".db_cleanup.log",
        ] {
            assert!(
                is_screener_owned_log_name(name),
        ...
        for name in [
            "private.log",
            "customer.log",
            "screener-secrets.log",
            "screener.2026-7-10.log",
            "screener.2026-07-10.backup.log",
        ] {
            assert!(
                !is_screener_owned_log_name(name),
