...
pub async fn restart_recording(app: tauri::AppHandle) {
    ...
    let teardown = crate::recording::bounded_teardown(
        ...
        crate::recording::stop_screener(app.state(), app.clone()),
    )
    .await;
    ...
    if let Err(e) = crate::recording::spawn_screener(app.state(), app.clone(), None).await {
...
/// breaking a real engine: `touch ~/.screener/.simulate-recording-failure`
...
pub fn simulated_break_active() -> bool {
    dirs::home_dir()
        .map(|h| h.join(".screener/.simulate-recording-failure").exists())
        .unwrap_or(false)
}
...
fn clear_simulated_break() {
    if let Some(h) = dirs::home_dir() {
        let _ = std::fs::remove_file(h.join(".screener/.simulate-recording-failure"));
    }
}
