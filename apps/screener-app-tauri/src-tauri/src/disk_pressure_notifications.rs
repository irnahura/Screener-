...
use screener_events::{DiskSpaceLowEvent, DiskSpaceRecoveredEvent};
...
const STORAGE_SETTINGS_DEEPLINK: &str = "screener://settings?section=storage";
...
impl Default for DiskPressureNotificationState {
    fn default() -> Self {
        Self::from_path(
            screener_core::paths::default_screener_data_dir().join(NOTIFICATION_STATE_FILE),
        )
    }
}
...
impl DiskPressureNotificationState {
    ...
    async fn complete_notification(&self) -> Result<(), String> {
        ...
        let write_result = screener_core::memories::external_sync::write_atomic_full(
            &self.path, &raw,
        )
    ...
    pub(crate) async fn recover(&self, event: &DiskSpaceRecoveredEvent) -> Result<bool, String> {
        if event.available_bytes < screener_events::LOW_DISK_RECOVERY_THRESHOLD_BYTES {
            return Ok(false);
        }
...
pub fn start(app: AppHandle) {
    ...
    tauri::async_runtime::spawn(async move {
        let mut events = screener_events::subscribe_to_event::<DiskSpaceLowEvent>(
    ...
    tauri::async_runtime::spawn(async move {
        let mut events = screener_events::subscribe_to_event::<DiskSpaceRecoveredEvent>(
...
fn low_disk_body(
    ...
    format!(
        "screener stopped capture because only {available} is free (safety threshold: \
...
mod tests {
    ...
    async fn recovery_requires_hysteresis_and_the_same_data_volume() {
        ...
        let too_soon = DiskSpaceRecoveredEvent::new(
            screener_events::LOW_DISK_RECOVERY_THRESHOLD_BYTES - 1,
            "test-data".to_string(),
        );
        ...
        let other_volume = DiskSpaceRecoveredEvent::new(
            screener_events::LOW_DISK_RECOVERY_THRESHOLD_BYTES,
