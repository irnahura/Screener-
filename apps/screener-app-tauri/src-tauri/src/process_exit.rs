...
//!   teardown (2026-07-01 report on screener 2.5.82)
//!
...
pub fn setup_terminate_interceptor(app_handle: AppHandle) {
    ...
    unsafe {
        ...
        let Some(mut decl) =
            objc::declare::ClassDecl::new("ScreenerTerminateDelegate", superclass)
        else {
...
/// True when any screener window is showing. Decides whether the quit
...
fn show_quit_alert(app: &AppHandle, show_minimize: bool, message: &str) {
    ...
    const QUIT_BUTTON: &str = "Quit screener";
    ...
    unsafe {
        ...
        let title = NSString::alloc(nil).init_str("Quit screener?");
        ...
        let plist = r#"
            <plist version="1.0">
              <dict>
                ...
                <string>screener</string>
                <key>CFBundleExecutable</key>
                <string>screener-app</string>
              </dict>
            </plist>
        "#;
