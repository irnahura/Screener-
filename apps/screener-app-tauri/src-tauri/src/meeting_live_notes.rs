...
pub fn start(app: AppHandle) {
    ...
    tauri::async_runtime::spawn(async move {
        let mut sub =
            screener_events::subscribe_to_event::<Vec<CalendarEventSignal>>("calendar_events");
    ...
    tauri::async_runtime::spawn(async move {
        let mut sub =
            screener_events::subscribe_to_event::<MeetingPrewarmEvent>("meeting_about_to_start");
        while let Some(event) = sub.next().await {
            ...
            client::send_typed_with_actions_and_priority(
                &header,
                format!("screener is ready to transcribe: {title}"),
                "meeting",
                Some(30_000),
                actions,
    ...
    tauri::async_runtime::spawn(async move {
        let mut sub =
            screener_events::subscribe_to_event::<MeetingStartedEvent>("meeting_started");
        ...
        while let Some(event) = sub.next().await {
            ...
            // a static note pane.
            let url = format!("screener://meeting/{meeting_id}");
            ...
            client::send_typed_with_actions_and_priority(
                "meeting detected",
                format!("screener is saving this meeting for transcription: {title}"),
                "meeting",
                Some(30_000),
                actions,
                NotificationPriority::High,
...
fn forward_screener_event(app: AppHandle, source: &'static str, target: &'static str) {
    tauri::async_runtime::spawn(async move {
        let mut sub = screener_events::subscribe_to_event::<serde_json::Value>(source);
        while let Some(event) = sub.next().await {
