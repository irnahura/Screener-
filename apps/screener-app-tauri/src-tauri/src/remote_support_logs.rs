...
mod imp {
    ...
    const DEFAULT_API_BASE: &str = "https://screener.com";
    ...
    fn api_base_url() -> String {
        ...
        #[cfg(debug_assertions)]
        if let Some(value) = std::env::var("SCREENER_SUPPORT_API_URL")
            .ok()
                        ...
                        match fulfill(&app, &api, &ctx, &request).await {
                            Ok(()) => {
                                ...
                                let _ = app
                                    ...
                                    .body("Filtered app diagnostics were shared with screener support.")
                                    .show();
                                status.set("ready");
                            }
                            Err(error) => {
    ...
    mod tests {
        ...
        fn jwt_subject_requires_a_non_empty_sub_claim() {
            ...
            let missing_sub = URL_SAFE_NO_PAD.encode(r#"{"aud":"screener"}"#);
