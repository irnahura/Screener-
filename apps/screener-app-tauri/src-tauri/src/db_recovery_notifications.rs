...
use screener_events::{DbRecoveryEvent, DbRecoveryState};

const RECOVERY_DEEPLINK: &str = "screener://database-recovery";
...
pub fn start(app: AppHandle) {
    ...
    tauri::async_runtime::spawn(async move {
        let mut sub =
            screener_events::subscribe_to_event::<DbRecoveryEvent>("db_recovery_restart_failed");
    ...
    tauri::async_runtime::spawn(async move {
        let mut sub =
            screener_events::subscribe_to_event::<DbRecoveryEvent>("db_recovery_needs_recovery");
        while let Some(event) = sub.next().await {
...
fn notify(app: &AppHandle, state: DbRecoveryState) {
    ...
    let (title, body) = match state {
        DbRecoveryState::RestartFailed => (
            "recording paused",
            "screener couldn't restart recording after a database error. reopen screener to continue with protected recovery.",
        ),
        DbRecoveryState::NeedsRecovery => (
            "recording paused",
            "your original database is protected. reopen screener to continue with protected recovery.",
        ),
    };
...
fn send_recovery_offer() {
    client::send_typed_with_actions_and_priority(
        ...
        "your original database is protected. screener can repair a verified copy and preserve the original. keep at least twice the database size free while it works.",
        "db_recovery",
        Some(0),
...
pub fn start_quarantined_database_recovery(app: AppHandle) -> Result<(), String> {
    ...
    if RECOVERY_ACTIVE.swap(true, Ordering::SeqCst) {
        client::send_typed_with_priority(
            ...
            "keep screener open while it builds and verifies a fresh database.",
            "db_recovery",
            Some(8_000),
            NotificationPriority::High,
        );
    ...
    client::send_typed_with_priority(
        ...
        "your original data is protected. screener is building and verifying a fresh copy. keep screener open.",
        "db_recovery",
        Some(0),
        NotificationPriority::High,
    );
    ...
    tauri::async_runtime::spawn(async move {
        match screener_engine::cli::db::recover_quarantined_database(&data_dir).await {
            Ok(()) => {
                RECOVERY_ACTIVE.store(false, Ordering::SeqCst);
                client::send_typed_with_priority(
                    "database repaired",
                    "your original database was preserved. screener is reopening and checking recording.",
                    "db_recovery",
                    Some(0),
                    NotificationPriority::High,
                );
            ...
            Err(recovery_error) => {
                ...
                client::send_typed_with_actions_and_priority(
                    ...
                    "your original data is still protected. screener couldn't finish the repair. check free disk space, then retry. technical details were saved to the logs.",
                    "db_recovery",
                    Some(0),
...
pub fn notify_recovery_quit_blocked() {
    ...
    client::send_typed_with_priority(
        ...
        "keep screener open until the repair finishes. your original data remains protected.",
        "db_recovery",
        Some(8_000),
        NotificationPriority::High,
    );
