...
use screener_core::pipes::install_bundled_pipe;
use screener_engine::live_views::{
...
use screener_engine::structured_outputs::{
...
fn active_screener_dir(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    let settings = SettingsStore::get(app)
        .map_err(|error| format!("failed to read Screener settings: {error}"))?
        .unwrap_or_default();
...
fn canvas_document_path(screener_dir: &Path, view_id: &str) -> PathBuf {
    screener_dir
        .join("live-views")
        .join("canvas")
...
fn read_canvas_document(
    ...
    let path = canvas_document_path(screener_dir, view_id);
    if !path.exists() {
        return Ok(None);
    }
...
fn save_canvas_document(
    ...
    let template = list_live_view_templates(screener_dir)
    ...
    let existing = read_canvas_document(screener_dir, &request.view_id)?;
    match &existing {
    ...
    let path = canvas_document_path(screener_dir, &document.view_id);
...
fn remove_canvas_document(screener_dir: &Path, view_id: &str) -> Result<(), String> {
    let _guard = canvas_store_lock()
        .lock()
        .map_err(|_| "canvas store lock was poisoned".to_string())?;
    let path = canvas_document_path(screener_dir, view_id);
...
pub async fn list_brain_views(app: tauri::AppHandle) -> Result<Vec<BrainViewDefinition>, String> {
    list_live_views(&active_screener_dir(&app)?)
...
pub async fn load_brain_view_canvas(
    ...
    let screener_dir = active_screener_dir(&app)?;
    let template = list_live_view_templates(&screener_dir)
        .map_err(|error| error.to_string())?
        .into_iter()
    ...
    let _guard = canvas_store_lock()
        .lock()
        .map_err(|_| "canvas store lock was poisoned".to_string())?;
    let document = read_canvas_document(&screener_dir, &view_id)?;
...
pub async fn save_brain_view_canvas(
    ...
    save_canvas_document(&active_screener_dir(&app)?, request)
}
...
pub async fn save_brain_view(
    ...
    save_live_view(&active_screener_dir(&app)?, request.into())
        .map(Into::into)
        .map_err(|error| error.to_string())
}
...
pub async fn delete_brain_view(app: tauri::AppHandle, id: String) -> Result<(), String> {
    let screener_dir = active_screener_dir(&app)?;
    delete_live_view(&screener_dir, &id).map_err(|error| error.to_string())?;
    remove_canvas_document(&screener_dir, &id)
}
