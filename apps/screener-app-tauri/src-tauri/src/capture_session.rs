...
use screener_audio::audio_manager::builder::AudioManagerOptions;
use screener_audio::core::device::resolve_audio_devices_for_capture;
use screener_audio::core::engine::AudioTranscriptionEngine;
use screener_audio::meeting_detector::MeetingDetector;
use screener_audio::transcription::deepgram::{
...
use screener_events::{send_event, PermissionEvent, PermissionKind};
...
pub struct CaptureSession {
    ...
    ui_recorder_handle: Option<screener_engine::UiRecorderHandle>,
    audio_manager: Arc<screener_audio::audio_manager::AudioManager>,
    audio_disabled: bool,
    /// Cleared on stop so `/vision/device/*` stops pointing at a shut-down manager.
    vision_manager_handle:
        Option<Arc<arc_swap::ArcSwap<Option<Arc<screener_engine::vision_manager::VisionManager>>>>>,
}
...
impl CaptureSession {
    ...
    pub async fn start(
        ...
        let mut capture_trigger_tx: Option<screener_engine::event_driven_capture::TriggerSender> =
            None;
        // --- Frame-linker sender (set by VisionManager, consumed by UI recorder + capture loops) ---
        let mut linker_tx: Option<screener_engine::frame_linker_actor::LinkerSender> = None;
        ...
        // spawn_screener is called again from onboarding after the user grants access.
        //
        ...
        let screen_recording_permitted =
            screener_core::permissions::check_screen_recording_tauri().is_granted();
        #[cfg(not(target_os = "macos"))]
        let screen_recording_permitted = true;

        if !config.disable_vision && !screen_recording_permitted {
            warn!("Screen recording permission not yet granted — skipping VisionManager to avoid native TCC dialog; will start on next spawn_screener after onboarding grants access");
        ...
        // --- Audio recording ---
        let audio_start_task = if !config.disable_audio {
            ...
            Some(tokio::spawn(async move {
                tokio::select! {
                    ...
                    _ = shutdown_rx.recv() => {
                        info!("Audio manager startup cancelled during capture shutdown");
                    }
                }
            }))
        } else {
        ...
        let accessibility_permitted =
            screener_core::permissions::check_accessibility().is_granted();
        #[cfg(not(target_os = "macos"))]
        let accessibility_permitted = true;

        let ui_recorder_handle = if !accessibility_permitted {
            warn!("Accessibility permission not yet granted — skipping UI event recording to avoid native TCC dialog; will start on next spawn_screener after onboarding grants access");
            // Emit permission_needed event so frontend can trigger onboarding/permission flow
            let _ = send_event(
            ...
            .await
            {
                Ok(handle) => {
                    ...
                    screener_engine::drm_detector::set_ui_recorder_stop_flag(handle.stop_flag());
                    Some(handle)
                }
                Err(e) => {
        ...
        // --- Speaker identification ---
        let _speaker_id_handle = screener_engine::start_speaker_identification(
            server.db.clone(),
    ...
    pub(crate) fn start_disk_pressure_monitor(&self) {
        screener_engine::disk_pressure::start_disk_pressure_monitor(
    ...
    pub async fn stop(mut self) {
        ...
        // Own and cancel delayed startup so a fast stop cannot start audio
        // after the capture session has already ended.
        if let Some(audio_start_task) = self.audio_start_task.take() {
            audio_start_task.abort();
            let _ = audio_start_task.await;
        }
