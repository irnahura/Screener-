...
pub fn start(app: AppHandle) {
    ...
    tauri::async_runtime::spawn(async move {
        let mut sub =
            screener_events::subscribe_to_event::<StallEvent>("meeting_streaming_audio_stall");
    ...
    tauri::async_runtime::spawn(async move {
        let mut sub = screener_events::subscribe_to_event::<StallEvent>(
...
fn handle_audio_stall(app: &AppHandle, event: StallEvent) {
    ...
    // surfaces even when the meeting window is on top of screener. Use
    ...
    std::thread::spawn(move || {
        if let Err(e) = app_for_notify
            .notification()
            .builder()
            .title("screener is not capturing audio")
            .body(format!(
...
fn handle_transcript_stall(app: &AppHandle, event: StallEvent) {
    ...
    client::send_typed_with_actions_and_priority(
        ...
        format!("audio is still being captured, but the transcript is delayed by {elapsed}s — keep the meeting open while screener retries"),
        "meeting",
        Some(30_000),
        Vec::new(),
