...
pub async fn try_resolve_quarantine(database_path: &Path) -> Result<(), SelfHealSkip> {
    if !screener_db::sqlite_quarantine_exists(database_path) {
        return Err(SelfHealSkip::NotQuarantined);
    }
    if screener_db::sqlite_hard_fault_latched(database_path) {
        return Err(SelfHealSkip::AlreadyLatchedThisProcess);
    }

    let marker = match screener_db::read_sqlite_quarantine(database_path) {
    ...
    if !screener_db::sqlite_quarantine_is_self_healable(code) {
        info!(
    ...
    let probe = match screener_db::probe_quarantined_generation_health(database_path).await {
        Ok(probe) => probe,
        Err(error) => {
            warn!(
                error = %error,
    ...
    match screener_db::resolve_verified_sqlite_quarantine(database_path, &archive) {
        Ok(Some(archived)) => {
            info!(
                sqlite_extended_code = code,
...
pub async fn try_self_heal_at_launch(app: AppHandle, database_path: PathBuf) -> bool {
    ...
    if let Err(error) = crate::recording::spawn_screener(app.state(), app.clone(), None).await {
    ...
    client::send_typed_with_priority(
        "recording resumed",
        "screener hit a temporary database error. your existing database passed a health check, so recording resumed without a rebuild.",
        "db_recovery",
        Some(10_000),
...
mod tests {
    ...
    fn archive_path_is_unique_per_fault() {
        let db = Path::new("/tmp/screener/db.sqlite");
    ...
    async fn launch_path_refuses_every_unproven_io_error_code() {
        ...
        for (recorded, expected) in [
            ...
            screener_db::persist_sqlite_quarantine(&db, recorded, "disk I/O error")
            ...
            assert!(screener_db::sqlite_quarantine_exists(&db));
        }
    }
}
