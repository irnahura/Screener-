...
///
/// Walking the screener data dir is O(files) and that tree reaches hundreds
...
pub struct CachedDiskUsage {
    pub timestamp: i64,
    pub usage: DiskUsage,
    /// The screener data directory this entry was computed for. Used to
...
pub fn get_cache_dir() -> Result<Option<PathBuf>, String> {
    ...
    Ok(Some(proj_dirs.join("screener")))
}
...
/// Everything `disk_usage` needs from the screener directory tree.
///
...
pub struct TreeScan {
    /// Every file under the screener dir.
    pub total_bytes: u64,
    ...
    pub pipes_bytes: u64,
    /// `*.log` files sitting directly in the screener dir.
    pub logs_bytes: u64,
...
/// Single recursive pass over the screener directory.
///
...
/// Walk `screener_dir` once and collect every size `disk_usage` reports.
pub fn scan_tree(screener_dir: &Path) -> io::Result<TreeScan> {
    SCAN_COUNT.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    let mut scan = TreeScan::default();
    if !screener_dir.exists() {
        return Ok(scan);
    }
    scan_tree_inner(screener_dir, Subtree::Other, &mut scan)?;

    // Logs are only the `*.log` files directly in the screener dir.
    if let Ok(entries) = fs::read_dir(screener_dir) {
...
fn read_cache(
    ...
    // `screener_dir` defaults to "" on older cache entries — those predate
    ...
    let cached_key_normalized = canonical_dir_key(Path::new(&cached.screener_dir));
    if cached.screener_dir.is_empty() || cached_key_normalized != current_dir_key {
        info!(
            "Cache dir mismatch (cached={}, current={}), recalculating",
            cached.screener_dir, current_dir_key
        );
        return None;
    }
...
pub async fn disk_usage(
    ...
    disk_usage_in(screener_dir, &cache_dir, freshness).await
}
...
pub(crate) async fn disk_usage_in(
    ...
    info!(
        ...
        screener_dir.display(),
        freshness
    );
    let data_dir = screener_dir.join("data");
    let cache_file = cache_dir.join("disk_usage.json");

    let current_dir_key = canonical_dir_key(screener_dir);
    ...
    info!("Calculating disk usage for {}", screener_dir.display());
    ...
    let scan_dir = screener_dir.clone();
    ...
    for file_name in [
        ...
        let db_path = screener_dir.join(file_name);
        if db_path.exists() {
    ...
    let available_space = {
        ...
        let path_obj = Path::new(&screener_dir);
        let available = sys
            .disks()
            .iter()
...
mod tests {
    ...
    mod reference {
        ...
        pub fn logs(screener_dir: &Path) -> u64 {
            let mut logs = 0;
            if let Ok(entries) = fs::read_dir(screener_dir) {
                for entry in entries.flatten() {
                    let path = entry.path();
    ...
    fn build_tree(root: &Path) {
        ...
        write(&root.join("screener.2026-08-14.log"), 70);
    ...
    fn fixture() -> (tempfile::TempDir, PathBuf, PathBuf) {
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path().join("screener");
    ...
    async fn cache_is_invalidated_when_the_data_dir_changes() {
        ...
        let other = other_tmp.path().join("screener");
