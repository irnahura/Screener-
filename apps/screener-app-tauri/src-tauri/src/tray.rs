...
fn prefetch_tray_menu_data(app: &AppHandle) -> TrayMenuData {
    ...
    let mut show_shortcut = if settings.show_screener_shortcut.trim().is_empty() {
        default_show.to_string()
    } else {
        settings.show_screener_shortcut.clone()
    };
    ...
    if settings
        .disabled_shortcuts
        .iter()
        .any(|shortcut| shortcut == "showScreenerShortcut")
    {
        show_shortcut.clear();
    }
    if settings
...
pub(crate) fn force_tray_rebuild(app: &AppHandle) -> Result<()> {
    ...
    if let Some(tray) = app.tray_by_id("screener_main") {
...
pub fn log_tray_position(app: &AppHandle) {
    if let Some(tray) = app.tray_by_id("screener_main") {
        ...
        error!("tray icon 'screener_main' not found");
    }
}
    ...
    let _ = app.run_on_main_thread(move || {
        if let Err(e) = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            crate::window::with_autorelease_pool(|| {
                ...
                let _old = app.remove_tray_by_id("screener_main");
                ...
                let icon = match app.path().resolve(
                    "assets/screener-logo-tray-white.png",
                    tauri::path::BaseDirectory::Resource,
                ) {
                    Ok(path) => tauri::image::Image::from_path(path).ok(),
                    Err(_) => {
                        tauri::image::Image::from_path("assets/screener-logo-tray-white.png").ok()
                    }
                };

                let mut builder = TrayIconBuilder::<Wry>::with_id("screener_main")
...
fn create_dynamic_menu(
    ...
    if !data.onboarding_completed && !data.app_ui_hidden {
        menu_builder = menu_builder
            ...
            .item(&MenuItemBuilder::with_id("quit", "Quit screener").build(app)?);
    ...
    if !data.app_ui_hidden {
        menu_builder = menu_builder
            .item(&MenuItemBuilder::with_id("open_app", "Open screener").build(app)?)
            .item(&PredefinedMenuItem::separator(app)?);
    }
    ...
    let version_text = match (is_beta, is_enterprise) {
        (_, true) => format!("screener v{} (Enterprise)", app.package_info().version),
        (true, false) => format!("screener v{} (Beta)", app.package_info().version),
        (false, false) => format!("screener v{}", app.package_info().version),
    };
    menu_builder = menu_builder.item(
    ...
    menu_builder = menu_builder.item(
        &MenuItemBuilder::with_id("quit", "Quit screener")
    ...
    match event.id().as_ref() {
        ...
        id if id.starts_with("pause_") => {
            ...
            let handle = tauri::async_runtime::spawn(async move {
                ...
                send_notify("Recording resumed", "screener is recording again.");
            });
            *PAUSE_TIMER.lock().unwrap_or_else(|e| e.into_inner()) = Some(PauseTimer {
            ...
            send_notify(
                "Recording paused",
                format!("screener will auto-resume in {}.", pretty),
            );
        ...
        "releases" => {
            ...
            let _ = app_handle.run_on_main_thread(move || {
                let _ = app
                    .opener()
                    .open_url("https://screener.com/changelog", None::<&str>);
            });
        }
        "update_now" => {
            let app = app_handle.clone();
                    ...
                    tauri::async_runtime::spawn(async move {
                        ...
                        dialog.show(move |clicked_download| {
                            if clicked_download {
                                let _ = app
                                    .opener()
                                    .open_url("https://screener.com/download", None::<&str>);
                            } else {
                                let _ = app.opener().open_url(
                                    "https://github.com/screenpipe/screener/releases",
                                    None::<&str>,
                                );
                            }
                        });
        ...
        "book_call" => {
            ...
            let _ = app_handle.run_on_main_thread(move || {
                let _ = app
                    .opener()
                    .open_url("https://cal.com/team/screener/chat", None::<&str>);
            });
        }
        "onboarding" => {
            let app = app_handle.clone();
...
async fn update_menu_if_needed(
    ...
    let tooltip: String = if has_perm_issue {
        "screener — ⚠️ permissions needed".to_string()
    ...
    {
        "screener — screen recording; meeting detection unavailable".to_string()
    } else if effective_status == RecordingStatus::Recording
        && audio_capture_status == Some(AudioCaptureStatus::WaitingForMeeting)
    {
        "screener — screen recording; audio waiting for meeting".to_string()
    } else if effective_status == RecordingStatus::Paused {
        match pause_remaining() {
            Some(d) => format!("screener — paused, resumes in {}", format_remaining(d)),
            None => "screener — paused".to_string(),
        }
    } else if effective_status == RecordingStatus::ScheduledPause {
        "screener — outside work hours (paused by schedule)".to_string()
    } else {
        "screener".to_string()
    };
    let app_for_tooltip = app.clone();
    let _ = app.run_on_main_thread(move || {
        if let Some(tray) = app_for_tooltip.tray_by_id("screener_main") {
    ...
    if should_update {
        ...
        #[cfg(not(target_os = "macos"))]
        {
            ...
            if let Err(e) = app.run_on_main_thread(move || {
                if let Err(e) = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    let Some(tray) = app_for_thread.tray_by_id("screener_main") else {
