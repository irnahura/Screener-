...
pub async fn vault_status(_app: AppHandle) -> Result<String, String> {
    ...
    for name in ["db.sqlite", "secrets.sqlite"] {
        ...
        if path.exists() && screener_vault::crypto::is_encrypted_file(&path).unwrap_or(false) {
            return Ok("locked".to_string());
        }
    }
...
pub async fn vault_unlock(_app: AppHandle, password: String) -> Result<(), String> {
    ...
    let _ = std::fs::remove_file(screener_dir.join(".vault_locked"));
    ...
    tokio::spawn(async move {
        if data_dir.exists() {
            let (tx, _rx) =
                tokio::sync::watch::channel(screener_vault::migration::MigrationProgress {
                    total_files: 0,
                    processed_files: 0,
                    total_bytes: 0,
                    processed_bytes: 0,
            ...
            if let Err(e) =
                screener_vault::migration::decrypt_data_dir(&screener_dir, &data_dir, key, tx)
                    .await
            {
