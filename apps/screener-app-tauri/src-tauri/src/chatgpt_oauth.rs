...
//! key in OS keychain) via `screener_secrets::SecretStore`.
...
/// The legacy migration in screener-secrets also writes to this key
...
async fn open_secret_store() -> Result<screener_secrets::SecretStore, String> {
...
async fn write_tokens_to_store(tokens: &OAuthTokens) -> Result<(), String> {
    ...
    // Retry up to 3 times — the screener server may hold a brief write lock.
    let mut last_err = String::new();
    for attempt in 0..3u32 {
        if attempt > 0 {
            tokio::time::sleep(std::time::Duration::from_millis(200 * (attempt as u64))).await;
...
///
/// The existing `OAuthRefreshScheduler` only handles screener-connect
        ...
        match code {
            Some(code) => {
                let html = concat!(
                    ...
                    "<p>You can close this tab and return to screener.</p>",
...
pub async fn chatgpt_oauth_login(
    ...
    // Activate Screener without forcing the legacy `main` window or changing
