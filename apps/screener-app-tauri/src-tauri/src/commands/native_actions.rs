...
pub(crate) fn dispatch_notification_action(json: String) {
    ...
    if action_type == Some("source") {
        ...
        let is_in_app = url.starts_with("screener://");
        let app_clone = app.clone();
        std::thread::spawn(move || {
            if is_in_app {
    ...
    //   "deeplink"  → screener:// in-app route, dispatched to DeeplinkHandler
    //
    ...
    if action_type == Some("link") || action_type == Some("deeplink") {
        ...
        // screener:// URL into "link". We route on actual scheme, not on
...
fn is_meeting_deeplink(url: &str) -> bool {
    url.starts_with("screener://meeting/") || url.starts_with("screener://meeting?")
}
    ...
    if let Some(app) = GLOBAL_APP_HANDLE.get() {
        ...
        std::thread::spawn(move || {
            ...
            if let Some(meeting_id) = native_overlay_meeting_note_id(&action) {
                ...
                let deeplink_url = format!("screener://meeting/{meeting_id}");
                let meeting_page = meeting_page_with_id(&deeplink_url);
...
mod tests {
    ...
    fn parses_meeting_deeplink_path_id() {
        assert_eq!(
            parse_meeting_deeplink("screener://meeting/123"),
            Some((123, true))
        );
    }
    ...
    fn parses_meeting_deeplink_query_id_and_live_flag() {
        assert_eq!(
            parse_meeting_deeplink("screener://meeting?id=456&live=0"),
            Some((456, false))
        );
    }
    ...
    fn rejects_invalid_meeting_deeplink() {
        assert_eq!(
            parse_meeting_deeplink("screener://meeting/not-a-number"),
            None
        );
        assert_eq!(parse_meeting_deeplink("screener://settings"), None);
    }
    ...
    fn notification_source_action_accepts_source_and_deeplink_aliases() {
        for (field, expected) in [
            ("source_url", "screener://chat/source-url"),
            ("sourceUrl", "screener://chat/sourceUrl"),
            ("deeplink_url", "screener://meeting/123"),
            ("deeplinkUrl", "screener://meeting/456"),
        ] {
            let action = json!({
                "type": "source",
                "label": "source",
