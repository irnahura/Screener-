...
//! in-process. This is the headless `screener export` CLI's in-app twin:
...
async fn live_db(app: &tauri::AppHandle) -> Result<Arc<DatabaseManager>, String> {
    ...
    let core = guard
        .as_ref()
        .ok_or("recording isn't running yet — start screener, then export")?;
    Ok(core.db.clone())
}
