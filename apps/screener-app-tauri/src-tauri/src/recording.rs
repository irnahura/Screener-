...
//! Tauri commands for managing the screener server and capture session.
//!
...
use screener_engine::RecordingConfig;
...
pub async fn test_openai_compatible_transcription(
    ...
    let model = if model.trim().is_empty() {
        screener_audio::DEFAULT_OPENAI_COMPATIBLE_MODEL
    } else {
        model.trim()
    };

    let test_audio = vec![0.0_f32; 16_000];
    screener_audio::transcription::openai_compatible::batch::transcribe_with_openai_compatible(
        None,
        endpoint,
        api_key.as_deref(),
        model,
...
fn recording_access_policy(
    ...
    // The consumer app is local-first: recording and the local API must not
    // depend on a hosted account, plan lookup, or sign-in redirect. An
    // enterprise-only account may still be rejected when it is launched with
    // the consumer binary, so that deployment policy remains fail-closed.
    let _ = has_verified_local_plan;
    !consumer_requires_enterprise_app
}

/// Consumer builds record locally without an account. Enterprise builds keep
/// their native entitlement guard, and consumer builds reject accounts that
/// are required to use an enterprise binary.
...
fn require_recording_access(store: &SettingsStore) -> Result<(), String> {
    ...
    Err("account_required: sign in to start screener recording".to_string())
}
...
pub struct RecordingState {
    ...
    /// every on/off point — `spawn_screener`/`start_capture` set it,
    /// `stop_screener`/`stop_capture` clear it — because capture has two
...
impl RecordingState {
    ...
    /// stop: `start_capture` / `spawn_screener` set it on; `stop_capture` /
    /// `stop_screener` clear it. (Capture has two on-paths and two off-paths;
...
pub struct AudioDeviceInfo {
    ...
    /// (screener-audio's `device_detection::bluetooth_mic_allowed` +
...
pub async fn get_available_audio_devices() -> Result<Vec<AudioDeviceInfo>, String> {
    ...
    let devices = screener_audio::core::device::list_audio_devices()
        .await
        .map_err(|e| format!("Failed to list audio devices: {}", e))?;

    let default_input = screener_audio::core::device::default_input_device()
        .map(|d| d.to_string())
        .ok();
    let default_output = screener_audio::core::device::default_output_device()
        .await
        .map(|d| d.to_string())
        .ok();
    ...
    let result: Vec<AudioDeviceInfo> = devices
        ...
        .map(|d| {
            ...
            let is_combo_bluetooth_mic = d.device_type
                == screener_audio::core::device::DeviceType::Input
                && screener_audio::core::device_detection::InputDeviceKind::detect_input(&d.name)
                    == screener_audio::core::device_detection::InputDeviceKind::Bluetooth
                && screener_audio::core::device::bluetooth_input_is_combo_headset(&d.name);
            AudioDeviceInfo {
                name,
                is_default,
...
pub async fn get_available_monitors() -> Result<Vec<MonitorDevice>, String> {
    ...
    let monitors = screener_screen::monitor::list_monitors().await;
...
async fn restore_interrupted_meeting_for_capture_restart(
    ...
    if let Some(detector) = server.audio_manager.meeting_detector().await {
        ...
        if interrupted.manual {
            detector.set_active_meeting(Some(screener_audio::meeting_detector::ActiveMeeting {
                pid: None,
                bundle_id: None,
                manual: true,
            }));
    ...
    if let Err(e) = screener_events::send_event(
        screener_audio::meeting_streaming::MEETING_STREAMING_REATTACH_EVENT,
        serde_json::json!({
...
pub async fn start_capture(
    ...
    // concurrent `start_capture_internal` (called from spawn_screener's
...
/// Stop capture AND server so the next spawn_screener does a full restart.
...
pub async fn stop_screener(
    ...
    stop_screener_inner(&state).await
}

async fn stop_screener_inner(state: &RecordingState) -> Result<(), String> {
    info!("stop_screener: stopping capture and server");
    ...
    // Shut down the server so the next spawn_screener does a full restart
    // with fresh settings (auth key, port, etc.). Without this, spawn_screener
    ...
    // Reset flags so the next spawn_screener takes the full-start path
...
pub async fn spawn_screener(
    ...
    let Ok(_lifecycle_guard) = state.server_lifecycle.try_lock() else {
        info!("spawn_screener: startup already in progress, not queueing behind it");
        return Ok(());
    };
    spawn_screener_inner(&state, app).await
}

async fn spawn_screener_inner(
    state: &RecordingState,
    app: tauri::AppHandle,
) -> Result<(), String> {
    info!(
        "spawn_screener: starting server (capture intended: {})",
        state.capture_intended()
    );
    ...
    // `to_recording_config` applies SCREENER_PORT for isolated dev/E2E
    ...
    // helper in `apps/screener-app-tauri/lib/events/types.ts`).
    ...
    let owned_browser = screener_connect::connections::browser::OwnedBrowser::default_instance();
    ...
    match result_rx.await {
        Ok(Ok(())) => {
            info!("Screener started successfully");
        ...
        Ok(Err(e)) => {
            ...
            if e.contains("no monitors matched")
                || e.contains(screener_engine::vision_manager::ZERO_DISPLAYS_ENUMERATED)
            {
                crate::health::set_recording_status(crate::health::RecordingStatus::Error);
            }
            Err(e)
...
mod spawn_lifecycle_lock_tests {
    /// `spawn_screener` must not await `server_lifecycle`.
    ///
    ...
    #[test]
    fn spawn_screener_does_not_await_the_lifecycle_lock() {
        // Assembled so the needles never match their own source lines.
        let fn_needle = concat!("pub async fn ", "spawn_screener(");
        ...
        let start = source
            .find(fn_needle)
            .expect("spawn_screener renamed — repoint this guard at it");
        ...
        assert!(
            ...
            "spawn_screener awaits server_lifecycle again — this parks the \
...
mod recording_access_tests {
    ...
    #[test]
    fn consumer_with_unknown_plan_can_record_locally() {
        assert!(recording_access_policy(false, false, false, false, false));
    }

    #[test]
    fn signed_out_consumer_can_start_recording() {
        assert!(recording_access_policy(false, false, false, false, false));
    }
