...
//!
//! The Clerk session JWT used as the Bearer for screener cloud used to live
...
//!
//! ## Cross-platform
//! Paths come from `default_screener_data_dir()` and the SecretStore is
...
pub async fn migrate_plaintext_token(data_dir: &Path) -> Option<String> {
    let read_key = screener_secrets::keychain::get_key_for_read();
...
/// - `~/.pi/agent/auth.json` — drop only the `screener` provider entry,
///   other providers' credentials stay.
/// - `~/.pi/agent/models.json` — reset the screener provider's `apiKey`
///   back to the `SCREENER_API_KEY` placeholder (`build_models_json` embeds
...
/// Remove the `screener` entry from pi's `auth.json`, preserving every other
...
fn scrub_pi_auth_json(path: &Path) {
    rewrite_json_file(path, |json| {
        json.as_object_mut()?.remove("screener").map(|_| ())
    });
}

/// Reset pi's `models.json` screener provider `apiKey` to the signed-out
/// `SCREENER_API_KEY` placeholder, preserving the rest of the provider map.
fn scrub_pi_models_json(path: &Path) {
    rewrite_json_file(path, |json| {
        let api_key = json.pointer_mut("/providers/screener/apiKey")?;
        if api_key.as_str() == Some("SCREENER_API_KEY") {
            return None;
        }
        *api_key = serde_json::Value::String("SCREENER_API_KEY".to_string());
        Some(())
    });
}
...
mod tests {
    ...
    #[test]
    fn signout_scrub_removes_screener_from_pi_auth() {
        let dir = unique_dir("pi_auth");
        let p = dir.join("auth.json");
        std::fs::write(&p, br#"{"screener":"jwt-x","anthropic":"sk-other"}"#).unwrap();
        scrub_pi_auth_json(&p);
        let json: serde_json::Value = serde_json::from_slice(&std::fs::read(&p).unwrap()).unwrap();
        assert!(json.get("screener").is_none(), "screener entry removed");
    ...
    fn signout_scrub_resets_pi_models_api_key() {
        ...
        std::fs::write(
            &p,
            br#"{"providers":{"screener":{"apiKey":"jwt-x","baseUrl":"u"},"ollama":{"apiKey":"none"}}}"#,
        )
        .unwrap();
        scrub_pi_models_json(&p);
        let json: serde_json::Value = serde_json::from_slice(&std::fs::read(&p).unwrap()).unwrap();
        assert_eq!(
            json.pointer("/providers/screener/apiKey")
                .unwrap()
                .as_str(),
            Some("SCREENER_API_KEY")
        );
        assert_eq!(
            json.pointer("/providers/screener/baseUrl")
                .unwrap()
                .as_str(),
            Some("u"),
    ...
    async fn store_load_roundtrip_encrypted() {
        ...
        let raw = std::fs::read(screener_secrets::secrets_database_path(&dir)).unwrap();
        assert!(
