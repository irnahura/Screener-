...
use screener_core::agents::pi::{
    apply_custom_provider_compat, screener_cloud_models, PI_AI_PACKAGE, PI_NAMESPACE_DIR,
    PI_PACKAGE, SCREENER_API_URL,
};
...
fn build_command_for_path(path: &str) -> Command {
    if path.ends_with(".cmd") || path.ends_with(".bat") {
        ...
        // twice is harmless.
        let mut cmd = screener_core::no_window_command("cmd.exe");
        cmd.args(["/C", path]);
        cmd
        ...
        if let Some(bun) = find_bun_executable() {
            ...
            screener_core::no_window_command(path)
        } else {
            screener_core::no_window_command(path)
        }
...
/// On Unix, pi's shebang is `#!/usr/bin/env node` but screener only bundles
...
fn terminate_acp_process_tree(pid: u32) {
    #[cfg(unix)]
    {
        ...
        // negative pid targets only that process group, never Screener's.
        unsafe {
...
/// Get the Pi config directory — screener's isolated agent dir
/// (`~/.screener/pi-config`), never the user's global `~/.pi/agent`.
/// Delegates to screener-core, which also runs the one-time seed
/// migration from the global dir.
/// See https://github.com/screenpipe/screener/issues/4002
/// and https://github.com/screenpipe/screener/issues/3812.
fn get_pi_config_dir() -> Result<PathBuf, String> {
    screener_core::agents::pi::pi_config_dir().map_err(|e| e.to_string())
}

fn remove_screener_auth_from_path(auth_path: &Path) -> Result<(), String> {
    if !auth_path.exists() {
        return Ok(());
    }
...
pub(crate) fn clear_screener_auth_token_files() -> Result<(), String> {
    let auth_path = get_pi_config_dir()?.join("auth.json");
    remove_screener_auth_from_path(&auth_path)
}
...
/// Find pi executable
/// Returns the screener-managed pi install directory (`~/.screener/pi-agent/`).
fn pi_local_install_dir() -> Option<PathBuf> {
    Some(screener_core::paths::default_screener_data_dir().join("pi-agent"))
}
...
/// one implementation of the flag, in `screener_core::no_window`.
fn apply_no_window(cmd: &mut Command) {
    screener_core::apply_no_window(cmd);
}
...
fn bun_command(bun: &str) -> Command {
    ...
    // screener-mcp prewarm spawns this directly at every launch and waits up
    // to two minutes for it, so the missing flag was a terminal sitting on the
    // user's desktop for the whole startup.
    let mut cmd = screener_core::no_window_command(bun);
    // Single source of truth in screener-core: on Linux, bun subprocesses
    ...
    screener_core::agents::pi::scrub_bun_runtime_env(&mut cmd);
    cmd
}
...
fn format_install_failure(tool: &str, output: &Output) -> String {
    ...
    format!(
        ...
        tool,
        screener_core::agents::pi::describe_exit_status(&output.status),
        details
    )
}
...
fn run_pi_package_install_once(install_dir: &Path, bun: &str) -> Result<(), String> {
    ...
    info!(
        ...
        install_dir.display(),
        screener_core::agents::pi::bun_version_string(bun),
    );
    ...
    bun_cmd
        ...
        // can launch fresh console processes, so disable them for Screener's
...
fn find_pi_executable() -> Option<String> {
    // 1. Check screener-managed local install first (preferred — we control the deps)
...
/// Ensure the screener skills exist in the project's .pi/skills directory.
/// Delegates to screener-core's canonical implementation.
fn ensure_screener_skill(project_dir: &str) -> Result<(), String> {
    use screener_core::agents::pi::PiExecutor;
    PiExecutor::ensure_screener_skill(std::path::Path::new(project_dir))
        .map_err(|e| format!("Failed to install screener skills: {}", e))
}
...
fn ensure_enterprise_team_skill(project_dir: &str) -> Result<Option<std::path::PathBuf>, String> {
    ...
    PiExecutor::ensure_screener_team_skill(&skill_root)
...
/// Web search uses the screener cloud backend (Gemini + Google Search),
/// so we only enable it for screener-cloud presets to avoid sending
...
fn ensure_web_search_extension(
    ...
    let is_screener_cloud = match provider_config {
        Some(config) => matches!(config.provider.as_str(), "screener-cloud" | "pi"),
        None => true, // default preset = screener cloud
    };

    if is_screener_cloud {
        std::fs::create_dir_all(&ext_dir)
            .map_err(|e| format!("Failed to create extensions dir: {}", e))?;

        let api_url = crate::config::screener_ai_gateway_url()?;
        let ext_content = include_str!("../assets/extensions/web-search.ts")
            .replace(SCREENER_API_URL, &api_url);
        ...
        info!(
            "Web search extension removed (provider {:?} is not screener-cloud)",
...
/// populated exclusively by screener, so it is trusted by definition. Keyed
...
pub struct AcpAgentConfig {
    ...
    /// Send the agent's model calls through Screener Cloud instead of the
    ...
    #[serde(default)]
    pub use_screener_cloud: Option<bool>,
}
...
pub struct PiProviderConfig {
    ...
    /// Provider type: "openai", "native-ollama", "custom", "screener-cloud"
    pub provider: String,
    ...
    /// tokens, so Screener converts this value using four characters/token.
    #[serde(default)]
...
async fn build_models_json(
    ...
    build_models_json_with_api_url(user_token, provider_config, SCREENER_API_URL).await
}
...
async fn build_models_json_with_api_url(
    ...
    // Always add screener cloud provider. A real token is inlined as a
    // literal; the logged-out fallback must use `$` env-var syntax (pi >= 0.80
    // treats bare names as literal keys).
    let api_key_value = user_token.unwrap_or("$SCREENER_API_KEY");
    let models = screener_cloud_models(api_url, user_token).await;
    let screener_provider = json!({
        "baseUrl": api_url,
    ...
    providers_map.insert("screener".to_string(), screener_provider);

    // Add the user's selected provider (if not screener-cloud)
    if let Some(config) = provider_config {
        let provider_name = match config.provider.as_str() {
            ...
            "custom" => "custom",
            _ => "", // screener-cloud already added above
        };
...
async fn ensure_pi_config(
    ...
    // Subagents are a baseline Screener capability, not an optional Pi
    ...
    // -- auth.json: merge screener token, preserve other providers --
    ...
    if let Some(token) = user_token.filter(|token| !token.is_empty()) {
        ...
        // provider resolve to "Provider is not configured: screener".
        screener_core::agents::pi::upgrade_legacy_pi_credentials(&mut auth);

        if let Some(obj) = auth.as_object_mut() {
            obj.insert(
                "screener".to_string(),
                screener_core::agents::pi::api_key_credential(token),
            );
        }
...
/// catch-all used to answer that with "screener" — which sent the *agent id*
...
/// Screener Business": a billing dead end for what is really a broken preset,
...
fn pi_registry_provider(provider: &str, url: &str) -> Result<&'static str, String> {
    Ok(match provider {
        ...
        "native-ollama" => "ollama",
        "anthropic" => "anthropic-byok",
        // "custom" requires a valid URL; fall back to screener cloud if missing
        "custom" if !url.is_empty() => "custom",
        "acp" => return Err(ACP_PRESET_WITHOUT_BACKEND.to_string()),
        "screener-cloud" | "pi" | _ => "screener",
    })
}

/// Resolve a model name for the screener provider.
///
/// The gateway (api.screener.com) is the source of truth for model validation
...
fn resolve_screener_model(requested: &str, provider: &str) -> String {
    // Only touch screener provider — other providers use their own model names
...
fn resolve_pi_model(requested: &str, provider: &str) -> String {
    match provider {
        "screener" => resolve_screener_model(requested, provider),
...
pub async fn pi_start_inner(
    ...
    // Ensure screener skills exist in project
    ensure_screener_skill(&project_dir)?;
    ...
    // ACP agents receive Screener via MCP during session/new. Pi-only
    ...
    if !use_acp {
        // Install web-search extension only for screener-cloud presets
        ...
        // extension needs the screener-cloud JWT (which ACP sessions never
        ...
        // and its env (SCREENER_LOCAL_API_*) is already set on this path.
        ensure_live_views_extension(&project_dir)?;
        ensure_connection_gate_extension(&project_dir)?;
        ...
        // must get Screener's provider setup and the required pi-subagents
    ...
    let (pi_provider, pi_model) = match &provider_config {
        ...
        None => ("screener".to_string(), "auto".to_string()),
    };
                    ...
                    .ok_or_else(|| {
                        ...
                        if bun_found {
                            let install_err = take_pi_install_error()
                                ...
                                .unwrap_or_default();
                            format!("Pi not found after install attempt.{} Try restarting the app or delete ~/.screener/pi-agent and restart.", install_err)
                        } else {
                            format!("Pi not found: bun is not installed. Screener needs bun to run the AI assistant. Expected bundled bun next to the app executable.")
                        }
                    })?
            }
        }
    ...
    let mut cmd = if use_acp {
        ...
        let executable = std::env::current_exe()
            .map_err(|error| format!("Failed to locate Screener ACP runtime: {error}"))?;
        ...
        command.args([
            ...
            // populated exclusively by screener, so it is trusted by definition.
            "--approve",
            "--provider",
            &pi_provider,
            "--model",
    ...
    if use_acp {
        ...
        // silently strip screener's tools for a user who has a `screener`
        ...
        // Route the agent's model calls through Screener Cloud when the
        ...
        // else here forwards it (the ACP runtime scrubs `SCREENER_API_KEY`
        ...
        if let Some(routing) = routing {
            let gateway = crate::config::screener_ai_gateway_url().unwrap_or_default();
        ...
        cmd.env("SCREENER_ACP_ID", agent_id)
            .env("SCREENER_ACP_CWD", &project_dir)
            .env("SCREENER_BUN_PATH", &bun_path)
            .env(
                "SCREENER_ACP_ARGS_JSON",
                serde_json::to_string(&acp.args).map_err(|e| e.to_string())?,
            )
            .env(
                "SCREENER_ACP_ENV_JSON",
                serde_json::to_string(&resolved_env).map_err(|e| e.to_string())?,
            )
            .env(
                "SCREENER_ACP_SESSION_CONFIG_JSON",
            ...
            .env_remove("SCREENER_ACP_COMMAND")
            .env_remove("SCREENER_ACP_AUTH_METHOD")
            .env_remove("SCREENER_ACP_SYSTEM_PROMPT");
        if let Some(command) = acp
            .command
            .as_deref()
            .filter(|value| !value.trim().is_empty())
        {
            cmd.env("SCREENER_ACP_COMMAND", command);
        }
        if let Some(auth_method) = acp
            ...
            .filter(|value| !value.trim().is_empty())
        {
            cmd.env("SCREENER_ACP_AUTH_METHOD", auth_method);
        }
        ...
        {
            cmd.env("SCREENER_ACP_RESUME_SESSION_ID", resume_id);
        }
    }
    ...
    if bun_path != "NOT FOUND" {
        if let Some(bun_dir) = std::path::Path::new(&bun_path).parent() {
            ...
            let new_path = {
                ...
                let bash_result =
                    tokio::task::spawn_blocking(screener_core::agents::pi::ensure_bash_available)
                        .await
                        .unwrap_or_else(|e| {
            ...
            let path_for_pi =
                screener_core::agents::pi::pi_child_path(std::ffi::OsStr::new(&new_path))
    ...
    // screener-api skill file — they often skip reading skills on their own.
    let is_local_model = !use_acp && matches!(pi_provider.as_str(), "ollama" | "custom");
    if is_local_model {
        let api_hint = "IMPORTANT: You MUST read the screener-api skill file BEFORE making any API calls. It contains authentication instructions, endpoint docs, and examples. Without reading it first, your API calls will fail with 403 unauthorized.";
    ...
    // Scope pi to screener's isolated agent dir (never ~/.pi/agent).
    screener_core::agents::pi::apply_pi_isolation_env(&mut |k, v| {
        cmd.env(k, v);
    });

    if !use_acp {
        if let Some(ref token) = user_token {
            cmd.env("SCREENER_API_KEY", token);
        }
    }
    ...
    // Pass local API config so the Pi agent can authenticate to the runtime local API.
    {
        ...
        cmd.env("SCREENER_LOCAL_API_PORT", api.port.to_string());
        cmd.env("SCREENER_LOCAL_API_URL", api.url(""));
        if let Some(ref key) = api.api_key {
            cmd.env("SCREENER_LOCAL_API_KEY", key);
        }
    }
    ...
    // x-screener-session and the navigate handler rides it to the frontend.
    ...
    cmd.env("SCREENER_SESSION_ID", &sid);

    // Chat session ID for per-session artifact isolation
    cmd.env("SCREENER_CHAT_SESSION_ID", &sid);
    ...
    // screener-core.
    if let Ok(p) = screener_core::agents::bash_env::ensure_wrapper_in_default_dir() {
        cmd.env("BASH_ENV", p);
    }

    // Pass the user's API key as env var for non-screener providers
    if let Some(ref config) = provider_config {
    ...
    // TODO(remove next release): drop SCREENER_API_AUTH_KEY alias.
    if let Some(key) = crate::store::resolved_api_auth_key() {
        cmd.env("SCREENER_LOCAL_API_KEY", &key);
        cmd.env("SCREENER_API_AUTH_KEY", key); // deprecated alias
    }

    // ACP adapters authenticate through their own configured provider env or
    // login state. Never expose the signed-in user's Screener cloud JWT to a
            ...
            match parsed {
                Some(event) => {
                    ...
                    if matches!(
                        ...
                        // (`apps/screener-app-tauri/lib/events/bus.ts`).
    ...
    if let Some(stderr) = stderr {
        ...
        std::thread::spawn(move || {
            ...
            while let Some(line) = read_lines_lossy(&mut reader) {
                ...
                {
                    warn!(
                        "Pi extension startup failed for '{}'; the next automatic restart will load only Screener-managed extensions",
                        project_dir_stderr
                    );
                }
                ...
                match child.try_wait() {
                    Ok(Some(status)) => {
                        ...
                        let install_hint = take_pi_install_error()
                            .map(|e| format!(" The Pi install previously failed: {} Restart screener to retry the install automatically.", e))
                            .unwrap_or_default();
...
/// here (desktop-side) via the SecretStore; the "screener" built-in server
...
async fn resolve_user_mcp_servers_json() -> Option<String> {
    use screener_connect::mcp_servers::{McpServerStore, McpTransport};
    let data_dir = screener_core::paths::default_screener_data_dir();
    ...
    let mut out = Vec::new();
    for cfg in servers {
        if !cfg.enabled || cfg.name.eq_ignore_ascii_case("screener") {
            continue;
        }
...
async fn open_secret_store_for_connection_context() -> Option<screener_secrets::SecretStore> {
    let data_dir = screener_core::paths::default_screener_data_dir();
    screener_secrets::SecretStore::open_for_data_dir_with_vault_key(&data_dir)
        .await
        .ok()
}
...
async fn foreground_connections_context(app: &AppHandle) -> String {
    ...
    screener_connect::connections::render_context(&data_dir, api.port, store.as_ref()).await
}
...
async fn attach_foreground_connections_context(
    ...
    // chat titles and spawned a duplicate "Current Screener connected
    ...
    // TypeScript side.
    format!(
        "<connections_context>\nCurrent Screener connected integrations context, refreshed for this turn:\n{}\n</connections_context>\n\n{}",
        ctx, message
    )
}
...
/// MCP configs (e.g. `{ command: <bun>, args: ["x", "screener-mcp@latest"] }`)
/// instead of `npx -y screener-mcp`. npx requires a global Node install
...
pub async fn pi_acp_probe_agent(agent: AcpAgentConfig) -> Result<String, String> {
    ...
    cmd.arg(crate::acp_runtime::RUNTIME_ARG)
        ...
        .env_remove("SCREENER_ACP_AUTH_METHOD")
        .env_remove("SCREENER_ACP_SYSTEM_PROMPT")
        .env_remove("SCREENER_ACP_SESSION_CONFIG_JSON")
...
fn validate_pi_extension_package_source(source: &str) -> Result<String, String> {
    ...
    Err("Only npm: packages and GitHub package URLs can be installed from screener".to_string())
}
...
fn ensure_pi_cli_for_package_command() -> Result<(String, String), String> {
    let bun = find_bun_executable().ok_or(
        "Could not find bundled bun. Restart screener or reinstall the app before installing Pi extensions.",
    )?;
    ...
    let pi_path = find_local_pi_entrypoint()
        ...
        .ok_or_else(|| {
            "Pi could not be installed. Restart screener and try again.".to_string()
        })?;
...
fn run_pi_package_command_blocking(args: Vec<String>) -> Result<(), String> {
    ...
    let project_dir = screener_core::paths::default_screener_data_dir().join("pi-chat");
    std::fs::create_dir_all(&project_dir)
...
pub async fn pi_remove_extension_package(
    ...
    if is_required_pi_extension_package_source(&source) {
        return Err("Subagents are required by screener and cannot be disabled".to_string());
    }
    stop_idle_pi_sessions_for_package_change(&state).await?;
...
pub async fn pi_update_config(
    ...
    let project_dir = screener_core::paths::default_screener_data_dir()
        .join("pi-chat")
        .to_string_lossy()
        .to_string();
...
pub(crate) fn find_bun_executable() -> Option<String> {
    ...
    // 0xC000001D at spawn. Delegate to screener-core, which waits for the
    // verified baseline build and fails closed instead of returning stock bun.
    #[cfg(windows)]
    if !screener_core::cpu_features::has_avx2() {
        return screener_core::agents::pi::find_bun_executable();
    }

    // First check next to our own executable (bundled bun in AppData/Local/screener/)
...
/// Installs pi into `~/.screener/pi-agent/` (local install, not global)
...
/// Warm bun's default package cache with the pinned screener-mcp so the first
...
/// `bun x screener-mcp` fetch is why those tools sometimes never registered
...
fn prewarm_screener_mcp(bun: &str) {
    use std::process::Stdio;
    let mut cmd = bun_command(bun);
    cmd.arg("x")
        .arg(crate::acp_runtime::SCREENER_MCP_PKG)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null());
    match cmd.spawn() {
        Ok(mut child) => {
            ...
            info!("screener-mcp prewarm complete ({})", crate::acp_runtime::SCREENER_MCP_PKG);
        }
        Err(e) => warn!("screener-mcp prewarm could not spawn bun: {}", e),
    }
}

pub fn ensure_pi_installed_background() {
    // Warm the screener-mcp cache in the background so the first ACP session
    ...
    if let Some(bun) = find_bun_executable() {
        let _ = std::thread::Builder::new()
            .name("screener-mcp-prewarm".to_string())
            .spawn(move || prewarm_screener_mcp(&bun));
    }
    ...
    #[cfg(windows)]
    {
        let _ = std::thread::Builder::new()
            ...
            .spawn(|| {
                screener_core::agents::pi::ensure_bash_available();
            });
    }
...
mod tests {
    ...
    fn identical_pi_launches_share_a_fingerprint() {
        let config = super::PiProviderConfig {
            backend: None,
            acp_agent: None,
            provider: "screener-cloud".to_string(),
            url: String::new(),
...
"#,
    ...
    #[test]
    fn clear_screener_auth_preserves_other_provider_tokens() {
        ...
        super::remove_screener_auth_from_path(&auth_path).expect("clear screener auth");

        let auth: Value =
            serde_json::from_str(&std::fs::read_to_string(&auth_path).unwrap()).unwrap();
        assert!(auth.get("screener").is_none());
    ...
    /// "Provider is not configured: screener" even though core was fixed.
    #[test]
    fn app_writer_emits_tagged_screener_credential() {
        let mut auth = json!({"screener": "bare-jwt", "anthropic": {"apiKey": "sk-ant"}});

        screener_core::agents::pi::upgrade_legacy_pi_credentials(&mut auth);
        auth.as_object_mut().unwrap().insert(
            "screener".to_string(),
            screener_core::agents::pi::api_key_credential("fresh-jwt"),
        );
    ...
    #[test]
    fn clear_screener_auth_missing_file_is_noop() {
        let dir = tempfile::tempdir().expect("tempdir");
        let auth_path = dir.path().join("missing-auth.json");

        super::remove_screener_auth_from_path(&auth_path).expect("missing auth is ok");
...
error: InstallFailed extracting tarball"#;
    ...
    fn test_pi_rpc_prompt_response() {
        let mut child = match spawn_pi_rpc("screener", "claude-haiku-4-5") {
            Some(c) => c,
            None => {
    ...
    fn test_pi_rpc_restart_race() {
        let mut child1 = match spawn_pi_rpc("screener", "claude-haiku-4-5") {
            Some(c) => c,
            None => {
                eprintln!("SKIP: pi executable not found");
                return;
        ...
        let mut child2 = match spawn_pi_rpc("screener", "claude-haiku-4-5") {
            Some(c) => c,
            None => {
    ...
    fn test_resolve_pi_model_maps_unsupported_chatgpt_codex_model() {
        ...
        assert_eq!(
            resolve_pi_model("gpt-5.5-codex", "screener"),
            "gpt-5.5-codex"
        );
    }

    #[tokio::test]
    async fn test_build_models_json_default_has_screener_provider() {
        let config = build_models_json(None, None).await;
        let providers = config["providers"].as_object().unwrap();
        assert!(providers.contains_key("screener"));
        assert_eq!(providers.len(), 1);

        let sp = &providers["screener"];
        assert_eq!(sp["baseUrl"], "https://api.screener.com/v1");
        assert_eq!(sp["api"], "openai-completions");
        // `$` prefix is required: pi >= 0.80 treats bare names as literal keys
        assert_eq!(sp["apiKey"], "$SCREENER_API_KEY");
    ...
    async fn test_build_models_json_uses_resolved_gateway_url() {
        ...
        assert_eq!(
            config["providers"]["screener"]["baseUrl"],
    ...
    /// as a model name, and the 403 surfaced as "upgrade to Screener
    ...
    fn known_providers_still_map_as_before() {
        ...
        assert_eq!(pi_registry_provider("custom", "").unwrap(), "screener");
        assert_eq!(
            pi_registry_provider("screener-cloud", "").unwrap(),
            "screener"
        );
    }
    ...
    async fn test_build_models_json_with_user_token() {
        ...
        let sp = &config["providers"]["screener"];
        assert_eq!(sp["apiKey"], "tok_abc123");
    }

    #[tokio::test]
    async fn test_build_models_json_screener_cloud_no_extra_provider() {
        let pc = make_provider_config("screener-cloud", "auto");
        let config = build_models_json(None, Some(&pc)).await;
        let providers = config["providers"].as_object().unwrap();
        // screener-cloud maps to "" (empty), so only the screener provider is added
        assert_eq!(providers.len(), 1);
        assert!(providers.contains_key("screener"));
    }
    ...
    fn test_extension_safe_mode_loads_only_managed_project_extensions() {
        ...
        let header = "// screener — AI that knows everything you've seen, said, or heard\n";
    ...
    fn test_tool_allowlist_restricts_bounded_pi_sessions() {
        let mut config = make_provider_config("screener-cloud", "auto");
        config.allowed_tools = Some(vec!["screener_live_view".to_string()]);
        let mut command = Command::new("pi");
        ...
        assert_eq!(args, vec!["--tools", "screener_live_view"]);
    }
    ...
    async fn test_build_models_json_custom_with_empty_url_skipped() {
        ...
        let config = build_models_json(None, Some(&pc)).await;
        let providers = config["providers"].as_object().unwrap();
        assert_eq!(providers.len(), 1); // only screener
    ...
    async fn test_build_models_json_custom_with_url() {
        ...
        assert_eq!(providers["custom"]["headers"]["User-Agent"], "screener");
    }
    ...
    async fn test_build_models_json_repairs_ai_genesis_custom_url() {
        for base_url in ["https://ai.ai-genesis.app", "https://api.ai-genesis.app/"] {
            ...
            assert_eq!(custom["headers"]["User-Agent"], "screener");
        }
    }
    ...
    async fn test_build_models_json_no_stale_providers() {
        ...
        // Only "screener" — no leftover providers
        assert_eq!(providers.len(), 1);

        // Every model has required fields for pi-coding-agent schema
        let models = providers["screener"]["models"].as_array().unwrap();
        for m in models {
    ...
    /// baseline seeds Screener skills, the Live Views extension, and registers
    /// the required pi-subagents package — the exact pieces the pi-acp path now
    /// calls ensure_screener_skill / ensure_live_views_extension /
    ...
    #[test]
    fn fresh_profile_seeds_screener_baseline() {
        let project = tempfile::tempdir().expect("project dir");
        let project_dir = project.path().to_str().expect("utf8 path");

        // Screener skills materialize under the fresh project's .pi/skills.
        super::ensure_screener_skill(project_dir).expect("seed skills");
        ...
        assert!(
            std::fs::read_dir(&skills_dir)
                ...
                .any(|e| e.file_name().to_string_lossy().starts_with("screener")),
            "at least one screener skill must be seeded on a fresh profile"
        );
