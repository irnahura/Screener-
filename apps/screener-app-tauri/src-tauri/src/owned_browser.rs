...
use screener_connect::connections::browser::{EvalResult, OwnedWebviewHandle};
use serde::Serialize;
...
/// already persisted, show Screener's own warning before the first Keychain
...
fn session_prompt_timeout_error(host: &str) -> String {
    format!(
        "Timed out waiting for browser-session permission for {host}. Approve the Screener browser prompt or log in inside the owned browser, then retry."
    )
}
...
impl OwnedWebviewHandle for TauriOwnedHandle {
    ...
    async fn navigate(&self, url: &str, owner: Option<&str>) -> Result<(), String> {
        ...
        // OwnedBrowser trait impl in screener-connect); without this
...
pub fn spawn_install_when_ready(
    ...
    owned_browser: std::sync::Arc<screener_connect::connections::browser::OwnedBrowser>,
) {
    tauri::async_runtime::spawn(async move {
        ...
        for attempt in 1..=MAX_ATTEMPTS {
            match install(&app, screener_dir.clone()).await {
                Ok(handle) => {
...
async fn browser_session_decision_for_url(
    ...
    if !crate::owned_browser_cookies::has_cookies_for_host(&host_key).await {
        ...
        {
            // If the Screener Browser Bridge extension is already connected,
