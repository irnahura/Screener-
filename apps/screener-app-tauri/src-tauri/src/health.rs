...
/// Vision is off because screener turned it off, not because anything
...
fn respawn_engine_if_crashed(
                    ...
                    if port_free {
                        ...
                        *last_restart_triggered = Some(now);
                        ...
                        tokio::spawn(async move {
                            match crate::recording::spawn_screener(
    ...
    let check = EngineRespawnCheck {
        ...
        in_restart_grace: last_restart_triggered
            ...
            .unwrap_or(false),
        recently_woke: screener_engine::sleep_monitor::recently_woke_from_sleep(),
        start_in_progress,
...
pub async fn start_health_check(app: tauri::AppHandle) -> Result<()> {
    ...
    tokio::spawn(async move {
        loop {
            ...
            // `screener-engine::permission_monitor` + capture-module emissions.
            ...
            if current_status != last_status || theme != last_theme {
                ...
                let icon_path = if is_unhealthy_icon(current_status) {
                    if theme == Mode::Light {
                        "assets/screener-logo-tray-black-failed.png"
                    } else {
                        "assets/screener-logo-tray-white-failed.png"
                    }
                } else {
                    if theme == Mode::Light {
                        "assets/screener-logo-tray-black.png"
                    } else {
                ...
                let _ = app.run_on_main_thread(move || {
                    crate::window::with_autorelease_pool(|| {
                        if let Some(main_tray) = app_clone.tray_by_id("screener_main") {
                            if let Err(e) =
            ...
            // When spawn_screener completes, it updates last_spawn_epoch.
            ...
            if should_track_capture_stalls_after(
                ...
                if let Ok(ref health) = health_result {
                    ...
                    let woke = screener_engine::sleep_monitor::recently_woke_from_sleep();
            ...
            let recently_woke = screener_engine::sleep_monitor::recently_woke_from_sleep();
            let intentionally_paused = !capture_intended
                || schedule_paused
                || matches!(&health_result, Ok(health) if health.drm_content_paused)
                || screener_config::should_pause_audio_for_lock()
...
async fn show_capture_stall_notification(app: &tauri::AppHandle, system: &str) -> Result<()> {
    ...
    let payload = serde_json::json!({
        ...
        "body": format!("screener has not received recent {} updates. this may recover on its own, but restarting usually fixes it.", system),
        "actions": [
            { "label": "RESTART", "action": "restart_recording", "primary": true }
        ],
        "autoDismissMs": 30000
