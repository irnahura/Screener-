...
//! suggestions (via screener cloud AI) during idle/charging periods.
...
//! panics (SCREENER-APP-97 and predecessors). `deny(clippy::string_slice)`
//! forces every byte-index slice through [`screener_core::strings`] so we
...
use screener_core::strings::{safe_byte_prefix, truncate_string};
...
/// When enabled, uses screener cloud (api.screener.com) instead of Apple
...
pub struct EnhancedAIConfig {
    pub enabled: bool,
    /// User's Clerk JWT token for authenticating with screener cloud.
    pub token: String,
}
...
/// Enable or disable enhanced AI suggestions (uses screener cloud).
#[tauri::command]
#[specta::specta]
...
async fn fetch_app_activity(api: &LocalApiContext) -> Result<Vec<AppActivity>, String> {
    ...
    let resp = api
        ...
        .json(&serde_json::json!({
            "query": "SELECT app_name, COUNT(*) as cnt FROM frames WHERE datetime(timestamp) > datetime('now', '-30 minutes') AND app_name != '' AND app_name != 'screener' AND app_name != 'screener-app' GROUP BY app_name ORDER BY cnt DESC LIMIT 15"
        }))
...
async fn fetch_window_activity(api: &LocalApiContext) -> Result<Vec<WindowActivity>, String> {
    ...
    let resp = api
        .apply_auth(client.post(api.url("/raw_sql")))
        .json(&serde_json::json!({
            "query": "SELECT app_name, window_name, COUNT(*) as cnt FROM frames WHERE datetime(timestamp) > datetime('now', '-30 minutes') AND app_name != '' AND app_name != 'screener' AND app_name != 'screener-app' AND window_name != '' GROUP BY app_name, window_name ORDER BY cnt DESC LIMIT 20"
        }))
        .timeout(std::time::Duration::from_secs(10))
        .send()
        .await
...
async fn fetch_accessibility_snippets(api: &LocalApiContext) -> Vec<AccessibilitySnippet> {
    ...
    let resp = api
        .apply_auth(client.post(api.url("/raw_sql")))
        .json(&serde_json::json!({
            "query": "SELECT app_name, window_name, SUBSTR(full_text, 1, 200) as snippet FROM frames WHERE datetime(timestamp) > datetime('now', '-15 minutes') AND LENGTH(full_text) > 30 AND app_name != 'screener' AND full_text IS NOT NULL ORDER BY timestamp DESC LIMIT 8"
        }))
...
const AI_SYSTEM_PROMPT: &str = r#"Analyze this screener user's activity (records screen/audio 24/7). Return a JSON object with "suggestions" and "tags".
...
async fn generate_ai_suggestions(
    ...
    debug!(
        "suggestions: AI prompt ~{} tokens, backend=screener-cloud",
        context.len() / 4
    );

    let client = reqwest::Client::new();
    let gateway_url = match crate::config::screener_ai_gateway_url() {
        Ok(url) => url,
        Err(error) => {
    ...
    let resp = client
        ...
        .header("x-screener-latency", "background")
        .json(&serde_json::json!({
            "model": "auto",
            "messages": [
...
mod tests {
    ...
    // ─── Benchmark tests ─────────────────────────────────────────────────────
    // Run with: cargo test -p screener-app -- --ignored benchmark --nocapture
    // Requires: screener local API running

    #[tokio::test]
    #[ignore] // requires screener running locally
    async fn benchmark_data_sources() {
        ...
        assert!(!apps.is_empty(), "no app data — is screener running?");
