...
/// `SCREENER_E2E_ALLOW_ACTIVATION=1`.
///
...
pub fn window_activation_allowed() -> bool {
    ...
    INIT.call_once(|| {
        let opted_in = matches!(
            std::env::var("SCREENER_E2E_ALLOW_ACTIVATION").as_deref(),
            Ok("1") | Ok("true")
        );
...
/// lets screener draw on top of whatever you are doing, including another
...
///
/// `makeKeyWindow` is what actually pulls screener to the foreground (and, for
...
///
/// The gates above stop screener *taking focus*, but focus is only half of
...
///
/// The per-callsite gates above stop screener *asking* for the foreground, but
