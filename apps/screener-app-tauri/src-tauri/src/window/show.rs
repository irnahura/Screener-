...
impl RewindWindowId {
    ...
    pub fn title(&self) -> &str {
        match self {
            RewindWindowId::Main => "screener",
            RewindWindowId::Home => "screener",
            RewindWindowId::Search => "search",
            RewindWindowId::Onboarding => "onboarding",
...
impl ShowRewindWindow {
    ...
    fn show_existing_main(
        ...
        if overlay_mode == "window" {
            ...
            #[cfg(target_os = "windows")]
            {
                ...
                // Without this, re-showing an already-created overlay when another screener
    ...
    pub fn show(&self, app: &AppHandle) -> tauri::Result<WebviewWindow> {
        ...
        let window = match self {
                                    ...
                                    std::thread::spawn(move || {
                                        ...
                                        #[cfg(target_os = "macos")]
                                        {
                                            ...
                                            let _ = app.run_on_main_thread(move || {
                                                ...
                                                // screener window (Settings, Chat), just clear.
                            ...
                            if !is_focused {
                                ...
                                std::thread::spawn(move || {
                                    ...
                                    #[cfg(target_os = "macos")]
                                    {
                                        ...
                                        let _ = app.run_on_main_thread(move || {
                                            // Conditional restore: if focus moved to another
                                            // screener window, just clear. Only activate
            ...
            ShowRewindWindow::Home { page } => {
                ...
                // when Home blurs and no screener window has focus (user left the app).
                #[cfg(not(target_os = "macos"))]
                {
                        ...
                        if let tauri::WindowEvent::Focused(focused) = event {
                            if !focused {
                                ...
                                std::thread::spawn(move || {
                                    ...
                                    if crate::commands::any_screener_webview_has_focus(&app_c) {
                                        return;
                                    }
                                    if crate::commands::is_main_close_in_progress() {
                                        return;
                                    ...
                                    std::thread::spawn(move || {
                                        info!(
                                            "shortcut-sync: unregister execute (reason=home_blur_no_screener_focus)"
                                        );
                                        let _ = crate::commands::unregister_window_shortcuts_if_generation_unchanged(
                                            app2,
                                            expected_gen,
                                            "home_blur_no_screener_focus",
                                        );
                                    });
                                });
                            } else {
    ...
    /// Used when transitioning from Main to another screener window (e.g. Home/Settings)
        ...
        if id.label() == RewindWindowId::Main.label() {
            #[cfg(target_os = "macos")]
            {
                ...
                run_on_main_thread_safe(app, move || {
                    ...
                    // we're transitioning to another screener window.
                });
            }
