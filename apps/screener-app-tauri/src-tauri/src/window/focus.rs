...
/// clicks/shows from replacing the real origin with screener itself.
#[cfg(target_os = "macos")]
...
/// panel must not overwrite it after macOS has activated screener.
#[cfg(target_os = "macos")]
...
/// activation to screener while the non-activating panel was being used,
/// reactivate the original app so dismissing Search does not reveal screener.
...
/// Check if our own app (screener) is still the active macOS application.
/// When the overlay panel loses focus to another screener window (Settings,
...
/// If our own app is still active (focus moved to another screener window
/// like Settings), we just clear without restoring — otherwise Settings/Chat
/// would lose keyboard focus. Also skip restoring if any screener window
...
pub(crate) fn restore_frontmost_app_if_external_with_app(app: Option<&AppHandle>) {
    ...
    if is_own_app_still_active() {
        info!("Focus moved to another screener window, clearing saved app without restoring");
        clear_frontmost_app();
        return;
    }
    ...
    if let Some(app) = app {
        ...
        for label in &non_panel_labels {
            if app.get_webview_window(label).is_some() {
                info!(
                    "Screener '{}' window still open, clearing saved app without restoring",
                    label
                );
                clear_frontmost_app();
                return;
...
mod tests {
    ...
    #[test]
    fn restores_only_when_search_activated_screener() {
