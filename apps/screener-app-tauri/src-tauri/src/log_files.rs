...
pub async fn get_log_files(app: AppHandle) -> Result<Vec<LogFile>, String> {
    let data_dir = get_screener_data_dir(&app).map_err(|e| e.to_string())?;
    let screener_data_dir = get_data_dir(&app).map_err(|e| e.to_string())?;
    Ok(collect_log_files(&[data_dir, screener_data_dir]).await)
}
...
/// `dataDir`, and it also honors `SCREENER_DATA_DIR` in isolated E2E runs.
#[tauri::command]
#[specta::specta]
...
pub fn get_data_dir(app: &AppHandle) -> anyhow::Result<PathBuf> {
    let default_path = screener_core::paths::default_screener_data_dir();
...
pub fn get_screener_data_dir(_app: &AppHandle) -> anyhow::Result<PathBuf> {
    Ok(screener_core::paths::default_screener_data_dir())
}
...
mod tests {
    ...
    async fn collects_only_log_files() {
        ...
        write_file(dir.path(), "screener.log", "hello").await;
        ...
        assert_eq!(names(&files), HashSet::from(["screener.log".to_string()]));
    }
