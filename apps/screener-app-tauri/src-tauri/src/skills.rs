...
//! device, copy chosen ones into the screener skills store
//! (`<data_dir>/skills/<name>/`), and list / remove what's been imported.
//!
//! The store is the source of truth. `screener-core`'s pi executor mirrors it
...
fn background_ai_tools_home() -> Option<PathBuf> {
    #[cfg(feature = "e2e")]
    {
        ...
        return std::env::var_os("SCREENER_E2E_AI_TOOLS_HOME").map(PathBuf::from);
    }
    #[cfg(not(feature = "e2e"))]
    {
...
fn background_bun_path() -> Option<PathBuf> {
    #[cfg(feature = "e2e")]
    if let Some(path) = std::env::var_os("SCREENER_E2E_BUN_PATH") {
    ...
    tauri::async_runtime::spawn(async move {
        ...
        for attempt in 1..=3 {
            ...
            match tokio::task::spawn_blocking(move || {
                screener_engine::cli::agent::setup_all_detected_desktop_in(
                    &home,
                    &bun_path,
                    api_key.as_deref(),
                    &api_url,
...
/// Baseline skills screener writes itself on every session. Importing a skill
/// under one of these names would clobber them, so we reject it.
const RESERVED_SKILL_NAMES: [&str; 3] = ["screener-api", "screener-cli", "screener-team"];
...
/// A skill currently sitting in the screener store.
#[derive(Debug, Clone, Serialize, Deserialize, Type)]
...
/// Install the two built-in screener skills into a supported external agent.
...
pub fn install_external_agent_skills(target: String) -> Result<Vec<String>, String> {
    ...
    screener_engine::cli::agent::install_skills(cli_target, "http://localhost:3030")
        .map(|paths| {
            paths
                .into_iter()
...
/// Remove the two built-in screener skills from a supported external agent.
...
pub fn remove_external_agent_skills(target: String) -> Result<Vec<String>, String> {
    ...
    screener_engine::cli::agent::remove_skills(cli_target)
        .map(|paths| {
            paths
                .into_iter()
                .map(|path| path.to_string_lossy().to_string())
...
pub struct RegistrySkill {
    ...
    pub path: String,
    /// Provenance for the badge: `anthropic` | `openai` | `screener` | `community`.
    #[serde(default)]
    pub source: String,
...
const REMOTE_REGISTRY_URL: &str = "https://raw.githubusercontent.com/screener/screener/main/apps/screener-app-tauri/src-tauri/skills-registry.json";
...
fn skills_store_dir() -> PathBuf {
    screener_core::paths::default_screener_data_dir().join("skills")
}
...
/// List skills currently in the screener store.
#[tauri::command]
#[specta::specta]
...
/// Copy a skill folder into the screener store. `source_path` is the folder
...
pub fn import_skill(source_path: String) -> Result<ImportedSkill, String> {
    ...
    if RESERVED_SKILL_NAMES.contains(&key.as_str()) {
        return Err(format!("\"{key}\" is a reserved screener skill name"));
    }
    ...
    screener_core::paths::copy_dir_all(&src, &dest)
...
pub fn remove_imported_skill(name: String) -> Result<(), String> {
    ...
    let chat_copy = screener_core::paths::default_screener_data_dir()
        .join("pi-chat")
        .join(".pi")
        .join("skills")
        .join(&key);
...
fn http_client() -> Result<reqwest::Client, String> {
    reqwest::Client::builder()
        ...
        .user_agent("screener-app (+https://screener.com)")
        .build()
