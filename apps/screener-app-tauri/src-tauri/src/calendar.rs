...
pub async fn calendar_status() -> Result<CalendarStatus, String> {
    #[cfg(target_os = "macos")]
    {
        ...
        let auth_status = ScreenerCalendar::authorization_status();
...
pub async fn start_calendar_events_publisher() {
    ...
    loop {
        ...
        if let Err(e) = screener_events::send_event("calendar_events", items) {
...
async fn collect_calendar_events() -> Vec<CalendarEventItem> {
    #[cfg(target_os = "macos")]
    {
        ...
        let status = ScreenerCalendar::authorization_status();
        if format!("{}", status) != "Full Access"
            && !ScreenerCalendar::access_granted_this_session()
        {
            return Vec::new();
        }
...
#[cfg(target_os = "macos")]
fn calendar_event_to_item(event: screener_connect::calendar::CalendarEvent) -> CalendarEventItem {
...
fn calendar_event_to_item_win(
    event: screener_connect::calendar_windows::CalendarEvent,
) -> CalendarEventItem {
    let start_display = event.start_local.format("%-I:%M %p").to_string();
    let end_display = event.end_local.format("%-I:%M %p").to_string();
