...
//! The screener control-plane base URL, baked at BUILD time.
//!
//! One knob for the whole app: `NEXT_PUBLIC_SCREENER_WEB_URL` — the same
//! variable the TS side already honors (`lib/web-url.ts`) — is also read
//! here via `option_env!`, so a single
//!
//!   NEXT_PUBLIC_SCREENER_WEB_URL=http://192.168.10.161:3000 bun tauri build
//!
...
//! screener.com. Unset = production behavior, byte-for-byte.
//!
//! Runtime env overrides (SCREENER_ENTERPRISE_INGEST_URL etc.) still win
//! over the baked base where they exist — MDM stays authoritative.

const DEFAULT_WEB_BASE: &str = "https://screener.com";

/// Base origin for control-plane calls: the build-time override when the
/// build was produced with `NEXT_PUBLIC_SCREENER_WEB_URL`, else prod.
pub fn screener_web_base() -> String {
    match option_env!("NEXT_PUBLIC_SCREENER_WEB_URL") {
        Some(v) => {
...
pub fn screener_web_url(path: &str) -> String {
    let base = screener_web_base();
    if path.starts_with('/') {
        format!("{base}{path}")
    } else {
        format!("{base}/{path}")
...
mod tests {
    ...
    fn urls_join_cleanly() {
        ...
        assert_eq!(
            screener_web_url("/api/enterprise/ingest"),
            format!("{base}/api/enterprise/ingest")
        );
        assert_eq!(screener_web_url("login"), format!("{base}/login"));
    }
}
