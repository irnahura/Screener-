...
pub async fn install_specific_version(app: &tauri::AppHandle, version: &str) -> Result<(), String> {
    ...
    let rollback_url = format!(
        "https://screener.com/api/app-update/rollback/{}/{}",
        target_arch, version
    );
...
pub async fn restart_for_update(
    ...
    match bounded_teardown(
        ...
        stop_screener(app.state::<RecordingState>(), app.clone()),
    )
    .await
    {
        ...
        TeardownOutcome::Failed(err) => {
            warn!(
                "banner restart: stop_screener failed (continuing): {}",
                err
            )
        }
        ...
        match restart_for_update(app.clone(), None).await {
            ...
            Err(e) => {
                ...
                notify_update_state(
                    ...
                    "screener couldn't save settings before restarting. try again, or quit and reopen the app.",
                );
            }
        }
...
impl UpdatesManager {
    ...
    pub async fn check_for_updates(
        ...
        // explicitly allows the Screener updater for this install context.
            ...
            *self.pending_update.lock().await = Some(PendingUpdateSnapshot {
...

            {
                ...
                let icon_path = if theme == Mode::Light {
                    "assets/screener-logo-tray-updates-black.png"
                } else {
                    "assets/screener-logo-tray-updates-white.png"
                };
                ...
                let _ = self.app.run_on_main_thread(move || {
                    crate::window::with_autorelease_pool(|| {
                        if let Some(tray) = app_clone.tray_by_id("screener_main") {
                            if let Err(e) =
            ...
            if let Some(ref item) = self.update_menu_item {
                ...
                item.set_text("Downloading latest version of screener")?;
            }

            #[cfg(target_os = "windows")]
            {
                // Windows: stop screener before replacing the binary
                if let Err(err) =
                    stop_screener(self.app.state::<RecordingState>(), self.app.clone()).await
                {
                        ...
                        || err_str.contains("Forbidden")
                    {
                        ...
                        std::thread::spawn(move || {
                            let _ = app_notif
                                .notification()
                                .builder()
                                .title("screener update available")
                    ...
                    *self.pending_update.lock().await = None;
                    ...
                    std::thread::spawn(move || {
                        let _ = app_notif
                            .notification()
                            .builder()
                            .title("screener update failed")
                            .body(format!(
                                "v{} couldn't download — open screener to retry",
                                version_str
                            ))
                            .show();
                    });
            ...
            if auto_update && *self.update_installed.lock().await {
                ...
                match bounded_teardown(
                    PRE_EXIT_TEARDOWN_TIMEOUT,
                    stop_screener(self.app.state::<RecordingState>(), self.app.clone()),
                )
                .await
                {
                    TeardownOutcome::Completed => {}
...
fn check_whats_new(app: &tauri::AppHandle) {
    ...
    tokio::spawn(async move {
        ...
        let body = if release_notes.is_empty() {
            format!(
                "screener updated to **v{}**! check the [changelog](https://screener.com/changelog) for details.",
                current_version
            )
        } else {
            ...
            format!(
                "screener updated to **v{}**!\n\n{}\n\n[full changelog](https://screener.com/changelog)",
        ...
        match client
            ...
            .json(&serde_json::json!({
                ...
                "type": "app-update",
                "title": format!("🎉 screener v{} — what's new", current_version),
                "body": body,
            }))
            .send()
            .await
...
pub fn start_update_check(
    ...
    {
        let rollback_dir = screener_core::paths::default_screener_data_dir().join("rollback");
...
mod tests {
    ...
    fn sweep_removes_installer_leftovers_only() {
        ...
        std::fs::write(dir.path().join("screener.exe.sp-old2"), b"old").unwrap();
        // Live app files must survive.
        std::fs::write(dir.path().join("bun.exe"), b"new").unwrap();
        std::fs::write(dir.path().join("screener.exe"), b"new").unwrap();

        assert_eq!(sweep_sp_old_files(dir.path()), 2);
        assert!(!dir.path().join("bun.exe.sp-old").exists());
        assert!(!dir.path().join("screener.exe.sp-old2").exists());
        assert!(dir.path().join("bun.exe").exists());
        assert!(dir.path().join("screener.exe").exists());
    }
