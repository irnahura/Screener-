...
// screener brand: black & white geometric minimalism
...
struct MarkdownText: View {
    ...
    /// `viewerOverridePath` is set when the link is a `screener://view?path=…`
    ...
    /// If `url` is `screener://view?path=…`, return the decoded path. Else nil.
    fileprivate static func viewerOverridePath(for url: URL) -> String? {
        guard url.scheme == "screener" else { return nil }
                ...
                if let closeBracket = afterBracket.firstIndex(of: "]") {
                    ...
                    if afterClose.hasPrefix("(") {
                        ...
                        if let closeParen = urlPart.firstIndex(of: ")") {
                            ...
                            // `%2F`s in a `screener://view?path=…` link
...
/// Tiny ↗ button rendered next to a `screener://view?path=…` link so the
...
/// Open a URL with the right transport: in-app for screener:// (no
...
public func notifShow(_ jsonPtr: UnsafePointer<CChar>) -> Int32 {
    ...
    guard let data = json.data(using: .utf8) else {
        NSLog("[screener-notif] failed to convert json to utf8 data")
        return -1
    }
    let payload: NotificationPayload
    do {
        ...
        NSLog("[screener-notif] json decode failed: \(error) — json: \(json.prefix(500))")
        return -1
    }
