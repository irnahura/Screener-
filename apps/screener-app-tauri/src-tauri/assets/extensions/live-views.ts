...
const blockSchema = {
  ...
  properties: {
    ...
    pipeName: {
      type: ["string", "null"],
      description:
        "Installed scheduled task that fills this Block, or null when none fits. Find it with screener_live_view action=pipes.",
    },
  },
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    name: "screener_live_view_propose",
    ...
    promptGuidelines: [
      "Call this exactly once, as the last step, after inspecting current values with screener_live_view action=values.",
      ...
      if (problems.length > 0) {
        return {
          content: [
            {
              type: "text" as const,
              text: `Proposal rejected. Fix these and call screener_live_view_propose again:\n- ${problems.join("\n- ")}`,
            },
          ],
          isError: true,
        };
  ...
  pi.registerTool({
    name: "screener_live_view",
    label: "Screener Live View",
    description:
      "Read or edit the user's saved Screener Live Views on demand. Use only when the user asks about a dashboard or Live View; its contents are not preloaded into chat.",
    promptSnippet:
      "Read or edit a Screener Live View only when the user asks",
    promptGuidelines: [
