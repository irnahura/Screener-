...
const API_BASE =
  ...
  `http://localhost:${process.env.SCREENER_LOCAL_API_PORT || process.env.SCREENER_PORT || "3030"}`;
const AUTH_KEY =
  process.env.SCREENER_LOCAL_API_KEY ||
  process.env.SCREENER_API_AUTH_KEY ||
  "";
...
// registers one shared MCP server whose URL points at screener.com's
...
const connectParams = {
  ...
  properties: {
    connectionId: {
      type: "string",
      description:
        "The Screener connection id required for the task, e.g. linear, notion, github, google-docs.",
    },
    reason: {
      type: "string",
      description:
...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    name: "screener_list_connections",
    label: "List Screener connections",
    description:
      "List Screener app connections and whether each one is connected. Use this before sending, pushing, creating, or reading from an external app.",
    promptSnippet:
      "List Screener app connections and connected status before using external apps",
  ...
  pi.registerTool({
    ...
    label: "Connect Screener app",
    description:
      "Ask the user to connect a required Screener app inline and wait for their answer. Call this when the task depends on an app that is not connected. Do not continue the dependent task until this returns connected.",
    promptSnippet:
      "Ask the user to connect a required Screener app inline and wait before continuing",
    promptGuidelines: [
      "Before using an external app, call screener_list_connections or otherwise verify it is connected.",
      "If the required app is not connected, call screener_connect_app and wait for the result.",
      "Do not continue a task that depends on an unconnected app while screener_connect_app is waiting.",
    ...
    async execute(
      ...
      try {
        ...
        if (!ctx.hasUI || !ctx.ui) {
          return {
            content: [
              {
                type: "text" as const,
                text: `${name} is not connected. Connect ${name} in Screener to continue.`,
              },
            ],
        ...
        const title = `screener:connect:${connectionId}:${name}`;
