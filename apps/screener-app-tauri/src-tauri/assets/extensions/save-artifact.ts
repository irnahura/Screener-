...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(
      ...
      // crates/screener-core/src/pipes/mod.rs — the same file saved from a
      ...
      const sessionId = process.env.SCREENER_CHAT_SESSION_ID || "chat";
      ...
      try {
        const apiUrl =
          ...
          `http://localhost:${
            process.env.SCREENER_LOCAL_API_PORT ||
            process.env.SCREENER_PORT ||
            "3030"
          }`;
        const authKey = process.env.SCREENER_LOCAL_API_KEY || "";
