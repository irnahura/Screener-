...
const API_BASE = `http://localhost:${process.env.SCREENER_PORT || 3030}/mcp-servers`;
const AUTH_KEY =
  process.env.SCREENER_LOCAL_API_KEY ||
  process.env.SCREENER_API_AUTH_KEY || // deprecated alias, drop next release
  "";
const SESSION_ID = process.env.SCREENER_SESSION_ID || "";
const MCP_ALLOWLIST_RAW = process.env.SCREENER_MCP_SERVER_ALLOWLIST;
const MCP_ALLOWLIST =
...
function authHeaders(): Record<string, string> {
  return {
    ...
    ...(SESSION_ID ? { "x-screener-session": SESSION_ID } : {}),
  };
}
