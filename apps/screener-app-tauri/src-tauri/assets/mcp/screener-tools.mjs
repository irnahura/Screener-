...
// core screener capabilities: query_recordings, list_connections,
// save_artifact, sp_web_search, screener_connect_app, live_view, and the
// sp_mcp_* bridge. It speaks newline-delimited JSON-RPC on stdin/stdout (MCP
// stdio) or, when SCREENER_TOOLS_HTTP_PORT is set, a stateless Streamable-HTTP
...
// screener HTTP server does not expose, so those agents reach parity. Uses
// only runtime built-ins, so it needs no npm install and runs from the bundled
// bun. It talks to the local screener engine over REST; it never sees the
...
const SERVER_INFO = { name: "screener-tools", version: "0.1.0" };

function apiBase() {
  const url =
    process.env.SCREENER_API_URL ||
    process.env.SCREENER_LOCAL_API_URL ||
    `http://localhost:${
      process.env.SCREENER_LOCAL_API_PORT || process.env.SCREENER_PORT || "3030"
    }`;
  return url.replace(/\/+$/, "");
}

function authHeaders() {
  const key = process.env.SCREENER_LOCAL_API_KEY || "";
...
// "screener -> http://localhost:3030/mcp" is the classic case) only 404s and
...
function chatSessionId() {
  return process.env.SCREENER_CHAT_SESSION_ID || "chat";
}

// Like authHeaders, but also carries the session tag the engine's per-session
// MCP-server access gate reads (x-screener-session). Matches the Pi
...
// path sets SCREENER_CHAT_SESSION_ID; the pi path sets SCREENER_SESSION_ID)
// — prefer whichever is present.
function mcpBridgeHeaders() {
  const headers = authHeaders();
  const session =
    process.env.SCREENER_SESSION_ID || process.env.SCREENER_CHAT_SESSION_ID || "";
  if (session) headers["x-screener-session"] = session;
  return headers;
}
...
const COMPOSIO_URL_RE = /^https:\/\/(www\.)?(screener\.com|screenpi\.pe)\/api\/composio\/mcp\/?$/;
...
// Live Views (dashboards) — mirrors the Pi-only screener_live_view extension.
...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      "List the user's Screener app connections (Gmail, Notion, Slack, Linear, calendars, and other integrations), whether each is connected, and how to use it (native proxy vs MCP OAuth). Use this before an action that depends on a connected app.",
  ...
  },
  {
    ...
    async run(args) {
      ...
      const tmpDir = join(tmpdir(), "screener-artifacts", sessionId);
  ...
  },
  {
    name: "sp_web_search",
    description:
      "Search the public internet via Google Search. Use ONLY for public, external information the user explicitly asks about (current events, news, public people or companies, public product docs). Do NOT use it for the user's own screener data. When unsure, do not search. Returns results with sources.",
    inputSchema: {
      type: "object",
      properties: {
    ...
    async run(args) {
      ...
      if (!res.ok) {
        ...
        if (res.status === 503) {
          throw new Error("web search unavailable: sign in to screener first");
        }
  ...
  {
    // Parity with the Pi-only `screener_live_view` extension: read or edit the
    // user's saved Screener Live Views (dashboards) on demand, so every
  ...
  },
  {
    ...
    // this, ACP agents are told by list_connections / screener_connect_app to
...
// runs the core `screener` server over HTTP — but that transport exposes
...
const HTTP_PARITY_TOOLS = [
  ...
  },
  {
    name: "health_check",
    description:
      "Check whether the screener engine is running and healthy (recording status, frame and audio stats). Proxies the local engine /health.",
    inputSchema: { type: "object", properties: {}, additionalProperties: false },
    async run() {
...
const httpPort = Number.parseInt(process.env.SCREENER_TOOLS_HTTP_PORT || "", 10);
...
// because the core screener server exposes only search_content over HTTP.
...
function startStdioServer() {
  ...
  process.stdin.on("data", (chunk) => {
    ...
    if (buffer.length > 4_000_000) {
      process.stderr.write("screener-tools: stdin line exceeded 4MB, exiting\n");
      process.exit(1);
    }
    let index;
...
function startHttpServer(port) {
  ...
  server.listen(port, "127.0.0.1", () => {
    process.stderr.write(`[screener-tools] http mcp listening on http://127.0.0.1:${port}/mcp\n`);
  });
}
