...
const CHROME_WEBSTORE_URL =
  "https://chromewebstore.google.com/search/screener%20browser%20bridge";

interface BrowserSidebarProps {
  conversationId: string | null;
  /** Session id the on-screen chat's agent process runs under (the value the
   *  agent's `x-screener-session` header carries). Used alongside
...
export function BrowserSidebar({
  ...
  useEffect(() => {
    ...
    window.addEventListener("screener:browser-sidebar-toggle", handler);
    return () => {
      window.removeEventListener("screener:browser-sidebar-toggle", handler);
    };
  }, [toggleFromHeader]);
            ...
            <div className="absolute inset-0 z-40 flex items-center justify-center bg-background p-4">
                <div className="w-full max-w-sm border border-border bg-card p-4 shadow-sm">
                  ...
                  <p className="text-xs leading-5 text-muted-foreground">
                    {sessionAccessRequest.alreadyGranted
                      ? "Screener is about to copy browser session cookies. macOS may ask for browser Safe Storage access next."
                      : "Screener can use your browser sessions so the agent opens sites already signed in. This applies to all sites. It does not read saved passwords."}
                  </p>
                  ...
                  {v20CookieBlock.reason === "locked" ? (
                    <>
                      <p className="text-xs leading-5 text-muted-foreground">
                        ...
                        database. Screener cannot read it while the browser is
                        open.
                      </p>
                      <p className="mt-2 text-xs leading-5 text-muted-foreground">
                        Connect the Screener Browser Bridge extension to share
                    ...
                    <>
                      <p className="text-xs leading-5 text-muted-foreground">
                        ...
                        app-bound encryption prevents Screener from reusing
                        them directly.
                      </p>
                      <p className="mt-2 text-xs leading-5 text-muted-foreground">
                        Connect the Screener Browser Bridge extension to reuse
