...
export default function Timeline({ embedded = false }: { embedded?: boolean }) {
	...
	// Listen for navigate-to-frame events (deep link: screener://frame/12345)
	useEffect(() => {
	...
	// e.g. app opened from cold start via screener://frame/23 — Timeline mounts late
	useEffect(() => {
		...
		<TimelineProvider>
			...
			>
						...
						<div className="absolute inset-0 flex items-center justify-center bg-gradient-to-b from-background via-background to-muted/20">
							{isScreenRecordingOff(health) ? (
								...
								<div className="text-center p-8 max-w-md">
									...
									<p className="text-muted-foreground mb-6 leading-relaxed">
										Screener is recording your screen activity. Your timeline will appear here as frames are captured.
									</p>
				...
				{(() => {
					...
					// browser_url from screener often lacks a protocol (e.g. "github.com/foo");
				...
				{error && (
					<div className="absolute inset-0 z-50 flex items-center justify-center bg-background/90">
						...
						<div className="bg-destructive/20 border border-destructive/30 text-foreground p-6 rounded-2xl text-center space-y-4 max-w-md mx-4">
							...
							<p className="text-sm text-foreground">
								Unable to reach your screener data. Please verify that the
								screener turned on.
							</p>
							<button
