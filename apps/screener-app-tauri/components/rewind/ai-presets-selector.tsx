...
export function AIProviderConfig({
  ...
  useEffect(() => {
    if (!acpEnabled && selectedProvider === "acp") {
      setSelectedProvider("screener-cloud");
    }
  ...
  const selectedModelMetadata = useMemo(
    () => selectedProvider === "screener-cloud"
        ...
        <div className="grid grid-cols-3 gap-2">
          {piAvailable && (
            <Button
              ...
              variant={selectedProvider === "screener-cloud" ? "default" : "outline"}
          ...
          <div className="space-y-1.5">
            ...
            {selectedProvider !== "screener-cloud" &&
              ...
              <div className="space-y-1">
                ...
                />
                <p className="text-[10px] text-muted-foreground">
                  use this only when the endpoint does not publish a context window; Screener agents need at least 32,768
                </p>
              </div>
            )}
            {selectedProvider !== "screener-cloud" &&
...
export const AIPresetDialog = ({
  ...
  const handleProviderSubmit = (providerData: any) => {
    ...
    // Screener Cloud gets its output budget from the gateway catalog. Direct
    // providers retain the resolved value or the user's unknown-model fallback.
    if (providerData.provider !== "screener-cloud") {
  ...
  const defaultPreset = preset
    ? {
        ...
        ...(preset.provider !== "screener-cloud"
...
export const AIPresetsSelector = ({
  ...
  const selectedPresetRequiresLogin = useMemo(() => {
    ...
    return preset?.provider === "screener-cloud" && !settings?.user?.token;
  ...
  const handleRemovePreset = (preset: AIPreset) => {
    ...
    // Safety net: prevent deletion of the last screener-cloud preset for subscribers
    if (preset.provider === "screener-cloud" && settings.user?.cloud_subscribed) {
  ...
  // Hide delete button on the last remaining screener-cloud preset for subscribers
  const cloudPresetCount = useMemo(
    () => (settings?.aiPresets || []).filter((p) => p.provider === "screener-cloud").length,
    [settings?.aiPresets]
  );
        ...
        {!isControlled && selectedPresetRequiresLogin && !showModelOnly && (
          <div className="flex items-center gap-2 p-2 text-sm bg-muted border border-border rounded-lg">
            ...
            <span className="text-foreground flex-1">
              Login required to use Screener Cloud
            </span>
            {showLoginCta && (
              <Button
                variant="outline"
              ...
              <CommandList>
                ...
                <CommandGroup>
                  {aiPresets.map((preset) => {
                    const isCloud = preset.provider === "screener-cloud";
