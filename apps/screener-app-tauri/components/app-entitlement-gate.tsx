...
const E2E_ACCOUNT_USER_KEY = "screener_e2e_account_user";
const E2E_ACCOUNT_USER_EVENT = "screener-e2e-seed-account-user";
...
// RIOM is local-first: using the recorder, local search, and local settings
// must not require a screener.com account. Enterprise deployments still use
// the managed-policy gate below, and individual AI providers may separately
// request their own credentials when the user chooses them.
const REQUIRE_CONSUMER_APP_AUTH =
  process.env.NEXT_PUBLIC_REQUIRE_CONSUMER_APP_AUTH === "true";
...
export function AppEntitlementGate({
  ...
  // `-shm` desync, or the user runs `screener db recover`), the token returns
  ...
  useEffect(() => {
    ...
    commands.stopScreener().catch((err) => {
      console.warn("failed to stop screener after entitlement gate:", err);
    });
  }, [
    enterpriseAuthenticationPending,
    isManagedDeployment,
  ...
  // macOS, so paste the login token (or the whole screener://...api_key=...
  ...
  const resumeRecordingAfterGate = useCallback((requireGateStop: boolean) => {
    ...
    void (async () => {
      try {
        await commands.stopScreener();
        await new Promise((r) => setTimeout(r, 500));
        await commands.spawnScreener(null);
        recorderStoppedByGateRef.current = false;
      } catch (err) {
        console.warn("failed to restart screener after access restored:", err);
      } finally {
  ...
  const devLoginBlock = isDevLoginEnabled() ? (
    <div className="mt-5 border-t border-border pt-4">
      ...
      <input
        ...
        placeholder="paste token or screener://…api_key=…"
        spellCheck={false}
  ...
  if (!isSettingsLoaded) {
    return (
      <EntitlementShell
        title="loading"
        description="checking local settings before starting screener."
      >
  ...
  if (!isManagedDeploymentResolved) {
    if (managedDeploymentResolutionError) {
      return (
        <EntitlementShell
          ...
          description="screener could not confirm which build is installed. it will retry automatically."
        >
          <Button
    ...
    return (
      <EntitlementShell
        ...
        description="checking which screener build is installed on this device."
      >
        <div className="h-10 w-full animate-pulse bg-muted" />
      </EntitlementShell>
    );
  ...
  if (shouldGateForEnterpriseApp) {
    return (
      <EntitlementShell
        ...
        description={`${email} belongs to ${enterpriseOrgName}. download the screener enterprise app so this device follows workspace policy and uploads to your org storage.`}
      >
        <div className="flex flex-col gap-3">
  ...
  if (!user?.token) {
    return (
      <EntitlementShell
        ...
        description="create or sign in to a screener account to start the free plan."
      >
        <div className="flex flex-col gap-3">
  ...
  return (
    <EntitlementShell
      ...
      description={
        needsRefresh
          ? `${email} has saved app access, but screener needs to verify it again before recording starts.`
          : shouldVerifyPlan
            ? `screener could not verify the plan for ${email}. refresh the account before recording starts.`
