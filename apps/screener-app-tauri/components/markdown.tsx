...
export function createScreenerUrlTransform(allowedHosts: readonly string[]) {
  const allowed = new Set(allowedHosts);

  return (url: string): string => {
    try {
      const parsed = new URL(url);
      if (parsed.protocol === "screener:" && allowed.has(parsed.host)) {
        return url;
      }
    } catch {
...
export const notificationUrlTransform = createScreenerUrlTransform(["view"]);
export const viewerUrlTransform = createScreenerUrlTransform(["view"]);
export const chatUrlTransform = createScreenerUrlTransform(["timeline", "frame", "view"]);

export function screenerViewerPathFromHref(href: string): string | null {
  try {
    const url = new URL(href);
    if (url.protocol !== "screener:" || url.host !== "view") {
      return null;
    }
...
export async function openScreenerViewerLink(href: string): Promise<boolean> {
  const path = screenerViewerPathFromHref(href);
...
export function resolveLocalPathFromMarkdownUrl(url: string): string | null {
  ...
  if (!raw || raw.startsWith("screener://")) {
    return null;
  }
...
export function rewriteLocalMarkdownLinksForChat(text: string): string {
  return rewriteLocalMediaLinksForChat(text).replace(
    ...
    (match, sigil: string, label: string, rawUrl: string) => {
      ...
      return `[${label}](screener://view?path=${encodeURIComponent(localPath)})`;
    },
  );
}
