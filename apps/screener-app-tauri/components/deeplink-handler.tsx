...
import {
  openScreenerViewerLink,
  screenerViewerPathFromHref,
...
export function DeeplinkHandler() {
  ...
  useEffect(() => {
    ...
    const processDeepLinkUrl = async (url: string) => {
      ...
      // screener" button opens this link. Refresh the authenticated account
        ...
        if (userToken) {
          try {
            ...
            toast({
              ...
              description: "Screener Business is ready",
            });
          } catch (error) {
            const msg = error instanceof Error ? error.message : String(error);
            toast({
      ...
      //   screener://subscription-success?purchase_token=<token>
      ...
      if (
        ...
        if (purchaseToken) {
          try {
            ...
            toast({
              title: "welcome to screener business!",
      ...
      //   screener[-enterprise]://oauth/connections/callback?code=...&state=...
      //   screener[-enterprise]://oauth/mcp/<serverId>/callback?code=...&state=...
      ...
      if (parsedUrl.host === "oauth") {
        ...
        try {
          ...
          toast({
            ...
            description: msg || "couldn't reach the local screener engine",
            variant: "destructive",
          });
        }
      }
      ...
      //   screener://timeline?timestamp=ISO8601
      //   screener://timeline?start_time=ISO8601&end_time=ISO8601
      ...
      // Handle pipe install deep links: screener://install-pipe?url=<encoded-url>
      if (
      ...
      //   screener://chat/<conversationId>?message=<messageId>
      //   screener://chat?conversation=<conversationId>&message=<messageId>
      ...
      // Handle in-app file viewer: screener://view?path=<encoded-path>
      ...
      if (parsedUrl.host === "view" || parsedUrl.pathname === "view") {
        const filePath = screenerViewerPathFromHref(url);
        if (filePath) {
          try {
            await openScreenerViewerLink(url);
          } catch (error) {
      ...
      // Handle frame deep links: screener://frame/12345
      ...
      //   screener://meeting/123?live=1
      //   screener://meeting?id=123
