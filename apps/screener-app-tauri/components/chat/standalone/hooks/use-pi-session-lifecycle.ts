...
export function usePiSessionLifecycle({
  ...
  const needsLogin = activePreset?.provider === "screener-cloud" && !userToken;
  ...
  const buildProviderConfig = useCallback((preset?: AIPreset | null): ResolvedPiProviderConfig | null => {
    ...
    // agents read live screen context through the screener MCP server
  ...
  useEffect(() => {
    ...
    // prompt into an initialize failure. ACP sessions get screener context
