...
// app_name=X in the query string — see crates/screener-core/assets/pipes/.
// The endpoint/method card and its rich metadata are curl-based. Raw pi sends a
// bash `curl`; ACP screener MCP tools send a name + structured args, which we
// map to the equivalent curl so the SAME classifier drives both. Any other tool
// (a native ACP Read/Edit, a non-screener MCP call) has no curl and no card.
function effectiveCommand(toolCall: ToolCall): string | null {
  if (toolCall.toolName === "bash") {
    return String(toolCall.args?.command ?? "") || null;
  }
  return mcpScreenerCommand(toolCall.toolName, (toolCall.args ?? {}) as Record<string, unknown>);
}
...
function bashToolDetailsPresentation(toolCall: ToolCall): BashToolDetailsPresentation | null {
  ...
  const localUrl = urlsInCommand(command).find(isLocalScreenerUrl);
...
function FriendlyToolDetails({ toolCall }: { toolCall: ToolCall }) {
  ...
  // Raw pi's bash curl AND screener MCP tool calls both resolve to a local
...
// existing assets in apps/screener-app-tauri/public/images/.
