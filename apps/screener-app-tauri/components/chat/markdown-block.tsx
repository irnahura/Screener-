    ...
    <MemoizedReactMarkdown
      ...
      components={{
        ...
        a({ href, children, ...props }) {
          if (
            href?.startsWith("screener://timeline") ||
            href?.startsWith("screener://frame") ||
            href?.startsWith("screener://view")
          ) {
            const handleScreenerLinkClick = async (
              e: React.MouseEvent<HTMLAnchorElement>,
            ) => {
              e.preventDefault();
              try {
                if (href.startsWith("screener://view")) {
                  const viewerPath = screenerViewerPathFromHref(href);
                  if (viewerPath && onOpenViewerPath) {
                    onOpenViewerPath(viewerPath);
                    return;
                  }
                  if (await openScreenerViewerLink(href)) return;
                }

                if (href.startsWith("screener://frame")) {
                ...
                console.error("Failed to open screener link:", error);
              }
            };
            ...
            return (
              <a
                href="#"
                onClick={handleScreenerLinkClick}
