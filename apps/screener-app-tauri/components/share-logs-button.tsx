    ...
    try {
      ...
      if (onBackgroundStart) {
        toast({
          ...
          description:
            video?.status === "ready"
              ? "you can keep using screener. keep the app running while the video uploads; we'll notify you when it's sent."
              : "you can keep using screener. please keep the app running for the next minute; we'll notify you when it's sent.",
        });
        onBackgroundStart();
      }
    } catch (err) {
