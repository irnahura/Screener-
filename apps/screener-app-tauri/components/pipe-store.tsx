...
function DiscoverView({ onInstalled }: { onInstalled?: () => void }) {
  ...
  useEffect(() => {
    if (!localStorage.getItem("screener:pipes-welcome-dismissed")) {
      setShowWelcome(true);
    }
  }, []);
  ...
  const dismissWelcome = () => {
    ...
    localStorage.setItem("screener:pipes-welcome-dismissed", "1");
  };
...
function PublisherIdentity({
  ...
  return (
    <div
      ...
      aria-label={publisher.isScreenerTeam ? "official Screener publisher" : undefined}
      title={compact && publisher.isScreenerTeam ? "built by screener team" : undefined}
      className={cn(
    ...
    >
      ...
      >
        {compact ? (
          ...
          <>
            {publisher.isScreenerTeam ? "built by " : "by "}
            <span className="font-medium text-foreground">{publisher.name}</span>
          </>
        )}
      </span>
      {publisher.verified && !publisher.isScreenerTeam && (
        <BadgeCheck
...
IMPORTANT: first read the screener skill file to understand how pipes work, then ask the user how they want to customize/improve this pipe for their specific needs. do NOT auto-send or auto-create — have a conversation first to understand what they want to change.
