...
async function getWindowsUpdateOptions() {
  ...
  return {
    checkOptions: {
      endpoints: [
        screenerWebUrl(`/api/app-update/${channel}/windows-${cpuArch}/{{current_version}}`, "https://screener.com"),
      ],
    ...
    try {
      ...
      if (os === "windows") {
        ...
        if (gate !== "proceed") {
          setIsInstalling(false);
          toast({
            title: "screener is still starting up",
            description:
              gate === "errored"
        ...
        // Stop screener before update on Windows
        try {
          await commands.stopScreener();
        } catch (e) {
          console.warn("failed to stop screener:", e);
        }
        ...
        toast({
          ...
          description: "screener will restart automatically",
          duration: 10000,
        });
    ...
    return (
      <div className={cn(
        ...
        <div className="flex items-center gap-2 flex-1">
          ...
          <span>
            screener <span className="font-medium">v{authRequired.version}</span> is available — sign in to download
          </span>
        </div>
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center gap-2 flex-1">
        <Sparkles className="h-4 w-4 text-primary" />
        <span>
          screener <span className="font-medium">v{updateInfo.version}</span> is ready
        </span>
      </div>
      <div className="flex items-center gap-2">
        <Button
