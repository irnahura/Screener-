 ...
 *   │ (~/.screener/chats/) │         │ listens app-wide for pi_event  │
...
const DELETED_PIPE_EXECUTIONS_KEY = "screener:deleted-pipe-executions";
...
export function ChatSidebar({ className, onViewAll }: ChatSidebarProps) {
  ...
  const [collapsedRecentsSections, setCollapsedRecentsSections] = useState<Set<string>>(() => {
    ...
    try {
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (k?.startsWith("screener:recents-subsection-collapsed:")) {
          if (localStorage.getItem(k) === "true") {
            set.add(k.slice("screener:recents-subsection-collapsed:".length));
          }
        }
      }
    } catch { /* ignore */ }
    ...
    setCollapsedRecentsSections((prev) => {
      ...
      try {
        localStorage.setItem(
          `screener:recents-subsection-collapsed:${key}`,
          String(!collapsed),
        );
      } catch { /* ignore */ }
      return next;
  ...
  const [expandedGroups, setExpandedGroups] = useState<Set<string>>(() => {
    ...
    try {
      for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
        if (k?.startsWith("screener:group-expanded:")) {
          if (localStorage.getItem(k) === "true") {
            set.add(k.slice("screener:group-expanded:".length));
          }
        }
      }
    } catch { /* ignore */ }
  ...
  useEffect(() => {
    ...
    try {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
        if (k?.startsWith("screener:group-expanded:")) {
          const groupKey = k.slice("screener:group-expanded:".length);
  ...
  useEffect(() => {
    try {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
        if (k?.startsWith("screener:recents-subsection-collapsed:")) {
          const sectionKey = k.slice("screener:recents-subsection-collapsed:".length);
  ...
  const [pinnedCollapsed, setPinnedCollapsed] = useCollapsedPref(
    "screener:pinned-collapsed",
    true
  );
  const [recentsCollapsed, setRecentsCollapsed] = useCollapsedPref("screener:recents-collapsed");
  const [archivedCollapsed, setArchivedCollapsed] = useCollapsedPref(
    "screener:closed-collapsed",
    true
  );
