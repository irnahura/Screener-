...
export function useChatConversations(opts: UseChatConversationsOpts) {
  ...
  const [showHistory, setShowHistoryRaw] = useState(() => {
    try { return localStorage.getItem("screener:chat-history-open") === "true"; } catch { return false; }
  });
  const setShowHistory = useCallback((v: boolean | ((prev: boolean) => boolean)) => {
    setShowHistoryRaw((prev) => {
      const next = typeof v === "function" ? v(prev) : v;
      try { localStorage.setItem("screener:chat-history-open", String(next)); } catch {}
      return next;
    });
  }, []);
  ...
  const saveConversation = async (
    ...
    // plumbing wrappers (<attached file: ...>, <screener-large-context>, etc.)
    ...
    if (
      ...
      // (e.g. Screener Cloud) to make a "smart" title would silently leak it.
