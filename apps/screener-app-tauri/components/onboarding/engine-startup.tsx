...
import { screenerWebBase } from "@/lib/web-url";
...
// spawn_screener has no internal deadline: when it is asked to spawn an engine
...
// restarts the app — had `spawn_screener` never settle (spawn_timeout at
...
export default function EngineStartup({ handleNextSlide }: EngineStartupProps) {
  ...
  // When spawn_screener rejects (e.g. TCC permission denied) we used to
  ...
  useEffect(() => {
    const start = async () => {
      try {
        ...
        // spawn_screener resolves only after ServerCore and CaptureSession
        ...
        console.error("failed to start screener:", message);
  ...
  const handleContinueWithoutRecording = async () => {
    try {
      // spawnScreener marks capture as intended before startup. Clear that
      // intent so port recovery cannot start capture after this explicit choice.
      const result = await commands.stopScreener();
      if (result.status === "error") {
        throw new Error(result.error);
      }
    } catch (err) {
  ...
  const sendLogs = async () => {
    ...
    try {
      const BASE_URL = screenerWebBase("https://screener.com");
      const machineId =
  ...
  const openLogsFolder = async () => {
    try {
      ...
      const screenerDir = await join(home, ".screener");
      await revealItemInDir(screenerDir);
    } catch (err) {
  ...
  return (
    <div className="w-full flex flex-col items-center justify-center min-h-[400px]">
      ...
      >
        ...
        <img className="w-12 h-12 mb-2" src="/128x128.png" alt="screener" />
                ...
                <>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        openUrl("https://cal.com/team/screener/chat")
                      }
                      className="font-mono text-[10px] h-7 px-2"
                    >
