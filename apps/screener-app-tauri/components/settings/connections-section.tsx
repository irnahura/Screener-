...
import { screenerWebUrl } from "@/lib/web-url";
...
const GITHUB_RELEASES_API = "https://api.github.com/repos/screener/screener/releases";
...
import {
  ...
  getInstalledClaudeScreenerEntry,
  isStaleClaudeScreenerEntry,
  isCodexMcpInstalled,
...
async function installGrokMcp(): Promise<void> {
  ...
  const next = servers.filter((s) => s?.id !== "screener");
...
async function uninstallGrokMcp(): Promise<void> {
  ...
  mcp.servers = (mcp.servers as Record<string, unknown>[]).filter((s) => s?.id !== "screener");
  await writeFile(configPath, new TextEncoder().encode(JSON.stringify(config, null, 2)));
}
...
const INTEGRATION_ICONS: Record<string, React.ReactNode> = {
    ...
    // user-browser: your real Chrome/Arc/Edge via the screener extension.
...
function ClaudePanel({ onConnected, onDisconnected }: { onConnected?: () => void; onDisconnected?: () => void }) {
  ...
  useEffect(() => {
    getInstalledClaudeScreenerEntry().then(async (entry) => {
      if (!entry) return;
      ...
      if (isStaleClaudeScreenerEntry(entry)) {
        try {
          const next = await buildMcpConfig({ client: "claude" });
          if (next.env?.SCREENER_LOCAL_API_KEY) {
  ...
  const handleConnect = async () => {
    try {
      ...
      if (mcp.command === "npx") {
        await message(
          "connected, but screener couldn't find its bundled runtime, so it wrote a config that needs Node.js installed.\n\nif Claude can't start screener, install Node (https://nodejs.org) or reinstall the screener app, then reconnect.",
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground">
        Install the screener MCP plus API and CLI skills for Claude in one click.
      </p>
...
function CodexPanel({ onConnected, onDisconnected }: { onConnected?: () => void; onDisconnected?: () => void }) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground">Install the screener MCP plus API and CLI skills for Codex in one click.</p>
      <div className="flex flex-wrap gap-2">
        {state === "installed" ? (
          <Button onClick={handleDisconnect} variant="outline" size="sm" className="gap-1.5 h-7 text-xs normal-case font-sans tracking-normal">
            <LogOut className="h-3 w-3" />disconnect
...
function ClaudeCodePanel() {
  ...
  const cmd = "claude mcp add screener -- npx -y screener-mcp@latest";
...
// memory-sync feature backed by the screener-connect Integrations of
  ...
  return (
    <div className="border-t border-border pt-3 mt-3 space-y-2">
      <div className="space-y-0.5">
        ...
        <p className="text-xs text-muted-foreground">
          writes your screener memories into {targetFilename} so {assistantName} sees them
...
// user's pipes write to, and vice-versa. Writes a single screener-owned
// note `<vault>/<folder>/screener-memories.md`, rewritten end-to-end on
// each 5-minute scheduler tick.
const OBSIDIAN_MEMORIES_ID = "obsidian-memories";
const OBSIDIAN_DEFAULT_FOLDER = "screener";
...
function ObsidianMemorySyncSubsection() {
  ...
  const notePath = `${vaultPath.replace(/[\\/]+$/, "")}/${folderClean}/screener-memories.md`;
  ...
  return (
    <div className="border-t border-border pt-3 mt-1 space-y-2">
      <div className="space-y-0.5">
        <p className="text-xs font-medium text-foreground">memory sync (beta)</p>
        <p className="text-xs text-muted-foreground">
          writes your screener memories into a note in this vault so they show up
...
function MstyPanel() {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-muted-foreground">
        3. Give the tool a name (e.g. <strong>screener</strong>) and click <strong>Add</strong>
      </p>
...
function OllamaPanel() {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground">
        Use Ollama as a local AI provider for screener.
      </p>
...
function LMStudioPanel() {
  const [status, setStatus] = useState<"idle" | "checking" | "connected" | "error">("idle");
  const [models, setModels] = useState<string[]>([]);
  const deeplink = "lmstudio://add_mcp?name=screener&config=eyJjb21tYW5kIjoibnB4IiwiYXJncyI6WyIteSIsInNjcmVlbnBpcGUtbWNwIl19";
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground">
        Connect LM Studio to screener&apos;s screen &amp; audio data, or use it as a local AI provider.
      </p>
      <div className="flex flex-wrap gap-2">
        <Button onClick={() => openUrl(deeplink)} size="sm" className="gap-1.5 h-7 text-xs normal-case font-sans tracking-normal">
          <Download className="h-3 w-3" /> add screener MCP to LM Studio
        </Button>
...
// server-side (screener-connect), so the UI can never request arbitrary
...
> = {
  slack: [
    { id: "send", label: "Send only", description: "Post messages as you. Screener can't read your Slack." },
...
// OAuth flow in one click. A provider may register screener as a *confidential*
// client (client_secret_basic) during dynamic client registration; that is
// handled generically in screener-connect's mcp_servers.rs, so a new provider
...
export const MCP_OAUTH_PROVIDERS: {
  ...
  { id: "linear", name: "Linear", url: "https://mcp.linear.app/mcp", description: <>Connect Linear so your AI can search and manage your issues, projects, and cycles. Sign-in uses Linear&apos;s OAuth — no API key, and screener never sees your password.</> },
  { id: "stripe", name: "Stripe", url: "https://mcp.stripe.com", description: <>Connect Stripe so your AI can query your customers, payments, invoices, and subscriptions. Sign-in uses Stripe&apos;s OAuth — no API key, and screener never sees your password.</> },
  { id: "sentry", name: "Sentry", url: "https://mcp.sentry.dev/mcp", description: <>Connect Sentry so your AI can search your issues, events, and releases. Sign-in uses Sentry&apos;s OAuth — no API key, and screener never sees your password.</> },
  { id: "intercom", name: "Intercom", url: "https://mcp.intercom.com/mcp", description: <>Connect Intercom so your AI can search your conversations, contacts, and help content. Sign-in uses Intercom&apos;s OAuth — no API key, and screener never sees your password.</> },
  { id: "asana", name: "Asana", url: "https://mcp.asana.com/mcp", description: <>Connect Asana so your AI can search and manage your tasks, projects, and portfolios. Sign-in uses Asana&apos;s OAuth — no API key, and screener never sees your password.</> },
  { id: "monday", name: "monday.com", url: "https://mcp.monday.com/mcp", description: <>Connect monday.com so your AI can work with your boards, items, and updates. Sign-in uses monday&apos;s OAuth — no API key, and screener never sees your password.</> },
  { id: "clickup", name: "ClickUp", url: "https://mcp.clickup.com/mcp", description: <>Connect ClickUp so your AI can search and manage your tasks, docs, and spaces. Sign-in uses ClickUp&apos;s OAuth — no API key, and screener never sees your password.</> },
  { id: "airtable", name: "Airtable", url: "https://mcp.airtable.com/mcp", description: <>Connect Airtable so your AI can read and update your bases, tables, and records. Sign-in uses Airtable&apos;s OAuth — no API key, and screener never sees your password.</> },
  { id: "confluence", name: "Confluence", url: "https://mcp.atlassian.com/v1/mcp", description: <>Connect Atlassian so your AI can search and edit your Confluence pages (and Jira issues). Sign-in uses Atlassian&apos;s OAuth — no API key, and screener never sees your password.</> },
  { id: "jira", name: "Jira", url: "https://mcp.atlassian.com/v1/mcp", description: <>Connect Atlassian so your AI can search and manage your Jira issues (and Confluence pages). Sign-in uses Atlassian&apos;s OAuth — no API key, and screener never sees your password.</> },
  { id: "notion", name: "Notion", url: "https://mcp.notion.com/mcp", description: <>Connect Notion so your AI can search, read, and write your pages and databases. Sign-in uses Notion&apos;s OAuth — no API key, and screener never sees your password.</> },
];
    ...
    try {
      ...
      const poll = async () => {
        ...
        if (Date.now() - started < 120_000) {
          ...
          setWaiting(false);
          setStatusMsg(
            "Sign-in was not completed — if your browser blocks http://localhost (e.g. Safari HTTPS-Only mode), click \"Open screener\" on the confirmation page"
          );
        }
      };
      timerRef.current = setTimeout(poll, 2000);
...
export function ConnectionsSection({
  ...
  // Composio-backed connections (managed auth through screener.com; see
  ...
  useEffect(() => {
    ...
    fetch(screenerWebUrl("/api/composio/status", "https://screener.com"), {
  ...
  const renderPanel = () => {
    ...
    switch (selected) {
      ...
      case "krisp": return <OAuthMcpPanel
        ...
        description={<>Connect Krisp so your AI can search your meeting transcripts, notes, and action items. Sign-in is handled by Krisp&apos;s OAuth, so screener never sees your password.</>}
      ...
      case "plaud": return <OAuthMcpPanel
        ...
        description={<>Connect Plaud so your AI can search your Plaud recordings, transcripts, summaries, and notes. Sign-in is handled by Plaud&apos;s OAuth, so screener never sees your password.</>}
      ...
      default:
        ...
        return (
          <div className="space-y-2">
            <p className="text-xs text-muted-foreground">
              couldn&apos;t load connection metadata. the local screener server may be starting up.
            </p>
            <Button
              size="sm"
              variant="outline"
