...
// ---------------------------------------------------------------------------
// Canonical screener SKILL.md content. Re-exported from a generated module so
// the "copy / save SKILL.md" text the cards serve is always the source of truth
// from the repo: .claude/skills/{screener-api,screener-cli}/SKILL.md
...
export {
  SCREENER_API_SKILL_MD,
  SCREENER_CLI_SKILL_MD,
} from "@/lib/generated/screener-skills";
...
export const SECOND_BRAIN_PROMPT = `you have access to screener, a local tool that records everything i see, say, and
...
i work — by watching my activity through screener in the background, so i never
have to re-explain my context. think of it as a digital clone of my working context.

## how to read my activity

if you have the screener MCP tools (search-content, activity-summary, list-meetings,
update-memory), use them. otherwise query the local REST API at http://localhost:3030
(or http://SCREENER_IP:3030 if i run screener on another machine):
...
so you don't pull too much. if screener skills are available, load them first.
...
- if you have the screener update-memory tool: write each durable fact as a memory
  with namespaced tags (person:, project:, topic:) and importance 0-1. retrieve later
  with search-content content_type='memory'. this is the same memory the screener
...
/** One installable screener skill (e.g. the API skill or the CLI skill). */
...
export type AgentCardProps = {
  ...
  /** One or more installable screener skills (e.g. API + CLI). First is shown by default. */
  skills: SkillVariant[];
  sync: {
    /** Default remote path on the VPS where ~/.screener gets pushed. */
    defaultRemotePath: string;
    /** Prefix used for localStorage keys + posthog event names. */
    storageKeyPrefix: string;
  };
  /** If set, renders a "Connect" tab for entering credentials that screener pipes use to call this agent. */
  connect?: {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground leading-relaxed">
        Register screener as an MCP server in {name}. Best when {name} runs on the
        same machine as screener.
      </p>
...
// ---------------------------------------------------------------------------
// Skill section — install screener SKILL.md into the agent's skills dir
// ---------------------------------------------------------------------------
...
function SkillSection({ name, skills }: { name: string; skills: SkillVariant[] }) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground leading-relaxed">
        Drop a screener SKILL.md into {name}&apos;s skills directory and {name} loads it
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-1.5 pt-2 border-t border-border">
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          ...
          host. To let {name} reach the screener data on <em>this</em> machine, use the{" "}
...
// ---------------------------------------------------------------------------
// Sync section — SSH/rsync ~/.screener to a remote VPS
...
function RemoteSyncSection({
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground leading-relaxed">
        Push your <code className="bg-muted px-1 rounded">~/.screener</code> over
...
// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------
// ConnectSection — credential form for screener pipes calling back to the agent
// ---------------------------------------------------------------------------
...
export function ConnectSection({ integrationId, fields }: { integrationId: string; fields: ConnectField[] }) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground leading-relaxed">
        Let screener scheduled tasks call back to this agent. Enter the gateway credentials so scheduled tasks can send events and messages directly to it.
      </p>
      {fields.map((field) => (
...
function SecondBrainCallout({ name }: { name: string }) {
  ...
  const saveMd = useCallback(async () => {
    setIsSaving(true);
    try {
      await writeTextFile("screener-second-brain.md", SECOND_BRAIN_PROMPT, {
        baseDir: BaseDirectory.Download,
      });
      const dir = await downloadDir();
      setSavedPath(await join(dir, "screener-second-brain.md"));
      toast({ title: "saved to Downloads", description: "screener-second-brain.md" });
