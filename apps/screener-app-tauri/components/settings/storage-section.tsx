...
export function StorageSection() {
  ...
  const handleApplyRestart = useCallback(async () => {
    ...
    try {
      ...
      toast({
        title: "restarted",
        description: "screener restarted with the new data directory",
      });
    } catch (error) {
      console.error("failed to restart screener:", error);
      toast({
        title: "restart failed",
        description: "please restart screener manually for the change to apply",
        variant: "destructive",
        duration: 5000,
      });
    } finally {
