...
// Hermes cards. Pick an agent, then set screener up on the machine where that
...
// `screener agent setup` CLI already covers cursor/windsurf too.
...
const TOML_SNIPPET = `[mcp_servers.screener]
command = "npx"
args = ["-y", "screener-mcp@latest"]
`;
  ...
  return [
    {
      ...
      md: SCREENER_API_SKILL_MD,
      downloadName: "screener-api-SKILL.md",
      localPath: `${skillsDir}/screener-api/SKILL.md`,
    },
    {
      id: "cli",
      label: "CLI",
      ...
      md: SCREENER_CLI_SKILL_MD,
      downloadName: "screener-cli-SKILL.md",
      localPath: `${skillsDir}/screener-cli/SKILL.md`,
    },
  ];
}
...
const TARGETS: { id: TargetId; label: string; props: AgentCardProps }[] = [
  {
    ...
    props: {
      ...
      description:
        "Run any AI agent on your VPS 24/7. Wire it to screener — register the MCP server, install the skill, or sync your data.",
      homepage: "https://github.com/openclaw/openclaw",
      mcp: { format: "json", configPath: "~/openclaw/mcp.json", snippet: JSON_SNIPPET },
      skills: skillVariants("~/openclaw/skills"),
      sync: { defaultRemotePath: "~/screener-data", storageKeyPrefix: "openclaw" },
      connect: {
  ...
  },
  {
    ...
    props: {
      ...
      description:
        "Self-improving agent with messaging gateways (Telegram, Discord, Slack, WhatsApp). Wire it to screener.",
      homepage: "https://hermes-agent.nousresearch.com",
      mcp: { format: "yaml", configPath: "~/.hermes/config.yaml", snippet: YAML_SNIPPET },
      skills: skillVariants("~/.hermes/skills"),
      sync: { defaultRemotePath: "~/screener-data", storageKeyPrefix: "hermes" },
      connect: {
  ...
  },
  {
    ...
    props: {
      ...
      description:
        "Anthropic's terminal/IDE agent. Give it screener via MCP + skills, locally or on a remote box.",
      homepage: "https://claude.com/claude-code",
      mcp: { format: "json", configPath: "~/.claude.json", snippet: JSON_SNIPPET },
      skills: skillVariants("~/.claude/skills"),
      sync: { defaultRemotePath: "~/screener-data", storageKeyPrefix: "claude-code" },
    },
  },
  {
    id: "claude-desktop",
    ...
    props: {
      name: "Claude Desktop",
      iconSrc: "/images/claude-ai.svg",
      description: "The Claude desktop app. MCP-only — register screener as an MCP server.",
      ...
      skills: [],
      sync: { defaultRemotePath: "~/screener-data", storageKeyPrefix: "claude-desktop" },
    },
  },
  {
    id: "codex",
    ...
    props: {
      ...
      description: "OpenAI's Codex CLI. MCP-only — registers screener in ~/.codex/config.toml.",
      homepage: "https://developers.openai.com/codex",
      mcp: { format: "toml", configPath: "~/.codex/config.toml", snippet: TOML_SNIPPET },
      skills: [],
      sync: { defaultRemotePath: "~/screener-data", storageKeyPrefix: "codex" },
    },
  },
];
...
export function RemoteAgentCard() {
  ...
  const setupCmd = `npx -y screener@latest agent setup ${target.id}`;
  ...
  const aiPrompt = `set up screener on the machine where my "${name}" agent runs (ssh into it if it's remote). run this there:

  ${setupCmd}

that installs the screener MCP server${hasSkills ? " + skill" : ""} so ${name} can search my screen + audio history. then restart ${name}. if screener runs on a different host than ${name}, append \`--api-url http://<that-host>:3030\`.`;
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-md border border-border bg-muted/40 p-3 space-y-3">
        <p className="text-xs text-muted-foreground leading-relaxed">
          <span className="text-foreground font-medium">Set screener up where {name} runs</span>{" "}
        ...
        <p className="text-[11px] text-muted-foreground leading-relaxed">
          then push your screener data to that box via the{" "}
