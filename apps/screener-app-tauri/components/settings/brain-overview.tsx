...
import { qualifiedValue, onboardingFunnel } from "@/lib/analytics";
import {
  AlertCircle,
  Check,
  CheckCircle2,
                ...
                body: JSON.stringify({
                  run_context: {
                    ...
                    instruction: requestedSlots
                      ? "Refresh only the listed structured output targets for the supplied time_range. Call structured_output get_targets first, query only source-backed Screener APIs inside that exact time window, and submit an evidence-backed value for each listed target before completing."
                      : "Refresh every structured output target assigned to this pipe for the supplied time_range. Call structured_output get_targets first, query only source-backed Screener APIs inside that exact time window, and submit every valid target before completing.",
                  },
                }),
              },
            );
  ...
  const runLiveViewBuilderAgentInline = async ({
    ...
    try {
      const generated = await generateLiveViewWithPi({
        ...
        preset: selectedAiPreset,
        userToken:
          selectedAiPreset.provider === "screener-cloud"
