...
export function PrivacySection() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Screener has been restarted with new settings",
      });
    } catch (error) {
  ...
  //
  //   usePiiRemoval            → hot-path regex (screener-core)
  //   asyncPiiRedaction        → text reconciliation worker (screener-redact)
  ...
  // screener_redact::parse_allow_list); the rest are opt-in. Stored
  ...
  // (see `RedactColumns` in screener-redact). The core keys are always
    ...
    try {
      ...
      if (
        ...
        toast({
          ...
          description: "allow screener to control your browser in macOS Automation settings",
          variant: "destructive",
        });
        return;
      }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                <h3 className="text-sm font-medium text-foreground flex items-center gap-1.5">
                  Capture clipboard
                  <HelpTooltip text="when on, screener records clipboard copy/paste events and contents. turn off if you ship ~/.screener to a remote LLM or share it — passwords, API keys, and private keys frequently pass through the clipboard." />
                </h3>
                <p className="text-xs text-muted-foreground">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                <h3 className="text-sm font-medium text-foreground flex items-center gap-1.5">
                  Capture clicks
                  <HelpTooltip text="when on, screener records mouse click events (where and what you clicked). on by default — clicks carry no text payload and power workflow analysis and task mining. turning this off only skips the click rows; clicks still trigger screen captures." />
                </h3>
                <p className="text-xs text-muted-foreground">
          ...
          <CardContent className="px-3 py-2.5">
            <div className="flex items-center space-x-2.5">
              ...
              <h3 className="text-sm font-medium text-foreground flex items-center gap-1.5">
                ...
                <HelpTooltip text="macOS permission that lets screener capture keystrokes and mouse clicks. without it, capture runs in reduced mode — clipboard and app/window switches still work, but keyboard and click recording is dropped." />
              </h3>
            </div>
            ...
            {aiPiiRemovalEnabled && (
              <div className="mt-3 ml-6 space-y-2 border-l-2 border-border pl-3">
                ...
                <p className="text-[11px] text-muted-foreground">
                  ...
                  hardware. Cloud uses screener&apos;s attested
 ...
 * `read:search` / `read:records` scopes that the `screener-team` pi
 * skill calls v1 endpoints with. An admin mints one at
 * https://screener.com/enterprise?tab=tokens, pastes it here, and the
 * desktop persists it to ~/.screener/enterprise.json. Every new pi
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                ...
                <button
                  ...
                  onClick={() =>
                    openUrl(screenerWebUrl("/enterprise?tab=tokens", "https://screener.com"))
                  }
                >
                  screener.com/enterprise → api tokens
                </button>
                .
              </p>
            </div>
