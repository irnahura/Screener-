...
// Auth + tool calls run through screener's server (screener.com/api/composio),
  ...
  const connect = async (alias?: string) => {
    ...
    try {
      ...
      const msg = e?.message === "Load failed" || e?.name === "TypeError"
        ? "couldn't reach screener.com — check your internet connection and try again"
  ...
  if (!token) {
    return (
      <div className="space-y-3">
        {valueLine}
        <p className="text-xs text-muted-foreground">log in to your screener account to connect {label}.</p>
        {privacyNote}
      </div>
    );
  }
