    ...
    const checkAndRequestPermission = async () => {
      ...
      if (permission) {
        try {
          ...
          if (!welcomeShown) {
            sendNotification({
              title: "welcome to screener",
              body: "thank you for using screener! we're dedicated to help you get the most out of screener.",
            });
    ...
    const unlisten = listen<string>("native-notification-action", async (event) => {
      ...
      try {
        ...
        if (action.type === "mute" && action.pipe_name) {
          const localforage = (await import("localforage")).default;
          const raw = await localforage.getItem<string>("screener-settings");
          ...
          await localforage.setItem("screener-settings", JSON.stringify(settings));
          return;
        }
        ...
        if (action.type === "source") {
          const url =
            ...
            action.deeplinkUrl;
          if (!url) return;
          if (typeof url === "string" && url.startsWith("screener://")) {
        ...
        if (action.type === "api" && action.url) {
          ...
          if (action.action === "record-hd" && res.ok) {
            ...
            if (typeof noteUrl === "string" && noteUrl.startsWith("screener://")) {
        ...
        if (action.type === "meeting_join" && action.url) {
          ...
          if (typeof deeplink === "string" && deeplink.startsWith("screener://")) {
        ...
        //   "deeplink"  — screener:// in-app route
        //
        ...
        if ((action.type === "link" || action.type === "deeplink") && action.url) {
          if (typeof action.url === "string" && action.url.startsWith("screener://")) {
        ...
        if (action.action === "open_timeline") {
          ...
          try {
            try { await commands.stopScreener(); } catch {}
