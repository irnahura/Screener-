...
export function StandaloneChat({
  ...
  const [showConnectBanner, setShowConnectBanner] = useState(() => {
    try { return localStorage.getItem("screener_connect_banner_dismissed") !== "true"; } catch { return true; }
  });
  const {
    allConnectionItems,
  ...
  const toggleBrowserPanel = useCallback(() => {
    ...
    if (activeSideView === "side" && browserPanelState.hasUrl) {
      window.dispatchEvent(
        new CustomEvent("screener:browser-sidebar-toggle", {
  ...
  // Chat conversations — stored as individual JSON files in ~/.screener/chats/
  const {
    showHistory,
    setShowHistory,
    historySearch,
  ...
  useEffect(() => {
    ...
    const unlisten = listen<AgentEventEnvelope>(AGENT_TOPICS.event, (event) => {
      ...
      if (!title.startsWith("screener:connect:")) return;
  ...
  // ACP connect broker: an agent's screener_connect_app tool blocks on
  ...
  const handleAgentActionEvent = useCallback((value: unknown, sessionId: string): boolean => {
    const trace = (stage: string, details?: Record<string, unknown>) => {
      if (process.env.NEXT_PUBLIC_SCREENER_E2E !== "true" || typeof window === "undefined") return;
  ...
  const acpDefaultPresetLabel =
    settings?.aiPresets?.find((preset) => preset.defaultPreset)?.id ?? "screener-cloud";
  ...
  return (
    <div ref={dropRootRef} className={cn("flex flex-col bg-background", className ?? "h-screen")} data-testid="section-home">
      ...
      <ChatComposer
        ...
        connectBanner={{
          ...
          onDismiss: () => {
            ...
            try { localStorage.setItem("screener_connect_banner_dismissed", "true"); } catch {}
          },
        }}
        onStop={handleStop}
      />
      ...
      <BrowserSidebar
        ...
        // navigation `owner` via x-screener-session). Lets the sidebar reveal
