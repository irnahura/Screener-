...
//      (~/.cache/screener/frontend-out/<hash>) -> copy it into out/. No build.
...
// this machine. Override with SCREENER_FRONTEND_CACHE_DIR (set to '' or
// 'off' to disable caching entirely).
const cacheEnv = process.env.SCREENER_FRONTEND_CACHE_DIR
const cacheRoot =
	cacheEnv === '' || cacheEnv === 'off'
		? null
		: cacheEnv ||
			path.join(process.env.XDG_CACHE_HOME || path.join(os.homedir(), '.cache'), 'screener', 'frontend-out')
...
// changed: rebaking `NEXT_PUBLIC_SCREENER_WEB_URL` with untouched sources
...
async function main() {
	const forced = ['1', 'true'].includes(String(process.env.SCREENER_FORCE_FRONTEND_BUILD).toLowerCase())
	...
	console.log(
		forced
			? '[build-frontend] SCREENER_FORCE_FRONTEND_BUILD set — building unconditionally'
