...
const port = process.env.SCREENER_WEB_API_PORT ?? "3030";
const scenario = process.env.SCREENER_WEB_SCENARIO ?? "ready";
const apiKey = process.env.SCREENER_LOCAL_API_KEY ?? "";

if (mode === "live" && !apiKey) {
  console.warn(
    "[web dev] SCREENER_LOCAL_API_KEY is unset; authenticated engine routes may return 401/403.",
  );
}
...
const child = Bun.spawn(
  ["bun", "x", "next", "dev", "-H", "127.0.0.1", "-p", "1420"],
  {
    ...
    env: {
      ...process.env,
      NEXT_PUBLIC_SCREENER_WEB_DEV: mode,
      NEXT_PUBLIC_SCREENER_WEB_SCENARIO: scenario,
      NEXT_PUBLIC_SCREENER_WEB_API_PORT: port,
      NEXT_PUBLIC_SCREENER_WEB_API_KEY: mode === "live" ? apiKey : "",
    },
    stdin: "inherit",
    stdout: "inherit",
    stderr: "inherit",
