...
const config = {
	...
	macos: {
		...
		mlxMetallibUrl: 'https://github.com/screenpipe/screener/releases/download/mlx-metallib-v0.2.0/mlx.metallib',
	},
}
...
async function copyBunBinary() {
	...
	if (platform === 'windows') {
		...
		const bunVersion = '1.3.10';
		const releaseTarget = process.env.SCREENER_RELEASE_TARGET;
		...
		// In CI we set SCREENER_RELEASE_TARGET per-matrix-entry and only need
		...
		if (wanted.length === 0) {
			throw new Error(`unknown SCREENER_RELEASE_TARGET for macOS: ${releaseTarget}`);
		}
...
if (!process.env.GITHUB_ENV) {
	console.log('\nCommands to build 🔨:')
	// Get relative path to screener-app-tauri folder
