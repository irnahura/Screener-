...
async function getChatsDir(): Promise<string> {
  ...
  // Resolve under the active data dir (honors SCREENER_DATA_DIR / a relocated
  ...
  _chatsDir = await join(home, ".screener", "chats");
  return _chatsDir;
}
...
export async function migrateFromStoreBin(): Promise<void> {
  try {
    ...
    console.log(
      `[chat-storage] migrating ${history.conversations.length} conversations from store.bin to ~/.screener/chats/`
    );
