...
export interface AnnouncementCta {
   ...
   *  consent surface as a `screener://install-pipe` deep link, showing
...
export const DISMISSED_STORAGE_KEY = "screener-announcements-dismissed-v1";
...
export const PREVIEW_STORAGE_KEY = "screener-announcement-preview";
...
export function areRemoteAnnouncementsDisabled(
  ...
  return (
    ...
    env.NEXT_PUBLIC_SCREENER_E2E === "true" ||
    env.NEXT_PUBLIC_SCREENER_DISABLE_REMOTE_ANNOUNCEMENTS === "true" ||
...
export function safeAnnouncementExternalUrl(raw: unknown): string | null {
  ...
  try {
    ...
    const screenerGithub =
      host === "github.com" &&
      (url.pathname === "/screener" ||
        url.pathname.startsWith("/screener/"));
    return firstParty || screenerGithub ? url.toString() : null;
  } catch {
    return null;
  }
}
