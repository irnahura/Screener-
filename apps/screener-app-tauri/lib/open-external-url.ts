...
export async function openExternalUrl(url: string): Promise<void> {
  const e2eWindow =
    typeof window !== "undefined"
      ? (window as Window & {
          __SCREENER_E2E_OPEN_URLS?: string[];
          __SCREENER_E2E_INTERCEPT_OPEN_URLS?: boolean;
        })
      : null;

  if (Array.isArray(e2eWindow?.__SCREENER_E2E_OPEN_URLS)) {
    e2eWindow.__SCREENER_E2E_OPEN_URLS.push(url);
  }
  if (e2eWindow?.__SCREENER_E2E_INTERCEPT_OPEN_URLS) return;
  await openUrl(url);
}
