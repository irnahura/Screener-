...
/**
 * Single source of truth for local screener API access.
 *
 * All API calls to the screener backend MUST go through this module.
 * It handles:
...
function applyApiConfig(config: LocalApiConfig): void {
  ...
  if (_authEnabled && _apiKey && typeof document !== "undefined") {
    document.cookie = `screener_auth=${_apiKey}; path=/; SameSite=Strict`;
  } else if (!_authEnabled && typeof document !== "undefined") {
    document.cookie = "screener_auth=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
  }
}
...
/**
 * Get the base URL for the local screener API.
 */
...
/**
 * Fetch wrapper for the local screener API.
 *
