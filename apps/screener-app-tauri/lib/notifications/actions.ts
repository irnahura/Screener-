...
// Route a screener:// deeplink to the window that can handle it. Meeting
// deeplinks belong to the Home window's meetings page; everything else to Main.
export function isMeetingDeeplink(url: string) {
  return url.startsWith("screener://meeting/") ||
    url.startsWith("screener://meeting?");
}
...
export async function executeNotificationAction(
  ...
  switch (action.type) {
    ...
    case "api": {
      if (action.url) {
        // An `api` action calls back into the local screener API with the
    ...
    case "deeplink": {
      if (action.url) {
        if (action.url.startsWith("screener://")) {
    ...
    case "source": {
      const sourceUrl =
        ...
        ctx.sourceUrl;
      if (sourceUrl) {
        if (sourceUrl.startsWith("screener://")) {
