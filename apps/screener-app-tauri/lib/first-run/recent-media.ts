 ...
 * or has not caught up yet. Audio-named mp4s are excluded — screener writes
...
export async function fetchFirstRunMedia(
  ...
  try {
    const params = new URLSearchParams({
      // NOT `ocr`. screener captures the accessibility tree first and falls
...
export async function preserveFirstRunMedia(
  ...
  try {
    ...
    const dir = await join(await homeDir(), ".screener", PRESERVED_DIR);
