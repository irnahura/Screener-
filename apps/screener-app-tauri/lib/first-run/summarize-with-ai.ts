...
function buildSummaryPrompt(facts: string): string {
  return `You are screener. You have been recording this person's screen for a few minutes, right after they finished setup. Below is EVERYTHING you observed. Write the first thing you ever say to them about their own work.
...
export async function summarizeFirstRunWithAi(
  ...
  if (preset.provider === "screener-cloud" && !options.userToken) {
  ...
  try {
    ...
    const dir = await join(home, ".screener", SUMMARY_PROJECT_DIR);
