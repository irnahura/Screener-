...
export interface WatchAuthorizationStatusOptions {
  ...
  /**
   * Continue polling even when `document.hidden` is true (e.g. the screener
...
// ---------------------------------------------------------------------------
// Controller — Screener-facing wrappers around the permission-flow plugin
// ---------------------------------------------------------------------------

/**
 * Screener-facing alias: every OS permission we surface in onboarding,
...
function isScreenerAppBundle(
  appPath: string | null | undefined,
): appPath is string {
  if (!appPath || !appPath.endsWith(".app")) return false;
  return /screener/i.test(appPath);
}
...
async function getScreenerAppPath(): Promise<string | null> {
  if (state.appPath !== undefined) return state.appPath;
  const appPath = await suggestedHostAppPath();
  state.appPath = isScreenerAppBundle(appPath) ? appPath : null;
...
async function restoreAlwaysOnTopWhenScreenerRefocuses(): Promise<void> {
  ...
  try {
    ...
    const unlisten = await window.onFocusChanged(async ({ payload: focused }) => {
      ...
      // Hide the drag panel when screener is frontmost — it should only
...
async function letSystemSettingsComeForward(): Promise<void> {
  try {
    ...
    await restoreAlwaysOnTopWhenScreenerRefocuses();
  } catch {
...
async function reclaimScreenerWindow(): Promise<void> {
  if (state.pendingFocusUnlisten) {
    try { state.pendingFocusUnlisten(); } catch {}
    state.pendingFocusUnlisten = null;
  }
  ...
  try {
    ...
    console.error(
      "failed to refocus screener window after permission flow:",
      error,
    );
  }
}
...
export async function requestPermissionWithFlow(
  ...
  const appPath = dragPermission ? await getScreenerAppPath() : null;
