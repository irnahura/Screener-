...
//! in-app file viewer.
//!
//! screener captures the user's entire screen/audio history, and the HTML
...
export const HUMAN_RENDER_MARKER =
  /<!--\s*screener:render\s*=\s*(?:human|html)\s*-->/i;

/**
 * Meta form: `<meta name="screener:render" content="human">`. Survives HTML
 * minifiers that strip comments; attribute order is not significant.
 */
function hasMetaRenderMarker(text: string): boolean {
  const tag = text.match(/<meta\b[^>]*\bscreener:render\b[^>]*>/i);
