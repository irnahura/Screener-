...
// Paths mirror the CLI's layout() in crates/screener-engine/src/cli/agent.rs.
...
export async function areExternalAgentSkillsInstalled(
  ...
  try {
    await Promise.all([
      readTextFile(await join(skillsRoot, "screener-api", "SKILL.md")),
      readTextFile(await join(skillsRoot, "screener-cli", "SKILL.md")),
    ]);
    return true;
  } catch {
    return false;
