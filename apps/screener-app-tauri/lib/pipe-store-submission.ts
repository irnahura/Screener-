...
export function buildPipeStoreSubmissionMailto({
  ...
  const subject = label
    ? `Screener Store ${action}: ${label}`
    : `Screener Store ${action}`;

  const body = [
    "Hi Louis,",
    "",
    kind === "update"
      ? "I'd like to submit an update to a scheduled task in the Screener Store."
      : "I'd like to submit a scheduled task to the Screener Store.",
