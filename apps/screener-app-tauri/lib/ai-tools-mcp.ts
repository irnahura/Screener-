...
// now owned by native Rust in crates/screener-engine/src/cli/agent.rs; this
...
async function resolveLocalApiKeyForMcp(): Promise<string | undefined> {
  ...
  if (!cfg?.key && cfg?.auth_enabled !== false) {
    ...
    if (!cfg?.key && cfg?.auth_enabled !== false) {
      throw new Error(
        "screener's local API key isn't available yet (engine still starting?) — try connecting again in a moment"
      );
    }
  }
...
/**
 * MCP install config for screener. Prefers the `bun` binary we ship with the
...
export async function buildMcpConfig(opts?: {
  ...
  const env: Record<string, string> = {
    SCREENER_MCP_CLIENT: opts?.client ?? "unknown",
    ...(apiKey ? { SCREENER_LOCAL_API_KEY: apiKey } : {}),
  };

  if (opts?.forceNpx) return { command: "npx", args: ["-y", "screener-mcp@latest"], env };
  try {
    const res = await commands.bunCheck();
    if (res.status === "ok" && res.data.available && res.data.path) {
      return { command: res.data.path, args: ["x", "screener-mcp@latest"], env };
    }
  ...
  return { command: "npx", args: ["-y", "screener-mcp@latest"], env };
}
...
async function readJsonConfigStrict(configPath: string): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    throw new Error(
      `${configPath} is not valid JSON (${e instanceof Error ? e.message : e}) — fix or remove it; screener won't overwrite it`
    );
  }
}
...
async function backupConfigIfExists(configPath: string): Promise<void> {
  ...
  await copyFile(configPath, `${configPath}.screener-backup-${ts}`);
  ...
  try {
    ...
    const backups = entries
      .map((e) => e.name)
      .filter((n): n is string => !!n && n.startsWith(`${base}.screener-backup-`))
      .sort();
...
/**
 * Delete only mcpServers.screener. Missing file / no entry is a no-op;
 * an invalid file throws so disconnect shows an honest per-tool error.
 */
async function removeScreenerFromJsonConfig(configPath: string): Promise<void> {
  const config = await readJsonConfigStrict(configPath);
  const servers = config.mcpServers as Record<string, unknown> | undefined;
  if (!servers?.screener) return;
  delete servers.screener;
  await replaceConfig(configPath, JSON.stringify(config, null, 2));
}
...
export async function installClaudeMcp(): Promise<McpCommand> {
  ...
  (config.mcpServers as Record<string, unknown>).screener = mcp;
...
export function buildCodexMcpToml(config: McpCommand): string {
  ...
  if (envEntries.length > 0) {
    lines.push("", "[mcp_servers.screener.env]");
...
export async function installCodexMcp(): Promise<McpCommand> {
  ...
  const withoutScreener = removeCodexMcpConfig(existing);
  const next = `${withoutScreener}${withoutScreener ? "\n\n" : ""}${buildCodexMcpToml(config)}\n`;
...
export async function uninstallClaudeMcp(): Promise<void> {
  ...
  await removeScreenerFromJsonConfig(configPath);
}

export async function uninstallCursorMcp(): Promise<void> {
  await removeScreenerFromJsonConfig(await getCursorMcpConfigPath());
}

const CODEX_SCREENER_TABLE = /(?:^|\n)\[mcp_servers\.screener\][\s\S]*?(?=\n\[(?!mcp_servers\.screener(?:\.|\]))[^\]]+\]|\s*$)/;

export function removeCodexMcpConfig(content: string): string {
  return content
    .replace(CODEX_SCREENER_TABLE, "")
    .replace(/^\n+/, "")
    .replace(/\n{3,}/g, "\n\n")
    .trimEnd();
}
...
export async function isOpenclawMcpInstalled(): Promise<boolean> {
  try {
    ...
    return !!JSON.parse(content)?.mcpServers?.screener;
  } catch { return false; }
}
...
export async function installOpenclawMcp(): Promise<McpCommand> {
  ...
  // and only set mcpServers.screener.
  const config = await readJsonConfigStrict(configPath);
  const mcp = await buildMcpConfig({ client: "openclaw" });
  if (!config.mcpServers || typeof config.mcpServers !== "object") config.mcpServers = {};
  (config.mcpServers as Record<string, unknown>).screener = { ...mcp, transport: "stdio" };
  await writeJsonConfig(configPath, config);
  return mcp;
}

export async function uninstallOpenclawMcp(): Promise<void> {
  await removeScreenerFromJsonConfig(await getOpenclawMcpConfigPath());
}
...
function hermesHasScreener(content: string): boolean {
  return content.split("\n").some(
    (l) =>
      !l.trimStart().startsWith("#") &&
      (/^\s+screener:/.test(l) || l.includes("screener-mcp"))
  );
}

export async function isHermesMcpInstalled(): Promise<boolean> {
  try {
    return hermesHasScreener(await readTextFile(await getHermesConfigPath()));
  } catch { return false; }
}
...
export async function installHermesMcp(): Promise<McpCommand> {
  ...
  if (HERMES_MCP_BLOCK.test(existing)) {
    ...
    // silent fake success.
    throw new Error(
      "~/.hermes/config.yaml already has an mcp_servers block — add the screener server manually"
    );
  }
...
export async function uninstallHermesMcp(): Promise<void> {
  ...
  // plus its indented children — but only when screener is its sole child.
  ...
  const onlyScreener = topLevelChildren.length === 1 && topLevelChildren[0] === "screener";
  const blockText = lines.slice(start, end).join("\n");
  if (!onlyScreener || !blockText.includes("screener-mcp")) {
    throw new Error(
      "~/.hermes/config.yaml has a customized mcp_servers block — remove the screener entry manually"
    );
  }
...
export async function uninstallWindsurfMcp(): Promise<void> {
  await removeScreenerFromJsonConfig(await getWindsurfMcpConfigPath());
}
...
export async function isRunnerMcpInstalled(): Promise<boolean> {
  try {
    ...
    return JSON.parse(content)?.mcpServers?.screener?.type === "stdio";
  } catch { return false; }
}
...
export async function installRunnerMcp(): Promise<McpCommand> {
  ...
  if (!config.mcpServers || typeof config.mcpServers !== "object") config.mcpServers = {};
  (config.mcpServers as Record<string, unknown>).screener = { type: "stdio", ...mcp };
  await writeJsonConfig(configPath, config);
  return mcp;
}

export async function uninstallRunnerMcp(): Promise<void> {
  await removeScreenerFromJsonConfig(await getRunnerMcpConfigPath());
}
...
export function friendlyToolError(err: unknown): FriendlyToolError {
  ...
  if (detail.includes("mcp_servers block")) {
    return { message: "config has a custom mcp_servers block — add screener there manually", path, detail };
  }
  if (detail.includes("local API key isn't available")) {
    ...
    return { message: "screener isn't responding — give it a few seconds and try again", detail };
  }
  ...
  try {
    switch (id) {
      ...
      case "hermes": {
        ...
        // Our one refusal: a hand-authored mcp_servers block without screener.
        return hermesHasScreener(text) || !HERMES_MCP_BLOCK.test(text);
      }
    }
  } catch {
    return false;
