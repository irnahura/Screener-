...
export type AecMode = "off" | "screener" | "macos" | "windows";
...
const AEC_MODES = new Set<AecMode>(["off", "screener", "macos", "windows"]);
...
export function getAecModeSettings(mode: AecMode) {
  return {
    aecMode: mode,
    screenerAecEnabled: mode === "screener",
...
export function buildDesktopRemoteControlPatch(
  ...
  if (
    ...
    Boolean(settings.screenerAecEnabled) !==
      aecSettings.screenerAecEnabled ||
