...
import { PROD_WEB_BASE, screenerWebBase } from "@/lib/web-url";

/** Overall deadline against the production control plane / public internet. */
export const TAURI_FETCH_TIMEOUT_MS = 30_000;
/**
 * Overall deadline when NEXT_PUBLIC_SCREENER_WEB_URL points at a control
 ...
 * (`https://screener.corp.example.com`) is production, not a dev server —
...
export function isLocalControlPlaneBase(): boolean {
  const base = screenerWebBase(PROD_WEB_BASE);
