...
export interface SourceCitation {
  ...
  // local (screener) citation, the footer makes the row jump into the
...
function sourceCitationsFromToolCall(toolCall: ToolCallLike | undefined): SourceCitation[] {
  ...
  // Screener timeline just because its bare name collides with a screener
  ...
  const isForeignMcp = mcpServer !== null && mcpServer !== "screener";
  ...
  if (toolName === "screener_search") {
    return [screenerToolCitation(args)];
  }

  // screener MCP tools (ACP agents surface them as mcp__screener__<name>,
...
function screenerToolCitation(args: Record<string, unknown>): SourceCitation {
  ...
  return {
    id: stableId(["screener-search-tool", contentType, appName, query, range]),
    kind: "screener",
...
function bashCitations(command: string, resultText: string): SourceCitation[] {
  ...
  for (const call of extractScreenerApiCalls(command)) {
    citations.push(screenerApiCitation(call));
  }

  for (const link of extractWebLinks(command)) {
    if (isLocalScreenerUrl(link.url)) continue;
    citations.push({
      id: stableId(["web", link.url]),
      kind: "web",
...
function screenerApiCitation(call: string): SourceCitation {
  const path = extractPath(call);
  const query = extractQuery(call);
  const title = screenerTitle(path);
  const kind = screenerKind(path);
  ...
  const timestamp = kind === "screener" ? navTimestamp(params.start_time) : undefined;
  const searchText = path === "/search" ? params.q : undefined;

  return {
    id: stableId(["screener", path, query]),
    kind,
    title,
    subtitle: screenerSubtitle(path, query),
    timestamp,
    query: searchText,
  };
}

function screenerKind(path: string): SourceCitationKind {
...
function screenerTitle(path: string): string {
  if (path === "/search") return "Screener search";
  if (path === "/activity-summary") return "Activity summary";
  if (path === "/raw_sql") return "Local database query";
  if (path === "/memories") return "Screener memories";
  ...
  if (path === "/health") return "Screener health";
  if (path.startsWith("/connections/")) return `${titleCase(path.split("/")[2] ?? "connector")} connection`;
  return "Screener API";
}

function screenerSubtitle(path: string, query: string): string | undefined {
  const params = queryParams(query);
  if (path === "/search") {
    const parts = [
  ...
  if (path.startsWith("/connections/perplexity/")) {
    return "external web context via Screener connection";
  }
...
function fileKind(path: string): SourceCitationKind {
  ...
  if (path.includes("/.screener/pipes/")) return "pipe";
  if (path.includes("/.screener/chats/")) return "screener";
  return "file";
}
...
function normalizeToolName(toolName: string): string {
  return toolName
    ...
    // mcp__screener__search-content) or a human title ("Grep"); strip the MCP
...
function extractScreenerApiCalls(command: string): string[] {
...
function isSourceKind(kind: string): kind is SourceCitationKind {
  return ["screener", "database", "connector", "web", "file", "memory", "pipe", "command"].includes(kind);
}
...
function isLocalScreenerUrl(url: string): boolean {
  try {
