...
function normalizeManagedPreset(value: unknown, index: number): EnterpriseManagedAiPreset | null {
  ...
  const model =
    provider === "screener-cloud"
      ? stringValue(value.model, "screener-cloud")
  ...
  return {
    ...
    provider,
    url: provider === "screener-cloud" ? "" : stringValue(value.url),
    model,
    api_key:
      provider === "screener-cloud" || provider === "native-ollama" || provider === "openai-chatgpt"
        ? ""
...
export function normalizeEnterpriseAiPresetPolicy(value: unknown): EnterpriseAiPresetPolicy {
  ...
  const allowScreenerCloud = value.allow_screener_cloud !== false;
  ...
  const validDefaults = new Set([
    ...(allowScreenerCloud ? ["screener-cloud"] : []),
    ...managedPresets.map((preset) => preset.id),
  ]);

  return {
    version: 2,
    allow_screener_cloud: allowScreenerCloud,
    ...
    default_preset_id: validDefaults.has(defaultPresetId)
      ? defaultPresetId
      : managedPresets[0]?.id ?? (allowScreenerCloud ? "screener-cloud" : null),
...
export function isPresetAllowedByEnterprisePolicy(
  ...
  if (preset.provider === "screener-cloud") return policy.allow_screener_cloud;
