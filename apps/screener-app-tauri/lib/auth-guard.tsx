...
import { PROD_WEB_BASE, screenerWebBase, screenerWebUrl } from "@/lib/web-url";
...
function openLogin() {
  ...
  const loginUrl = screenerWebUrl("/login", "https://screener.com");
...
function showSignedOutToast() {
  ...
  toast({
    ...
    duration: 30000,
    action: (
      <ToastAction altText="Sign in to screener" onClick={openLogin}>
        sign in
      </ToastAction>
    ),
  });
...
// True for any screener CLOUD host (screenpi.pe / screener.com and their
// subdomains).
//
// Match on the URL *host* — never a substring of the whole URL. The local
// engine at localhost:3030 routinely carries a screener-domain value in the
...
// connected-account email is @screenpi.pe / @screener.com.
export function isScreenerApi(url: string): boolean {
...
// website auth surface (screener.com/api/user and the OAuth/session
...
// NEXT_PUBLIC_SCREENER_WEB_URL repoints the app (baked enterprise build,
// Vercel preview, local control plane), THAT deployment minted the token and
// only its 401s may clear it — a 401 from prod screener.com then just means
...
//
// The AI inference gateway `api.screener.com` is EXCLUDED: it is fail-open on
...
// inference subdomain.
export function isScreenerAuthApi(
  url: string,
  webBase: string = screenerWebBase(PROD_WEB_BASE)
): boolean {
  ...
  if (!isScreenerApi(url)) return false;
  const host = cloudRequestHost(url);
  return host !== "api.screener.com" && host !== "api.screenpi.pe";
}
...
// Patches window.fetch once to detect 401s from screener API calls.
...
export function installAuthInterceptor(
  ...
  window.fetch = async function patchedFetch(
    ...
    if (isScreenerAuthApi(url) && res.status === 401) {
