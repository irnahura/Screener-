...
export function useDeviceMonitor() {
  ...
  // Auto-discover screener instances on the network
  ...
  const discoverDevices = useCallback(async () => {
    ...
    try {
      ...
      // Probe each host on port 3030 to find screener instances
      const PROBE_TIMEOUT_MS = 3_000;
      const seenHostnames = new Set<string>();
      if (localHostname) seenHostnames.add(localHostname);

      // Try common screener ports
