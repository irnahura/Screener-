 ...
 * Writes managed pipe.md files to ~/.screener/pipes/{name}/
...
async function enterprisePipesDir(): Promise<string> {
  try {
    ...
    console.warn(
      "[enterprise-pipes] failed to resolve active data dir, using ~/.screener:",
      result.error,
    );
  } catch (error) {
    console.warn(
      "[enterprise-pipes] active data dir command unavailable, using ~/.screener:",
      error,
    );
  }

  return await join(await homeDir(), ".screener", "pipes");
}
...
/**
 * Gather pipe execution statuses from the local screener API.
