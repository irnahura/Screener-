...
async function getCacheDir(): Promise<string> {
  ...
  if (res.status === "ok") {
    ...
    const baseRes = await commands.getScreenerBaseDir();
    if (baseRes.status === "ok") {
      baseDir = baseRes.data;
    } else {
      // last resort
      const { homeDir } = await import("@tauri-apps/api/path");
      baseDir = await join(await homeDir(), ".screener");
    }
  }
 ...
 * Cache lives inside the screener data dir (~/.screener/cache/).
 */
