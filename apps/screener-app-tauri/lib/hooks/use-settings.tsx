...
import { captureSettingsChange } from "@/lib/analytics";
...
export type Settings = SettingsStore & {
	...
	/** Where the user says they found screener, answered once during setup.
	...
	meetingLiveTranscriptionProvider?: "selected-engine" | "screener-cloud" | "disabled" | "deepgram-live";
	 ...
	 * video frames, and images from screener data. Default true. When
	 * false, the "Cloud audio + video + image analysis" section is
	 * stripped from `~/.claude/skills/screener-api/SKILL.md` so agents
	...
	/** Hide screener windows from screenshots and screen-sharing viewers while
	...
	/** Global blanket permission: allow screener to copy browser cookies
...
// Default screener-cloud presets on first install — every seed runs on
...
// Non-pro users get a single "screener" preset on auto — auto handles
// model routing without needing the user to know what to pick.
const SCREENER_PRESET_ID = "screener";
...
// every paying user — not just Cloud Sync subscribers — gets Screener Cloud
...
const applyProCloudAudioDefaults = (settings: Settings): Settings => {
	...
	const userChoseCustomEngine =
		...
		settings.audioTranscriptionEngine !== "screener-cloud";
	...
	settings.meetingLiveTranscriptionProvider = "screener-cloud";
...
let DEFAULT_SETTINGS: Settings = {
			...
			// in screener-core (emails, phone numbers, SSNs, card numbers, API
		...
		updateChannel: "stable",
			...
			showScreenerShortcut: "Control+Super+S",
...
export function createDefaultSettingsObject(): Settings {
	try {
		...
		DEFAULT_SETTINGS.showScreenerShortcut = p === "windows" ? "Alt+S" : "Control+Super+S";
...
async function waitForE2eSettingsWriteDelay(): Promise<void> {
	if (
		process.env.NEXT_PUBLIC_SCREENER_E2E !== "true" ||
...
export const getStore = async () => {
	if (!_store) {
		_store = (async () => {
			if (!isTauriRuntime()) {
				const key = "screener-web-settings";
				...
				return {
					async get<T>(name: string) {
						return (values[name] as T | undefined) ?? null;
					},
			...
			// store.bin as Rust (get_base_dir honors SCREENER_DATA_DIR); a
			// hardcoded ~/.screener here splits the settings store in two
			// whenever that override is set.
			let baseDir: string | null = null;
			try {
				const res = await commands.getScreenerBaseDir();
				if (res.status === "ok") {
					baseDir = res.data;
				} else {
					console.warn("get_screener_base_dir failed, using ~/.screener:", res.error);
				}
			} catch (e) {
				console.warn("get_screener_base_dir unavailable, using ~/.screener:", e);
			}
			if (!baseDir) {
				baseDir = `${await homeDir()}/.screener`;
			}
 ...
 *
 * Enterprise and consumer installs share `~/.screener`, so a machine that ran
...
function createSettingsStore() {
	const get = async (): Promise<Settings> => {
		...
		// "screener" placeholder with the pro pair (chat + pipes) IF they're pro.
		// Anonymous users keep the placeholder forever (which is correct — non-pro
		// stays on the single "screener" auto preset). Existing users with their
		...
		if (!(settings as any)._presetsSeededForUser && settings.user?.token) {
			...
			const isAnonymousPlaceholder =
				presets.length === 1 &&
				(presets[0] as any)?.id === SCREENER_PRESET_ID &&
				(presets[0] as any)?.provider === "screener-cloud";
		...
		// Migration: Rename "pi" provider to "screener-cloud" for clarity
		if (settings.aiPresets?.some((p: any) => p.provider === "pi")) {
			settings.aiPresets = settings.aiPresets.map((p: any) =>
				p.provider === "pi" ? { ...p, provider: "screener-cloud" } : p
			);
			needsUpdate = true;
		}

		// Migration: Rename "pi-agent" preset id to "screener-cloud"
		if (settings.aiPresets?.some((p: any) => p.id === "pi-agent")) {
			settings.aiPresets = settings.aiPresets.map((p: any) =>
				p.id === "pi-agent" ? { ...p, id: "screener-cloud" } : p
			);
			needsUpdate = true;
		}

		// Migration: Add screener-cloud preset for existing users (without touching their existing presets)
		const hasCloudPreset = settings.aiPresets?.some(
			(p: any) => p.id === "screener-cloud" || p.provider === "screener-cloud"
		);
		...
		// Does NOT set screener-cloud here because user may not be logged in yet.
		...
		if (!(settings as any)._parakeetDefaultMigrationDone) {
			...
			if (isWhisperVariant || engine === "screener-cloud" || engine === "parakeet") {
		...
		// both background and live transcription to Screener Cloud. The marker
		...
		//   - only if provider is still screener-cloud and model is exactly the seeded
...
export const SettingsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
	...
	useEffect(() => {
		const loadSettings = async () => {
			try {
				...
				// such as SCREENER_PORT. Refresh after applying the persisted
	...
	// Install global fetch interceptor to catch 401s from screener.com
	...
	// Keep logout idempotent so repeated sign-out events cannot resurrect state.
	...
	useEffect(() => {
		...
		const unlistenPromise = listen("screener-auth-signout", () => {
	...
	useEffect(() => {
		...
		//  - the environment guard (CI / SCREENER_DISABLE_TELEMETRY), which is
	...
	const updateSettings = async (updates: Partial<Settings>) => {
		...
		// before it leaves — telemetry is intentionally disabled in the MVP.
		...
		if (clearsAccount) {
			...
			emit("screener-auth-signout").catch(() => {});
		}
	...
	const loadUser = async (token: string, verify = false) => {
		...
		try {
			const response = await fetch(
				screenerWebUrl("/api/user", "https://screener.com"),
				{
					method: "POST",
					headers: {
