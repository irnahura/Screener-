...
const API = screenerWebUrl("/api/team", "https://screener.com");

// key stored under "team_key_<team_id>" in ~/.screener/store.bin
...
export function useTeam() {
  ...
  const createInviteLink = useCallback(
    async (teamName: string, key: CryptoKey, forceNew = false) => {
      ...
      return screenerWebUrl(`/join/${tokenData.invite_token}#key=${encodeURIComponent(base64Key)}`, "https://screener.com");
    },
    [headers]
  );
