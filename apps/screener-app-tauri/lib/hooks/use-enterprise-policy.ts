...
interface EnterprisePolicy {
  ...
  orgName: string;
  /** Admin requires employees to sign in with their screener account —
...
const ACCOUNT_LOGIN_REQUIRED_ERROR =
  "your organization requires signing in with your screener account";
...
/**
 * Same check when NEXT_PUBLIC_SCREENER_WEB_URL points at a control plane on
...
export const E2E_ENTERPRISE_POLICY_KEY = "screener_e2e_enterprise_policy";
export const E2E_ENTERPRISE_HEARTBEAT_STATUS_KEY = "screener_e2e_enterprise_heartbeat_status";
export const E2E_ENTERPRISE_SKIP_SAVED_LICENSE_KEY =
  "screener_e2e_enterprise_skip_saved_license";

function enterpriseE2eMocksEnabled(): boolean {
  return process.env.NEXT_PUBLIC_SCREENER_E2E === "true" && typeof window !== "undefined";
}
...
function toLocalAiPreset(
  ...
  const provider = preset.provider === "screener-cloud" ? "screener-cloud" : preset.provider;
  return {
    id: preset.id,
...
async function applyManagedDeviceSettings(lockedSettings: Record<string, unknown>): Promise<void> {
  ...
  try {
    await withTimeout(
      "enterprise input capture stopScreener",
      commands.stopScreener(),
      ENGINE_RESTART_COMMAND_TIMEOUT_MS
    );
    await new Promise((resolve) => setTimeout(resolve, 1000));
    await withTimeout(
      "enterprise input capture spawnScreener",
      commands.spawnScreener(null),
      ENGINE_RESTART_COMMAND_TIMEOUT_MS
    );
  } finally {
...
async function sendHeartbeat(
  ...
  try {
    ...
    const res = await tauriFetchWithDeadline(
      screenerWebUrl("/api/enterprise/heartbeat", "https://screener.com"),
      {
        method: "POST",
        headers: {
          ...credentialHeaders,
...
export function useEnterprisePolicyRuntime() {
  ...
  const fetchPolicy = useCallback(async (
    ...
    try {
      ...
      // Fallback: read directly from ~/.screener/auth.json when the
      ...
      if (e2ePolicy.present) {
        ...
        const res = await tauriFetchWithDeadline(
          screenerWebUrl("/api/enterprise/policy", "https://screener.com"),
          {
            method: "GET",
            headers,
      ...
      if (result.aiPresetPolicy) {
        try {
          ...
          console.log(
            `[enterprise] applied AI preset policy: cloud=${result.aiPresetPolicy.allow_screener_cloud}, employee=${result.aiPresetPolicy.allow_employee_custom_presets}, managed=${result.aiPresetPolicy.managed_presets.length}`
          );
        } catch (e) {
      ...
      // Persist admin status into ~/.screener/enterprise.json so the
      // Enterprise app can decide whether to inject the screener-team
      ...
      if (cloudToken) {
        ...
        console.warn(
          "[enterprise] no cloud token available — skipping team-config persist (sign in to screener cloud to enable team queries)"
        );
      }
      return { ok: true, policy: result };
    } catch (e) {
  ...
  const startPolling = useCallback((credential: EnterpriseCredential) => {
    ...
    intervalRef.current = setInterval(async () => {
      ...
      if (result.ok) {
        ...
        // fall back to the signed-in screener account when one exists,
  ...
  const authenticateCredential = useCallback(async (
    ...
    if (result.ok) {
      ...
      // employee to sign in with their screener account. Device policy was
   ...
   * API, saves it to ~/.screener/enterprise.json, and starts fetching policy.
   */
  ...
  useEffect(() => {
    ...
    (async () => {
      // Read license key from enterprise.json (MDM or ~/.screener/)
