...
export async function getInstalledMcpVersion(): Promise<string | null> {
  try {
    ...
    return config?.mcpServers?.screener ? "installed" : null;
  } catch { return null; }
}

// The exact `args` arrays our own installers (and the README) have ever
// written for the screener entry. Auto-repair ONLY touches entries matching
// one of these verbatim — never a hand-customized entry.
const KNOWN_DEFAULT_MCP_ARGS: readonly string[][] = [
  ["-y", "screener-mcp@latest"], // npx (current)
  ["-y", "screener-mcp"],        // npx (older README snippet)
  ["x", "screener-mcp@latest"],  // bundled bun / bunx (current)
  ["x", "screener-mcp"],         // bundled bun / bunx (older)
];
...
// A screener entry is "stale" — written by an older build of ours or copied
...
// `--screener-url`, a custom `--port`, extra args, or any env key other than
// the one we manage — is NOT flagged, so auto-repair can never clobber a
// user's bespoke setup.
export function isStaleClaudeScreenerEntry(entry: unknown): boolean {
  ...
  const hasKey = !!e.env?.SCREENER_LOCAL_API_KEY;
  const hasClaudeClient = e.env?.SCREENER_MCP_CLIENT === "claude";
  return !hasKey || !hasClaudeClient;
}

// Read the currently-installed Claude screener entry (or null). Used to decide
// whether an auto-repair rewrite is warranted.
export async function getInstalledClaudeScreenerEntry(): Promise<unknown | null> {
  try {
    const configPath = await getClaudeConfigPath();
    if (!configPath) return null;
    const config = JSON.parse(await readTextFile(configPath));
    return config?.mcpServers?.screener ?? null;
  } catch { return null; }
}
...
export function hasEnabledCodexMcp(content: string): boolean {
  const table = content.match(CODEX_SCREENER_TABLE)?.[0] ?? "";
...
export function hasEnabledGrokMcp(content: string): boolean {
  try {
    ...
    return servers.some((s) => s?.id === "screener" && s?.enabled !== false);
  } catch { return false; }
}
