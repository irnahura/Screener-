    ...
    .map((pipe, index) => {
      ...
      return {
        ...
        candidate: {
          ...
          name: slug,
          title,
          description: `${title}. ${description || "Screener scheduled task"}`,
          category,
...
async function waitForServer(maxWaitMs = 20_000): Promise<void> {
  ...
  throw new OnboardingLiveViewSetupError(
    "server_not_ready",
    "planning",
    "Screener is still starting. Try again in a moment.",
  );
}
          ...
          `/pipes/${encodeURIComponent(pipeName)}/run`,
          {
            ...
            body: JSON.stringify({
              ...
              run_context: {
                ...
                instruction:
                  "Build this first dashboard from source-backed Screener data. Call structured_output get_targets first and submit every listed target that has enough evidence. Never invent a positive result when evidence is missing.",
              },
            }),
          },
        );
