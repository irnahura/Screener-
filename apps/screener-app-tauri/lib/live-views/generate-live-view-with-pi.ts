...
const PROPOSE_TOOL = "screener_live_view_propose";
...
/**
 * Reads the schema-validated `screener_live_view_propose` arguments. Shape
...
screener_live_view_propose({"operations":[{"op":"update","blockId":"focus-time","block":{"component":"table.v1","width":12,"intent":"Rows of app or project with focused minutes for the selected period, largest first, omitting apps with no recorded time."}}],"note":"Focus time becomes a per-app table instead of one total."})

Example. The user asks to add a missing outcome:
screener_live_view_propose({"operations":[{"op":"add","block":{"id":"meeting-followups","title":"Meeting follow-ups","intent":"Open commitments captured in meetings during the selected period, with who owes what, or an empty state when no meeting was recorded.","component":"list.v1","width":6,"pipeName":"meeting-summary"}}],"note":"Adds an open follow-ups list from meeting-summary."})`;
...
${JSON.stringify(
  options.pipes.map((pipe) => ({
    ...
    description: pipe.description.slice(0, 240),
      ...
      : `
Find scheduled tasks with screener_live_view action=pipes and a short query. Bind pipeName only to a name that search returned; use null when nothing fits.`;

  return `Design a Screener Live View change for the user to review, then submit it with screener_live_view_propose. The app applies nothing until the user accepts, so never call action=save.

${scopeInstruction}

Work in this order:
1. ${editing ? `Call screener_live_view action=values for ${JSON.stringify(options.currentViewRef?.id ?? "")} to see what the Blocks currently render. A Block shows its bound task's last payload, so an intent-only edit changes nothing the user can see.` : "Decide the outcomes the user wants to see."}
...
3. Call screener_live_view_propose once with the finished change. Fix and retry if it reports problems.
...
function providerConfig(preset: AIPreset): PiProviderConfig {
  ...
  return {
    ...
    allowedTools: ["screener_live_view", PROPOSE_TOOL],
  };
}

async function liveViewProjectDir(): Promise<string> {
  // Resolve beside the active screener data directory so relocated installs
  ...
  return await join(await homeDir(), ".screener", PROJECT_DIR);
}
...
async function runGeneration(
  ...
  const handler = (envelope: AgentEventEnvelope) => {
    ...
    if (event.type === "tool_execution_start") {
      ...
      if (event.toolName !== "screener_live_view") {
