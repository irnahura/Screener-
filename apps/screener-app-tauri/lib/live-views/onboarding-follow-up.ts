...
function liveViewDeepLink(viewId: string): string {
  return `screener://live-view/${encodeURIComponent(viewId)}`;
}
          ...
          `/pipes/${encodeURIComponent(pipeName)}/run`,
          {
            ...
            body: JSON.stringify({
              ...
              run_context: {
                ...
                instruction:
                  "Refresh this Live View with the latest source-backed Screener data. Call structured_output get_targets first and submit every listed target that has enough evidence. Never invent a positive result when evidence is missing.",
              },
            }),
          },
        );
