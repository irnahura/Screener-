...
const defaultPlansUrl = () => screenerWebUrl("/onboarding", "https://screenpi.pe");
...
/** Accept only Screener's HTTPS pricing/billing pages before opening. */
...
export function freeWallSheetSeenKey(resetsAt: string | null): string {
  return `screener:free-wall-sheet-seen:${resetsAt ?? "unknown"}`;
}
