...
const SAFETY_REFUSAL_MESSAGE =
  "The selected model declined this request because of its safety policy. This is not a screener outage. Start a new chat and revise the request with clear authorized context, or ask for high-level guidance.";
...
export function isHostedScreenerProvider(provider?: string | null): boolean {
  // screener's own hosted gateway (default chat preset + the Pi agent both
  // route through api.screener.com). A connection failure here is on us,
  // not the user's machine.
  return provider === "screener-cloud" || provider === "pi";
}

export function buildCloudConnectionMessage(): string {
  return "Can't reach screener cloud right now — this is usually a brief outage on our end, not your setup. Wait a few seconds and try again.";
}
...
export function buildContextOverflowMessage(errorStr = ""): string {
  ...
  if (requested && available) {
    if (available < MIN_AGENT_CONTEXT_TOKENS) {
      return `This provider exposes ${tokenFormatter.format(available)} context tokens, but Screener's agent request already needs ${tokenFormatter.format(requested)}. Increase the provider's context window (for example, Ollama num_ctx) to at least ${tokenFormatter.format(MIN_AGENT_CONTEXT_TOKENS)}, or choose a larger-context model, then retry in a new chat.`;
    }
...
export function buildAccountStandingMessage(errorStr: string): string | null {
  ...
  return "screener cloud AI is blocked for this account — it's flagged as not in good standing. Signing out and back in can refresh your account status. If you believe this is a mistake, contact screener support. Local models and your own provider keys keep working in Settings → AI.";
}
 ...
 * organization policy to be enabled"), which reads like a screener crash and
...
export function buildAgentRefusalMessage(
  ...
  if (mentionsPermission && mentionsPolicy) {
    return {
      ...
      retryable: false,
      message: `${agent} signed in fine, but its own service refused this account: it needs a plan or organization policy you don't have. Signing in again won't change that — it isn't a screener limit and screener can't grant it.\n\nWhat does work: switch this preset's model calls to Screener Cloud in Settings → AI presets, if this agent supports it, or pick a different agent. Ask whoever administers the account to enable it if you need this one specifically.`,
    };
  }
...
function buildGenericProviderErrorMessage(
  ...
  if (isHostedScreenerProvider(provider)) {
    ...
    if (normalized.includes("free_plan_hosted_background_disabled")) {
      return "screener cloud AI for background scheduled tasks requires a paid plan. You can still run this scheduled task with Ollama or your own provider key.";
    }
    if (normalized.includes("free_chat_client_update_required")) {
      return "Update screener to use your 2 daily free AI messages.";
    }
  ...
  if (isConnectionLikeError(errorStr)) {
    return isHostedScreenerProvider(provider)
...
export function buildProviderErrorPresentation(
  ...
  // text: the raw upstream prose reads like a screener crash and says nothing
