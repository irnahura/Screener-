...
export function isLocalScreenerUrl(url: URL): boolean {
...
export function webTargetFromUrl(url: URL, kind: WebTargetKind): WebTargetPresentation | null {
  if (isLocalScreenerUrl(url)) return null;
  return {
    url: url.toString(),
...
// Maps pi's bash curl calls to the local screener API into a human label.
...
export function classifyCurl(cmd: string): CurlPresentation | null {
  ...
  const url = urls.find(isLocalScreenerUrl);
  if (!url) {
...
function curlActivity(command: string): ToolActivityPresentation | null {
  ...
  const localUrl = urls.find(isLocalScreenerUrl);
  ...
  if (
    ...
    return activity("Checking Screener", "Checked Screener");
  }
  if (path === "/export") {
    return activity("Preparing an export", "Prepared an export");
  }

  return activity("Working in Screener", "Completed work in Screener");
}
...
// screener's MCP server exposes tools that mirror the local REST endpoints
// (activity-summary → /activity-summary, list-pipes → /pipes, search-content →
// /search, …). ACP agents surface these as `mcp__screener__<name>` tool calls
...
const MCP_SCREENER_ENDPOINTS: Record<string, { path: string; method: string }> = {
  ...
  screener: { path: "/search", method: "GET" },
...
/**
 * If `toolName` is a screener MCP tool, synthesize the equivalent local curl
 ...
 */
export function mcpScreenerCommand(
  toolName: string,
  args: Record<string, unknown> = {},
): string | null {
  let name: string;
  if (/^mcp__screener__/i.test(toolName)) {
    name = toolName.replace(/^mcp__screener__/i, "");
  } else if (!MCP_PREFIX_RE.test(toolName) && MCP_SCREENER_ENDPOINTS[toolName]) {
    // Some agents drop the mcp__ prefix; still recognize a known screener tool.
    name = toolName;
  } else {
    return null;
  }
  ...
  const endpoint = MCP_SCREENER_ENDPOINTS[name];
  if (!endpoint) {
    // A screener MCP tool we haven't mapped: a bare local URL still reads as
    // "Working in Screener" rather than a generic background step.
...
export function presentToolActivity(toolCall: PresentableToolCall): ToolActivityPresentation {
  ...
  // screener MCP tools mirror the local REST endpoints — reuse the curl path.
  const mcpCommand = mcpScreenerCommand(rawName, args);
