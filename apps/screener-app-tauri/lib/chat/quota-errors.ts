...
export function validateQuotaUpgradeAction({
  ...
  try {
    ...
    if (
      url.protocol !== "https:" ||
      !["screenpi.pe", "screener.com"].includes(url.hostname) ||
 ...
 * arbitrary nested provider errors. The URL is accepted only for Screener's
 ...
 *
 * This used to be one hardcoded "upgrade to Screener Business" string, which
...
export function buildModelNotAllowedMessage(errorStr: string): string {
  ...
  const named = modelNotAllowedName(errorStr)
    ...
    : "This model isn't";
  return `${named} available on Screener Cloud. Switch to Auto, or use your own provider key. If you picked a coding agent, re-select it in Settings → AI presets.`;
}
