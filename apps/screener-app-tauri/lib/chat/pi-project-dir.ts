...
export async function piProjectDirForSession(
  ...
  const defaultBaseDir = await join(home, ".screener");
  ...
  try {
    const result = await commands.getScreenerBaseDir();
    if (result.status === "ok" && result.data) baseDir = result.data;
  } catch {
    // Production defaults to ~/.screener. Isolated app/E2E data roots are
    // returned by getScreenerBaseDir, so a command failure safely falls back.
  }
