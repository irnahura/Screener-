...
export function buildAutomateMyWorkPrompt(existingPipes: AutomationPipeInventory[] = []) {
  return `<role>
You are a screener automation expert. Find one repeated, costly workflow that could become a useful LOW-RISK automation ("pipe"). Your first job is discovery, not creation. A repair recommendation or no recommendation is better than manufacturing a generic pipe.
</role>

Read the screener skill first so you know the API and how pipes work. During discovery, use progressive disclosure and the screener API only. Authenticate every localhost:3030 request with \`Authorization: Bearer $SCREENER_LOCAL_API_KEY\`; \`SCREENER_API_KEY\` is for the hosted API and is not proof that the local API is misconfigured. Never estimate time from frame counts and never use /raw_sql for this task.
...
The recommendation must be read-only: it may query screener and write one declared result inside its own output directory, but it must not send messages, call outbound services, modify user files, or take destructive action.
...
**Inputs:** [specific local screener data]
 ...
 * pipe.md files in crates/screener-core/assets/pipes/.
 */
export const FALLBACK_TEMPLATES: TemplatePipe[] = [
  ...
  },
  {
    ...
    icon: "\u{1F4CB}",
    featured: true,
    prompt: `Analyze my screen and audio recordings from today (last 16 hours). Read the screener skill first. Use limit=10 per search, max 5 searches total. For app-usage totals, aggregate by app over the time range using whatever screener query tool you have (a COUNT/GROUP BY query or the activity summary). Use only screener's recorded data, not this project's files or other apps' source.
...
Only report what you can verify from the data. End with: "**Next step:** [most important thing to continue]"`,
  },
  {
    ...
    icon: "⏱",
    featured: true,
    prompt: `Analyze my app usage from today (last 12 hours). Read the screener skill first. Use limit=10 per search, max 4 searches. For time per app, aggregate frames by app over the range using whatever screener query tool you have (a COUNT/GROUP BY query or the activity summary). Use only screener's recorded data, not this project's files or other apps' source.
...
End with: "**Suggestion:** [one specific change to improve tomorrow]"`,
  },
  {
    ...
    icon: "✅",
    featured: true,
    prompt: `Find action items and to-dos from the last 3 days that I may have missed. Read the screener skill first. Use limit=10 per search, max 5 searches over the last 3 days. Use only screener's recorded data, not this project's files or other apps' source.
